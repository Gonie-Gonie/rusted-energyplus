...
function Invoke-ReportSkeletonCase {
    ...
    $summaryPath = Join-Path $ReportRoot "$CaseId\compare-summary.json"
    ...
    Assert-Contains -Text $report -Pattern "| key | variable | frequency | class | source | baseline_samples | first | last | baseline_min | baseline_mean | baseline_max | baseline_nonzero_count | status |" -Description "$CaseId report source column"
    ...
    Assert-Contains -Text $summary -Pattern '"baseline_mean"' -Description "$CaseId summary baseline mean"
