...
$CaseId = "calendar_epw_dst_southern_wrap_hourly_exact_001"
...
$IdfName = "calendar_epw_dst_southern_wrap_hourly_exact.idf"
$WeatherName = "calendar_epw_dst_southern_wrap_hourly_exact.epw"
...
$ExpectedDataPeriod = "DATA PERIODS,1,1,Data,Saturday,3/27,3/29"
$ExpectedFirstTimestamp = "env=CALENDAR EPW DST SOUTHERN WRAP RUN PERIOD;day=1;month=3;date=27;dst=1;hour=1;start=0.00;end=60.00;day_type=Saturday"
$ExpectedLastTimestamp = "env=CALENDAR EPW DST SOUTHERN WRAP RUN PERIOD;day=3;month=3;date=29;dst=0;hour=24;start=0.00;end=60.00;day_type=Monday"
$ExpectedDates = @(27, 28, 29)
$ExpectedDst = @(1, 1, 0)
...
Assert-Contains -Text $caseText -Pattern 'script = "scripts/dev.cmd compare-calendar-epw-dst-southern-wrap-exact"' -Description "manifest gate attribution"
...
for ($rowIndex = 0; $rowIndex -lt 72; ++$rowIndex) {
    ...
    if ($fields.Count -ne 35 -or $fields[0] -ne "2032" -or $fields[1] -ne "3" -or
        [int]$fields[2] -ne $ExpectedDates[$dayIndex] -or [int]$fields[3] -ne $expectedHour -or
        $fields[4] -ne "60") {
        throw "EPW row $rowIndex must retain the exact 2032-03-27 through 2032-03-29 date/hour order"
    }
...
Assert-Contains -Text $idfText -Pattern "Calendar EPW DST Southern Wrap Run Period" -Description "RunPeriod identity"
...
$expectedRunPeriodFields = @(
    "Calendar EPW DST Southern Wrap Run Period",
    "3", "27", "2032", "3", "29", "2032", "Saturday",
    "No", "Yes", "No", "No", "No", "No"
)
if (($runPeriodFields -join '|') -cne ($expectedRunPeriodFields -join '|')) {
    throw "EPW southern-wrap daylight-saving fixture must retain the exact 2032-03-27 through 2032-03-29 Saturday RunPeriod and explicit policies"
}
...
Write-Host "Running exact EPW last-Sunday October through last-Sunday March daylight-saving gate."
...
if ($summary.case_id -cne $CaseId -or $summary.oracle_version -cne "26.1.0" -or
    ...
    $summary.gate.script -cne "scripts/dev.cmd compare-calendar-epw-dst-southern-wrap-exact" -or
...
for ($index = 0; $index -lt 72; ++$index) {
    ...
    if (-not $timestampMatch.Success -or
        ...
        [int]$timestampMatch.Groups[2].Value -ne 3 -or
...
Write-Host "Exact EPW last-Sunday October through last-Sunday March daylight-saving gate passed."
