...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-outdoor-air-co2-dcv-conformance\26.1.0"
$CaseId = "ideal_loads_outdoor_air_co2_dcv_conformance_candidate_001"
...
foreach ($path in @(
    ...
    Assert-FileExists -Path $path -Description "required IdealLoads outdoor-air CO2Setpoint DCV conformance input"
}
...
Write-Host "Generating IdealLoads outdoor-air CO2Setpoint DCV conformance candidate artifacts."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "IdealLoads outdoor-air CO2Setpoint DCV conformance candidate comparison failed."
}
...
Assert-FileExists -Path $summaryPath -Description "IdealLoads outdoor-air CO2Setpoint DCV conformance compare summary"
Assert-FileExists -Path $reportPath -Description "IdealLoads outdoor-air CO2Setpoint DCV conformance markdown report"
Assert-FileExists -Path $selectedOutputsPath -Description "IdealLoads outdoor-air CO2Setpoint DCV conformance selected outputs"
Assert-FileExists -Path $resultStorePath -Description "IdealLoads outdoor-air CO2Setpoint DCV conformance Rust result store"
Assert-FileExists -Path $variableDeltasPath -Description "IdealLoads outdoor-air CO2Setpoint DCV conformance variable deltas"
Assert-FileExists -Path $firstDivergencePath -Description "IdealLoads outdoor-air CO2Setpoint DCV conformance first divergence CSV"
Assert-FileExists -Path $toleranceFailuresPath -Description "IdealLoads outdoor-air CO2Setpoint DCV conformance tolerance failures CSV"
Assert-FileExists -Path $stageSummaryPath -Description "IdealLoads outdoor-air CO2Setpoint DCV conformance stage summary"
...
if ($summary.conformance_claim -ne $true) {
    throw "IdealLoads outdoor-air CO2Setpoint DCV conformance summary must set conformance_claim=true"
}
if ($summary.status -ne "pass") {
    throw "Unexpected IdealLoads outdoor-air CO2Setpoint DCV conformance status: $($summary.status)"
}
if ($summary.tolerance_failures -ne 0) {
    throw "IdealLoads outdoor-air CO2Setpoint DCV conformance should have zero tolerance failures: $($summary.tolerance_failures)"
}
...
if ($summary.demand_controlled_ventilation_type -ne "CO2Setpoint") {
    throw "Summary must record CO2Setpoint DCV, got $($summary.demand_controlled_ventilation_type)"
}
if ($summary.current_people_count_min -ne 5.0 -or $summary.current_people_count_max -ne 5.0) {
    throw "Unexpected current people count range: min=$($summary.current_people_count_min) max=$($summary.current_people_count_max)"
}
if ($summary.co2_setpoint_required_mass_flow_rate_max_kg_per_s -le $summary.outdoor_air_mass_flow_rate_kg_per_s) {
    throw "CO2Setpoint DCV proof input should exceed design OA mass flow: co2_max=$($summary.co2_setpoint_required_mass_flow_rate_max_kg_per_s) design=$($summary.outdoor_air_mass_flow_rate_kg_per_s)"
}
if ($summary.co2_setpoint_required_mass_flow_rate_min_kg_per_s -lt 0.0) {
    throw "CO2Setpoint required mass flow should not be negative, got $($summary.co2_setpoint_required_mass_flow_rate_min_kg_per_s)"
}
if ($summary.outdoor_air_mass_flow_rate_min_kg_per_s -lt ($summary.outdoor_air_mass_flow_rate_kg_per_s - 0.000000001)) {
    throw "CO2Setpoint DCV minimum OA mass flow should stay at or above design mass flow: min=$($summary.outdoor_air_mass_flow_rate_min_kg_per_s) design=$($summary.outdoor_air_mass_flow_rate_kg_per_s)"
}
if ($summary.outdoor_air_mass_flow_rate_max_kg_per_s -le $summary.outdoor_air_mass_flow_rate_kg_per_s) {
    throw "CO2Setpoint DCV max OA mass flow should exceed design mass flow: max=$($summary.outdoor_air_mass_flow_rate_max_kg_per_s) design=$($summary.outdoor_air_mass_flow_rate_kg_per_s)"
}
...
if ($conformanceRows.Count -ne 22) {
    throw "Expected twenty-two outdoor-air CO2Setpoint DCV conformance rows, found $($conformanceRows.Count)"
}
if (@($conformanceRows | Where-Object { $_.status -ne "pass" }).Count -ne 0) {
    throw "All outdoor-air CO2Setpoint DCV conformance rows must pass"
}
...
if ($massRow.Count -ne 1 -or $massRow[0].units -ne "kg/s" -or $massRow[0].rust_source -ne "rust-ideal-loads-outdoor-air-co2-setpoint-dcv") {
    throw "Missing kg/s CO2Setpoint DCV outdoor-air mass-flow row"
}
$volumeRow = @($rows | Where-Object { $_.variable -eq "Zone Ideal Loads Outdoor Air Standard Density Volume Flow Rate" })
if ($volumeRow.Count -ne 1 -or $volumeRow[0].units -ne "m3/s" -or $volumeRow[0].rust_source -ne "rust-ideal-loads-outdoor-air-co2-setpoint-dcv") {
    throw "Missing m3/s CO2Setpoint DCV outdoor-air standard-density volume-flow row"
}
...
if ($sensibleHeatingRow[0].max_abs_delta -gt 4.0 -or $sensibleHeatingRow[0].rmse_delta -gt 1.0) {
...
if ($totalHeatingRow[0].max_abs_delta -gt 4.0 -or $totalHeatingRow[0].rmse_delta -gt 1.0) {
...
if ($economizerActiveRow[0].max_abs_delta -gt 0.000000001 -or $economizerActiveRow[0].rmse_delta -gt 0.000000001) {
    throw "Inactive economizer active-time row exceeded zero conformance tolerance: max_abs=$($economizerActiveRow[0].max_abs_delta) rmse=$($economizerActiveRow[0].rmse_delta)"
}
...
if ($stageSummary.demand_controlled_ventilation_type -ne "CO2Setpoint") {
    throw "Stage summary must record CO2Setpoint DCV, got $($stageSummary.demand_controlled_ventilation_type)"
}
...
if ($stageSummary.current_people_count_min -ne 5.0 -or $stageSummary.current_people_count_max -ne 5.0) {
    throw "Stage summary must record current people range 5..5 for CO2Setpoint design occupancy, got min=$($stageSummary.current_people_count_min) max=$($stageSummary.current_people_count_max)"
}
if ($stageSummary.co2_setpoint_required_mass_flow_rate_max_kg_per_s -le $stageSummary.outdoor_air_mass_flow_rate_kg_per_s) {
    throw "Stage summary must record active CO2Setpoint demand above design OA mass flow"
}

$reportText = Get-Content -LiteralPath $reportPath -Raw
Assert-Contains -Text $reportText -Pattern "claim_boundary: conformance IdealLoads outdoor-air Flow/Person CO2Setpoint DCV branch for declared variables only" -Description "markdown claim boundary"
...
Assert-Contains -Text $reportText -Pattern "outdoor_air_source: DesignSpecification:OutdoorAir Flow/Person with blank OA schedule, EnergyPlus StdRhoAir from Site:Location, and source-order zone/OA/mixed-air state proof rows plus EnergyPlus CO2Setpoint DCV ZoneSysContDemand(ZoneNum).OutputRequiredToCO2SP through the Zone Air CO2 Predicted Load to Setpoint Mass Flow Rate proof input and CalcPurchAirMinOAMassFlow max(minimum OA, CO2 demand)" -Description "markdown OA source"
Assert-Contains -Text $reportText -Pattern "outdoor_air_schedule: blank-always-1.0" -Description "markdown OA schedule guard"
Assert-Contains -Text $reportText -Pattern "demand_controlled_ventilation_type: CO2Setpoint" -Description "markdown DCV guard"
Assert-Contains -Text $reportText -Pattern "design_people_count: 5.000000000000000" -Description "markdown design people count"
Assert-Contains -Text $reportText -Pattern "current_people_count_min: 5.000000000000000" -Description "markdown current people minimum"
Assert-Contains -Text $reportText -Pattern "current_people_count_max: 5.000000000000000" -Description "markdown current people maximum"
Assert-Contains -Text $reportText -Pattern "co2_setpoint_required_mass_flow_rate_max_kg_per_s:" -Description "markdown CO2 demand maximum"
...
Assert-Contains -Text $reportText -Pattern "| ZONE ONE IDEAL LOADS | Zone Ideal Loads Economizer Active Time | conformance" -Description "markdown inactive economizer active-time row"
...
Write-Host "IdealLoads outdoor-air CO2Setpoint DCV conformance candidate artifacts generated."
