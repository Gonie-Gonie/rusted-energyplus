...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-outdoor-air-differential-enthalpy-economizer-conformance\26.1.0"
$CaseId = "ideal_loads_outdoor_air_differential_enthalpy_economizer_conformance_candidate_001"
...
foreach ($path in @(
    ...
    Assert-FileExists -Path $path -Description "required IdealLoads outdoor-air DifferentialEnthalpy economizer conformance input"
}
...
Write-Host "Generating IdealLoads outdoor-air DifferentialEnthalpy economizer conformance candidate artifacts."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "IdealLoads outdoor-air DifferentialEnthalpy economizer conformance candidate comparison failed."
}
...
Assert-FileExists -Path $summaryPath -Description "IdealLoads outdoor-air DifferentialEnthalpy economizer conformance compare summary"
Assert-FileExists -Path $reportPath -Description "IdealLoads outdoor-air DifferentialEnthalpy economizer conformance markdown report"
Assert-FileExists -Path $selectedOutputsPath -Description "IdealLoads outdoor-air DifferentialEnthalpy economizer conformance selected outputs"
Assert-FileExists -Path $resultStorePath -Description "IdealLoads outdoor-air DifferentialEnthalpy economizer conformance Rust result store"
Assert-FileExists -Path $variableDeltasPath -Description "IdealLoads outdoor-air DifferentialEnthalpy economizer conformance variable deltas"
Assert-FileExists -Path $firstDivergencePath -Description "IdealLoads outdoor-air DifferentialEnthalpy economizer conformance first divergence CSV"
Assert-FileExists -Path $toleranceFailuresPath -Description "IdealLoads outdoor-air DifferentialEnthalpy economizer conformance tolerance failures CSV"
Assert-FileExists -Path $stageSummaryPath -Description "IdealLoads outdoor-air DifferentialEnthalpy economizer conformance stage summary"
...
if ($summary.conformance_claim -ne $true) {
    throw "IdealLoads outdoor-air DifferentialEnthalpy economizer conformance summary must set conformance_claim=true"
}
if ($summary.status -ne "pass") {
    throw "Unexpected IdealLoads outdoor-air conformance status: $($summary.status)"
}
if ($summary.tolerance_failures -ne 0) {
    throw "IdealLoads outdoor-air DifferentialEnthalpy economizer conformance should have zero tolerance failures: $($summary.tolerance_failures)"
}
...
if ($conformanceRows.Count -ne 15) {
    throw "Expected fifteen outdoor-air DifferentialEnthalpy economizer conformance rows, found $($conformanceRows.Count)"
}
...
if ($massRow.Count -ne 1 -or $massRow[0].units -ne "kg/s" -or $massRow[0].rust_source -ne "rust-ideal-loads-outdoor-air-differential-enthalpy-economizer") {
    throw "Missing kg/s differential enthalpy economizer outdoor-air mass-flow row"
}
$volumeRow = @($rows | Where-Object { $_.variable -eq "Zone Ideal Loads Outdoor Air Standard Density Volume Flow Rate" })
if ($volumeRow.Count -ne 1 -or $volumeRow[0].units -ne "m3/s" -or $volumeRow[0].rust_source -ne "rust-ideal-loads-outdoor-air-differential-enthalpy-economizer") {
    throw "Missing m3/s differential enthalpy economizer outdoor-air standard-density volume-flow row"
}
...
if ($economizerActiveRow.Count -ne 1 -or $economizerActiveRow[0].units -ne "hr" -or $economizerActiveRow[0].rust_source -ne "rust-ideal-loads-outdoor-air-differential-enthalpy-economizer") {
    throw "Missing hr differential enthalpy economizer active-time row"
}
if ($economizerActiveRow[0].max_abs_delta -gt 0.000000001 -or $economizerActiveRow[0].rmse_delta -gt 0.000000001) {
    throw "Differential enthalpy economizer active-time row exceeded conformance tolerance: max_abs=$($economizerActiveRow[0].max_abs_delta) rmse=$($economizerActiveRow[0].rmse_delta)"
}
...
if ($maxEconomizerActiveTime -le 0.0) {
    throw "Expected at least one active DifferentialEnthalpy economizer timestep"
}
...
if ($stageSummary.economizer -ne "DifferentialEnthalpy") {
    throw "Stage summary must record economizer=DifferentialEnthalpy, got $($stageSummary.economizer)"
}
