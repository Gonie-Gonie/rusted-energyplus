...
$CaseId = "calendar_schedule_external_interface_initial_value_exact_001"
$CaseRoot = Join-Path $RepoRoot "data\conformance_cases\$CaseId"
$CasePath = Join-Path $CaseRoot "case.toml"
$IdfPath = Join-Path $CaseRoot "calendar_schedule_external_interface_initial_value_exact.idf"
$WeatherPath = Join-Path $CaseRoot "calendar_schedule_external_interface_initial_value_exact.epw"
$GateCommand = "scripts/dev.cmd compare-calendar-schedule-external-interface-initial-value-exact"
...
foreach ($path in @(
    ...
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Missing required ExternalInterface:Schedule exact-case file: $path"
    }
}
...
foreach ($contract in @(
    ...
    'abs_tol = 0.0',
    'rmse_tol = 0.0',
    'exactly 96 ordered, unique Timestep Schedule Value samples and timestamps, each equal to 0.375',
    'No ExternalInterface activation object is present',
    'BCVTB/FMU live exchange',
    'mutable updates or currentVal state',
    'warmup and sizing exchange lifecycle',
    'EMS priority',
    'generated internal day/week EIO parity',
    'downstream consumers',
    'multi-object and diagnostic parity',
    'FunctionalMockupUnitImport/Export schedule families',
    'script = "scripts/dev.cmd compare-calendar-schedule-external-interface-initial-value-exact"',
    'blocking = true'
)) {
...
$expectedVectors = @(
    "Version|26.1",
    "Building|External Interface Schedule Initial Value Exact Fixture|0.0|Suburbs|0.04|0.4|FullExterior|25|6",
    "Timestep|4",
    "GlobalGeometryRules|UpperLeftCorner|CounterClockWise|World",
    "RunPeriod|External Interface Schedule Initial Value Run Period|1|1|2032|1|1|2032|Thursday|No|No|No|No|No|No",
    "ScheduleTypeLimits|Any Number",
    "ExternalInterface:Schedule|External Initial Value|Any Number|0.375",
    "Output:Variable|EXTERNAL INITIAL VALUE|Schedule Value|Timestep"
)
...
$mutated = $idfText.Replace(
    "Version,26.1;",
    "Version,26.1; ExternalInterface,PtolemyServer;"
)
...
if ($mutatedVectors.Count -ne ($expectedVectors.Count + 1) -or
    $mutatedVectors[1] -cne "ExternalInterface|PtolemyServer") {
...
$expectedHeaders = @(
    "LOCATION,External Interface Schedule Initial Value Exact Fixture,CO,USA,Synthetic,999999,39.74,-105.18,-7.0,1829.0",
    "DESIGN CONDITIONS,0",
    "TYPICAL/EXTREME PERIODS,0",
    "GROUND TEMPERATURES,0",
    "HOLIDAYS/DAYLIGHT SAVINGS,No,0,0,0",
    "COMMENTS 1,Deterministic one-day ExternalInterface Schedule inactive initial-value fixture",
...
if ($LASTEXITCODE -ne 0) {
    throw "ExternalInterface:Schedule report failed with exit code $LASTEXITCODE $($output -join [Environment]::NewLine)"
}
...
foreach ($path in @(
    ...
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Missing ExternalInterface:Schedule report artifact: $path"
    }
}
...
if ($summary.case_id -cne $CaseId -or $summary.oracle_version -cne "26.1.0" -or
    ...
    throw "Unexpected ExternalInterface:Schedule report summary contract"
}
$seriesRows = @($summary.series | Where-Object {
    $_.key -eq "EXTERNAL INITIAL VALUE" -and $_.variable -eq "Schedule Value"
})
if ($seriesRows.Count -ne 1) {
    throw "Expected exactly one external initial-value Schedule Value series"
}
$series = $seriesRows[0]
$firstTimestamp = "env=EXTERNAL INTERFACE SCHEDULE INITIAL VALUE RUN PERIOD;day=1;month=1;date=1;dst=0;hour=1;start=0.00;end=15.00;day_type=Thursday"
$lastTimestamp = "env=EXTERNAL INTERFACE SCHEDULE INITIAL VALUE RUN PERIOD;day=1;month=1;date=1;dst=0;hour=24;start=45.00;end=60.00;day_type=Thursday"
if ($series.level -cne "conformance" -or $series.class -cne "schedule" -or
    ...
    throw "External initial-value samples and timestamps must match exactly at zero tolerance"
}
...
$scheduleFamily = $converted."ExternalInterface:Schedule"
$convertedSchedule = $scheduleFamily."External Initial Value"
$scheduleTypeFamily = $converted."ScheduleTypeLimits"
if (@($scheduleFamily.PSObject.Properties).Count -ne 1 -or
    @($convertedSchedule.PSObject.Properties).Count -ne 2 -or
    $convertedSchedule.schedule_type_limits_name -cne "Any Number" -or
    [double]$convertedSchedule.initial_value -ne 0.375 -or
    ...
    throw "Converted epJSON changed the exact external schedule/type-limit fields or added ExternalInterface activation"
}
Write-Host "OK converted epJSON exact inactive ExternalInterface:Schedule shape"

$esoLines = @(Get-Content -LiteralPath $oracleEsoPath -Encoding UTF8)
$dictionaryRows = @($esoLines | Where-Object {
    $_ -ceq "7,1,EXTERNAL INITIAL VALUE,Schedule Value [] !TimeStep"
})
...
for ($index = 0; $index -lt 96; ++$index) {
    if ($values[$index] -ne 0.375) {
        throw "Unexpected raw external initial value at sample $($index): $($values[$index])"
    }
    ...
    if (-not $timestampMatch.Success -or
        ...
        throw "Unexpected raw external-schedule timestamp at sample $($index): $($timestamps[$index])"
    }
}
...
$environmentRow = "Environment,EXTERNAL INTERFACE SCHEDULE INITIAL VALUE RUN PERIOD,WeatherFileRunPeriod,01/01/2032,01/01/2032,Thursday,1,Use RunPeriod Specified Day,No,No,No,No,No,Clark and Allen"
...
$warningHeader = '   ** Warning ** IDF file contains object "ExternalInterface:Schedule",'
...
foreach ($reportContract in @(
    ...
    "| EXTERNAL INITIAL VALUE | Schedule Value | conformance | schedule | timestep | eso | timestamp | 96 | 96 | 96 | 0.000000000000 | 0.000000000000 | 0.000000000000 |"
)) {
    Assert-Contains -Text $reportText -Pattern $reportContract -Description "markdown external initial-value contract"
}

Write-Host "ExternalInterface:Schedule inactive initial-value exact gate passed."
