...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-flow-limit-conformance\26.1.0"
$CaseId = "ideal_loads_flow_limit_conformance_001"
...
Write-Host "Generating IdealLoads no-OA flow-limit conformance comparison artifacts."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "IdealLoads no-OA flow-limit conformance comparison failed."
}
...
if ($summary.conformance_claim -ne $true) {
    throw "IdealLoads flow-limit conformance summary must set conformance_claim=true"
}
if ($summary.status -ne "pass") {
    throw "Unexpected IdealLoads flow-limit conformance status: $($summary.status)"
}
if ($summary.tolerance_failures -ne 0) {
    throw "IdealLoads flow-limit conformance should have zero tolerance failures: $($summary.tolerance_failures)"
}
...
if ($summary.zone_demand_synthetic_rc_model -ne $false) {
    throw "IdealLoads flow-limit conformance must not synthesize zone demand from an RC shortcut"
}
...
if (@($conformanceRows | Where-Object { $_.status -ne "pass" }).Count -ne 0) {
    throw "All conformance-level flow-limit output rows must pass"
}
...
if ($stageSummary.selected_purchased_air_branch -ne "finite_flow") {
    throw "Unexpected PurchasedAir branch: $($stageSummary.selected_purchased_air_branch)"
}
if ($stageSummary.declared_ideal_loads_branch -ne "finite_flow") {
...
if ($stageSummary.finite_limit_conformance -ne $true) {
    throw "Stage summary must mark finite_limit_conformance=true for the promoted flow-limit case"
}
...
Assert-Contains -Text $reportText -Pattern "selected_purchased_air_branch: finite_flow" -Description "markdown PurchasedAir branch"
Assert-Contains -Text $reportText -Pattern "declared_ideal_loads_branch: finite_flow" -Description "markdown declared branch"
...
Write-Host "IdealLoads no-OA flow-limit conformance comparison artifacts generated."
