...
Assert-Contains -Text $reportText -Pattern "claim_boundary: conformance no-OA ConstantSupplyHumidityRatio cooling IdealLoads branch for declared heating/cooling rate rows, supply-node rows, ReportPurchasedAir energy rows, blank fuel-efficiency rows, and hourly facility meters only" -Description "markdown claim boundary"
