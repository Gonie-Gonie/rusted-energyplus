...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-humidistat-dehumidification-conformance\26.1.0"
$CaseId = "ideal_loads_humidistat_dehumidification_conformance_candidate_001"
...
foreach ($path in @(
    ...
    Assert-FileExists -Path $path -Description "required IdealLoads humidistat-dehumidification conformance input"
}
...
Write-Host "Generating IdealLoads humidistat-dehumidification conformance candidate artifacts."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "IdealLoads humidistat-dehumidification conformance candidate comparison failed."
}
...
Assert-FileExists -Path $summaryPath -Description "IdealLoads humidistat-dehumidification conformance compare summary"
Assert-FileExists -Path $reportPath -Description "IdealLoads humidistat-dehumidification conformance markdown report"
Assert-FileExists -Path $resultStorePath -Description "IdealLoads humidistat-dehumidification conformance Rust result store"
Assert-FileExists -Path $toleranceFailuresPath -Description "IdealLoads humidistat-dehumidification conformance tolerance failures CSV"
Assert-FileExists -Path $stageSummaryPath -Description "IdealLoads humidistat-dehumidification conformance stage summary"
...
if ($summary.conformance_claim -ne $true) {
    throw "IdealLoads humidistat-dehumidification conformance summary must set conformance_claim=true"
}
...
$zoneTotalCooling = Get-ResultSeries -ResultStore $resultStore -Variable "Zone Ideal Loads Zone Total Cooling Rate"
$zoneSensibleCooling = Get-ResultSeries -ResultStore $resultStore -Variable "Zone Ideal Loads Zone Sensible Cooling Rate"
$zoneLatentCooling = Get-ResultSeries -ResultStore $resultStore -Variable "Zone Ideal Loads Zone Latent Cooling Rate"
$supplyTotalCooling = Get-ResultSeries -ResultStore $resultStore -Variable "Zone Ideal Loads Supply Air Total Cooling Rate"
$supplySensibleCooling = Get-ResultSeries -ResultStore $resultStore -Variable "Zone Ideal Loads Supply Air Sensible Cooling Rate"
...
$maxZoneTotalCooling = ($zoneTotalCooling.values | Measure-Object -Maximum).Maximum
$maxZoneSensibleCooling = ($zoneSensibleCooling.values | Measure-Object -Maximum).Maximum
$maxZoneLatentCooling = ($zoneLatentCooling.values | Measure-Object -Maximum).Maximum
$maxSupplyTotalCooling = ($supplyTotalCooling.values | Measure-Object -Maximum).Maximum
$maxSupplySensibleCooling = ($supplySensibleCooling.values | Measure-Object -Maximum).Maximum
...
if ($maxZoneTotalCooling -le 0.0) {
    throw "Expected active zone total cooling in humidistat-dehumidification conformance candidate"
}
if ($maxZoneSensibleCooling -le 0.0) {
    throw "Expected active zone sensible cooling in humidistat-dehumidification conformance candidate"
}
if ($maxZoneLatentCooling -le 0.0) {
    throw "Expected active zone latent cooling in humidistat-dehumidification conformance candidate"
}
if ($maxSupplyTotalCooling -le 0.0) {
    throw "Expected active supply-air total cooling in humidistat-dehumidification conformance candidate"
}
if ($maxSupplySensibleCooling -le 0.0) {
    throw "Expected active supply-air sensible cooling in humidistat-dehumidification conformance candidate"
}
if ($maxSupplyLatentCooling -le 0.0) {
    throw "Expected active supply-air latent cooling in humidistat-dehumidification conformance candidate"
}
if ($minDehumidifyingMoistureDemand -ge 0.0) {
    throw "Expected active negative dehumidifying moisture demand in humidistat-dehumidification conformance candidate"
}
if ([Math]::Abs($minSupplyHumidity - 0.0077) -gt 0.000000001) {
    throw "Expected humidistat-dehumidification to use the minimum cooling supply humidity ratio"
}
...
if ($stageSummary.selected_purchased_air_branch -ne "humidistat_dehumidification") {
    throw "Unexpected PurchasedAir branch: $($stageSummary.selected_purchased_air_branch)"
}
if ($stageSummary.declared_ideal_loads_branch -ne "humidistat_dehumidification") {
...
Assert-Contains -Text $reportText -Pattern "claim_boundary: conformance no-OA Humidistat dehumidification IdealLoads branch for declared heating/cooling rate rows, supply-node rows, ReportPurchasedAir energy rows, blank fuel-efficiency rows, and hourly facility meters only" -Description "markdown claim boundary"
...
Assert-Contains -Text $reportText -Pattern "selected_purchased_air_branch: humidistat_dehumidification" -Description "markdown PurchasedAir branch"
Assert-Contains -Text $reportText -Pattern "declared_ideal_loads_branch: humidistat_dehumidification" -Description "markdown declared branch"
...
Write-Host "IdealLoads humidistat-dehumidification conformance candidate artifacts generated."
