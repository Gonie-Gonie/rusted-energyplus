...
Write-Host "Generating IdealLoads no-OA sensible diagnostic comparison artifacts."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "IdealLoads no-OA sensible diagnostic comparison failed."
}

$text = ($output -join "`n")
Assert-Contains -Text $text -Pattern "IdealLoads No-OA Sensible Diagnostic Report" -Description "report header"
...
if ($summary.conformance_claim -ne $false) {
    throw "IdealLoads diagnostic summary must keep conformance_claim=false"
}
if ($summary.status -ne "diagnostic") {
    throw "Unexpected IdealLoads diagnostic status: $($summary.status)"
}
if ($summary.tolerance_failures -ne 0) {
    throw "IdealLoads diagnostic comparison must have zero tolerance failures before promotion: $($summary.tolerance_failures)"
}
...
if ($summary.zone_demand_synthetic_rc_model -ne $false) {
    throw "IdealLoads diagnostic must not synthesize zone demand from an RC shortcut"
}
...
if ($nodeFlow[0].status -ne "pass") {
    throw "System Node Mass Flow Rate must pass in diagnostic comparison before promotion"
}
...
Write-Host "IdealLoads no-OA sensible diagnostic comparison artifacts generated."
