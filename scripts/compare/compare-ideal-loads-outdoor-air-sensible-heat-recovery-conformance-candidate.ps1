...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-outdoor-air-sensible-heat-recovery-conformance\26.1.0"
$CaseId = "ideal_loads_outdoor_air_sensible_heat_recovery_conformance_candidate_001"
...
foreach ($path in @(
    ...
    Assert-FileExists -Path $path -Description "required IdealLoads outdoor-air Sensible heat-recovery conformance input"
}
...
Write-Host "Generating IdealLoads outdoor-air Sensible heat-recovery conformance candidate comparison artifacts."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "IdealLoads outdoor-air design-flow conformance candidate comparison failed."
}
...
Assert-FileExists -Path $summaryPath -Description "IdealLoads outdoor-air Sensible heat-recovery conformance compare summary"
Assert-FileExists -Path $reportPath -Description "IdealLoads outdoor-air Sensible heat-recovery conformance markdown report"
Assert-FileExists -Path $selectedOutputsPath -Description "IdealLoads outdoor-air Sensible heat-recovery conformance selected outputs"
Assert-FileExists -Path $resultStorePath -Description "IdealLoads outdoor-air Sensible heat-recovery conformance Rust result store"
Assert-FileExists -Path $variableDeltasPath -Description "IdealLoads outdoor-air Sensible heat-recovery conformance variable deltas"
Assert-FileExists -Path $firstDivergencePath -Description "IdealLoads outdoor-air Sensible heat-recovery conformance first divergence CSV"
Assert-FileExists -Path $toleranceFailuresPath -Description "IdealLoads outdoor-air Sensible heat-recovery conformance tolerance failures CSV"
Assert-FileExists -Path $stageSummaryPath -Description "IdealLoads outdoor-air Sensible heat-recovery conformance stage summary"
...
if ($conformanceRows.Count -ne 22) {
    throw "Expected twenty-two outdoor-air Sensible heat-recovery conformance rows, found $($conformanceRows.Count)"
}
if (@($conformanceRows | Where-Object { $_.status -ne "pass" }).Count -ne 0) {
    throw "All outdoor-air Sensible heat-recovery conformance rows must pass"
}
...
$heatRecoverySource = "rust-ideal-loads-outdoor-air-sensible-heat-recovery"
foreach ($heatRecoveryVariable in @(
    ...
    if ($heatRecoveryRow.Count -ne 1 -or $heatRecoveryRow[0].units -ne "W" -or $heatRecoveryRow[0].rust_source -ne $heatRecoverySource) {
        throw "Missing W sensible heat-recovery row: $heatRecoveryVariable"
    }
    if ($heatRecoveryRow[0].max_abs_delta -gt 1.0 -or $heatRecoveryRow[0].rmse_delta -gt 1.0) {
        throw "Sensible heat-recovery row exceeded diagnostic tolerance: variable=$heatRecoveryVariable max_abs=$($heatRecoveryRow[0].max_abs_delta) rmse=$($heatRecoveryRow[0].rmse_delta)"
    }
}
foreach ($heatRecoveryVariable in @(
    ...
    $heatRecoveryRow = @($rows | Where-Object { $_.variable -eq $heatRecoveryVariable })
    if ($heatRecoveryRow.Count -ne 1 -or $heatRecoveryRow[0].units -ne "W" -or $heatRecoveryRow[0].rust_source -ne $heatRecoverySource) {
        throw "Missing W sensible heat-recovery latent row: $heatRecoveryVariable"
    }
    if ($heatRecoveryRow[0].max_abs_delta -gt 0.000000001 -or $heatRecoveryRow[0].rmse_delta -gt 0.000000001) {
        throw "Sensible heat-recovery latent row exceeded zero conformance tolerance: variable=$heatRecoveryVariable max_abs=$($heatRecoveryRow[0].max_abs_delta) rmse=$($heatRecoveryRow[0].rmse_delta)"
    }
}
...
if ($heatRecoveryActiveRow.Count -ne 1 -or $heatRecoveryActiveRow[0].units -ne "hr" -or $heatRecoveryActiveRow[0].rust_source -ne $heatRecoverySource) {
    throw "Missing hr sensible heat-recovery active-time row"
}
if ($heatRecoveryActiveRow[0].max_abs_delta -gt 0.000000001 -or $heatRecoveryActiveRow[0].rmse_delta -gt 0.000000001) {
    throw "Sensible heat-recovery active-time row exceeded diagnostic tolerance: max_abs=$($heatRecoveryActiveRow[0].max_abs_delta) rmse=$($heatRecoveryActiveRow[0].rmse_delta)"
}
...
if ($maxHeatRecoveryActiveTime -le 0.0) {
    throw "Expected at least one active Sensible heat-recovery timestep"
}
...
if ($maxHeatRecoverySensibleHeating -le 0.0) {
    throw "Expected Sensible heat recovery to report nonzero sensible heating"
}
...
if ($stageSummary.heat_recovery -ne "Sensible") {
    throw "Stage summary must record heat_recovery=Sensible, got $($stageSummary.heat_recovery)"
}

$reportText = Get-Content -LiteralPath $reportPath -Raw
Assert-Contains -Text $reportText -Pattern "claim_boundary: conformance IdealLoads outdoor-air Sensible heat recovery branch for declared variables only" -Description "markdown claim boundary"
...
Assert-Contains -Text $reportText -Pattern "outdoor_air_source: DesignSpecification:OutdoorAir Flow/Zone with blank OA schedule, EnergyPlus StdRhoAir from Site:Location, and source-order zone/OA/mixed-air state proof rows plus EnergyPlus Sensible heat recovery OA tempering when recirculation air can beneficially warm or cool outdoor air" -Description "markdown OA source"
...
Assert-Contains -Text $reportText -Pattern "| ZONE ONE IDEAL LOADS | Zone Ideal Loads Heat Recovery Sensible Heating Rate | conformance" -Description "markdown sensible heat-recovery sensible heating row"
Assert-Contains -Text $reportText -Pattern "| ZONE ONE IDEAL LOADS | Zone Ideal Loads Heat Recovery Total Cooling Rate | conformance" -Description "markdown sensible heat-recovery total cooling row"
...
Assert-Contains -Text $reportText -Pattern "| ZONE ONE IDEAL LOADS | Zone Ideal Loads Heat Recovery Active Time | conformance" -Description "markdown sensible heat-recovery active-time row"

Write-Host "IdealLoads outdoor-air Sensible heat-recovery conformance candidate comparison artifacts generated."
