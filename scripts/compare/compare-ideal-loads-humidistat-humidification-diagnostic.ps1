...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-humidistat-humidification\26.1.0"
$CaseId = "ideal_loads_humidistat_humidification_diagnostic_001"
...
foreach ($path in @(
    ...
    Assert-FileExists -Path $path -Description "required IdealLoads humidistat-humidification compare input"
}
...
Write-Host "Generating IdealLoads humidistat-humidification diagnostic comparison artifacts."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "IdealLoads humidistat-humidification diagnostic comparison failed."
}
...
Assert-FileExists -Path $summaryPath -Description "IdealLoads humidistat-humidification compare summary"
Assert-FileExists -Path $reportPath -Description "IdealLoads humidistat-humidification markdown report"
Assert-FileExists -Path $resultStorePath -Description "IdealLoads humidistat-humidification Rust result store"
Assert-FileExists -Path $toleranceFailuresPath -Description "IdealLoads humidistat-humidification tolerance failures CSV"
Assert-FileExists -Path $stageSummaryPath -Description "IdealLoads humidistat-humidification stage summary"
...
if ($summary.conformance_claim -ne $false) {
    throw "IdealLoads humidistat-humidification diagnostic must not set conformance_claim=true"
}
...
if (@($summary.series | Where-Object { $_.level -ne "diagnostic" }).Count -ne 0) {
    throw "All humidistat-humidification diagnostic rows must remain diagnostic-level"
}
if (@($summary.series | Where-Object { $_.status -ne "pass" }).Count -ne 0) {
    throw "All humidistat-humidification diagnostic rows must pass"
}
...
if ($maxZoneLatentHeating -le 0.05) {
    throw "Expected active zone latent heating in humidistat-humidification diagnostic"
}
if ($maxSupplyLatentHeating -le 0.05) {
    throw "Expected active supply-air latent heating in humidistat-humidification diagnostic"
}
if ($maxHumidifyingMoistureDemand -le 1.0e-8) {
    throw "Expected active positive humidifying moisture demand in humidistat-humidification diagnostic"
}
...
Assert-Contains -Text $reportText -Pattern "| ZONE ONE IDEAL LOADS | Zone Ideal Loads Zone Latent Heating Rate | diagnostic" -Description "markdown zone latent heating row"
Assert-Contains -Text $reportText -Pattern "| ZONE ONE IDEAL LOADS | Zone Ideal Loads Supply Air Latent Heating Rate | diagnostic" -Description "markdown supply latent heating row"
Assert-Contains -Text $reportText -Pattern "| ZONE ONE | Zone System Predicted Moisture Load to Humidifying Setpoint Moisture Transfer Rate | diagnostic" -Description "markdown humidifying moisture demand row"

Write-Host "IdealLoads humidistat-humidification diagnostic comparison passed."
