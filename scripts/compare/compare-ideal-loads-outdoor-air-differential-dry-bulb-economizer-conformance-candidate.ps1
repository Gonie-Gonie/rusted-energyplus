...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-outdoor-air-differential-dry-bulb-economizer-conformance\26.1.0"
$CaseId = "ideal_loads_outdoor_air_differential_dry_bulb_economizer_conformance_candidate_001"
...
foreach ($path in @(
    ...
    Assert-FileExists -Path $path -Description "required IdealLoads outdoor-air DifferentialDryBulb economizer conformance input"
}
...
Write-Host "Generating IdealLoads outdoor-air DifferentialDryBulb economizer conformance candidate artifacts."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "IdealLoads outdoor-air DifferentialDryBulb economizer conformance candidate comparison failed."
}
...
Assert-FileExists -Path $summaryPath -Description "IdealLoads outdoor-air DifferentialDryBulb economizer conformance compare summary"
Assert-FileExists -Path $reportPath -Description "IdealLoads outdoor-air DifferentialDryBulb economizer conformance markdown report"
Assert-FileExists -Path $selectedOutputsPath -Description "IdealLoads outdoor-air DifferentialDryBulb economizer conformance selected outputs"
Assert-FileExists -Path $resultStorePath -Description "IdealLoads outdoor-air DifferentialDryBulb economizer conformance Rust result store"
Assert-FileExists -Path $variableDeltasPath -Description "IdealLoads outdoor-air DifferentialDryBulb economizer conformance variable deltas"
Assert-FileExists -Path $firstDivergencePath -Description "IdealLoads outdoor-air DifferentialDryBulb economizer conformance first divergence CSV"
Assert-FileExists -Path $toleranceFailuresPath -Description "IdealLoads outdoor-air DifferentialDryBulb economizer conformance tolerance failures CSV"
Assert-FileExists -Path $stageSummaryPath -Description "IdealLoads outdoor-air DifferentialDryBulb economizer conformance stage summary"
...
if ($summary.conformance_claim -ne $true) {
    throw "IdealLoads outdoor-air DifferentialDryBulb economizer conformance summary must set conformance_claim=true"
}
if ($summary.status -ne "pass") {
    throw "Unexpected IdealLoads outdoor-air conformance status: $($summary.status)"
}
if ($summary.tolerance_failures -ne 0) {
    throw "IdealLoads outdoor-air DifferentialDryBulb economizer conformance should have zero tolerance failures: $($summary.tolerance_failures)"
}
...
if ($summary.design_volume_flow_rate_m3_per_s -ne 0.001) {
...
if ($conformanceRows.Count -ne 15) {
    throw "Expected fifteen outdoor-air DifferentialDryBulb economizer conformance rows, found $($conformanceRows.Count)"
}
if (@($conformanceRows | Where-Object { $_.status -ne "pass" }).Count -ne 0) {
    throw "All outdoor-air DifferentialDryBulb economizer conformance rows must pass"
}
$diagnosticRows = @($rows | Where-Object { $_.level -eq "diagnostic" })
if ($diagnosticRows.Count -ne 7) {
    throw "Expected seven outdoor-air inactive proof diagnostic rows, found $($diagnosticRows.Count)"
}
...
if ($massRow.Count -ne 1 -or $massRow[0].units -ne "kg/s" -or $massRow[0].rust_source -ne "rust-ideal-loads-outdoor-air-differential-dry-bulb-economizer") {
    throw "Missing kg/s differential dry-bulb economizer outdoor-air mass-flow row"
}
$volumeRow = @($rows | Where-Object { $_.variable -eq "Zone Ideal Loads Outdoor Air Standard Density Volume Flow Rate" })
if ($volumeRow.Count -ne 1 -or $volumeRow[0].units -ne "m3/s" -or $volumeRow[0].rust_source -ne "rust-ideal-loads-outdoor-air-differential-dry-bulb-economizer") {
    throw "Missing m3/s differential dry-bulb economizer outdoor-air standard-density volume-flow row"
}
...
if ($supplyMassRow[0].max_abs_delta -gt 0.000003 -or $supplyMassRow[0].rmse_delta -gt 0.000001) {
...
if ($supplyVolumeRow[0].max_abs_delta -gt 0.000003 -or $supplyVolumeRow[0].rmse_delta -gt 0.000001) {
...
if ($supplyHumidityRatioRow[0].max_abs_delta -gt 0.00001 -or $supplyHumidityRatioRow[0].rmse_delta -gt 0.000001) {
...
if ($mixedAirHumidityRatioRow[0].max_abs_delta -gt 0.00001 -or $mixedAirHumidityRatioRow[0].rmse_delta -gt 0.000001) {
...
if ($economizerActiveRow.Count -ne 1 -or $economizerActiveRow[0].units -ne "hr" -or $economizerActiveRow[0].rust_source -ne "rust-ideal-loads-outdoor-air-differential-dry-bulb-economizer") {
    throw "Missing hr differential dry-bulb economizer active-time row"
}
if ($economizerActiveRow[0].max_abs_delta -gt 0.000000001 -or $economizerActiveRow[0].rmse_delta -gt 0.000000001) {
    throw "Differential dry-bulb economizer active-time row exceeded diagnostic tolerance: max_abs=$($economizerActiveRow[0].max_abs_delta) rmse=$($economizerActiveRow[0].rmse_delta)"
}
...
$economizerActiveSeries = @($resultStore.series | Where-Object { $_.variable_name -eq "Zone Ideal Loads Economizer Active Time" })
if ($economizerActiveSeries.Count -ne 1) {
    throw "Missing economizer active-time result-store series"
}
$maxEconomizerActiveTime = @($economizerActiveSeries[0].values | Measure-Object -Maximum)[0].Maximum
if ($maxEconomizerActiveTime -le 0.0) {
    throw "Expected at least one active DifferentialDryBulb economizer timestep"
}
$outdoorAirMassSeries = @($resultStore.series | Where-Object { $_.variable_name -eq "Zone Ideal Loads Outdoor Air Mass Flow Rate" })
if ($outdoorAirMassSeries.Count -ne 1) {
    throw "Missing outdoor-air mass-flow result-store series"
}
$maxOutdoorAirMassFlow = @($outdoorAirMassSeries[0].values | Measure-Object -Maximum)[0].Maximum
if ($maxOutdoorAirMassFlow -le $summary.outdoor_air_mass_flow_rate_kg_per_s) {
    throw "Expected economizer to raise outdoor-air mass flow above the design minimum"
}
...
if ($stageSummary.economizer -ne "DifferentialDryBulb") {
    throw "Stage summary must record economizer=DifferentialDryBulb, got $($stageSummary.economizer)"
}

$reportText = Get-Content -LiteralPath $reportPath -Raw
Assert-Contains -Text $reportText -Pattern "claim_boundary: conformance IdealLoads outdoor-air DifferentialDryBulb economizer branch for declared variables only" -Description "markdown claim boundary"
...
Assert-Contains -Text $reportText -Pattern "outdoor_air_source: DesignSpecification:OutdoorAir Flow/Zone with blank OA schedule, EnergyPlus StdRhoAir from Site:Location, and source-order zone/OA/mixed-air state proof rows plus EnergyPlus DifferentialDryBulb economizer OA flow reset when outdoor dry-bulb is below recirculation dry-bulb" -Description "markdown OA source"
...
Assert-Contains -Text $reportText -Pattern "| ZONE ONE IDEAL LOADS | Zone Ideal Loads Economizer Active Time | conformance" -Description "markdown differential dry-bulb economizer active-time row"
...
Write-Host "IdealLoads outdoor-air DifferentialDryBulb economizer conformance candidate artifacts generated."
