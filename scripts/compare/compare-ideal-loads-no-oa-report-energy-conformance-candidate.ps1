...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-no-oa-report-energy-conformance\26.1.0"
$CaseId = "ideal_loads_no_oa_report_energy_conformance_candidate_001"
...
Write-Host "Generating IdealLoads no-OA ReportPurchasedAir energy conformance comparison artifacts."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "IdealLoads no-OA ReportPurchasedAir energy conformance comparison failed."
}
...
if ($summary.conformance_claim -ne $true) {
    throw "IdealLoads report-energy conformance summary must set conformance_claim=true"
}
if ($summary.status -ne "pass") {
    throw "Unexpected IdealLoads report-energy conformance status: $($summary.status)"
}
...
if ($summary.tolerance_failures -ne 0) {
    throw "IdealLoads report-energy conformance comparison must have zero tolerance failures: $($summary.tolerance_failures)"
}
...
if ($conformanceRows.Count -ne 4) {
    throw "Expected 4 conformance-level ReportPurchasedAir energy rows, found $($conformanceRows.Count)"
}
if (@($conformanceRows | Where-Object { $_.status -ne "pass" }).Count -ne 0) {
    throw "All conformance-level ReportPurchasedAir energy rows must pass"
}
$expectedReportEnergyVariables = @(
    "Zone Ideal Loads Supply Air Total Heating Energy",
    "Zone Ideal Loads Supply Air Total Cooling Energy",
    "Zone Ideal Loads Zone Total Heating Energy",
    "Zone Ideal Loads Zone Total Cooling Energy"
)
foreach ($expectedVariable in $expectedReportEnergyVariables) {
    $matches = @($conformanceRows | Where-Object { $_.variable -eq $expectedVariable -and $_.rust_source -eq "rust-ideal-loads-report-time-step-energy" -and $_.units -eq "J" })
    if ($matches.Count -ne 1) {
        throw "Missing conformance ReportPurchasedAir energy row for $expectedVariable"
    }
}
$diagnosticRows = @($summary.series | Where-Object { $_.level -eq "diagnostic" })
if ($diagnosticRows.Count -ne 24) {
    throw "Expected 24 diagnostic ESO output rows, found $($diagnosticRows.Count)"
}
...
if ($nodeFlow[0].level -ne "diagnostic") {
    throw "System Node Mass Flow Rate must remain diagnostic-level in the report-energy candidate"
}
...
if ($energyRows.Count -ne 8) {
    throw "Expected 8 IdealLoads energy rows, found $($energyRows.Count)"
}
if (@($energyRows | Where-Object { $_.units -ne "J" -or $_.status -ne "pass" }).Count -ne 0) {
    throw "IdealLoads energy rows must be passing joule rows"
}
$reportEnergyRows = @($energyRows | Where-Object { $_.variable -notlike "*Fuel Energy" })
if (@($reportEnergyRows | Where-Object { $_.level -ne "conformance" -or $_.rust_source -ne "rust-ideal-loads-report-time-step-energy" }).Count -ne 0) {
    throw "Non-fuel ReportPurchasedAir energy rows must be conformance-level TimeStepSysSec rows"
}
$fuelEnergyRows = @($energyRows | Where-Object { $_.variable -like "*Fuel Energy" })
if (@($fuelEnergyRows | Where-Object { $_.level -ne "diagnostic" -or $_.rust_source -ne "rust-ideal-loads-blank-fuel-efficiency-time-step-energy" }).Count -ne 0) {
...
if (@($meterRows | Where-Object { $_.level -ne "diagnostic" -or $_.status -ne "pass" }).Count -ne 0) {
    throw "All IdealLoads facility meter diagnostic rows must pass"
}
...
if ($stageSummary.energy_output_level_policy -ne "conformance for declared no-OA non-fuel ReportPurchasedAir energy rows only") {
...
Assert-Contains -Text $reportText -Pattern "claim_boundary: conformance no-OA ReportPurchasedAir rate-to-TimeStepSysSec energy for declared non-fuel energy rows only" -Description "markdown claim boundary"
...
Assert-Contains -Text $reportText -Pattern "| ZONE ONE IDEAL LOADS | Zone Ideal Loads Supply Air Total Heating Energy | conformance" -Description "markdown conformance supply heating energy row"
Assert-Contains -Text $reportText -Pattern "| ZONE ONE IDEAL LOADS | Zone Ideal Loads Zone Total Cooling Energy | conformance" -Description "markdown conformance zone cooling energy row"
...
Write-Host "IdealLoads no-OA ReportPurchasedAir energy conformance comparison artifacts generated."
