...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-constant-fuel-efficiency-conformance\26.1.0"
$CaseId = "ideal_loads_constant_fuel_efficiency_conformance_candidate_001"
...
Write-Host "Generating IdealLoads constant fuel-efficiency conformance comparison artifacts."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "IdealLoads constant fuel-efficiency conformance comparison failed."
}
...
if ($summary.conformance_claim -ne $true) {
    throw "IdealLoads constant fuel-efficiency conformance summary must set conformance_claim=true"
}
if ($summary.status -ne "pass") {
    throw "Unexpected IdealLoads constant fuel-efficiency conformance status: $($summary.status)"
}
...
if ($summary.tolerance_failures -ne 0) {
    throw "IdealLoads constant fuel-efficiency conformance should have zero tolerance failures: $($summary.tolerance_failures)"
}
...
if ($summary.fuel_energy_rate_source -ne "EnergyPlus ReportPurchasedAir constant Schedule:Constant fuel-efficiency schedule branch") {
...
if ($summary.zone_demand_synthetic_rc_model -ne $false) {
    throw "IdealLoads constant fuel-efficiency conformance must not synthesize zone demand from an RC shortcut"
}
...
if ($summary.rust_meter_time_series_comparison -ne $true) {
    throw "IdealLoads constant fuel-efficiency conformance must compare Rust hourly facility meter diagnostics"
}
...
if ($summary.fuel_energy_output_level_policy -ne "conformance for declared no-OA constant Schedule:Constant fuel-efficiency rows only") {
...
if ($conformanceRows.Count -ne 8) {
    throw "Expected 8 conformance-level fuel-efficiency rows, found $($conformanceRows.Count)"
}
if (@($conformanceRows | Where-Object { $_.status -ne "pass" }).Count -ne 0) {
    throw "All IdealLoads constant fuel-efficiency conformance rows must pass"
}
...
if (@($diagnosticRows | Where-Object { $_.status -ne "pass" }).Count -ne 0) {
    throw "All IdealLoads constant fuel-efficiency diagnostic rows must pass"
}
...
if (@($rateRows | Where-Object { $_.level -ne "conformance" -or $_.rust_source -ne "rust-ideal-loads-constant-fuel-efficiency" }).Count -ne 0) {
    throw "Fuel energy-rate rows must be conformance rows using the constant fuel-efficiency Rust source"
}
...
if (@($fuelEnergyRows | Where-Object { $_.level -ne "conformance" -or $_.rust_source -ne "rust-ideal-loads-constant-fuel-efficiency-time-step-energy" }).Count -ne 0) {
    throw "Fuel energy rows must be conformance rows using the constant fuel-efficiency TimeStepSysSec source"
}
...
if ($stageSummary.fuel_energy_output_level_policy -ne "conformance for declared no-OA constant Schedule:Constant fuel-efficiency rows only") {
...
Assert-Contains -Text $reportText -Pattern "claim_boundary: conformance no-OA constant Schedule:Constant IdealLoads fuel-efficiency for declared fuel-energy rows only" -Description "markdown claim boundary"
...
Assert-Contains -Text $reportText -Pattern "fuel_energy_rate_source: EnergyPlus ReportPurchasedAir constant Schedule:Constant fuel-efficiency schedule branch" -Description "markdown fuel source"
...
Assert-Contains -Text $reportText -Pattern "fuel_energy_output_level_policy: conformance for declared no-OA constant Schedule:Constant fuel-efficiency rows only" -Description "markdown fuel energy output level policy"
...
Write-Host "IdealLoads constant fuel-efficiency conformance comparison artifacts generated."
