...
if ($diagnosticRows.Count -ne 13) {
    throw "Expected 13 diagnostic proof rows, found $($diagnosticRows.Count)"
}
...
Assert-Contains -Text $reportText -Pattern "claim_boundary: conformance no-OA ConstantSupplyHumidityRatio heating IdealLoads branch for declared variables, ReportPurchasedAir energy rows, blank fuel-efficiency rows, and hourly facility meters only" -Description "markdown claim boundary"
