...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-outdoor-air-differential-enthalpy-economizer\26.1.0"
$CaseId = "ideal_loads_outdoor_air_differential_enthalpy_economizer_diagnostic_001"
...
Write-Host "Generating IdealLoads outdoor-air differential enthalpy economizer diagnostic comparison artifacts."
...
if ($economizerActiveRow[0].max_abs_delta -gt 0.000000001 -or $economizerActiveRow[0].rmse_delta -gt 0.000000001) {
    throw "Differential enthalpy economizer active-time row exceeded diagnostic tolerance: max_abs=$($economizerActiveRow[0].max_abs_delta) rmse=$($economizerActiveRow[0].rmse_delta)"
}
...
Assert-Contains -Text $reportText -Pattern "| ZONE ONE IDEAL LOADS | Zone Ideal Loads Economizer Active Time | diagnostic" -Description "markdown differential enthalpy economizer active-time row"
...
Write-Host "IdealLoads outdoor-air differential enthalpy economizer diagnostic comparison artifacts generated."
