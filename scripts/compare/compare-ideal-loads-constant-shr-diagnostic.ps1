...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-constant-shr\26.1.0"
$CaseId = "ideal_loads_constant_shr_diagnostic_001"
...
foreach ($path in @(
    ...
    Assert-FileExists -Path $path -Description "required IdealLoads constant-SHR compare input"
}
...
Write-Host "Generating IdealLoads constant-SHR diagnostic comparison artifacts."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "IdealLoads constant-SHR diagnostic comparison failed."
}
...
Assert-FileExists -Path $summaryPath -Description "IdealLoads constant-SHR compare summary"
Assert-FileExists -Path $reportPath -Description "IdealLoads constant-SHR markdown report"
Assert-FileExists -Path $resultStorePath -Description "IdealLoads constant-SHR Rust result store"
Assert-FileExists -Path $toleranceFailuresPath -Description "IdealLoads constant-SHR tolerance failures CSV"
Assert-FileExists -Path $stageSummaryPath -Description "IdealLoads constant-SHR stage summary"
...
if ($summary.conformance_claim -ne $false) {
    throw "IdealLoads constant-SHR diagnostic must not set conformance_claim=true"
}
...
if (@($summary.series | Where-Object { $_.level -ne "diagnostic" }).Count -ne 0) {
    throw "All constant-SHR diagnostic rows must remain diagnostic-level"
}
if (@($summary.series | Where-Object { $_.status -ne "pass" }).Count -ne 0) {
    throw "All constant-SHR diagnostic rows must pass"
}
...
foreach ($variable in @(
    ...
    if ($rows.Count -lt 1) {
        throw "Missing expected constant-SHR summary row: $variable"
    }
...
Write-Host "IdealLoads constant-SHR diagnostic comparison passed."
