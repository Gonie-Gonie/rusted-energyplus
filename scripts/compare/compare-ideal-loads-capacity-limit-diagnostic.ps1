...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-capacity-limit\26.1.0"
$CaseId = "ideal_loads_capacity_limit_diagnostic_001"
...
Write-Host "Generating IdealLoads no-OA capacity-limit diagnostic comparison artifacts."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "IdealLoads no-OA capacity-limit diagnostic comparison failed."
}
...
if ($summary.conformance_claim -ne $false) {
    throw "IdealLoads capacity-limit diagnostic summary must set conformance_claim=false"
}
...
if ($summary.zone_demand_synthetic_rc_model -ne $false) {
    throw "IdealLoads capacity-limit diagnostic must not synthesize zone demand from an RC shortcut"
}
...
if ($resultStore.series_count -ne 16 -or $resultStore.sample_count -ne 188) {
...
Write-Host "IdealLoads no-OA capacity-limit diagnostic comparison artifacts generated."
