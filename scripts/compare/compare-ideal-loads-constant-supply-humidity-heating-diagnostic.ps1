...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-constant-supply-humidity-heating\26.1.0"
$CaseId = "ideal_loads_constant_supply_humidity_heating_diagnostic_001"
...
foreach ($path in @(
    ...
    Assert-FileExists -Path $path -Description "required IdealLoads constant-supply-humidity heating compare input"
}
...
Write-Host "Generating IdealLoads constant-supply-humidity heating diagnostic comparison artifacts."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "IdealLoads constant-supply-humidity heating diagnostic comparison failed."
}
...
Assert-FileExists -Path $summaryPath -Description "IdealLoads constant-supply-humidity heating compare summary"
Assert-FileExists -Path $reportPath -Description "IdealLoads constant-supply-humidity heating markdown report"
Assert-FileExists -Path $resultStorePath -Description "IdealLoads constant-supply-humidity heating Rust result store"
Assert-FileExists -Path $toleranceFailuresPath -Description "IdealLoads constant-supply-humidity heating tolerance failures CSV"
Assert-FileExists -Path $stageSummaryPath -Description "IdealLoads constant-supply-humidity heating stage summary"
...
if ($summary.conformance_claim -ne $false) {
    throw "IdealLoads constant-supply-humidity heating diagnostic must not set conformance_claim=true"
}
...
if (@($summary.series | Where-Object { $_.level -ne "diagnostic" }).Count -ne 0) {
    throw "All constant-supply-humidity heating diagnostic rows must remain diagnostic-level"
}
if (@($summary.series | Where-Object { $_.status -ne "pass" }).Count -ne 0) {
    throw "All constant-supply-humidity heating diagnostic rows must pass"
}
...
Write-Host "IdealLoads constant-supply-humidity heating diagnostic comparison passed."
