...
Assert-Contains -Text $reportText -Pattern "claim_boundary: conformance no-OA Humidistat humidification IdealLoads branch for declared heating/cooling rate rows, supply-node rows, moisture-demand rows, ReportPurchasedAir energy rows, blank fuel-efficiency rows, and hourly/monthly/run-period facility meters only" -Description "markdown claim boundary"
