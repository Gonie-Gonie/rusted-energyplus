...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-humidistat-humidification-conformance\26.1.0"
$CaseId = "ideal_loads_humidistat_humidification_conformance_candidate_001"
...
foreach ($path in @(
    ...
    Assert-FileExists -Path $path -Description "required IdealLoads humidistat-humidification conformance input"
}
...
Write-Host "Generating IdealLoads humidistat-humidification conformance candidate artifacts."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "IdealLoads humidistat-humidification conformance candidate comparison failed."
}
...
Assert-FileExists -Path $summaryPath -Description "IdealLoads humidistat-humidification conformance compare summary"
Assert-FileExists -Path $reportPath -Description "IdealLoads humidistat-humidification conformance markdown report"
Assert-FileExists -Path $resultStorePath -Description "IdealLoads humidistat-humidification conformance Rust result store"
Assert-FileExists -Path $toleranceFailuresPath -Description "IdealLoads humidistat-humidification conformance tolerance failures CSV"
Assert-FileExists -Path $stageSummaryPath -Description "IdealLoads humidistat-humidification conformance stage summary"
...
if ($summary.conformance_claim -ne $true) {
    throw "IdealLoads humidistat-humidification conformance summary must set conformance_claim=true"
}
if ($summary.status -ne "pass") {
    throw "Unexpected IdealLoads humidistat-humidification status: $($summary.status)"
}
...
if ($summary.series_count -ne 38) {
    throw "Unexpected IdealLoads humidistat-humidification series count: $($summary.series_count)"
}
...
foreach ($expected in @(
    ...
    @("ZONE ONE", "Zone System Predicted Moisture Load to Humidifying Setpoint Moisture Transfer Rate", "diagnostic"),
...
foreach ($variable in @(
    ...
    if ($rows.Count -lt 1) {
        throw "Missing expected humidistat-humidification summary row: $variable"
    }
...
$zoneTotalHeating = Get-ResultSeries -ResultStore $resultStore -Variable "Zone Ideal Loads Zone Total Heating Rate"
$zoneSensibleHeating = Get-ResultSeries -ResultStore $resultStore -Variable "Zone Ideal Loads Zone Sensible Heating Rate"
$zoneLatentHeating = Get-ResultSeries -ResultStore $resultStore -Variable "Zone Ideal Loads Zone Latent Heating Rate"
$supplyTotalHeating = Get-ResultSeries -ResultStore $resultStore -Variable "Zone Ideal Loads Supply Air Total Heating Rate"
$supplySensibleHeating = Get-ResultSeries -ResultStore $resultStore -Variable "Zone Ideal Loads Supply Air Sensible Heating Rate"
$supplyLatentHeating = Get-ResultSeries -ResultStore $resultStore -Variable "Zone Ideal Loads Supply Air Latent Heating Rate"
$humidifyingMoistureDemand = Get-ResultSeries -ResultStore $resultStore -Key "ZONE ONE" -Variable "Zone System Predicted Moisture Load to Humidifying Setpoint Moisture Transfer Rate"
...
$maxZoneTotalHeating = ($zoneTotalHeating.values | Measure-Object -Maximum).Maximum
$maxZoneSensibleHeating = ($zoneSensibleHeating.values | Measure-Object -Maximum).Maximum
$maxZoneLatentHeating = ($zoneLatentHeating.values | Measure-Object -Maximum).Maximum
$maxSupplyTotalHeating = ($supplyTotalHeating.values | Measure-Object -Maximum).Maximum
$maxSupplySensibleHeating = ($supplySensibleHeating.values | Measure-Object -Maximum).Maximum
$maxSupplyLatentHeating = ($supplyLatentHeating.values | Measure-Object -Maximum).Maximum
$maxHumidifyingMoistureDemand = ($humidifyingMoistureDemand.values | Measure-Object -Maximum).Maximum
...
if ($maxZoneTotalHeating -le 0.0) {
    throw "Expected active zone total heating in humidistat-humidification conformance candidate"
}
if ($maxZoneSensibleHeating -le 0.0) {
    throw "Expected active zone sensible heating in humidistat-humidification conformance candidate"
}
if ($maxZoneLatentHeating -le 0.0) {
    throw "Expected active zone latent heating in humidistat-humidification conformance candidate"
}
if ($maxSupplyTotalHeating -le 0.0) {
    throw "Expected active supply-air total heating in humidistat-humidification conformance candidate"
}
if ($maxSupplySensibleHeating -le 0.0) {
    throw "Expected active supply-air sensible heating in humidistat-humidification conformance candidate"
}
if ($maxSupplyLatentHeating -le 0.0) {
    throw "Expected active supply-air latent heating in humidistat-humidification conformance candidate"
}
if ($maxHumidifyingMoistureDemand -le 1.0e-8) {
    throw "Expected active positive humidifying moisture demand in humidistat-humidification conformance candidate"
}
if ($maxSupplyHumidity -lt 0.014) {
    throw "Expected humidistat-humidification to raise supply humidity ratio"
}
if ($maxSupplyMinusReturnHumidity -le 1.0e-8) {
    throw "Expected supply humidity ratio to exceed return humidity ratio during humidification"
}
...
if ($stageSummary.selected_purchased_air_branch -ne "humidistat_humidification") {
    throw "Unexpected PurchasedAir branch: $($stageSummary.selected_purchased_air_branch)"
}
if ($stageSummary.declared_ideal_loads_branch -ne "humidistat_humidification") {
...
Assert-Contains -Text $reportText -Pattern "claim_boundary: conformance no-OA Humidistat humidification IdealLoads branch for declared heating/cooling rate rows, supply-node rows, ReportPurchasedAir energy rows, blank fuel-efficiency rows, and hourly/monthly/run-period facility meters only" -Description "markdown claim boundary"
...
Assert-Contains -Text $reportText -Pattern "selected_purchased_air_branch: humidistat_humidification" -Description "markdown PurchasedAir branch"
Assert-Contains -Text $reportText -Pattern "declared_ideal_loads_branch: humidistat_humidification" -Description "markdown declared branch"
Assert-Contains -Text $reportText -Pattern "inactive_branches: outdoor_air, economizer, heat_recovery, dcv, autosizing, saturation_limit" -Description "markdown inactive branches"
...
Assert-Contains -Text $reportText -Pattern "| ZONE ONE | Zone System Predicted Moisture Load to Humidifying Setpoint Moisture Transfer Rate | diagnostic" -Description "markdown humidifying moisture demand proof row"
...
Write-Host "IdealLoads humidistat-humidification conformance candidate artifacts generated."
