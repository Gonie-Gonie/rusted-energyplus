...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-blank-fuel-efficiency-conformance\26.1.0"
$CaseId = "ideal_loads_blank_fuel_efficiency_conformance_candidate_001"
...
Write-Host "Generating IdealLoads blank fuel-efficiency conformance comparison artifacts."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "IdealLoads blank fuel-efficiency conformance comparison failed."
}
...
if ($summary.conformance_claim -ne $true) {
    throw "IdealLoads blank fuel-efficiency conformance summary must set conformance_claim=true"
}
if ($summary.status -ne "pass") {
    throw "Unexpected IdealLoads blank fuel-efficiency conformance status: $($summary.status)"
}
...
if ($summary.tolerance_failures -ne 0) {
    throw "IdealLoads blank fuel-efficiency conformance should have zero tolerance failures: $($summary.tolerance_failures)"
}
...
if ([Math]::Abs([double]$summary.heating_fuel_efficiency - 1.0) -gt 1.0e-12) {
    throw "Unexpected heating fuel efficiency: $($summary.heating_fuel_efficiency)"
}
if ([Math]::Abs([double]$summary.cooling_fuel_efficiency - 1.0) -gt 1.0e-12) {
    throw "Unexpected cooling fuel efficiency: $($summary.cooling_fuel_efficiency)"
}
if ($summary.fuel_energy_rate_source -ne "EnergyPlus ReportPurchasedAir blank fuel-efficiency schedule branch") {
    throw "Unexpected fuel energy rate source: $($summary.fuel_energy_rate_source)"
}
if ($summary.fuel_energy_rate_rust_source -ne "rust-ideal-loads-blank-fuel-efficiency") {
    throw "Unexpected fuel energy rate Rust source: $($summary.fuel_energy_rate_rust_source)"
}
if ($summary.fuel_energy_rust_source -ne "rust-ideal-loads-blank-fuel-efficiency-time-step-energy") {
    throw "Unexpected fuel energy Rust source: $($summary.fuel_energy_rust_source)"
}
if ($summary.fuel_energy_output_level_policy -ne "conformance for declared no-OA blank fuel-efficiency rows only") {
...
if ($summary.zone_demand_synthetic_rc_model -ne $false) {
    throw "IdealLoads blank fuel-efficiency conformance must not synthesize zone demand from an RC shortcut"
}
...
if (@($meterRows | Where-Object { $_.status -ne "pass" }).Count -ne 0) {
    throw "All IdealLoads blank fuel-efficiency facility meter diagnostics must pass"
}

$conformanceRows = @($summary.series | Where-Object { $_.level -eq "conformance" })
if ($conformanceRows.Count -ne 8) {
    throw "Expected 8 conformance-level blank fuel-efficiency rows, found $($conformanceRows.Count)"
}
if (@($conformanceRows | Where-Object { $_.status -ne "pass" }).Count -ne 0) {
    throw "All IdealLoads blank fuel-efficiency conformance rows must pass"
}
...
if (@($diagnosticRows | Where-Object { $_.status -ne "pass" }).Count -ne 0) {
    throw "All IdealLoads blank fuel-efficiency diagnostic rows must pass"
}
...
if (@($rateRows | Where-Object { $_.level -ne "conformance" -or $_.rust_source -ne "rust-ideal-loads-blank-fuel-efficiency" }).Count -ne 0) {
    throw "Fuel energy-rate rows must be conformance rows using the blank fuel-efficiency Rust source"
}
...
if (@($fuelEnergyRows | Where-Object { $_.level -ne "conformance" -or $_.rust_source -ne "rust-ideal-loads-blank-fuel-efficiency-time-step-energy" }).Count -ne 0) {
    throw "Fuel energy rows must be conformance rows using the blank fuel-efficiency TimeStepSysSec source"
}
...
if ([Math]::Abs([double]$stageSummary.heating_fuel_efficiency - 1.0) -gt 1.0e-12) {
    throw "Unexpected stage heating fuel efficiency: $($stageSummary.heating_fuel_efficiency)"
}
if ([Math]::Abs([double]$stageSummary.cooling_fuel_efficiency - 1.0) -gt 1.0e-12) {
    throw "Unexpected stage cooling fuel efficiency: $($stageSummary.cooling_fuel_efficiency)"
}
if ($stageSummary.fuel_energy_output_level_policy -ne "conformance for declared no-OA blank fuel-efficiency rows only") {
...
Assert-Contains -Text $reportText -Pattern "claim_boundary: conformance no-OA blank IdealLoads fuel-efficiency for declared fuel-energy rows only" -Description "markdown claim boundary"
...
Assert-Contains -Text $reportText -Pattern "fuel_energy_rate_source: EnergyPlus ReportPurchasedAir blank fuel-efficiency schedule branch" -Description "markdown fuel source"
Assert-Contains -Text $reportText -Pattern "fuel_efficiency: heating=1.000000000000 cooling=1.000000000000" -Description "markdown fuel efficiency values"
Assert-Contains -Text $reportText -Pattern "fuel_energy_output_level_policy: conformance for declared no-OA blank fuel-efficiency rows only" -Description "markdown fuel energy output level policy"
...
Write-Host "IdealLoads blank fuel-efficiency conformance comparison artifacts generated."
