...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-outdoor-air-occupancy-dcv-conformance\26.1.0"
$CaseId = "ideal_loads_outdoor_air_occupancy_dcv_conformance_candidate_001"
...
foreach ($path in @(
    ...
    Assert-FileExists -Path $path -Description "required IdealLoads outdoor-air OccupancySchedule DCV conformance input"
}
...
Write-Host "Generating IdealLoads outdoor-air OccupancySchedule DCV conformance candidate artifacts."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "IdealLoads outdoor-air OccupancySchedule DCV conformance candidate comparison failed."
}
...
Assert-FileExists -Path $summaryPath -Description "IdealLoads outdoor-air OccupancySchedule DCV conformance compare summary"
Assert-FileExists -Path $reportPath -Description "IdealLoads outdoor-air OccupancySchedule DCV conformance markdown report"
Assert-FileExists -Path $selectedOutputsPath -Description "IdealLoads outdoor-air OccupancySchedule DCV conformance selected outputs"
Assert-FileExists -Path $resultStorePath -Description "IdealLoads outdoor-air OccupancySchedule DCV conformance Rust result store"
Assert-FileExists -Path $variableDeltasPath -Description "IdealLoads outdoor-air OccupancySchedule DCV conformance variable deltas"
Assert-FileExists -Path $firstDivergencePath -Description "IdealLoads outdoor-air OccupancySchedule DCV conformance first divergence CSV"
Assert-FileExists -Path $toleranceFailuresPath -Description "IdealLoads outdoor-air OccupancySchedule DCV conformance tolerance failures CSV"
Assert-FileExists -Path $stageSummaryPath -Description "IdealLoads outdoor-air OccupancySchedule DCV conformance stage summary"
...
if ($summary.conformance_claim -ne $true) {
    throw "IdealLoads outdoor-air OccupancySchedule DCV conformance summary must set conformance_claim=true"
}
if ($summary.status -ne "pass") {
    throw "Unexpected IdealLoads outdoor-air OccupancySchedule DCV conformance status: $($summary.status)"
}
if ($summary.tolerance_failures -ne 0) {
    throw "IdealLoads outdoor-air OccupancySchedule DCV conformance should have zero tolerance failures: $($summary.tolerance_failures)"
}
...
if ($summary.demand_controlled_ventilation_type -ne "OccupancySchedule") {
    throw "Summary must record OccupancySchedule DCV, got $($summary.demand_controlled_ventilation_type)"
}
if ($summary.current_people_count_min -ne 0.0 -or $summary.current_people_count_max -ne 5.0) {
    throw "Unexpected current people count range: min=$($summary.current_people_count_min) max=$($summary.current_people_count_max)"
}
if ($summary.outdoor_air_mass_flow_rate_min_kg_per_s -ne 0.0) {
    throw "OccupancySchedule DCV should reach zero minimum OA mass flow, got $($summary.outdoor_air_mass_flow_rate_min_kg_per_s)"
}
if ([Math]::Abs($summary.outdoor_air_mass_flow_rate_max_kg_per_s - $summary.outdoor_air_mass_flow_rate_kg_per_s) -gt 0.000000001) {
    throw "OccupancySchedule DCV max OA mass flow should match design mass flow: max=$($summary.outdoor_air_mass_flow_rate_max_kg_per_s) design=$($summary.outdoor_air_mass_flow_rate_kg_per_s)"
}
...
if ($conformanceRows.Count -ne 22) {
    throw "Expected twenty-two outdoor-air OccupancySchedule DCV conformance rows, found $($conformanceRows.Count)"
}
if (@($conformanceRows | Where-Object { $_.status -ne "pass" }).Count -ne 0) {
    throw "All outdoor-air OccupancySchedule DCV conformance rows must pass"
}
...
if ($massRow.Count -ne 1 -or $massRow[0].units -ne "kg/s" -or $massRow[0].rust_source -ne "rust-ideal-loads-outdoor-air-occupancy-schedule-dcv") {
    throw "Missing kg/s OccupancySchedule DCV outdoor-air mass-flow row"
}
$volumeRow = @($rows | Where-Object { $_.variable -eq "Zone Ideal Loads Outdoor Air Standard Density Volume Flow Rate" })
if ($volumeRow.Count -ne 1 -or $volumeRow[0].units -ne "m3/s" -or $volumeRow[0].rust_source -ne "rust-ideal-loads-outdoor-air-occupancy-schedule-dcv") {
    throw "Missing m3/s OccupancySchedule DCV outdoor-air standard-density volume-flow row"
}
...
if ($stageSummary.demand_controlled_ventilation_type -ne "OccupancySchedule") {
    throw "Stage summary must record OccupancySchedule DCV, got $($stageSummary.demand_controlled_ventilation_type)"
}
...
if ($stageSummary.current_people_count_min -ne 0.0 -or $stageSummary.current_people_count_max -ne 5.0) {
    throw "Stage summary must record current people range 0..5, got min=$($stageSummary.current_people_count_min) max=$($stageSummary.current_people_count_max)"
}
...
Assert-Contains -Text $reportText -Pattern "current_people_count_min: 0.000000000000000" -Description "markdown current people minimum"
...
Write-Host "IdealLoads outdoor-air OccupancySchedule DCV conformance candidate artifacts generated."
