...
$CaseId = "calendar_epw_dst_southern_wrap_start_hourly_exact_001"
...
$IdfName = "calendar_epw_dst_southern_wrap_start_hourly_exact.idf"
$WeatherName = "calendar_epw_dst_southern_wrap_start_hourly_exact.epw"
...
$ExpectedHeader = "HOLIDAYS/DAYLIGHT SAVINGS,Yes,Last Sunday in October,Last Sunday in March,0"
$ExpectedDataPeriod = "DATA PERIODS,1,1,Data,Saturday,10/30,11/1"
$ExpectedFirstTimestamp = "env=CALENDAR EPW DST SOUTHERN WRAP START RUN PERIOD;day=1;month=10;date=30;dst=0;hour=1;start=0.00;end=60.00;day_type=Saturday"
$ExpectedLastTimestamp = "env=CALENDAR EPW DST SOUTHERN WRAP START RUN PERIOD;day=3;month=11;date=1;dst=1;hour=24;start=0.00;end=60.00;day_type=Monday"
$ExpectedMonths = @(10, 10, 11)
$ExpectedDates = @(30, 31, 1)
$ExpectedDst = @(0, 1, 1)
$ExpectedDayTypes = @("Saturday", "Sunday", "Monday")
...
foreach ($path in @(
    ...
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Missing required EPW southern-wrap daylight-saving conformance file: $path"
    }
}
...
if ($weatherLines.Count -ne 80 -or
    @($weatherLines | Where-Object { [string]::IsNullOrWhiteSpace($_) }).Count -ne 0) {
    throw "EPW southern-wrap daylight-saving fixture must contain exactly eight headers and 72 nonblank data rows"
}
...
Assert-Contains -Text $caseText -Pattern 'script = "scripts/dev.cmd compare-calendar-epw-dst-southern-wrap-start-exact"' -Description "manifest gate attribution"
...
if ($calendarHeaders.Count -ne 1 -or $calendarHeaders[0] -cne $ExpectedHeader) {
    throw "EPW must contain exactly the expected last-Sunday October through last-Sunday March daylight-saving header"
}
...
if ($dataPeriodHeaders.Count -ne 1 -or $dataPeriodHeaders[0] -cne $ExpectedDataPeriod) {
    throw "EPW must contain exactly the expected three-day Saturday-start DATA PERIODS header"
}
$weatherRows = @($weatherLines | Select-Object -Skip 8 | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
if ($weatherRows.Count -ne 72) {
    throw "EPW southern-wrap daylight-saving fixture must contain 72 hourly rows, found $($weatherRows.Count)"
}
$orderedPayloads = @()
for ($rowIndex = 0; $rowIndex -lt 72; ++$rowIndex) {
    ...
    if ($fields.Count -ne 35 -or $fields[0] -ne "2032" -or [int]$fields[1] -ne $ExpectedMonths[$dayIndex] -or
        [int]$fields[2] -ne $ExpectedDates[$dayIndex] -or [int]$fields[3] -ne $expectedHour -or
        $fields[4] -ne "60") {
        throw "EPW row $rowIndex must retain the exact 2032-10-30 through 2032-11-01 date/hour order"
    }
    $orderedPayloads += ($fields[5..34] -join ',')
}
if (@($orderedPayloads | Select-Object -Unique).Count -ne 1) {
    throw "EPW southern-wrap daylight-saving weather payload must remain constant across all 72 ordered rows"
}
$weatherPayloads = @()
for ($dayIndex = 0; $dayIndex -lt $ExpectedDates.Count; ++$dayIndex) {
    $day = $ExpectedDates[$dayIndex]
    $month = $ExpectedMonths[$dayIndex]
    $date = "2032,$month,$day"
    $dateRows = @($weatherRows | Where-Object { $_ -match ('^' + [regex]::Escape($date) + ',') })
    if ($dateRows.Count -ne 24) {
        throw "EPW southern-wrap daylight-saving fixture must contain 24 rows for $date, found $($dateRows.Count)"
    }
    $hours = @($dateRows | ForEach-Object { [int](($_ -split ',')[3]) })
    if (($hours -join ',') -cne ((1..24) -join ',')) {
        throw "EPW southern-wrap daylight-saving fixture must contain ordered hours 1..24 for $date"
    }
    foreach ($row in $dateRows) {
        $fields = $row -split ','
        if ($fields.Count -ne 35 -or $fields[0] -ne "2032" -or [int]$fields[1] -ne $ExpectedMonths[$dayIndex] -or
            [int]$fields[2] -ne $day -or $fields[4] -ne "60") {
            throw "EPW southern-wrap daylight-saving rows must contain the expected date, 35 fields, and minute 60"
        }
...
if ($weatherPayloads.Count -ne 1) {
    throw "EPW southern-wrap daylight-saving weather payload must remain constant across all 72 rows"
}
...
Assert-Contains -Text $idfText -Pattern "Calendar EPW DST Southern Wrap Start Run Period" -Description "RunPeriod identity"
...
if ($runPeriodObjects.Count -ne 1 -or
    ...
    throw "EPW southern-wrap daylight-saving fixture must contain exactly one RunPeriod and one Output:Variable"
}
...
$expectedRunPeriodFields = @(
    "Calendar EPW DST Southern Wrap Start Run Period",
    "10", "30", "2032", "11", "1", "2032", "Saturday",
    "No", "Yes", "No", "No", "No", "No"
)
if (($runPeriodFields -join '|') -cne ($expectedRunPeriodFields -join '|')) {
    throw "EPW southern-wrap daylight-saving fixture must retain the exact 2032-10-30 through 2032-11-01 Saturday RunPeriod and explicit policies"
}
...
if (($outputFields -join '|') -cne "Environment|Site Daylight Saving Time Status|Hourly") {
    throw "EPW southern-wrap daylight-saving fixture must request exactly the hourly Site Daylight Saving Time Status"
}
if ([regex]::Matches($idfText, '(?im)^\s*RunPeriodControl:SpecialDays\s*,').Count -ne 0) {
    throw "EPW southern-wrap daylight-saving fixture must not contain RunPeriodControl:SpecialDays"
}
if ([regex]::Matches($idfText, '(?im)^\s*RunPeriodControl:DaylightSavingTime\s*,').Count -ne 0) {
    throw "EPW southern-wrap daylight-saving fixture must not contain RunPeriodControl:DaylightSavingTime"
}
...
Write-Host "Running exact EPW last-Sunday October through last-Sunday March start-side daylight-saving gate."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "EPW southern-wrap daylight-saving rule case failed: $CaseId"
}
...
foreach ($path in @($summaryPath, $reportPath, $oracleEsoPath, $oracleEioPath, $oracleErrPath, $oracleEndPath)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Missing EPW southern-wrap daylight-saving comparison artifact: $path"
    }
}
...
if ($summary.case_id -cne $CaseId -or $summary.oracle_version -cne "26.1.0" -or
    ...
    $summary.gate.script -cne "scripts/dev.cmd compare-calendar-epw-dst-southern-wrap-start-exact" -or
    $summary.gate.blocking -ne $true) {
    throw "EPW southern-wrap daylight-saving case must retain its exact passing single-series conformance and blocking-gate contract"
}
...
if ($calendar.policy_applied -ne $true -or $calendar.weather_file_allows_leap_years -ne $true -or
    ...
    throw "Unexpected three-day leap-year weather calendar state"
}
...
if ($daylightSaving.weather_file_period_declared -ne $true -or
    ...
    throw "EPW southern-wrap daylight-saving declaration and RunPeriod policy must be active"
}
$resolvedPeriod = $daylightSaving.resolved_period
if ($resolvedPeriod.start_month -ne 10 -or $resolvedPeriod.start_day -ne 31 -or
    $resolvedPeriod.start_day_of_year -ne 305 -or
    $resolvedPeriod.end_month -ne 3 -or $resolvedPeriod.end_day -ne 28 -or
    $resolvedPeriod.end_day_of_year -ne 88 -or $resolvedPeriod.wraps_year -ne $true) {
    throw "Unexpected resolved last-Sunday October through last-Sunday March daylight-saving period"
}
if ($calendar.daylight_saving_hourly_samples -ne 48) {
    throw "Expected 48 DST-active hourly samples, found $($calendar.daylight_saving_hourly_samples)"
}
...
if ($specialDays.weather_file_declared -ne 0 -or $specialDays.run_period_uses_weather_file -ne $false -or
    ...
    throw "Southern-wrap daylight-saving fixture must not activate special days"
}
...
if ($values.Count -ne 72 -or $timestampRows.Count -ne 72) {
    throw "Expected 72 oracle daylight-saving values and timestamps"
}
...
if ($daylightSavingEioRows.Count -ne 1 -or
    [string]$daylightSavingEioRows[0] -cne "Environment:Daylight Saving,Yes,WeatherFile,10/31,03/28") {
...
if ([regex]::Matches($oracleErrText, '(?m)^\s*\*\* Warning \*\*').Count -ne 0 -or
    ...
    throw "EPW southern-wrap daylight-saving oracle must complete without warning or severe markers"
}
...
foreach ($entry in @(
    ...
    Assert-Contains -Text $reportText -Pattern $entry -Description "markdown southern-wrap daylight-saving state"
}

Write-Host "Exact EPW last-Sunday October through last-Sunday March start-side daylight-saving gate passed."
