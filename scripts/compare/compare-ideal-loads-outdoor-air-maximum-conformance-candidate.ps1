...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-outdoor-air-maximum-conformance\26.1.0"
$CaseId = "ideal_loads_outdoor_air_maximum_conformance_candidate_001"
...
foreach ($path in @(
    ...
    Assert-FileExists -Path $path -Description "required IdealLoads outdoor-air Maximum conformance input"
}
...
Write-Host "Generating IdealLoads outdoor-air Maximum conformance candidate artifacts."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "IdealLoads outdoor-air Maximum conformance candidate comparison failed."
}
...
Assert-FileExists -Path $summaryPath -Description "IdealLoads outdoor-air Maximum conformance compare summary"
Assert-FileExists -Path $reportPath -Description "IdealLoads outdoor-air Maximum conformance markdown report"
Assert-FileExists -Path $selectedOutputsPath -Description "IdealLoads outdoor-air Maximum conformance selected outputs"
Assert-FileExists -Path $resultStorePath -Description "IdealLoads outdoor-air Maximum conformance Rust result store"
Assert-FileExists -Path $variableDeltasPath -Description "IdealLoads outdoor-air Maximum conformance variable deltas"
Assert-FileExists -Path $firstDivergencePath -Description "IdealLoads outdoor-air Maximum conformance first divergence CSV"
Assert-FileExists -Path $toleranceFailuresPath -Description "IdealLoads outdoor-air Maximum conformance tolerance failures CSV"
Assert-FileExists -Path $stageSummaryPath -Description "IdealLoads outdoor-air Maximum conformance stage summary"
...
if ($summary.conformance_claim -ne $true) {
    throw "IdealLoads outdoor-air Maximum conformance summary must set conformance_claim=true"
}
if ($summary.status -ne "pass") {
    throw "Unexpected IdealLoads outdoor-air Maximum conformance status: $($summary.status)"
}
if ($summary.tolerance_failures -ne 0) {
    throw "IdealLoads outdoor-air Maximum conformance should have zero tolerance failures: $($summary.tolerance_failures)"
}
...
Assert-NearlyEqual -Actual $summary.design_volume_flow_rate_m3_per_s -Expected 0.05 -Tolerance 0.000000000001 -Description "outdoor-air Maximum final design volume flow"
...
if ($conformanceRows.Count -ne 22) {
    throw "Expected twenty-two outdoor-air Maximum conformance rows, found $($conformanceRows.Count)"
}
if (@($conformanceRows | Where-Object { $_.status -ne "pass" }).Count -ne 0) {
    throw "All outdoor-air Maximum conformance rows must pass"
}
...
Assert-Contains -Text $reportText -Pattern "outdoor_air_air_changes_m3_per_s: 0.050000000000000" -Description "markdown AirChanges/Hour component"
Assert-Contains -Text $reportText -Pattern "design_volume_flow_rate_m3_per_s: 0.050000000000000" -Description "markdown Maximum final design volume"
...
Write-Host "IdealLoads outdoor-air Maximum conformance candidate artifacts generated."
