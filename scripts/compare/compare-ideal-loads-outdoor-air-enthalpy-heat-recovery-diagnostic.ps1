...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-outdoor-air-enthalpy-heat-recovery\26.1.0"
$CaseId = "ideal_loads_outdoor_air_enthalpy_heat_recovery_diagnostic_001"
...
Write-Host "Generating IdealLoads outdoor-air enthalpy heat-recovery diagnostic comparison artifacts."
...
if ($supplyHumidityRatioRow[0].max_abs_delta -gt 0.00005 -or $supplyHumidityRatioRow[0].rmse_delta -gt 0.00001) {
...
if ($mixedAirHumidityRatioRow[0].max_abs_delta -gt 0.00005 -or $mixedAirHumidityRatioRow[0].rmse_delta -gt 0.00001) {
...
foreach ($heatRecoveryVariable in @(
    ...
    if ($heatRecoveryRow.Count -ne 1 -or $heatRecoveryRow[0].units -ne "W" -or $heatRecoveryRow[0].rust_source -ne $heatRecoverySource) {
        throw "Missing W enthalpy heat-recovery row: $heatRecoveryVariable"
    }
    if ($heatRecoveryRow[0].max_abs_delta -gt 1.0 -or $heatRecoveryRow[0].rmse_delta -gt 1.0) {
        throw "Enthalpy heat-recovery row exceeded diagnostic tolerance: variable=$heatRecoveryVariable max_abs=$($heatRecoveryRow[0].max_abs_delta) rmse=$($heatRecoveryRow[0].rmse_delta)"
    }
}
foreach ($heatRecoveryVariable in @(
    ...
    if ($heatRecoveryRow[0].max_abs_delta -gt 6.0 -or $heatRecoveryRow[0].rmse_delta -gt 1.0) {
        throw "Enthalpy heat-recovery cooling row exceeded diagnostic tolerance: variable=$heatRecoveryVariable max_abs=$($heatRecoveryRow[0].max_abs_delta) rmse=$($heatRecoveryRow[0].rmse_delta)"
    }
}
foreach ($heatRecoveryVariable in @(
    ...
    $heatRecoveryRow = @($rows | Where-Object { $_.variable -eq $heatRecoveryVariable })
    if ($heatRecoveryRow.Count -ne 1 -or $heatRecoveryRow[0].units -ne "W" -or $heatRecoveryRow[0].rust_source -ne $heatRecoverySource) {
        throw "Missing W enthalpy heat-recovery latent row: $heatRecoveryVariable"
    }
    if ($heatRecoveryRow[0].max_abs_delta -gt 1.0 -or $heatRecoveryRow[0].rmse_delta -gt 1.0) {
        throw "Enthalpy heat-recovery latent row exceeded diagnostic tolerance: variable=$heatRecoveryVariable max_abs=$($heatRecoveryRow[0].max_abs_delta) rmse=$($heatRecoveryRow[0].rmse_delta)"
    }
}
foreach ($heatRecoveryVariable in @(
    ...
    if ($heatRecoveryRow[0].max_abs_delta -gt 6.0 -or $heatRecoveryRow[0].rmse_delta -gt 1.0) {
        throw "Enthalpy heat-recovery latent cooling row exceeded diagnostic tolerance: variable=$heatRecoveryVariable max_abs=$($heatRecoveryRow[0].max_abs_delta) rmse=$($heatRecoveryRow[0].rmse_delta)"
    }
}
...
if ($heatRecoveryActiveRow.Count -ne 1 -or $heatRecoveryActiveRow[0].units -ne "hr" -or $heatRecoveryActiveRow[0].rust_source -ne $heatRecoverySource) {
    throw "Missing hr enthalpy heat-recovery active-time row"
}
if ($heatRecoveryActiveRow[0].max_abs_delta -gt 0.000000001 -or $heatRecoveryActiveRow[0].rmse_delta -gt 0.000000001) {
    throw "Enthalpy heat-recovery active-time row exceeded diagnostic tolerance: max_abs=$($heatRecoveryActiveRow[0].max_abs_delta) rmse=$($heatRecoveryActiveRow[0].rmse_delta)"
}
...
Assert-Contains -Text $reportText -Pattern "claim_boundary: diagnostic-only IdealLoads outdoor-air Flow/Zone mass, standard-density volume, outdoor-air report rates, supply-air state, mixed-air state, and Enthalpy heat recovery active-time/rate parity; DCV, economizer, humidity controls, saturation-limit branches, and broad OA conformance remain outside the claim" -Description "markdown claim boundary"
...
Assert-Contains -Text $reportText -Pattern "| ZONE ONE IDEAL LOADS | Zone Ideal Loads Heat Recovery Sensible Heating Rate | diagnostic" -Description "markdown enthalpy heat-recovery sensible heating row"
Assert-Contains -Text $reportText -Pattern "| ZONE ONE IDEAL LOADS | Zone Ideal Loads Heat Recovery Latent Heating Rate | diagnostic" -Description "markdown enthalpy heat-recovery latent heating row"
Assert-Contains -Text $reportText -Pattern "| ZONE ONE IDEAL LOADS | Zone Ideal Loads Heat Recovery Total Cooling Rate | diagnostic" -Description "markdown enthalpy heat-recovery total cooling row"
...
Assert-Contains -Text $reportText -Pattern "| ZONE ONE IDEAL LOADS | Zone Ideal Loads Heat Recovery Active Time | diagnostic" -Description "markdown enthalpy heat-recovery active-time row"

Write-Host "IdealLoads outdoor-air enthalpy heat-recovery diagnostic comparison artifacts generated."
