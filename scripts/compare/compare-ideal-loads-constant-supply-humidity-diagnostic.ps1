...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-constant-supply-humidity\26.1.0"
$CaseId = "ideal_loads_constant_supply_humidity_diagnostic_001"
...
foreach ($path in @(
    ...
    Assert-FileExists -Path $path -Description "required IdealLoads constant-supply-humidity compare input"
}
...
Write-Host "Generating IdealLoads constant-supply-humidity diagnostic comparison artifacts."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "IdealLoads constant-supply-humidity diagnostic comparison failed."
}
...
Assert-FileExists -Path $summaryPath -Description "IdealLoads constant-supply-humidity compare summary"
Assert-FileExists -Path $reportPath -Description "IdealLoads constant-supply-humidity markdown report"
Assert-FileExists -Path $resultStorePath -Description "IdealLoads constant-supply-humidity Rust result store"
Assert-FileExists -Path $toleranceFailuresPath -Description "IdealLoads constant-supply-humidity tolerance failures CSV"
Assert-FileExists -Path $stageSummaryPath -Description "IdealLoads constant-supply-humidity stage summary"
...
if ($summary.conformance_claim -ne $false) {
    throw "IdealLoads constant-supply-humidity diagnostic must not set conformance_claim=true"
}
...
if (@($summary.series | Where-Object { $_.level -ne "diagnostic" }).Count -ne 0) {
    throw "All constant-supply-humidity diagnostic rows must remain diagnostic-level"
}
if (@($summary.series | Where-Object { $_.status -ne "pass" }).Count -ne 0) {
    throw "All constant-supply-humidity diagnostic rows must pass"
}
...
Write-Host "IdealLoads constant-supply-humidity diagnostic comparison passed."
