...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-outdoor-air-flow-zone-conformance\26.1.0"
$CaseId = "ideal_loads_outdoor_air_flow_zone_conformance_candidate_001"
...
foreach ($path in @(
    ...
    Assert-FileExists -Path $path -Description "required IdealLoads outdoor-air Flow/Zone conformance input"
}
...
Write-Host "Generating IdealLoads outdoor-air Flow/Zone conformance candidate artifacts."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "IdealLoads outdoor-air Flow/Zone conformance candidate comparison failed."
}
...
Assert-FileExists -Path $summaryPath -Description "IdealLoads outdoor-air Flow/Zone conformance compare summary"
Assert-FileExists -Path $reportPath -Description "IdealLoads outdoor-air Flow/Zone conformance markdown report"
Assert-FileExists -Path $selectedOutputsPath -Description "IdealLoads outdoor-air Flow/Zone conformance selected outputs"
Assert-FileExists -Path $resultStorePath -Description "IdealLoads outdoor-air Flow/Zone conformance Rust result store"
Assert-FileExists -Path $variableDeltasPath -Description "IdealLoads outdoor-air Flow/Zone conformance variable deltas"
Assert-FileExists -Path $firstDivergencePath -Description "IdealLoads outdoor-air Flow/Zone conformance first divergence CSV"
Assert-FileExists -Path $toleranceFailuresPath -Description "IdealLoads outdoor-air Flow/Zone conformance tolerance failures CSV"
Assert-FileExists -Path $stageSummaryPath -Description "IdealLoads outdoor-air Flow/Zone conformance stage summary"
...
if ($summary.conformance_claim -ne $true) {
    throw "IdealLoads outdoor-air Flow/Zone conformance summary must set conformance_claim=true"
}
if ($summary.status -ne "pass") {
    throw "Unexpected IdealLoads outdoor-air Flow/Zone conformance status: $($summary.status)"
}
if ($summary.tolerance_failures -ne 0) {
    throw "IdealLoads outdoor-air Flow/Zone conformance should have zero tolerance failures: $($summary.tolerance_failures)"
}
...
if ($conformanceRows.Count -ne 22) {
    throw "Expected twenty-two outdoor-air Flow/Zone conformance rows, found $($conformanceRows.Count)"
}
if (@($conformanceRows | Where-Object { $_.status -ne "pass" }).Count -ne 0) {
    throw "All outdoor-air Flow/Zone conformance rows must pass"
}
...
Write-Host "IdealLoads outdoor-air Flow/Zone conformance candidate artifacts generated."
