...
$CaseId = "calendar_schedule_fmu_export_initial_value_exact_001"
$CaseRoot = Join-Path $RepoRoot "data\conformance_cases\$CaseId"
$CasePath = Join-Path $CaseRoot "case.toml"
$IdfPath = Join-Path $CaseRoot "calendar_schedule_fmu_export_initial_value_exact.idf"
$WeatherPath = Join-Path $CaseRoot "calendar_schedule_fmu_export_initial_value_exact.epw"
$GateCommand = "scripts/dev.cmd compare-calendar-schedule-fmu-export-initial-value-exact"
...
foreach ($path in @(
    ...
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Missing required ExternalInterface:FunctionalMockupUnitExport:To:Schedule exact-case file: $path"
    }
}
...
foreach ($contract in @(
    ...
    'abs_tol = 0.0',
    'rmse_tol = 0.0',
    'exactly 96 ordered, unique Timestep Schedule Value samples and timestamps, each equal to 0.875',
    'No ExternalInterface activation object or socket/provider configuration is present',
    'synthetic outer key ExternalInterface:FunctionalMockupUnitExport:To:Schedule 1',
    'socket.cfg and variables.cfg handling',
    'provider resolution',
    'FMU variable binding',
    'live exchange',
    'warmup handoff',
    'mutable updates or currentVal state',
    'termination or final exchange',
    'Ptolemy lifecycle',
    'global ExternalInterface behavior',
    'other ExternalInterface variable or actuator families',
    'exact duplicate diagnostic text',
    'bounded ScheduleTypeLimits parity',
    'unknown type-limit warning parity',
    'downstream consumers',
    'multiple objects',
    'actual/design-day/multi-environment execution',
    'script = "scripts/dev.cmd compare-calendar-schedule-fmu-export-initial-value-exact"',
    'blocking = true'
)) {
...
$expectedVectors = @(
    "Version|26.1",
    "Building|FMU Export Schedule Initial Value Exact Fixture|0.0|Suburbs|0.04|0.4|FullExterior|25|6",
    "Timestep|4",
    "GlobalGeometryRules|UpperLeftCorner|CounterClockWise|World",
    "RunPeriod|FMU Export Schedule Initial Value Run Period|1|1|2032|1|1|2032|Thursday|No|No|No|No|No|No",
    "ScheduleTypeLimits|Any Number",
    "ExternalInterface:FunctionalMockupUnitExport:To:Schedule|FMU Export Initial Value|Any Number|ProbeInput|0.875",
    "Output:Variable|FMU EXPORT INITIAL VALUE|Schedule Value|Timestep"
)
...
$mutated = $idfText.Replace(
    "Version,26.1;",
    "Version,26.1; ExternalInterface,FunctionalMockupUnitExport;"
)
...
if ($mutatedVectors.Count -ne ($expectedVectors.Count + 1) -or
    $mutatedVectors[1] -cne "ExternalInterface|FunctionalMockupUnitExport") {
...
$expectedHeaders = @(
    "LOCATION,FMU Export Schedule Initial Value Exact Fixture,CO,USA,Synthetic,999999,39.74,-105.18,-7.0,1829.0",
    "DESIGN CONDITIONS,0",
    "TYPICAL/EXTREME PERIODS,0",
    "GROUND TEMPERATURES,0",
    "HOLIDAYS/DAYLIGHT SAVINGS,No,0,0,0",
    "COMMENTS 1,Deterministic one-day FMU Export To Schedule inactive initial-value fixture",
...
if ($LASTEXITCODE -ne 0) {
    throw "ExternalInterface:FunctionalMockupUnitExport:To:Schedule report failed with exit code $LASTEXITCODE $($output -join [Environment]::NewLine)"
}
...
foreach ($path in @(
    ...
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Missing ExternalInterface:FunctionalMockupUnitExport:To:Schedule report artifact: $path"
    }
}
...
if ($summary.case_id -cne $CaseId -or $summary.oracle_version -cne "26.1.0" -or
    ...
    throw "Unexpected ExternalInterface:FunctionalMockupUnitExport:To:Schedule report summary contract"
}
$seriesRows = @($summary.series | Where-Object {
    $_.key -eq "FMU EXPORT INITIAL VALUE" -and $_.variable -eq "Schedule Value"
})
if ($seriesRows.Count -ne 1) {
    throw "Expected exactly one FMU export initial-value Schedule Value series"
}
$series = $seriesRows[0]
$firstTimestamp = "env=FMU EXPORT SCHEDULE INITIAL VALUE RUN PERIOD;day=1;month=1;date=1;dst=0;hour=1;start=0.00;end=15.00;day_type=Thursday"
$lastTimestamp = "env=FMU EXPORT SCHEDULE INITIAL VALUE RUN PERIOD;day=1;month=1;date=1;dst=0;hour=24;start=45.00;end=60.00;day_type=Thursday"
if ($series.level -cne "conformance" -or $series.class -cne "schedule" -or
    ...
    throw "FMU export initial-value samples and timestamps must match exactly at zero tolerance"
}
...
$scheduleFamily = $converted."ExternalInterface:FunctionalMockupUnitExport:To:Schedule"
$syntheticOuterKey = "ExternalInterface:FunctionalMockupUnitExport:To:Schedule 1"
$convertedSchedule = $scheduleFamily.$syntheticOuterKey
$scheduleTypeFamily = $converted."ScheduleTypeLimits"
if (@($scheduleFamily.PSObject.Properties).Count -ne 1 -or
    @($scheduleFamily.PSObject.Properties | Where-Object {
        $_.Name -ceq "FMU Export Initial Value"
    }).Count -ne 0 -or
    @($convertedSchedule.PSObject.Properties).Count -ne 4 -or
    $convertedSchedule.schedule_name -cne "FMU Export Initial Value" -or
    ...
    @($converted.PSObject.Properties | Where-Object {
        $_.Name -ceq "ExternalInterface"
    }).Count -ne 0) {
    throw "Converted epJSON changed the exact synthetic-outer-key FMU export schedule fields or added an activation family"
}
Write-Host "OK converted epJSON exact inactive ExternalInterface:FunctionalMockupUnitExport:To:Schedule shape"

$esoLines = @(Get-Content -LiteralPath $oracleEsoPath -Encoding UTF8)
$dictionaryRows = @($esoLines | Where-Object {
    $_ -ceq "7,1,FMU EXPORT INITIAL VALUE,Schedule Value [] !TimeStep"
})
...
for ($index = 0; $index -lt 96; ++$index) {
    if ($values[$index] -ne 0.875) {
        throw "Unexpected raw FMU export initial value at sample $($index): $($values[$index])"
    }
    ...
    if (-not $timestampMatch.Success -or
        ...
        throw "Unexpected raw FMU-export-schedule timestamp at sample $($index): $($timestamps[$index])"
    }
}
...
$environmentRow = "Environment,FMU EXPORT SCHEDULE INITIAL VALUE RUN PERIOD,WeatherFileRunPeriod,01/01/2032,01/01/2032,Thursday,1,Use RunPeriod Specified Day,No,No,No,No,No,Clark and Allen"
...
foreach ($reportContract in @(
    ...
    "| FMU EXPORT INITIAL VALUE | Schedule Value | conformance | schedule | timestep | eso | timestamp | 96 | 96 | 96 | 0.000000000000 | 0.000000000000 | 0.000000000000 |"
)) {
    Assert-Contains -Text $reportText -Pattern $reportContract -Description "markdown FMU export initial-value contract"
}

Write-Host "ExternalInterface:FunctionalMockupUnitExport:To:Schedule inactive initial-value exact gate passed."
