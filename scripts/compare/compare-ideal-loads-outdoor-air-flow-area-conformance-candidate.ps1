...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-outdoor-air-flow-area-conformance\26.1.0"
$CaseId = "ideal_loads_outdoor_air_flow_area_conformance_candidate_001"
...
foreach ($path in @(
    ...
    Assert-FileExists -Path $path -Description "required IdealLoads outdoor-air Flow/Area conformance input"
}
...
Write-Host "Generating IdealLoads outdoor-air Flow/Area conformance candidate artifacts."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "IdealLoads outdoor-air Flow/Area conformance candidate comparison failed."
}
...
Assert-FileExists -Path $summaryPath -Description "IdealLoads outdoor-air Flow/Area conformance compare summary"
Assert-FileExists -Path $reportPath -Description "IdealLoads outdoor-air Flow/Area conformance markdown report"
Assert-FileExists -Path $selectedOutputsPath -Description "IdealLoads outdoor-air Flow/Area conformance selected outputs"
Assert-FileExists -Path $resultStorePath -Description "IdealLoads outdoor-air Flow/Area conformance Rust result store"
Assert-FileExists -Path $variableDeltasPath -Description "IdealLoads outdoor-air Flow/Area conformance variable deltas"
Assert-FileExists -Path $firstDivergencePath -Description "IdealLoads outdoor-air Flow/Area conformance first divergence CSV"
Assert-FileExists -Path $toleranceFailuresPath -Description "IdealLoads outdoor-air Flow/Area conformance tolerance failures CSV"
Assert-FileExists -Path $stageSummaryPath -Description "IdealLoads outdoor-air Flow/Area conformance stage summary"
...
if ($summary.conformance_claim -ne $true) {
    throw "IdealLoads outdoor-air Flow/Area conformance summary must set conformance_claim=true"
}
if ($summary.status -ne "pass") {
    throw "Unexpected IdealLoads outdoor-air Flow/Area conformance status: $($summary.status)"
}
if ($summary.tolerance_failures -ne 0) {
    throw "IdealLoads outdoor-air Flow/Area conformance should have zero tolerance failures: $($summary.tolerance_failures)"
}
...
if ($summary.zone_floor_area_m2 -ne 1.0) {
    throw "Unexpected outdoor-air zone floor area: $($summary.zone_floor_area_m2)"
}
...
if ($conformanceRows.Count -ne 22) {
    throw "Expected twenty-two outdoor-air Flow/Area conformance rows, found $($conformanceRows.Count)"
}
if (@($conformanceRows | Where-Object { $_.status -ne "pass" }).Count -ne 0) {
    throw "All outdoor-air Flow/Area conformance rows must pass"
}
...
if ($stageSummary.zone_floor_area_m2 -ne 1.0) {
    throw "Stage summary must record zone_floor_area_m2=1, got $($stageSummary.zone_floor_area_m2)"
}

$reportText = Get-Content -LiteralPath $reportPath -Raw
Assert-Contains -Text $reportText -Pattern "claim_boundary: conformance IdealLoads outdoor-air Flow/Area branch for declared variables only" -Description "markdown claim boundary"
...
Assert-Contains -Text $reportText -Pattern "zone_floor_area_m2: 1.000000000000000" -Description "markdown zone floor area"
...
Write-Host "IdealLoads outdoor-air Flow/Area conformance candidate artifacts generated."
