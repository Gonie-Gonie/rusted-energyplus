...
$OutputRoot = Join-Path $RepoRoot ".runtime\ideal-loads-flow-capacity-limit-conformance\26.1.0"
$CaseId = "ideal_loads_flow_capacity_limit_conformance_001"
...
Write-Host "Generating IdealLoads no-OA flow-and-capacity-limit conformance comparison artifacts."
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "IdealLoads no-OA flow-and-capacity-limit conformance comparison failed."
}
...
if ($summary.conformance_claim -ne $true) {
    throw "IdealLoads flow-and-capacity-limit conformance summary must set conformance_claim=true"
}
if ($summary.status -ne "pass") {
    throw "Unexpected IdealLoads flow-and-capacity-limit conformance status: $($summary.status)"
}
if ($summary.tolerance_failures -ne 0) {
    throw "IdealLoads flow-and-capacity-limit conformance should have zero tolerance failures: $($summary.tolerance_failures)"
}
...
if ($summary.zone_demand_synthetic_rc_model -ne $false) {
    throw "IdealLoads flow-and-capacity-limit conformance must not synthesize zone demand from an RC shortcut"
}
...
if (@($conformanceRows | Where-Object { $_.status -ne "pass" }).Count -ne 0) {
    throw "All conformance-level flow-and-capacity-limit output rows must pass"
}
...
if ($stageSummary.selected_purchased_air_branch -ne "flow_and_capacity") {
    throw "Unexpected PurchasedAir branch: $($stageSummary.selected_purchased_air_branch)"
}
if ($stageSummary.declared_ideal_loads_branch -ne "flow_and_capacity") {
...
if ($stageSummary.finite_limit_conformance -ne $true) {
    throw "Stage summary must mark finite_limit_conformance=true for the promoted flow-and-capacity-limit case"
}
...
Assert-Contains -Text $reportText -Pattern "selected_purchased_air_branch: flow_and_capacity" -Description "markdown PurchasedAir branch"
Assert-Contains -Text $reportText -Pattern "declared_ideal_loads_branch: flow_and_capacity" -Description "markdown declared branch"
...
Write-Host "IdealLoads no-OA flow-and-capacity-limit conformance comparison artifacts generated."
