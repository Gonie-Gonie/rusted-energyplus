...
Assert-Contains -Text $mainSourceText -Pattern "execute heat-balance compatibility runtime using precomputed weather inputs; the schedule cache is prepared and profiled but is not yet consumed by heat-balance calculations" -Description "B5 schedule cache numerical nonclaim"
