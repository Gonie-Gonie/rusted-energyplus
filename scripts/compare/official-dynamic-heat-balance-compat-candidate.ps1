...
Assert-Contains -Text $algorithmSourceText -Pattern "pub(crate) struct HeatBalanceRuntimeConfig" -Description "B1 probe-agnostic heat-balance runtime config"
...
Assert-Contains -Text $algorithmSourceText -Pattern "Self::SourceOrder1ZoneOpaqueCompat => HeatBalanceRuntimeConfig" -Description "D2 compatibility candidate declares runtime flags directly"
...
Assert-Contains -Text $pipelineSourceText -Pattern "let weather_file = load_epw_weather_file(weather_path)" -Description "B5 rich EPW parse in runtime setup"
Assert-Contains -Text $pipelineSourceText -Pattern "let time_axis = build_hourly_time_axis_with_weather_metadata(" -Description "B5 metadata-aware TimeAxis in runtime setup"
Assert-Contains -Text $pipelineSourceText -Pattern "let environment_weather = select_epw_environment_weather(&weather_file, &time_axis)" -Description "B5 source-order environment weather selection in runtime setup"
Assert-Contains -Text $pipelineSourceText -Pattern "let weather_series = precompute_weather_timestep_series(" -Description "B5 weather timestep precompute in setup"
Assert-Contains -Text $pipelineSourceText -Pattern "environment_weather.hourly_records()," -Description "B5 selected hourly records feed weather precompute"
Assert-Contains -Text $pipelineSourceText -Pattern "time_axis.zone_timestep.timesteps_per_hour," -Description "B5 TimeAxis cadence feeds weather precompute"
Assert-Contains -Text $pipelineSourceText -Pattern "time_axis.first_hour_interpolation_starting_values," -Description "B5 TimeAxis first-hour policy feeds weather precompute"
...
Assert-Contains -Text $pipelineSourceText -Pattern "schedule_cache: ScheduleSeriesCache" -Description "B5 prepared immutable schedule cache"
Assert-Contains -Text $pipelineSourceText -Pattern "precompute_schedule_cache_for_time_axis" -Description "B5 schedule cache compile from TimeAxis"
Assert-Contains -Text $pipelineSourceText -Pattern "schedule_cache_profile: ScheduleCacheProfile" -Description "B5 deterministic schedule cache profile"
Assert-Contains -Text $pipelineSourceText -Pattern '"schedule_cache": schedule_cache_json(' -Description "B5 run summary schedule cache metadata"
Assert-NotContains -Text $pipelineSourceText -Pattern "schedule_series: Vec<ScheduleValueSeries>" -Description "B5 legacy prepared schedule series"
Assert-NotContains -Text $pipelineSourceText -Pattern "runtime_inputs.schedule_cache.get(" -Description "B5 unclaimed numerical cache lookup"
Assert-Contains -Text $mainSourceText -Pattern "ep_runtime consumes a separately prepared referenced-only 24-hour cache for validated hour-only OtherEquipment gains, while this full-axis schedule cache remains profile-only" -Description "B5 specialized heat-balance cache and full-axis profile-only boundary"
Assert-Contains -Text $runtimeSourceText -Pattern "precompute_hour_only_internal_gain_schedule_cache" -Description "B5 runtime prepares referenced-only internal-gain schedule cache"
...
Assert-Contains -Text $mainSourceText -Pattern "precompute_schedule_cache_for_time_axis" -Description "C1 immutable schedule cache precompute uses TimeAxis"
...
Assert-Contains -Text $timeAxisSourceText -Pattern "DEFAULT_RUN_PERIOD_YEAR: u32 = 2017" -Description "yearless non-leap run-period default year"
...
Assert-Contains -Text $algorithmSourceText -Pattern "HeatBalanceInteriorLongwaveMode::EnergyPlusScriptFFlatAccess" -Description "conformance longwave runtime-config owner"
...
Assert-Contains -Text $schedulesSourceText -Pattern "precompute_schedule_cache_for_time_axis(model, time_axis).into_traces()" -Description "legacy hourly adapter delegates to immutable schedule cache"
