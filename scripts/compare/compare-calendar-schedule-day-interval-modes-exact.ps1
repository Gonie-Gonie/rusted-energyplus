...
$CaseId = "calendar_schedule_day_interval_modes_exact_001"
$CaseRoot = Join-Path $RepoRoot "data\conformance_cases\$CaseId"
$CasePath = Join-Path $CaseRoot "case.toml"
$IdfPath = Join-Path $CaseRoot "calendar_schedule_day_interval_modes_exact.idf"
$WeatherPath = Join-Path $CaseRoot "calendar_schedule_day_interval_modes_exact.epw"
$GateCommand = "scripts/dev.cmd compare-calendar-schedule-day-interval-modes-exact"
...
foreach ($path in @(
    ...
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Missing required Schedule:Day:Interval modes file: $path"
    }
}
...
foreach ($contract in @(
    ...
    'source_file = "data/conformance_cases/calendar_schedule_day_interval_modes_exact_001/calendar_schedule_day_interval_modes_exact.idf"',
    'idf = "data/conformance_cases/calendar_schedule_day_interval_modes_exact_001/calendar_schedule_day_interval_modes_exact.idf"',
    'weather = "data/conformance_cases/calendar_schedule_day_interval_modes_exact_001/calendar_schedule_day_interval_modes_exact.epw"',
    'frequency = "timestep"',
    'timestamp_contract = "ordered-exact-unique"',
    'abs_tol = 0.0',
    'rmse_tol = 0.0',
    'genuinely blank Interpolate to Timestep field',
    'Thursday is [10] then 95 values of 175',
    'Friday is [10, 120] then 94 values of 175',
    'Saturday is [10, 40, 85, 130] then 92 values of 175',
    'exactly 0 Warning and 0 Severe errors',
    'nonaligned No behavior and warning parity',
    'Schedule:Day:List',
    ...
    'script = "scripts/dev.cmd compare-calendar-schedule-day-interval-modes-exact"',
    'blocking = true'
)) {
...
$defaultDay = "Interval Default No Day"
$averageDay = "Interval Average Day"
$linearDay = "Interval Linear Day"
$expectedVectors = @(
    "Version|26.1",
    "Building|Schedule Day Interval Modes Exact Fixture|0.0|Suburbs|0.04|0.4|FullExterior|25|6",
    "Timestep|4",
    "GlobalGeometryRules|UpperLeftCorner|CounterClockWise|World",
    "RunPeriod|Schedule Day Interval Modes Run Period|1|1|2032|1|3|2032|Thursday|No|No|No|No|No|No",
    "ScheduleTypeLimits|Any Number",
    "Schedule:Day:Interval|$defaultDay|Any Number||Until: 00:15|10|Until: 24:00|175",
    "Schedule:Day:Interval|$averageDay|Any Number|Average|Until: 00:20|10|Until: 01:15|175|Until: 24:00|175",
    "Schedule:Day:Interval|$linearDay|Any Number|Linear|Until: 00:20|10|Until: 01:15|175|Until: 24:00|175",
    "Schedule:Week:Daily|Interval Modes Week|$defaultDay|$defaultDay|$defaultDay|$defaultDay|$defaultDay|$averageDay|$linearDay|$defaultDay|$defaultDay|$defaultDay|$defaultDay|$defaultDay",
    "Schedule:Year|Day Interval Modes Schedule|Any Number|Interval Modes Week|1|1|12|31",
    "Output:Variable|DAY INTERVAL MODES SCHEDULE|Schedule Value|Timestep"
)
...
if (($actualVectors -join '||') -cne ($expectedVectors -join '||')) {
    throw "Fixture must retain the exact complete IDF object order, blank default field, and fields"
}
...
Write-Host "OK complete Schedule:Day:Interval fixture vectors and parser self-check"

$expectedHeaders = @(
    "LOCATION,Schedule Day Interval Modes Exact Fixture,CO,USA,Synthetic,999999,39.74,-105.18,-7.0,1829.0",
    "DESIGN CONDITIONS,0",
    "TYPICAL/EXTREME PERIODS,0",
    "GROUND TEMPERATURES,0",
    "HOLIDAYS/DAYLIGHT SAVINGS,No,0,0,0",
    "COMMENTS 1,Deterministic three-day Schedule Day Interval interpolation modes fixture",
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "Schedule:Day:Interval modes report failed with exit code $LASTEXITCODE $joinedOutput"
}
...
foreach ($path in @(
    ...
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Missing Schedule:Day:Interval modes report artifact: $path"
    }
}
...
if ($summary.case_id -cne $CaseId -or $summary.oracle_version -cne "26.1.0" -or
    ...
    throw "Unexpected Schedule:Day:Interval modes report summary contract"
}
...
$seriesRows = @($summary.series | Where-Object {
    $_.key -eq "DAY INTERVAL MODES SCHEDULE" -and $_.variable -eq "Schedule Value"
})
if ($seriesRows.Count -ne 1) {
    throw "Expected exactly one Schedule:Day:Interval Schedule Value series"
}
$series = $seriesRows[0]
$firstTimestamp = "env=SCHEDULE DAY INTERVAL MODES RUN PERIOD;day=1;month=1;date=1;dst=0;hour=1;start=0.00;end=15.00;day_type=Thursday"
$lastTimestamp = "env=SCHEDULE DAY INTERVAL MODES RUN PERIOD;day=3;month=1;date=3;dst=0;hour=24;start=45.00;end=60.00;day_type=Saturday"
if ($series.level -cne "conformance" -or $series.class -cne "schedule" -or
    ...
    throw "Schedule:Day:Interval values and exact first/last timestamps must match at zero delta"
}
...
$dayIntervalFamily = $converted."Schedule:Day:Interval"
$convertedDefault = $dayIntervalFamily.$defaultDay
$convertedAverage = $dayIntervalFamily.$averageDay
$convertedLinear = $dayIntervalFamily.$linearDay
if (@($dayIntervalFamily.PSObject.Properties).Count -ne 3 -or
    ...
    $convertedLinear.interpolate_to_timestep -cne "Linear") {
    throw "Converted epJSON must omit the default-No key and retain only explicit Average and Linear keys"
}
$defaultData = @($convertedDefault.data | ForEach-Object { "$($_.time)|$($_.value_until_time)" })
$averageData = @($convertedAverage.data | ForEach-Object { "$($_.time)|$($_.value_until_time)" })
$linearData = @($convertedLinear.data | ForEach-Object { "$($_.time)|$($_.value_until_time)" })
if (($defaultData -join '||') -cne "Until: 00:15|10||Until: 24:00|175" -or
    ($averageData -join '||') -cne "Until: 00:20|10||Until: 01:15|175||Until: 24:00|175" -or
    ($linearData -join '||') -cne "Until: 00:20|10||Until: 01:15|175||Until: 24:00|175") {
    throw "Converted epJSON must retain exact source interval data for all three day profiles"
}
Write-Host "OK staged IDF and converted epJSON blank/default ownership"

$esoLines = @(Get-Content -LiteralPath $oracleEsoPath -Encoding UTF8)
$dictionaryRows = @($esoLines | Where-Object {
    $_ -ceq "7,1,DAY INTERVAL MODES SCHEDULE,Schedule Value [] !TimeStep"
})
...
for ($index = 0; $index -lt 288; ++$index) {
    ...
    $expectedValue = switch ($dayIndex) {
        0 { if ($daySample -eq 0) { 10.0 } else { 175.0 } }
        1 {
            if ($daySample -eq 0) { 10.0 }
            elseif ($daySample -eq 1) { 120.0 }
            else { 175.0 }
        }
        2 {
            if ($daySample -eq 0) { 10.0 }
            elseif ($daySample -eq 1) { 40.0 }
            elseif ($daySample -eq 2) { 85.0 }
            elseif ($daySample -eq 3) { 130.0 }
            else { 175.0 }
        }
    }
    if ($values[$index] -ne $expectedValue) {
        throw "Unexpected raw Schedule:Day:Interval value at sample $($index): $($values[$index])"
    }
...
Write-Host "OK exact raw ESO default-No, Average, Linear values and 288 timestamps"

$eioLines = @(Get-Content -LiteralPath $oracleEioPath -Encoding UTF8)
$environmentRow = "Environment,SCHEDULE DAY INTERVAL MODES RUN PERIOD,WeatherFileRunPeriod,01/01/2032,01/03/2032,Thursday,3,Use RunPeriod Specified Day,No,No,No,No,No,Clark and Allen"
...
Assert-NotContains -Text $errText -Pattern 'Invalid "until"' -Description "non-multiple Until warning"
...
foreach ($reportContract in @(
    ...
    "| DAY INTERVAL MODES SCHEDULE | Schedule Value | conformance | schedule | timestep | eso | timestamp | 288 | 288 | 288 | 0.000000000000 | 0.000000000000 | 0.000000000000 |"
)) {
    Assert-Contains -Text $reportText -Pattern $reportContract -Description "markdown Schedule:Day:Interval contract"
}

Write-Host "Schedule:Day:Interval interpolation-modes exact gate passed."
