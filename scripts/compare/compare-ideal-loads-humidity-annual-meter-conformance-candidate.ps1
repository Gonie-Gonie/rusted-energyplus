...
Assert-Contains -Text $reportText -Pattern "ideal_loads_psychrometric_evaluation_policy: compatibility reports use source-order direct psychrometric evaluation; no cross-timestep cache or reordering is enabled" -Description "markdown psychrometric policy"
Assert-Contains -Text $reportText -Pattern "ideal_loads_psychrometric_cache_policy: future compatibility cache must key exact temperature, humidity ratio, and pressure tuple and preserve EnergyPlus evaluation order" -Description "markdown psychrometric cache"
