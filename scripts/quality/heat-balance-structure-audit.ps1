...
Assert-Contains -Path $pipeline -Pattern 'parse EPW weather once before the runtime loop' -Description "EPW parse is setup-only"
Assert-NotContains -Path $pipeline -Pattern '(?s)fn execute_rust_runtime.*load_epjson_file' -Description "epJSON parsing inside runtime execution"
Assert-NotContains -Path $pipeline -Pattern '(?s)fn execute_rust_runtime.*load_epw_records' -Description "EPW parsing inside runtime execution"
