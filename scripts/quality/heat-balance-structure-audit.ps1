...
Assert-Contains -Path $schedules -Pattern 'pub fn precompute_schedule_value_series_for_time_axis\s*\([\s\S]*compact_schedule_series_for_time_axis[\s\S]*detailed_schedule_lookup_state\(point\)[\s\S]*fn detailed_schedule_lookup_state\s*\([\s\S]*point\.hour\.clamp\(1, 24\) \+ u32::from\(point\.dst\)[\s\S]*point\.schedule_day_of_year[\s\S]*tomorrow_special_day_type[\s\S]*unwrap_or_else\(\|\| point\.tomorrow_day_of_week\.into\(\)\)' -Description "runtime compact schedule consumes calendar ordinal, current DST, tomorrow special-day precedence, and tomorrow weekday"
