...
Assert-Contains -Path $schedules -Pattern 'pub fn precompute_schedule_value_series_for_time_axis\s*\([\s\S]*compact_schedule_series_for_time_axis[\s\S]*detailed_schedule_lookup_state\(point\)[\s\S]*fn detailed_schedule_lookup_state\s*\([\s\S]*point\.schedule_day_of_year[\s\S]*tomorrow_special_day_type[\s\S]*unwrap_or_else\(\|\| point\.tomorrow_day_of_week\.into\(\)\)[\s\S]*fn detailed_schedule_lookup_state_from_input\s*\([\s\S]*input\.hour\.clamp\(1, 24\) \+ u32::from\(input\.dst\)' -Description "runtime compact schedule consumes calendar ordinal, current DST, tomorrow special-day precedence, and tomorrow weekday"
...
Assert-Contains -Path $compiler -Pattern 'fn compact_schedule_periods\s*\([\s\S]*compact_directive\(text, "Until"\)[\s\S]*compact_directive\(text, "Interpolate"\)[\s\S]*UnsupportedScheduleCompactInterpolation' -Description "compiler accepts omitted compact interpolation directive and rejects explicit directives"
Assert-Contains -Path $schedules -Pattern 'pub fn precompute_schedule_value_series_for_environment_time_axis\s*\([\s\S]*detailed_schedule_environment_lookup_state\(point\)[\s\S]*fn detailed_schedule_environment_lookup_state\s*\([\s\S]*timestep_end_minute: point\.end_minute' -Description "environment schedule series consumes each zone-timestep end minute"
...
Assert-Contains -Path $algorithmLedger -Pattern 'Compiler::parse_timestep[\s\S]*Compiler::parse_compact_schedules[\s\S]*Compiler::compact_schedule_periods[\s\S]*precompute_schedule_value_series_for_environment_time_axis[\s\S]*detailed_schedule_environment_lookup_state[\s\S]*precompile_compact_schedule_intervals[\s\S]*normalized_environment_timestep_timestamp_label' -Description "zone-timestep Rust target mapping"
