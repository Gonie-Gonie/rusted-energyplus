...
$script = Join-Path $RepoRoot "tools\docs\validate_diagnostic_probe_ledger.py"
if (-not (Test-Path -LiteralPath $script -PathType Leaf)) {
    throw "Missing diagnostic probe ledger validator: $script"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Diagnostic probe ledger check failed with exit code $LASTEXITCODE"
}
