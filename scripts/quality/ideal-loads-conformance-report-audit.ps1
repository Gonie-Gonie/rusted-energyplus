...
$expectedIdealLoadsPsychrometricEvaluationPolicy = "compatibility reports use source-order direct psychrometric evaluation with EnergyPlus Psat cache-temperature quantization; no reordering is enabled"
$expectedIdealLoadsPsychrometricCachePolicy = "saturation-pressure evaluation mirrors EnergyPlus default PsyPsatFnTemp cache temperature-key truncation before the raw polynomial"
...
foreach ($case in $promotedCases) {
    ...
    Assert-JsonPropertyEquals -Object $summary -PropertyName "zone_demand_cooling_sign_convention" -Expected "negative W requests cooling; positive means no active cooling request; exact zero priority depends on the demand input kind" -Description "$($case.Id) compare summary"
    ...
    $reportFieldExpectations = [ordered]@{
        ...
        "zone_demand_cooling_sign_convention" = "negative W requests cooling; positive means no active cooling request; exact zero priority depends on the demand input kind"
    ...
    Assert-JsonPropertyEquals -Object $stageSummary -PropertyName "zone_demand_cooling_sign_convention" -Expected "negative W requests cooling; positive means no active cooling request; exact zero priority depends on the demand input kind" -Description "$($case.Id) stage summary"
