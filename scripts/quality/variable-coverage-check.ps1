...
$script = Join-Path $RepoRoot "tools\docs\validate_variable_coverage.py"
if (-not (Test-Path -LiteralPath $script -PathType Leaf)) {
    throw "Missing variable coverage validator: $script"
}

& $python $script --repo-root $RepoRoot
if ($LASTEXITCODE -ne 0) {
    throw "Variable coverage check failed with exit code $LASTEXITCODE"
}
