...
function Assert-Cp348BindingContract {
    ...
    if ($cp347 -lt 0 -or $cp348 -le $cp347 -or $cp349 -le $cp348 -or $cp350 -le $cp349 -or $numerical -le $cp350) {
        throw "Binding must execute CP347 then CP348 then CP349 then CP350 before numerical coupling"
    }
...
Assert-Contains -Path $cp348PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp350_lifecycle_evidence' -Description "CP348 cumulative non-direct firewall"
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 288' -Description "CP348 script total"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| executable script records \| 288 \|' -Description "generated CP348 script total"
