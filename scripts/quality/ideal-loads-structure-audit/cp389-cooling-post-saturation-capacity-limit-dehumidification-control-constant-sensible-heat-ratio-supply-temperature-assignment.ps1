...
if ($cp388Index -lt 0 -or $cp389Index -le $cp388Index -or $numericalIndex -le $cp389Index) {
    throw "Binding must execute CP388, CP389, then unchanged numerical coupling"
}
...
Assert-Contains -Path $pipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp389_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
$nonDirectTest = Get-Cp389BraceBlock -Text $pipelineText -AnchorPattern 'fn\s+non_direct_runtime_rejects_cp316_through_cp389_lifecycle_evidence\s*\(' -Description "cumulative non-direct regression"
...
foreach ($historical in 334..388) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp389_lifecycle_evidence' -Description "historical cumulative firewall"
}
foreach ($historical in 335..388) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
...
foreach ($historical in 337..388) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'script_count = 327' -Description "historical current inventory total"
}
foreach ($historical in 367..388) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'Count -ne 87' -Description "historical current internal classification"
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern '240 public and 87 internal' -Description "historical current classification diagnostic"
}
...
foreach ($pattern in @('\$cp388Call\s*=', '\$cp389Call\s*=', 'CP388-to-CP389', 'CP389-to-numerical')) {
...
foreach ($historical in 377..388) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'CP389-to-numerical' -Description "historical CP389 terminal interval"
}
foreach ($historical in 385..388) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
...
if ($cp388AuditIndex -lt 0 -or $cp389AuditIndex -le $cp388AuditIndex -or $completionIndex -le $cp389AuditIndex) {
    throw "Master audit must dot-source CP389 after CP388 before completion"
}
...
if ([regex]::Matches($inventory, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($inventory, '(?m)^classification = "internal"$').Count -ne 87) {
    throw "CP389 inventory must be exactly 240 public and 87 internal scripts"
}
...
foreach ($pattern in @(
        '\| executable script records \| 327 \|', '\| public scripts \| 240 \|',
        '\| internal scripts \| 87 \|', '\| scripts without callers \| 0 \|'
    )) {
