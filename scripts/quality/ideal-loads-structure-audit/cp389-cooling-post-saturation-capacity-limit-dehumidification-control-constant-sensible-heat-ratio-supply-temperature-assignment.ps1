...
Assert-Contains -Path $pipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp393_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
$nonDirectTest = Get-Cp389BraceBlock -Text $pipelineText -AnchorPattern 'fn\s+non_direct_runtime_rejects_cp316_through_cp393_lifecycle_evidence\s*\(' -Description "cumulative non-direct regression"
...
foreach ($historical in 334..389) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp393_lifecycle_evidence' -Description "historical cumulative firewall"
}
...
foreach ($historical in 337..389) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'script_count = 331' -Description "historical current inventory total"
}
foreach ($historical in 367..389) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'Count -ne 91' -Description "historical current internal classification"
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern '240 public and 91 internal' -Description "historical current classification diagnostic"
}
...
foreach ($pattern in @('\$cp388Call\s*=', '\$cp389Call\s*=', '\$cp390Call\s*=', '\$cp391Call\s*=', '\$cp392Call\s*=', 'CP388-to-CP389', 'CP389-to-CP390', 'CP390-to-CP391', 'CP391-to-CP392', 'CP393-to-numerical')) {
...
foreach ($historical in 377..389) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'CP393-to-numerical' -Description "historical CP393 terminal interval"
}
...
if ([regex]::Matches($inventory, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($inventory, '(?m)^classification = "internal"$').Count -ne 91) {
    throw "CP389 inventory must be exactly 240 public and 91 internal scripts"
}
...
foreach ($pattern in @(
        '\| executable script records \| 331 \|', '\| public scripts \| 240 \|',
        '\| internal scripts \| 91 \|', '\| scripts without callers \| 0 \|'
    )) {
