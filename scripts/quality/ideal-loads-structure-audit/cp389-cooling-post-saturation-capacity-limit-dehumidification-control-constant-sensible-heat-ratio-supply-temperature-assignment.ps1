...
$nonDirectTest = Get-Cp389BraceBlock -Text $pipelineText -AnchorPattern 'fn\s+non_direct_runtime_rejects_cp316_through_cp421_lifecycle_evidence\s*\(' -Description "cumulative non-direct regression"
...
foreach ($historical in 337..389) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'script_count = 359' -Description "historical current inventory total"
}
foreach ($historical in 367..389) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'Count -ne 119' -Description "historical current internal classification"
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern '240 public and 119 internal' -Description "historical current classification diagnostic"
}
...
if ([regex]::Matches($inventory, '(?m)^classification = "public"$').Count -ne 240 -or
    ...
    throw "CP389 inventory must be exactly 240 public and 119 internal scripts"
}
