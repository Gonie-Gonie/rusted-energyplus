...
$nonDirectTest = Get-Cp388BraceBlock -Text $pipelineText -AnchorPattern 'fn\s+non_direct_runtime_rejects_cp316_through_cp441_lifecycle_evidence\s*\(' -Description "cumulative non-direct regression"
...
[regex]::Matches($inventory, '(?m)^classification = "internal"$').Count -ne 139) {
...
foreach ($pattern in @(
        '\| 379 \|', '\| public scripts \| 240 \|',
        '\| 139 \|', '\| scripts without callers \| 0 \|'
    )) {
