...
$nonDirectTest = Get-Cp388BraceBlock -Text $pipelineText -AnchorPattern 'fn\s+non_direct_runtime_rejects_cp316_through_cp435_lifecycle_evidence\s*\(' -Description "cumulative non-direct regression"
...
[regex]::Matches($inventory, '(?m)^classification = "internal"$').Count -ne 133) {
...
foreach ($pattern in @(
        '\| 373 \|', '\| public scripts \| 240 \|',
        '\| 133 \|', '\| scripts without callers \| 0 \|'
    )) {
