...
$nonDirectTest = Get-Cp388BraceBlock -Text $pipelineText -AnchorPattern 'fn\s+non_direct_runtime_rejects_cp316_through_cp431_lifecycle_evidence\s*\(' -Description "cumulative non-direct regression"
...
[regex]::Matches($inventory, '(?m)^classification = "internal"$').Count -ne 129) {
throw "CP388 inventory must be exactly 240 public and 122 internal scripts"
}
...
foreach ($pattern in @(
        '\| 369 \|', '\| public scripts \| 240 \|',
        '\| 129 \|', '\| scripts without callers \| 0 \|'
    )) {
