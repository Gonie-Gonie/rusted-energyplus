...
if ($cp387Index -lt 0 -or $cp388Index -le $cp387Index -or $numericalIndex -le $cp388Index) {
    throw "Binding must execute CP387, CP388, then unchanged numerical coupling"
}
...
$nonDirectTest = Get-Cp388BraceBlock -Text $pipelineText -AnchorPattern 'fn\s+non_direct_runtime_rejects_cp316_through_cp388_lifecycle_evidence\s*\(' -Description "cumulative non-direct regression"
...
foreach ($pattern in @('\$cp387Call\s*=', '\$cp388Call\s*=', 'CP387-to-CP388', 'CP388-to-numerical')) {
...
if ($cp387AuditIndex -lt 0 -or $cp388AuditIndex -le $cp387AuditIndex -or $completionIndex -le $cp388AuditIndex) {
    throw "Master audit must dot-source CP388 after CP387 before completion"
}
...
if ([regex]::Matches($inventory, '(?m)^classification = "public"$').Count -ne 240 -or
    ...
    throw "CP388 inventory must be exactly 240 public and 86 internal scripts"
}
