...
$nonDirectTest = Get-Cp388BraceBlock -Text $pipelineText -AnchorPattern 'fn\s+non_direct_runtime_rejects_cp316_through_cp436_lifecycle_evidence\s*\(' -Description "cumulative non-direct regression"
...
foreach ($pattern in @(
        '\| 374 \|', '\| public scripts \| 240 \|',
        '\| 134 \|', '\| scripts without callers \| 0 \|'
    )) {
