...
Assert-Contains -Path $cp379PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp400_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
[regex]::Matches($cp379InventoryText, '(?m)^classification = "internal"$').Count -ne 98) {
throw "CP379 inventory must be exactly 240 public and 98 internal scripts"
}
