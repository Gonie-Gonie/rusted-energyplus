...
if ($cp378BindingIndexForCp379 -lt 0 -or $cp379BindingIndex -le $cp378BindingIndexForCp379 -or $cp380BindingIndexForCp379 -le $cp379BindingIndex -or
    $cp381BindingIndexForCp379 -le $cp380BindingIndexForCp379 -or $cp379NumericalIndex -le $cp381BindingIndexForCp379) {
...
Assert-Contains -Path $cp379PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp381_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Contains -Path $cp345Audit -Pattern 'CP379-to-CP380' -Description "CP345 CP380 predecessor interval"; Assert-Contains -Path $cp345Audit -Pattern 'CP381-to-numerical' -Description "CP345 terminal interval"
...
foreach ($historical in 364, 373, 374, 376, 377, 378) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'cp380_assertions\.rs' -Description "historical CP380 arbitrary terminal"; Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'cp381_assertions\.rs' -Description "historical CP381 arbitrary terminal"
}
...
if ([regex]::Matches($cp379InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp379InventoryText, '(?m)^classification = "internal"$').Count -ne 79) {
    throw "CP379 inventory must be exactly 240 public and 79 internal scripts"
}
