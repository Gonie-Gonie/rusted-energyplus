...
Assert-Contains -Path $cp379PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp401_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
[regex]::Matches($cp379InventoryText, '(?m)^classification = "internal"$').Count -ne 99) {
throw "CP379 inventory must be exactly 240 public and 99 internal scripts"
}
