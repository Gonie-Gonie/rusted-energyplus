...
Assert-Contains -Path $cp379PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp387_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
if ([regex]::Matches($cp379InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp379InventoryText, '(?m)^classification = "internal"$').Count -ne 85) {
    throw "CP379 inventory must be exactly 240 public and 85 internal scripts"
}
