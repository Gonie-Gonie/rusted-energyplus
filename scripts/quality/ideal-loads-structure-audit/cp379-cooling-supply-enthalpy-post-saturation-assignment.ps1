...
Assert-Contains -Path $cp379PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp385_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Contains -Path $cp345Audit -Pattern 'CP379-to-CP380' -Description "CP345 CP380 predecessor interval"; Assert-Contains -Path $cp345Audit -Pattern 'CP385-to-numerical' -Description "CP345 terminal interval"
...
if ([regex]::Matches($cp379InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp379InventoryText, '(?m)^classification = "internal"$').Count -ne 83) {
    throw "CP379 inventory must be exactly 240 public and 83 internal scripts"
}
