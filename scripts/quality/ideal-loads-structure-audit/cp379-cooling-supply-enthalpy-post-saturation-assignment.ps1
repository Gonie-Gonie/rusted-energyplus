...
if ($cp378BindingIndexForCp379 -lt 0 -or $cp379BindingIndex -le $cp378BindingIndexForCp379 -or $cp380BindingIndexForCp379 -le $cp379BindingIndex -or
    $cp381BindingIndexForCp379 -le $cp380BindingIndexForCp379 -or $cp382BindingIndexForCp379 -le $cp381BindingIndexForCp379 -or $cp379NumericalIndex -le $cp382BindingIndexForCp379) {
...
Assert-Contains -Path $cp379PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp382_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Contains -Path $cp345Audit -Pattern 'CP379-to-CP380' -Description "CP345 CP380 predecessor interval"; Assert-Contains -Path $cp345Audit -Pattern 'CP382-to-numerical' -Description "CP345 terminal interval"
...
if ([regex]::Matches($cp379InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp379InventoryText, '(?m)^classification = "internal"$').Count -ne 80) {
    throw "CP379 inventory must be exactly 240 public and 80 internal scripts"
}
