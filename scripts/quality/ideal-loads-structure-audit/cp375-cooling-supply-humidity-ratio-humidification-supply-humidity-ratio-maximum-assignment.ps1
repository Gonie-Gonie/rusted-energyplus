...
Assert-Contains -Path $cp375PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp441_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 334..374) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp441_lifecycle_evidence' -Description "historical CP375 firewall"
}
...
Assert-Cp375TextContains -Text $cp375InventoryText -Pattern 'script_count = 379' -Description "script total"
...
[regex]::Matches($cp375InventoryText, '(?m)^classification = "internal"$').Count -ne 139) {
