...
if ($cp374BindingIndexForCp375 -lt 0 -or $cp375BindingIndex -le $cp374BindingIndexForCp375 -or
    $cp376BindingIndex -le $cp375BindingIndex -or $cp377BindingIndex -le $cp376BindingIndex -or $cp378BindingIndex -le $cp377BindingIndex -or $cp379BindingIndex -le $cp378BindingIndex -or $cp380BindingIndex -le $cp379BindingIndex -or $cp375NumericalIndex -le $cp380BindingIndex) {
...
Assert-Contains -Path $cp375PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp380_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 334..374) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp380_lifecycle_evidence' -Description "historical CP375 firewall"
}
...
Assert-Cp375TextContains -Text $cp375InventoryText -Pattern 'script_count = 318' -Description "script total"
...
if ([regex]::Matches($cp375InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp375InventoryText, '(?m)^classification = "internal"$').Count -ne 78) {
