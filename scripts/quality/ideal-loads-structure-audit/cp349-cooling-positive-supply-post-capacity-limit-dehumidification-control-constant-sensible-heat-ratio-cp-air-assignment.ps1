...
function Assert-Cp349BindingContract {
    ...
    if ($cp348 -lt 0 -or $cp349 -le $cp348 -or $cp350 -le $cp349 -or $cp351 -le $cp350 -or $cp352 -le $cp351 -or $numerical -le $cp352) {
        throw "Binding must execute CP348 then CP349 then CP350 then CP351 then CP352 before numerical coupling"
    }
...
Assert-Contains -Path $cp349PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp352_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 290' -Description "CP349 script total"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| executable script records \| 290 \|' -Description "generated CP349 script total"
