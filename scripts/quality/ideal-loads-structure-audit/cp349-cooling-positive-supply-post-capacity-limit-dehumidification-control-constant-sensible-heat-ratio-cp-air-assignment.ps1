...
function Assert-Cp349BindingContract {
    ...
    if ($cp348 -lt 0 -or $cp349 -le $cp348 -or $cp350 -le $cp349 -or $cp351 -le $cp350 -or $cp352 -le $cp351 -or $cp353 -le $cp352 -or $cp354 -le $cp353 -or $cp355 -le $cp354 -or $cp356 -le $cp355 -or $numerical -le $cp356) {
        throw "Binding must execute CP348 then CP349 then CP350 then CP351 then CP352 then CP353 then CP354 then CP355 then CP356 before numerical coupling"
    }
...
Assert-Contains -Path $cp349PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp356_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 294' -Description "CP349 script total"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| executable script records \| 294 \|' -Description "generated CP349 script total"
