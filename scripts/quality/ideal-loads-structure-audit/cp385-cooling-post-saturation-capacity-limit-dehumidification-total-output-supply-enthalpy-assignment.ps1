...
if ($cp384Index -lt 0 -or $cp385Index -le $cp384Index -or $cp386Index -le $cp385Index -or $cp387Index -le $cp386Index -or $cp388Index -le $cp387Index -or $cp389Index -le $cp388Index -or $numericalIndex -le $cp389Index) {
    throw "Binding must execute CP384, CP385, CP386, CP387, CP388, CP389, then unchanged numerical coupling"
}
...
if ($cp384AuditIndex -lt 0 -or $cp385AuditIndex -le $cp384AuditIndex -or $cp386AuditIndex -le $cp385AuditIndex -or $cp387AuditIndex -le $cp386AuditIndex -or $cp388AuditIndex -le $cp387AuditIndex -or $cp389AuditIndex -le $cp388AuditIndex -or $completionIndex -le $cp389AuditIndex) {
...
if ([regex]::Matches($inventory, '(?m)^classification = "public"$').Count -ne 240 -or
    ...
    throw "CP385 inventory must be exactly 240 public and 87 internal scripts"
}
