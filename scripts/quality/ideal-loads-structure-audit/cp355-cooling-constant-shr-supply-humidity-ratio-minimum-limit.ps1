...
function Assert-Cp355BindingContract {
    ...
    if ($cp354 -lt 0 -or $cp355 -le $cp354 -or $numerical -le $cp355) {
        throw "Binding must execute CP354 then CP355 before numerical coupling"
    }
...
Assert-Contains -Path $cp355PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp355_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 293' -Description "CP355 script total"
