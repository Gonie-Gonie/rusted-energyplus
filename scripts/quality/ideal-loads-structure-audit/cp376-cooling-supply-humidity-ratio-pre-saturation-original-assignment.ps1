...
Assert-Contains -Path $cp376PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp388_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Cp376TextContains -Text $cp376InventoryText -Pattern 'script_count = 326' -Description "script total"
...
if ([regex]::Matches($cp376InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp376InventoryText, '(?m)^classification = "internal"$').Count -ne 86) {
