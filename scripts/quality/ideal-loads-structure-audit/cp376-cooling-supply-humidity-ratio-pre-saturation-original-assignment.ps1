...
Assert-Contains -Path $cp376PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp410_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Cp376TextContains -Text $cp376InventoryText -Pattern 'script_count = 348' -Description "script total"
...
[regex]::Matches($cp376InventoryText, '(?m)^classification = "internal"$').Count -ne 108) {
