...
$cp377BindingIndexForCp376 = $cp376BindingText.IndexOf("let calculation_${cp377StemForCp376} ="); $cp378BindingIndexForCp376 = $cp376BindingText.IndexOf("let calculation_cooling_supply_humidity_ratio_saturation_limit_assignment =")
...
if ($cp375BindingIndexForCp376 -lt 0 -or $cp376BindingIndex -le $cp375BindingIndexForCp376 -or
    $cp377BindingIndexForCp376 -le $cp376BindingIndex -or $cp378BindingIndexForCp376 -le $cp377BindingIndexForCp376 -or $cp376NumericalIndex -le $cp378BindingIndexForCp376) {
...
Assert-Contains -Path $cp376PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp378_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Cp376TextContains -Text $cp376InventoryText -Pattern 'script_count = 316' -Description "script total"
...
if ([regex]::Matches($cp376InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp376InventoryText, '(?m)^classification = "internal"$').Count -ne 76) {
    throw "CP376 inventory must be exactly 240 public and 76 internal scripts"
}
