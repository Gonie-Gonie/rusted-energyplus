...
if ($cp375BindingIndexForCp376 -lt 0 -or $cp376BindingIndex -le $cp375BindingIndexForCp376 -or
    $cp377BindingIndexForCp376 -le $cp376BindingIndex -or $cp378BindingIndexForCp376 -le $cp377BindingIndexForCp376 -or $cp379BindingIndexForCp376 -le $cp378BindingIndexForCp376 -or $cp376NumericalIndex -le $cp379BindingIndexForCp376) {
...
Assert-Contains -Path $cp376PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp379_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Cp376TextContains -Text $cp376InventoryText -Pattern 'script_count = 317' -Description "script total"
...
if ([regex]::Matches($cp376InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp376InventoryText, '(?m)^classification = "internal"$').Count -ne 77) {
    throw "CP376 inventory must be exactly 240 public and 77 internal scripts"
}
