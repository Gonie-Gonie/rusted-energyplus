...
Assert-Contains -Path $cp373PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp417_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 334..372) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp417_lifecycle_evidence' -Description "historical CP373 firewall"
}
...
Assert-Cp373TextContains -Text $cp373InventoryText -Pattern 'script_count = 355' -Description "script total"
...
[regex]::Matches($cp373InventoryText, '(?m)^classification = "internal"$').Count -ne 115) {
