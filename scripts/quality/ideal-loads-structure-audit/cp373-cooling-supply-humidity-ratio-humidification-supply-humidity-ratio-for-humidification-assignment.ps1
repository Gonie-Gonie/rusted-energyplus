...
$cp376BindingIndexForCp373 = $cp373BindingText.IndexOf("let calculation_cooling_supply_humidity_ratio_pre_saturation_original_assignment ="); $cp377BindingIndexForCp373 = $cp373BindingText.IndexOf("let calculation_cooling_supply_humidity_ratio_saturation_assignment =")
...
if ($cp372BindingIndexForCp373 -lt 0 -or $cp373BindingIndex -le $cp372BindingIndexForCp373 -or
    ...
    $cp376BindingIndexForCp373 -le $cp375BindingIndexForCp373 -or $cp377BindingIndexForCp373 -le $cp376BindingIndexForCp373 -or $cp373NumericalIndex -le $cp377BindingIndexForCp373) {
...
Assert-Contains -Path $cp373PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp377_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 334..372) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp377_lifecycle_evidence' -Description "historical CP373 firewall"
}
...
Assert-Cp373TextContains -Text $cp373InventoryText -Pattern 'script_count = 315' -Description "script total"
...
if ([regex]::Matches($cp373InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp373InventoryText, '(?m)^classification = "internal"$').Count -ne 75) {
