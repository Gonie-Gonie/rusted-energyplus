...
Assert-Contains -Path $pipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp435_lifecycle_evidence' -Description 'CP434 non-direct firewall'
...
foreach ($file in $firewall) { $text = Read-RepoText -Path $file.FullName; if (-not $text.Contains('non_direct_runtime_rejects_cp316_through_cp435_lifecycle_evidence') -or $text.Contains($staleFirewall)) { throw "CP434 firewall propagation drift $($file.Name)" } }
foreach ($file in $script) { $text = Read-RepoText -Path $file.FullName; if (-not $text.Contains('script_count = 373') -or $text.Contains($staleScript)) { throw "CP434 inventory-total propagation drift $($file.Name)" } }
foreach ($file in $internal) { $text = Read-RepoText -Path $file.FullName; if ($text -notmatch '(?s)classification\s*=\s*"internal".{0,120}\.Count\s*-ne\s*133' -or $text -match '(?s)classification\s*=\s*"internal".{0,120}\.Count\s*-ne\s*131') { throw "CP434 internal propagation drift $($file.Name)" } }
foreach ($file in $generated) { $text = Read-RepoText -Path $file.FullName; if (-not $text.Contains('\| 373 \|') -or -not $text.Contains('\| 133 \|') -or $text.Contains($staleGeneratedTotal) -or $text.Contains($staleGeneratedInternal)) { throw "CP434 generated propagation drift $($file.Name)" } }
...
foreach ($file in $helpers) { $text = Read-RepoText -Path $file.FullName; if (-not $text.Contains("calculation_$stem") -or -not $text.Contains("advance_$stem") -or -not $text.Contains('CP340 through CP435')) { throw "CP434 helper strip/whitelist drift $($file.Name)" } }
...
$staleLiveBinding = 'current_12' + '5_snapshot_binding|binding_is_12' + '5|(?:binding|calculation_fields|scheduledFields|scheduled|pub calculation_)[^\r\n]{0,240}(?:Count-ne12' + '5|,\s*\\s\*12' + '5)'
foreach ($file in $live) { $text = Read-RepoText -Path $file.FullName; if ($text -notmatch 'current_126_snapshot_binding|binding_is_126|Count-ne126|,\s*\\s\*126' -or $text -match $staleLiveBinding) { throw "CP434 current-126 propagation drift $($file.Name)" } }
...
foreach ($file in $manualAudits) { if (-not (Read-RepoText -Path $file.FullName).Contains('+2169+3638')) { throw "CP434 manual cumulative propagation drift $($file.Name)" } }
$manuals = @(
...
foreach ($manual in $manuals) {
    ...
    if ($bytes.Length -ne $manual.Length + 2169 + 3638 -or [Array]::IndexOf($bytes,[byte]13) -ne -1 -or $bytes[-1] -ne 10) { throw "CP434 manual byte shape drift $($manual.Path)" }
...
foreach ($pattern in @('script_count = 373','dev_command_count = 238','unused_script_count = 0','unreachable_count = 0')) { if (-not $inventory.Contains($pattern)) { throw "CP434 inventory missing $pattern" } }
if ([regex]::Matches($inventory,'(?m)^classification = "public"\r?$').Count -ne 240 -or [regex]::Matches($inventory,'(?m)^classification = "internal"\r?$').Count -ne 133) { throw 'CP434 inventory classification drift' }
...
Assert-Contains -Path 'docs\src\generated\script-index.md' -Pattern '\| 373 \|' -Description 'generated total'; Assert-Contains -Path 'docs\src\generated\script-index.md' -Pattern '\| 133 \|' -Description 'generated internal'
