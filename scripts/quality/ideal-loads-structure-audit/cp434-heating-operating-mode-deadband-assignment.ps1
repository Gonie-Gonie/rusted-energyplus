...
if ($core.Count -ne 11 -or @(Get-ChildItem "$root\tests" -File -Filter '*.rs').Count -ne 1) { throw 'CP434 exact eleven-file/one-test topology drift' }
...
if ($scheduledFields.Count -ne 125 -or $scheduledFields[123] -cne 'calculation_heating_mode_guard_else_branch_entry' -or $scheduledFields[124] -cne 'calculation_heating_operating_mode_deadband_assignment') { throw 'CP434 exact binding index 123/124/current 125 drift' }
foreach ($test in @('scheduled_binding_advances_cp434_after_cp433_before_unchanged_coupling','cp434_adapter_accepts_only_the_cp433_snapshot_and_no_scalar_input','cp434_extends_current_scheduled_binding_from_124_to_125_snapshots')) { Assert-Contains -Path $adapterTests -Pattern $test -Description 'CP434 adapter/current-binding regression' }
...
Assert-Contains -Path $pipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp434_lifecycle_evidence' -Description 'CP434 non-direct firewall'
...
foreach ($file in $firewall) { $text = Read-RepoText -Path $file.FullName; if (-not $text.Contains('non_direct_runtime_rejects_cp316_through_cp434_lifecycle_evidence') -or $text.Contains($staleFirewall)) { throw "CP434 firewall propagation drift $($file.Name)" } }
foreach ($file in $script) { $text = Read-RepoText -Path $file.FullName; if (-not $text.Contains('script_count = 372') -or $text.Contains($staleScript)) { throw "CP434 inventory-total propagation drift $($file.Name)" } }
foreach ($file in $internal) { $text = Read-RepoText -Path $file.FullName; if ($text -notmatch '(?s)classification\s*=\s*"internal".{0,120}\.Count\s*-ne\s*132' -or $text -match '(?s)classification\s*=\s*"internal".{0,120}\.Count\s*-ne\s*131') { throw "CP434 internal propagation drift $($file.Name)" } }
foreach ($file in $generated) { $text = Read-RepoText -Path $file.FullName; if (-not $text.Contains('\| 372 \|') -or -not $text.Contains('\| 132 \|') -or $text.Contains($staleGeneratedTotal) -or $text.Contains($staleGeneratedInternal)) { throw "CP434 generated propagation drift $($file.Name)" } }
...
foreach ($file in $helpers) { $text = Read-RepoText -Path $file.FullName; if (-not $text.Contains("calculation_$stem") -or -not $text.Contains("advance_$stem") -or -not $text.Contains('CP340 through CP434')) { throw "CP434 helper strip/whitelist drift $($file.Name)" } }
...
foreach ($file in $terminal) { $text = Read-RepoText -Path $file.FullName; if (-not $text.Contains('$cp434Call') -or -not $text.Contains('CP433-to-CP434') -or -not $text.Contains('CP434-to-numerical') -or -not $text.Contains("-Pattern ('Assert-Contains[^\r\n]+CP433-to-' + 'numerical')")) { throw "CP434 terminal propagation drift $($file.Name)" } }
...
foreach ($file in $live) { $text = Read-RepoText -Path $file.FullName; if ($text -notmatch 'current_125_snapshot_binding|binding_is_125|Count-ne125|,\s*\\s\*125' -or $text -match 'current_124_snapshot_binding|binding_is_124|Count-ne124|,\s*\\s\*124') { throw "CP434 current-125 propagation drift $($file.Name)" } }
...
foreach ($file in $manualAudits) { if (-not (Read-RepoText -Path $file.FullName).Contains('+2169')) { throw "CP434 manual cumulative propagation drift $($file.Name)" } }
$manuals = @(
...
foreach ($manual in $manuals) {
    ...
    if ($bytes.Length -ne $manual.Length + 2169 -or [Array]::IndexOf($bytes,[byte]13) -ne -1 -or $bytes[-1] -ne 10) { throw "CP434 manual byte shape drift $($manual.Path)" }
...
foreach ($pattern in @('script_count = 372','dev_command_count = 238','unused_script_count = 0','unreachable_count = 0')) { if (-not $inventory.Contains($pattern)) { throw "CP434 inventory missing $pattern" } }
if ([regex]::Matches($inventory,'(?m)^classification = "public"\r?$').Count -ne 240 -or [regex]::Matches($inventory,'(?m)^classification = "internal"\r?$').Count -ne 132) { throw 'CP434 inventory classification drift' }
...
Assert-Contains -Path 'docs\src\generated\script-index.md' -Pattern '\| 372 \|' -Description 'generated total'; Assert-Contains -Path 'docs\src\generated\script-index.md' -Pattern '\| 132 \|' -Description 'generated internal'
...
if ((Get-Content 'scripts\quality\ideal-loads-structure-audit.ps1').Count -ne 4200 -or (Get-Content 'scripts\quality\ideal-loads-structure-audit\cp345-cooling-positive-supply-post-capacity-limit-humidity-ratio-mixed-air-assignment.ps1').Count -ne 1200 -or (Get-Content 'crates\ep_runtime\src\ideal_loads\calc.rs').Count -ne 99 -or (Get-Content 'crates\ep_runtime\src\ideal_loads\init\state\witnesses.rs').Count -ne 272 -or @(Get-Content 'crates\ep_runtime\src\ideal_loads\init\state.rs' | Where-Object { $_ -match '\S' }).Count -ne 380 -or (Get-Content $scheduled).Count -gt 500) { throw 'CP434 fixed cap drift' }
