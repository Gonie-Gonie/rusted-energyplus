...
Assert-Contains -Path $pipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp436_lifecycle_evidence' -Description 'CP434 non-direct firewall'
...
foreach ($file in $firewall) { $text = Read-RepoText -Path $file.FullName; if (-not $text.Contains('non_direct_runtime_rejects_cp316_through_cp436_lifecycle_evidence') -or $text.Contains($staleFirewall)) { throw "CP434 firewall propagation drift $($file.Name)" } }
foreach ($file in $script) { $text = Read-RepoText -Path $file.FullName; if (-not $text.Contains('script_count = 374') -or $text.Contains($staleScript)) { throw "CP434 inventory-total propagation drift $($file.Name)" } }
foreach ($file in $internal) { $text = Read-RepoText -Path $file.FullName; if ($text -notmatch '(?s)classification\s*=\s*"internal".{0,120}\.Count\s*-ne\s*134' -or $text -match '(?s)classification\s*=\s*"internal".{0,120}\.Count\s*-ne\s*131') { throw "CP434 internal propagation drift $($file.Name)" } }
foreach ($file in $generated) { $text = Read-RepoText -Path $file.FullName; if (-not $text.Contains('\| 374 \|') -or -not $text.Contains('\| 134 \|') -or $text.Contains($staleGeneratedTotal) -or $text.Contains($staleGeneratedInternal)) { throw "CP434 generated propagation drift $($file.Name)" } }
...
foreach ($file in $helpers) { $text = Read-RepoText -Path $file.FullName; if (-not $text.Contains("calculation_$stem") -or -not $text.Contains("advance_$stem") -or -not $text.Contains('CP340 through CP436')) { throw "CP434 helper strip/whitelist drift $($file.Name)" } }
...
foreach ($file in $live) { $text = Read-RepoText -Path $file.FullName; if ($text -notmatch 'current_127_snapshot_binding|binding_is_127|Count-ne127|,\s*\\s\*127' -or $text -match $staleLiveBinding) { throw "CP434 current-127 propagation drift $($file.Name)" } }
...
foreach ($file in $manualAudits) { if (-not (Read-RepoText -Path $file.FullName).Contains('+2169+3638+3009')) { throw "CP434 manual cumulative propagation drift $($file.Name)" } }
$manuals = @(
...
foreach ($manual in $manuals) {
    ...
    if ($bytes.Length -ne $manual.Length + 2169 + 3638 + 3009 -or [Array]::IndexOf($bytes,[byte]13) -ne -1 -or $bytes[-1] -ne 10) { throw "CP434 manual byte shape drift $($manual.Path)" }
...
foreach ($pattern in @('script_count = 374','dev_command_count = 238','unused_script_count = 0','unreachable_count = 0')) { if (-not $inventory.Contains($pattern)) { throw "CP434 inventory missing $pattern" } }
if ([regex]::Matches($inventory,'(?m)^classification = "public"\r?$').Count -ne 240 -or [regex]::Matches($inventory,'(?m)^classification = "internal"\r?$').Count -ne 134) { throw 'CP434 inventory classification drift' }
...
Assert-Contains -Path 'docs\src\generated\script-index.md' -Pattern '\| 374 \|' -Description 'generated total'; Assert-Contains -Path 'docs\src\generated\script-index.md' -Pattern '\| 134 \|' -Description 'generated internal'
...
if ((Get-Content 'scripts\quality\ideal-loads-structure-audit.ps1').Count -ne 4200 -or (Get-Content 'scripts\quality\ideal-loads-structure-audit\cp345-cooling-positive-supply-post-capacity-limit-humidity-ratio-mixed-air-assignment.ps1').Count -ne 1200 -or (Get-Content 'crates\ep_runtime\src\ideal_loads\calc.rs').Count -ne 99 -or (Get-Content 'crates\ep_runtime\src\ideal_loads\init\state\witnesses.rs').Count -ne 273 -or @(Get-Content 'crates\ep_runtime\src\ideal_loads\init\state.rs' | Where-Object { $_ -match '\S' }).Count -ne 380 -or (Get-Content $scheduled).Count -gt 500) { throw 'CP434 fixed cap drift' }
Write-Host 'CP434 heating operating-mode DeadBand assignment structure audit passed.'
}
Assert-Contains -Path 'crates\ep_runtime\src\ideal_loads\binding.rs' -Pattern 'calculation_heating_outdoor_air_maximum_flow_body_volume_flow_assignment' -Description 'CP436 recent binding propagation'
