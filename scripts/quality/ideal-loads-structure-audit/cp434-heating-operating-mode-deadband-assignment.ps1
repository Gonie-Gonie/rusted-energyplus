...
Assert-Contains -Path $pipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp438_lifecycle_evidence' -Description 'CP434 non-direct firewall'
...
foreach ($file in $firewall) { $text = Read-RepoText -Path $file.FullName; if (-not $text.Contains('non_direct_runtime_rejects_cp316_through_cp438_lifecycle_evidence') -or $text.Contains($staleFirewall)) { throw "CP434 firewall propagation drift $($file.Name)" } }
foreach ($file in $script) { $text = Read-RepoText -Path $file.FullName; if (-not $text.Contains('script_count = 376') -or $text.Contains($staleScript)) { throw "CP434 inventory-total propagation drift $($file.Name)" } }
foreach ($file in $internal) { $text = Read-RepoText -Path $file.FullName; if ($text -notmatch '(?s)classification\s*=\s*"internal".{0,120}\.Count\s*-ne\s*136' -or $text -match '(?s)classification\s*=\s*"internal".{0,120}\.Count\s*-ne\s*131') { throw "CP434 internal propagation drift $($file.Name)" } }
foreach ($file in $generated) { $text = Read-RepoText -Path $file.FullName; if (-not $text.Contains('\| 376 \|') -or -not $text.Contains('\| 136 \|') -or $text.Contains($staleGeneratedTotal) -or $text.Contains($staleGeneratedInternal)) { throw "CP434 generated propagation drift $($file.Name)" } }
...
foreach ($file in $helpers) { $text = Read-RepoText -Path $file.FullName; if (-not $text.Contains("calculation_$stem") -or -not $text.Contains("advance_$stem") -or -not $text.Contains('CP340 through CP438')) { throw "CP434 helper strip/whitelist drift $($file.Name)" } }
...
foreach ($file in $live) { $text = Read-RepoText -Path $file.FullName; if ($text -notmatch 'current_129_snapshot_binding|binding_is_129|Count-ne129|,\s*\\s\*129' -or $text -match $staleLiveBinding) { throw "CP434 current-129 propagation drift $($file.Name)" } }
...
foreach ($manual in $manuals) {
    ...
    if ($bytes.Length -ne $manual.Length + 2169 + 3638 + 3009 + 3458 + 3655 -or [Array]::IndexOf($bytes,[byte]13) -ne -1 -or $bytes[-1] -ne 10) { throw "CP434 manual byte shape drift $($manual.Path)" }
...
foreach ($pattern in @('script_count = 376','dev_command_count = 238','unused_script_count = 0','unreachable_count = 0')) { if (-not $inventory.Contains($pattern)) { throw "CP434 inventory missing $pattern" } }
if ([regex]::Matches($inventory,'(?m)^classification = "public"\r?$').Count -ne 240 -or [regex]::Matches($inventory,'(?m)^classification = "internal"\r?$').Count -ne 136) { throw 'CP434 inventory classification drift' }
...
Assert-Contains -Path 'docs\src\generated\script-index.md' -Pattern '\| 376 \|' -Description 'generated total'; Assert-Contains -Path 'docs\src\generated\script-index.md' -Pattern '\| 136 \|' -Description 'generated internal'
