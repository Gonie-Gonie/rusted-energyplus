...
$cp341BindingIntervalCode = [regex]::Replace(
    ...
    $cp341BindingIntervalCode = [regex]::Replace(
        $cp341BindingIntervalCode,
        '(?s)(?:let calculation_heating_mode_guard_else_branch_entry =\s*advance_heating_mode_guard_else_branch_entry\([^;]+?\)\?;|let calculation_heating_operating_mode_deadband_assignment =\s*advance_heating_operating_mode_deadband_assignment\([^;]+?\)\?;|let calculation_heating_outdoor_air_maximum_flow_guard =\s*advance_heating_outdoor_air_maximum_flow_guard\([^;]+?\)\?;|let calculation_heating_outdoor_air_maximum_flow_body_volume_flow_assignment =\s*advance_heating_outdoor_air_maximum_flow_body_volume_flow_assignment\([^;]+?\)\?;|let calculation_heating_outdoor_air_maximum_flow_first_warning_guard =\s*advance_heating_outdoor_air_maximum_flow_first_warning_guard\([^;]+?\)\?;)',
        ''
    )
...
Assert-Contains -Path $cp341CoupledRuntimeTests -Pattern 'cp347_direct_coupled_runtime_completes_none_case_after_g_f_l_and_skips_unit_off' -Description "cumulative direct coupled CP341-through-CP347 post-capacity join/skip regression"
...
Assert-Contains -Path $cp341PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp437_lifecycle_evidence' -Description "non-direct CP341 through CP363 evidence rejection"
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 375' -Description "CP341 cumulative inventory total through CP403"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| 375 \|' -Description "CP341 generated script count through CP403"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| 135 \|' -Description "CP341 generated internal script count through CP403"
