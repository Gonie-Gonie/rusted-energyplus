...
if ($cp386Index -lt 0 -or $cp387Index -le $cp386Index -or $cp388Index -le $cp387Index -or $cp389Index -le $cp388Index -or $numericalIndex -le $cp389Index) {
    throw "Binding must execute CP386, CP387, CP388, CP389, then unchanged numerical coupling"
}
...
$nonDirectTest = Get-Cp387BraceBlock -Text $pipelineText -AnchorPattern 'fn\s+non_direct_runtime_rejects_cp316_through_cp389_lifecycle_evidence\s*\(' -Description "cumulative non-direct regression"
...
foreach ($pattern in @('\$cp386Call\s*=', '\$cp387Call\s*=', '\$cp388Call\s*=', '\$cp389Call\s*=', 'CP386-to-CP387', 'CP387-to-CP388', 'CP388-to-CP389', 'CP389-to-numerical')) {
...
if ($cp386AuditIndex -lt 0 -or $cp387AuditIndex -le $cp386AuditIndex -or $cp388AuditIndex -le $cp387AuditIndex -or $cp389AuditIndex -le $cp388AuditIndex -or $completionIndex -le $cp389AuditIndex) {
...
throw "CP387 inventory must be exactly 240 public and 87 internal scripts"
}
