...
if ($cp386Index -lt 0 -or $cp387Index -le $cp386Index -or $cp388Index -le $cp387Index -or $cp389Index -le $cp388Index -or $cp390Index -le $cp389Index -or $numericalIndex -le $cp390Index) {
    throw "Binding must execute CP386, CP387, CP388, CP389, CP390, then unchanged numerical coupling"
}
...
$nonDirectTest = Get-Cp387BraceBlock -Text $pipelineText -AnchorPattern 'fn\s+non_direct_runtime_rejects_cp316_through_cp390_lifecycle_evidence\s*\(' -Description "cumulative non-direct regression"
...
if ($cp386AuditIndex -lt 0 -or $cp387AuditIndex -le $cp386AuditIndex -or $cp388AuditIndex -le $cp387AuditIndex -or $cp389AuditIndex -le $cp388AuditIndex -or $cp390AuditIndex -le $cp389AuditIndex -or $completionIndex -le $cp390AuditIndex) {
    throw "Master audit must dot-source CP387 through CP389 after CP386 before completion"
}
...
throw "CP387 inventory must be exactly 240 public and 88 internal scripts"
}
