...
Assert-Contains -Path $cp382PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp421_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($pattern in @(
        'script_count = 359', 'dev_command_count = 238',
...
if ([regex]::Matches($cp382InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp382InventoryText, '(?m)^classification = "internal"$').Count -ne 119) {
    throw "CP382 inventory must be exactly 240 public and 119 internal scripts"
}
