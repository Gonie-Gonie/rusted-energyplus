...
Assert-Contains -Path $cp382PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp390_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($pattern in @(
        'script_count = 328', 'dev_command_count = 238',
...
if ([regex]::Matches($cp382InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp382InventoryText, '(?m)^classification = "internal"$').Count -ne 88) {
    throw "CP382 inventory must be exactly 240 public and 88 internal scripts"
}
