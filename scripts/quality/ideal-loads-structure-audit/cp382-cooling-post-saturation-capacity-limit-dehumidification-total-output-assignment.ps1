...
Assert-Contains -Path $cp382PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp406_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($pattern in @(
        'script_count = 344', 'dev_command_count = 238',
...
if ([regex]::Matches($cp382InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp382InventoryText, '(?m)^classification = "internal"$').Count -ne 104) {
    throw "CP382 inventory must be exactly 240 public and 104 internal scripts"
}
...
foreach ($pattern in @(
        '\| executable script records \| 344 \|',
        '\| public scripts \| 240 \|',
        '\| internal scripts \| 104 \|',
