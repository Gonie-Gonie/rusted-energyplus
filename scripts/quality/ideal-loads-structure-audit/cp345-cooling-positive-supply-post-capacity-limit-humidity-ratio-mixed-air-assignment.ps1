...
function Assert-Cp345BindingContract {
    ...
    if (
        ...
        $numerical.Index -lt ($cp370Call.Index + $cp370Call.Length)
    ) {
        throw "Binding must execute CP344 through CP370 in source order before unchanged numerical coupling"
    }
    foreach ($interval in @(
            [PSCustomObject]@{
            ...
            [PSCustomObject]@{
                ...
                End = $numerical.Index
                Description = "CP370-to-numerical"
            }
        )) {
...
Assert-Contains -Path $cp345PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp370_lifecycle_evidence' -Description "pipeline cumulative non-direct CP362 firewall regression"
...
foreach ($audit in @(
    ...
    Assert-Contains -Path $audit -Pattern 'non_direct_runtime_rejects_cp316_through_cp370_lifecycle_evidence' -Description "historical non-direct firewall reaches CP362"
}
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 308' -Description "CP345 cumulative inventory total through CP358"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| executable script records \| 308 \|' -Description "CP345 generated script total through CP358"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| internal scripts \| 68 \|' -Description "CP345 generated internal script total through CP358"
