...
function Assert-Cp345BindingContract {
    ...
    if (
        ...
        -not $cp376Call.Success -or -not $cp377Call.Success -or -not $cp378Call.Success -or -not $cp379Call.Success -or -not $cp380Call.Success -or
        ...
        $cp376Call.Index -lt ($cp375Call.Index + $cp375Call.Length) -or $cp377Call.Index -lt ($cp376Call.Index + $cp376Call.Length) -or $cp378Call.Index -lt ($cp377Call.Index + $cp377Call.Length) -or $cp379Call.Index -lt ($cp378Call.Index + $cp378Call.Length) -or $cp380Call.Index -lt ($cp379Call.Index + $cp379Call.Length) -or
        $numerical.Index -lt ($cp380Call.Index + $cp380Call.Length)
    ) {
        throw "Binding must execute CP344 through CP380 in source order before unchanged numerical coupling"
    }
    foreach ($interval in @(
            [PSCustomObject]@{
            ...
            [PSCustomObject]@{ Start = $cp376Call.Index + $cp376Call.Length; End = $cp377Call.Index; Description = "CP376-to-CP377" }, [PSCustomObject]@{ Start = $cp377Call.Index + $cp377Call.Length; End = $cp378Call.Index; Description = "CP377-to-CP378" }, [PSCustomObject]@{ Start = $cp378Call.Index + $cp378Call.Length; End = $cp379Call.Index; Description = "CP378-to-CP379" }, [PSCustomObject]@{ Start = $cp379Call.Index + $cp379Call.Length; End = $cp380Call.Index; Description = "CP379-to-CP380" }, [PSCustomObject]@{ Start = $cp380Call.Index + $cp380Call.Length; End = $numerical.Index; Description = "CP380-to-numerical" }
        )) {
...
Assert-Contains -Path $cp345PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp380_lifecycle_evidence' -Description "pipeline cumulative non-direct CP362 firewall regression"
...
foreach ($audit in @(
    ...
    Assert-Contains -Path $audit -Pattern 'non_direct_runtime_rejects_cp316_through_cp380_lifecycle_evidence' -Description "historical non-direct firewall reaches CP362"
}
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 318' -Description "CP345 cumulative inventory total through CP358"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| executable script records \| 318 \|' -Description "CP345 generated script total through CP358"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| internal scripts \| 78 \|' -Description "CP345 generated internal script total through CP358"
