...
function Assert-Cp345BindingContract {
    ...
    foreach ($interval in @(
        ...
        $code = [regex]::Replace($code, '(?s)/\*.*?\*/|(?m)//.*$', ''); $code = [regex]::Replace($code, '(?s)let calculation_cooling_post_saturation_capacity_limit_dehumidification_supply_temperature_saturation_mixed_air_limit =\s*advance_cooling_post_saturation_capacity_limit_dehumidification_supply_temperature_saturation_mixed_air_limit\([^;]+?\)\?;', '')
...
Assert-Contains -Path $cp345PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp415_lifecycle_evidence' -Description "pipeline cumulative non-direct CP362 firewall regression"
...
foreach ($audit in @(
    ...
    Assert-Contains -Path $audit -Pattern 'non_direct_runtime_rejects_cp316_through_cp415_lifecycle_evidence' -Description "historical non-direct firewall reaches CP362"
}
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 353' -Description "CP345 cumulative inventory total through CP403"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| 353 \|' -Description "CP346 generated script total through CP403"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| 113 \|' -Description "CP346 generated internal script total through CP403"
