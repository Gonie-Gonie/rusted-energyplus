...
function Assert-Cp345BindingContract {
    ...
    $cp376Call = [regex]::Match($body, '(?s)let calculation_cooling_supply_humidity_ratio_pre_saturation_original_assignment =\s*advance_cooling_supply_humidity_ratio_pre_saturation_original_assignment\([^;]+?\)\?;')
    ...
    if (
        ...
        $cp376Call.Index -lt ($cp375Call.Index + $cp375Call.Length) -or
        $numerical.Index -lt ($cp376Call.Index + $cp376Call.Length)
    ) {
        throw "Binding must execute CP344 through CP376 in source order before unchanged numerical coupling"
    }
    foreach ($interval in @(
            [PSCustomObject]@{
            ...
            [PSCustomObject]@{ Start = $cp376Call.Index + $cp376Call.Length; End = $numerical.Index; Description = "CP376-to-numerical" }
        )) {
...
Assert-Contains -Path $cp345PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp376_lifecycle_evidence' -Description "pipeline cumulative non-direct CP362 firewall regression"
...
foreach ($audit in @(
    ...
    Assert-Contains -Path $audit -Pattern 'non_direct_runtime_rejects_cp316_through_cp376_lifecycle_evidence' -Description "historical non-direct firewall reaches CP362"
}
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 314' -Description "CP345 cumulative inventory total through CP358"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| executable script records \| 314 \|' -Description "CP345 generated script total through CP358"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| internal scripts \| 74 \|' -Description "CP345 generated internal script total through CP358"
