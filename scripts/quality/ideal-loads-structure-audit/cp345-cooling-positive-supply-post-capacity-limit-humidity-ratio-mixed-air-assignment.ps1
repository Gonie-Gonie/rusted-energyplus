...
function Assert-Cp345BindingContract {
    ...
    if (
        ...
        $numerical.Index -lt ($cp360Call.Index + $cp360Call.Length)
    ) {
        throw "Binding must execute CP344 then CP345 then CP346 then CP347 then CP348 then CP349 then CP350 then CP351 then CP352 then CP353 then CP354 then CP355 then CP356 then CP357 then CP358 then CP359 then CP360 before unchanged numerical coupling"
    }
    foreach ($interval in @(
            [PSCustomObject]@{
            ...
            [PSCustomObject]@{
                ...
                End = $numerical.Index
                Description = "CP360-to-numerical"
            }
        )) {
...
Assert-Contains -Path $cp345PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp360_lifecycle_evidence' -Description "pipeline cumulative non-direct CP357 firewall regression"
...
foreach ($audit in @(
    ...
    Assert-Contains -Path $audit -Pattern 'non_direct_runtime_rejects_cp316_through_cp360_lifecycle_evidence' -Description "historical non-direct firewall reaches CP357"
}
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 298' -Description "CP345 cumulative inventory total through CP358"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| executable script records \| 298 \|' -Description "CP345 generated script total through CP358"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| internal scripts \| 58 \|' -Description "CP345 generated internal script total through CP358"
