...
function Assert-Cp345BindingContract {
    ...
    if (
        ...
        -not $cp376Call.Success -or -not $cp377Call.Success -or -not $cp378Call.Success -or -not $cp379Call.Success -or -not $cp380Call.Success -or -not $cp381Call.Success -or -not $cp382Call.Success -or -not $cp383Call.Success -or -not $cp384Call.Success -or -not $cp385Call.Success -or -not $cp386Call.Success -or -not $cp387Call.Success -or -not $cp388Call.Success -or -not $cp389Call.Success -or -not $cp390Call.Success -or -not $cp391Call.Success -or -not $cp392Call.Success -or -not $cp393Call.Success -or -not $cp394Call.Success -or -not $cp395Call.Success -or -not $cp396Call.Success -or -not $cp397Call.Success -or
        ...
        $numerical.Index -lt ($cp397Call.Index + $cp397Call.Length)
    ) {
        throw "Binding must execute CP344 through CP388 in source order before unchanged numerical coupling"
    }
    foreach ($interval in @(
            [PSCustomObject]@{
...
Assert-Contains -Path $cp345PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp397_lifecycle_evidence' -Description "pipeline cumulative non-direct CP362 firewall regression"
...
foreach ($audit in @(
    ...
    Assert-Contains -Path $audit -Pattern 'non_direct_runtime_rejects_cp316_through_cp397_lifecycle_evidence' -Description "historical non-direct firewall reaches CP362"
}
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 335' -Description "CP345 cumulative inventory total through CP358"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| executable script records \| 335 \|' -Description "CP345 generated script total through CP358"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| internal scripts \| 95 \|' -Description "CP345 generated internal script total through CP358"
