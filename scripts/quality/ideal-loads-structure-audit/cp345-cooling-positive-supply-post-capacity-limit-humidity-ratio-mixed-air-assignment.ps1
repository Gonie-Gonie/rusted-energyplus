...
function Assert-Cp345BindingContract {
    ...
    if (
        ...
        $numerical.Index -lt ($cp350Call.Index + $cp350Call.Length)
    ) {
        throw "Binding must execute CP344 then CP345 then CP346 then CP347 then CP348 then CP349 then CP350 before unchanged numerical coupling"
    }
    foreach ($interval in @(
            [PSCustomObject]@{
            ...
            [PSCustomObject]@{
                ...
                End = $numerical.Index
                Description = "CP350-to-numerical"
            }
        )) {
...
Assert-Contains -Path $cp345PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp350_lifecycle_evidence' -Description "pipeline cumulative non-direct CP349 firewall regression"
...
foreach ($audit in @(
    ...
    Assert-Contains -Path $audit -Pattern 'non_direct_runtime_rejects_cp316_through_cp350_lifecycle_evidence' -Description "historical non-direct firewall reaches CP349"
}
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 288' -Description "CP345 cumulative inventory total through CP350"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| executable script records \| 288 \|' -Description "CP345 generated script total through CP350"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| internal scripts \| 48 \|' -Description "CP345 generated internal script total through CP350"
