...
function Assert-Cp345BindingContract {
    ...
    if (
        ...
        $numerical.Index -lt ($cp407Call.Index + $cp407Call.Length)
    ) {
        throw "Binding must execute CP344 through CP407 in source order before unchanged numerical coupling"
    }
    foreach ($interval in @(
            [PSCustomObject]@{
...
Assert-Contains -Path $cp345PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp407_lifecycle_evidence' -Description "pipeline cumulative non-direct CP362 firewall regression"
...
foreach ($audit in @(
    ...
    Assert-Contains -Path $audit -Pattern 'non_direct_runtime_rejects_cp316_through_cp407_lifecycle_evidence' -Description "historical non-direct firewall reaches CP362"
}
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 345' -Description "CP345 cumulative inventory total through CP403"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| 345 \|' -Description "CP345 generated script total through CP403"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| 105 \|' -Description "CP345 generated internal script total through CP403"
