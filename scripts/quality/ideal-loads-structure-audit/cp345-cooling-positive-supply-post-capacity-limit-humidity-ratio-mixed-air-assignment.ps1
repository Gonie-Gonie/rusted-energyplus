...
function Assert-Cp345BindingContract {
    ...
    if (
        ...
        $numerical.Index -lt ($cp355Call.Index + $cp355Call.Length)
    ) {
        throw "Binding must execute CP344 then CP345 then CP346 then CP347 then CP348 then CP349 then CP350 then CP351 then CP352 then CP353 then CP354 then CP355 before unchanged numerical coupling"
    }
    foreach ($interval in @(
            [PSCustomObject]@{
            ...
            [PSCustomObject]@{
                ...
                End = $numerical.Index
                Description = "CP355-to-numerical"
            }
        )) {
...
Assert-Contains -Path $cp345PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp355_lifecycle_evidence' -Description "pipeline cumulative non-direct CP355 firewall regression"
...
foreach ($audit in @(
    ...
    Assert-Contains -Path $audit -Pattern 'non_direct_runtime_rejects_cp316_through_cp355_lifecycle_evidence' -Description "historical non-direct firewall reaches CP355"
}
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 293' -Description "CP345 cumulative inventory total through CP355"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| executable script records \| 293 \|' -Description "CP345 generated script total through CP355"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| internal scripts \| 53 \|' -Description "CP345 generated internal script total through CP355"
