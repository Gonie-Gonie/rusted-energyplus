...
if ($cp385Index -lt 0 -or $cp386Index -le $cp385Index -or $cp387Index -le $cp386Index -or $cp388Index -le $cp387Index -or $cp389Index -le $cp388Index -or $cp390Index -le $cp389Index -or $cp391Index -le $cp390Index -or $numericalIndex -le $cp391Index) {
...
$nonDirectTest = Get-Cp386BraceBlock `
    -Text $pipelineText `
    -AnchorPattern 'fn\s+non_direct_runtime_rejects_cp316_through_cp391_lifecycle_evidence\s*\(' `
...
if ($cp385AuditIndex -lt 0 -or $cp386AuditIndex -le $cp385AuditIndex -or $cp387AuditIndex -le $cp386AuditIndex -or $cp388AuditIndex -le $cp387AuditIndex -or $cp389AuditIndex -le $cp388AuditIndex -or $cp390AuditIndex -le $cp389AuditIndex -or $cp391AuditIndex -le $cp390AuditIndex -or $completionIndex -le $cp391AuditIndex) {
...
throw "CP386 inventory must be exactly 240 public and 89 internal scripts"
}
