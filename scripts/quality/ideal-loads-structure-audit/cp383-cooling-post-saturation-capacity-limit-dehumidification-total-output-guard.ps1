...
Assert-Contains -Path $cp383PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp400_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
if ([regex]::Matches($cp383InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp383InventoryText, '(?m)^classification = "internal"$').Count -ne 98) {
    throw "CP383 inventory must be exactly 240 public and 98 internal scripts"
}
