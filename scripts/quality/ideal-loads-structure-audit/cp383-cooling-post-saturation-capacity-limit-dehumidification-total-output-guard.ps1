...
Assert-Contains -Path $cp383PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp431_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($pattern in @(
        'script_count = 369', 'dev_command_count = 238',
...
if ([regex]::Matches($cp383InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp383InventoryText, '(?m)^classification = "internal"$').Count -ne 129) {
