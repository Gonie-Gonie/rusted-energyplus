...
$cp383PrefixValidation = "$cp383Root\release\prefix_validation.rs"
...
Assert-Contains -Path $cp383PrefixValidation -Pattern 'cooling_post_saturation_capacity_limit_dehumidification_total_output_assignment_committed_latest_snapshot_is_consistent\s*\(' -Description "bounded CP382 committed predecessor proof"
Assert-NotContains -Path $cp383PrefixValidation -Pattern 'completed_direct_cooling_post_saturation_capacity_limit_dehumidification_total_output_assignment_is_consistent\s*\(' -Description "recursive CP382 predecessor completion"
...
Assert-Contains -Path $cp383PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp440_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($pattern in @(
        'script_count = 378', 'dev_command_count = 238',
...
if ([regex]::Matches($cp383InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp383InventoryText, '(?m)^classification = "internal"$').Count -ne 138) {
    throw "CP383 inventory must be exactly 240 public and 136 internal scripts"
}
