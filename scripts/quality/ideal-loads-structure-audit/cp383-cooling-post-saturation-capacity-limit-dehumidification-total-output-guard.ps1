...
Assert-Contains -Path $cp383PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp413_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($pattern in @(
        'script_count = 351', 'dev_command_count = 238',
...
if ([regex]::Matches($cp383InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp383InventoryText, '(?m)^classification = "internal"$').Count -ne 111) {
    throw "CP383 inventory must be exactly 240 public and 110 internal scripts"
}
