...
if ($cp377BindingIndexForCp378 -lt 0 -or $cp378BindingIndex -le $cp377BindingIndexForCp378 -or
    $cp379BindingIndex -le $cp378BindingIndex -or $cp380BindingIndex -le $cp379BindingIndex -or $cp381BindingIndex -le $cp380BindingIndex -or $cp382BindingIndex -le $cp381BindingIndex -or $cp383BindingIndex -le $cp382BindingIndex -or $cp384BindingIndexForCp378 -le $cp383BindingIndex -or $cp378NumericalIndex -le $cp384BindingIndexForCp378 -or $cp378ReconcileIndex -le $cp378NumericalIndex) {
...
Assert-Contains -Path $cp378PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp384_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Contains -Path $cp345Audit -Pattern 'CP378-to-CP379' -Description "CP345 CP379 predecessor interval"; Assert-Contains -Path $cp345Audit -Pattern 'CP379-to-CP380' -Description "CP345 CP380 predecessor interval"; Assert-Contains -Path $cp345Audit -Pattern 'CP384-to-numerical' -Description "CP345 terminal interval"
...
if ([regex]::Matches($cp378InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp378InventoryText, '(?m)^classification = "internal"$').Count -ne 82) {
    throw "CP378 inventory must be exactly 240 public and 82 internal scripts"
}
