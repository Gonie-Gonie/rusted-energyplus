...
Assert-Contains -Path $cp378PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp420_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
[regex]::Matches($cp378InventoryText, '(?m)^classification = "internal"$').Count -ne 118) {
throw "CP378 inventory must be exactly 240 public and 118 internal scripts"
}
