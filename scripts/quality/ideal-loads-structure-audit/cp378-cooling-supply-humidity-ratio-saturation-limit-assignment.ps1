...
Assert-Contains -Path $cp378PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp403_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
[regex]::Matches($cp378InventoryText, '(?m)^classification = "internal"$').Count -ne 101) {
throw "CP378 inventory must be exactly 240 public and 101 internal scripts"
}
