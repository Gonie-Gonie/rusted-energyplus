...
$cp378BindingIndex = $cp378BindingText.IndexOf("let calculation_${cp378Stem} =")
...
if ($cp377BindingIndexForCp378 -lt 0 -or $cp378BindingIndex -le $cp377BindingIndexForCp378 -or
    $cp378NumericalIndex -le $cp378BindingIndex -or $cp378ReconcileIndex -le $cp378NumericalIndex) {
    throw "Binding must execute CP377, CP378, unchanged numerical coupling, then reconciliation"
}
...
Assert-Contains -Path $cp378PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp378_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 334..377) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp378_lifecycle_evidence' -Description "historical firewall"
}
...
foreach ($historical in @('cp326-cooling-supply-mass-flow-limit-body.ps1') + @(329..359 | ForEach-Object { (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$($_)-*.ps1").Name })) {
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$historical" -Pattern "calculation_$cp378Stem" -Description "historical CP378 binding order"
}
foreach ($historical in 360..377) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'cp378BindingIndex' -Description "historical CP378 binding index"
}
foreach ($historical in @('cp326-cooling-supply-mass-flow-limit-body.ps1') + @(329..345 | ForEach-Object { (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$($_)-*.ps1").Name })) {
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$historical" -Pattern "advance_$cp378Stem" -Description "historical CP378 helper whitelist"
}
...
Assert-Contains -Path $cp345Audit -Pattern 'CP378-to-numerical' -Description "CP345 terminal interval"
...
foreach ($historical in 364, 373, 374, 376, 377) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'cp378_assertions\.rs' -Description "historical CP378 terminal source owner"
}
...
foreach ($pattern in @('script_count = 316', 'dev_command_count = 238', 'unused_script_count = 0', 'unreachable_count = 0')) {
...
if ([regex]::Matches($cp378InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp378InventoryText, '(?m)^classification = "internal"$').Count -ne 76) {
    throw "CP378 inventory must be exactly 240 public and 76 internal scripts"
}
