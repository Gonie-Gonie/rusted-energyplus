...
Assert-Contains -Path $cp378PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp386_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
if ([regex]::Matches($cp378InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp378InventoryText, '(?m)^classification = "internal"$').Count -ne 84) {
    throw "CP378 inventory must be exactly 240 public and 84 internal scripts"
}
