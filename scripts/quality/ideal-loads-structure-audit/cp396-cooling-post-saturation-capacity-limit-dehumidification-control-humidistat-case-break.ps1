...
foreach ($pattern in @('\$cp396Call\s*=', 'CP395-to-CP396', 'CP396-to-CP397', 'CP397-to-CP398', 'CP398-to-numerical')) { Assert-Contains -Path $cp345Audit -Pattern $pattern -Description "CP345 terminal chain" }
...
foreach ($pattern in @('script_count = 336', 'dev_command_count = 238', 'unused_script_count = 0', 'unreachable_count = 0')) { Assert-Cp396Text -Text $inventory -Pattern $pattern -Description "inventory" }
if ([regex]::Matches($inventory, '(?m)^classification = "public"$').Count -ne 240 -or [regex]::Matches($inventory, '(?m)^classification = "internal"$').Count -ne 96) { throw "CP396 inventory classification drift" }
