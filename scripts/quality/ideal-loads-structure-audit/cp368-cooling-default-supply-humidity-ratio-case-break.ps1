...
foreach ($historical in 334..368) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp421_lifecycle_evidence' -Description "historical CP368 firewall"
}
foreach ($historical in 326..333) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-NotContains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp421_lifecycle_evidence' -Description "out-of-range CP368 firewall token"
}
...
Assert-Cp368TextContains -Text $cp368InventoryText -Pattern 'script_count = 359' -Description "script total"
...
[regex]::Matches($cp368InventoryText, '(?m)^classification = "internal"$').Count -ne 119) {
throw "CP368 inventory must be exactly 240 public and 119 internal scripts"
}
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| 359 \|' -Description "CP368 generated total"
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| public scripts \| 240 \|' -Description "CP368 generated public"
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| 119 \|' -Description "CP368 generated internal"
