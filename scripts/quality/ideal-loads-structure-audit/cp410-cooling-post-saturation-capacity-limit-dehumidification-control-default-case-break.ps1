...
$successorStem = 'cooling_post_saturation_capacity_limit_dehumidification_supply_humidity_ratio_pre_saturation_original_assignment'
$terminalStem = 'cooling_post_saturation_capacity_limit_dehumidification_supply_humidity_ratio_saturation_assignment'
...
Assert-Contains -Path $prefixValidation -Pattern 'cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_case_break_committed_latest_snapshot_is_consistent\s*\(' -Description 'bounded CP409 committed predecessor proof'
Assert-NotContains -Path $prefixValidation -Pattern 'completed_direct_cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_case_break_is_consistent\s*\(' -Description 'recursive CP409 predecessor completion'
...
Assert-PatternsInOrder -Path $binding -Patterns @("let\s+calculation_$predecessorStem\s*=","let\s+calculation_$stem\s*=","let\s+calculation_$successorStem\s*=","let\s+calculation_$terminalStem\s*=",'let\s+unit_available\s*=','let\s+coupling\s*=') -Description 'CP409-to-CP410-to-CP411-to-CP412-to-CP413-to-CP414-to-CP415 binding order'
Assert-PatternsInOrder -Path $scheduledOutput -Patterns @("pub\s+calculation_$predecessorStem\s*:","pub\s+calculation_$stem\s*:","pub\s+calculation_$successorStem\s*:","pub\s+calculation_$terminalStem\s*:",'pub\s+coupling\s*:') -Description 'CP409-to-CP410-to-CP411-to-CP412 scheduled output order'
$bindingText = Read-RepoText -Path $binding
if ([regex]::Matches($bindingText,"\bcalculation_$predecessorStem\b").Count -ne 3 -or [regex]::Matches($bindingText,"\bcalculation_$stem\b").Count -ne 3) { throw 'CP410 binding evidence occurrence drift' }
...
if ($dto -match '(?i)cp409|cp410|cp411|cp412|default_case_break|pre_saturation_original_assignment|saturation_assignment|constant_supply_humidity_ratio_case_break') { throw 'CP409/CP410/CP411/CP412 evidence entered numerical DTO' }
...
Assert-PatternsInOrder -Path $pipelineRoot -Patterns @("$predecessorStem::\s*validate_direct_lifecycle","$stem::\s*validate_direct_lifecycle","$successorStem::\s*validate_direct_lifecycle","$terminalStem::\s*validate_direct_lifecycle") -Description 'pipeline CP409-to-CP410-to-CP411-to-CP412 order'
...
foreach ($file in $audits) {
    ...
    if ($number -ge 334 -and $number -le 409) { Assert-Contains -Path $file.FullName -Pattern 'non_direct_runtime_rejects_cp316_through_cp431_lifecycle_evidence' -Description 'historical non-direct firewall' }
    if ($number -ge 337 -and $number -le 409) { Assert-Contains -Path $file.FullName -Pattern 'script_count = 369' -Description 'historical script count' }
    if ($number -ge 367 -and $number -le 409) { Assert-Contains -Path $file.FullName -Pattern 'Count -ne 129' -Description 'historical classification count' }
    if ($number -ge 335 -and $number -le 409) {
        Assert-Contains -Path $file.FullName -Pattern ([regex]::Escape('\| 369 \|')) -Description 'historical generated total'
        Assert-Contains -Path $file.FullName -Pattern ([regex]::Escape('\| 129 \|')) -Description 'historical generated internal total'
    }
}
...
foreach ($pattern in @('\$cp410Call\s*=','\$cp411Call\s*=','\$cp412Call\s*=','\$cp413Call\s*=','\$cp414Call\s*=','CP409-to-CP410','CP410-to-CP411','CP411-to-CP412','CP412-to-CP413','CP413-to-CP414','CP414-to-CP415')) { Assert-Contains -Path $cp345 -Pattern $pattern -Description 'CP345 terminal chain' }
...
foreach ($file in @($audits | Where-Object { $_.BaseName -match '^cp(?<number>399|400|401|402|403|404|405|406|407|408|409|410)-' })) { Assert-Contains -Path $file.FullName -Pattern 'calculation_cooling_post_saturation_capacity_limit_dehumidification_supply_humidity_ratio_pre_saturation_original_assignment' -Description 'recent CP411 binding order' }
...
foreach ($pattern in @('script_count = 369','dev_command_count = 238','unused_script_count = 0','unreachable_count = 0')) { Assert-Cp410Text -Text $inventory -Pattern $pattern -Description 'inventory count' }
if ([regex]::Matches($inventory,'(?m)^classification = "public"\r?$').Count -ne 240 -or [regex]::Matches($inventory,'(?m)^classification = "internal"\r?$').Count -ne 129) { throw 'CP410 inventory classification drift' }
...
Assert-Contains -Path 'docs\src\generated\script-index.md' -Pattern '\| 369 \|' -Description 'generated script total'
Assert-Contains -Path 'docs\src\generated\script-index.md' -Pattern '\| public scripts \| 240 \|' -Description 'generated public total'
Assert-Contains -Path 'docs\src\generated\script-index.md' -Pattern '\| 129 \|' -Description 'generated internal total'
