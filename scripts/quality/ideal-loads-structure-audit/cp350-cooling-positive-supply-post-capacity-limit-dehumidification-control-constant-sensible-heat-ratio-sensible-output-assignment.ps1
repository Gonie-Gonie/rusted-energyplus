...
function Assert-Cp350BindingContract {
    ...
    if ($cp349 -lt 0 -or $cp350 -le $cp349 -or $cp351 -le $cp350 -or $cp352 -le $cp351 -or $cp353 -le $cp352 -or $numerical -le $cp353) {
        throw "Binding must execute CP349 then CP350 then CP351 then CP352 then CP353 before numerical coupling"
    }
...
Assert-Contains -Path $cp350PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp353_lifecycle_evidence' -Description "cumulative firewall"
...
foreach ($historical in 334..349) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp353_lifecycle_evidence' -Description "historical CP351 firewall"
}
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 291' -Description "CP350 script total"
