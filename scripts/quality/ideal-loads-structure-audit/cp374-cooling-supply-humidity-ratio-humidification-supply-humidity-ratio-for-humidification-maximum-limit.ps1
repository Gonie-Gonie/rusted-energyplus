...
$cp376BindingIndexForCp374 = $cp374BindingText.IndexOf("let calculation_cooling_supply_humidity_ratio_pre_saturation_original_assignment ="); $cp377BindingIndexForCp374 = $cp374BindingText.IndexOf("let calculation_cooling_supply_humidity_ratio_saturation_assignment =")
...
if ($cp373BindingIndexForCp374 -lt 0 -or $cp374BindingIndex -le $cp373BindingIndexForCp374 -or
    ...
    $cp376BindingIndexForCp374 -le $cp375BindingIndexForCp374 -or $cp377BindingIndexForCp374 -le $cp376BindingIndexForCp374 -or $cp374NumericalIndex -le $cp377BindingIndexForCp374) {
...
Assert-Contains -Path "crates\ep_run\tests\arbitrary_run_ideal_loads\cp377_assertions.rs" -Pattern 'assert_numerical_nonfeed\(' -Description "CP377 terminal nonfeed"
Assert-Contains -Path $cp374PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp377_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 334..373) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp377_lifecycle_evidence' -Description "historical CP374 firewall"
}
...
Assert-Cp374TextContains -Text $cp374InventoryText -Pattern 'script_count = 315' -Description "script total"
...
if ([regex]::Matches($cp374InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp374InventoryText, '(?m)^classification = "internal"$').Count -ne 75) {
    throw "CP374 inventory must be exactly 240 public and 75 internal scripts"
}
