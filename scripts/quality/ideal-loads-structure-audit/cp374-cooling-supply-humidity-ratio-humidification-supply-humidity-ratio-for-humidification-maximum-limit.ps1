...
if ($cp373BindingIndexForCp374 -lt 0 -or $cp374BindingIndex -le $cp373BindingIndexForCp374 -or
    ...
    $cp376BindingIndexForCp374 -le $cp375BindingIndexForCp374 -or $cp377BindingIndexForCp374 -le $cp376BindingIndexForCp374 -or $cp378BindingIndexForCp374 -le $cp377BindingIndexForCp374 -or $cp379BindingIndexForCp374 -le $cp378BindingIndexForCp374 -or $cp380BindingIndexForCp374 -le $cp379BindingIndexForCp374 -or $cp381BindingIndexForCp374 -le $cp380BindingIndexForCp374 -or $cp382BindingIndexForCp374 -le $cp381BindingIndexForCp374 -or $cp383BindingIndexForCp374 -le $cp382BindingIndexForCp374 -or $cp374NumericalIndex -le $cp383BindingIndexForCp374) {
...
Assert-Contains -Path $cp374PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp383_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 334..373) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp383_lifecycle_evidence' -Description "historical CP374 firewall"
}
...
Assert-Cp374TextContains -Text $cp374InventoryText -Pattern 'script_count = 321' -Description "script total"
...
if ([regex]::Matches($cp374InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp374InventoryText, '(?m)^classification = "internal"$').Count -ne 81) {
    throw "CP374 inventory must be exactly 240 public and 81 internal scripts"
}
