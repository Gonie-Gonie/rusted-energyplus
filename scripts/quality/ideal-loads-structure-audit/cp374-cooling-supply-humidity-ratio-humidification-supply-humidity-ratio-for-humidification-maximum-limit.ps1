...
Assert-Contains -Path $cp374PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp407_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 334..373) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp407_lifecycle_evidence' -Description "historical CP374 firewall"
}
...
Assert-Cp374TextContains -Text $cp374InventoryText -Pattern 'script_count = 345' -Description "script total"
...
[regex]::Matches($cp374InventoryText, '(?m)^classification = "internal"$').Count -ne 105) {
