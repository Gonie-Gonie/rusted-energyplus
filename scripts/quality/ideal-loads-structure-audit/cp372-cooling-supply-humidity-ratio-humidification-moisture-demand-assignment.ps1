...
if ($cp371BindingIndexForCp372 -lt 0 -or $cp372BindingIndex -le $cp371BindingIndexForCp372 -or
    ...
    $cp372NumericalIndex -le $cp373BindingIndexForCp372) {
...
Assert-Contains -Path $cp372PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp373_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 334..371) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp373_lifecycle_evidence' -Description "historical CP372 firewall"
}
...
if ($cp371AuditIndexForCp372 -lt 0 -or $cp372AuditIndex -le $cp371AuditIndexForCp372 -or
    $cp373AuditIndexForCp372 -le $cp372AuditIndex -or $cp372CompletionIndex -le $cp373AuditIndexForCp372) {
...
Assert-Cp372TextContains -Text $cp372InventoryText -Pattern 'script_count = 311' -Description "script total"
...
if ([regex]::Matches($cp372InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp372InventoryText, '(?m)^classification = "internal"$').Count -ne 71) {
    throw "CP372 inventory must be exactly 240 public and 71 internal scripts"
}
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| executable script records \| 311 \|' -Description "CP372 generated total"
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| public scripts \| 240 \|' -Description "CP372 generated public"
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| internal scripts \| 71 \|' -Description "CP372 generated internal"
