...
function Assert-Cp347BindingContract {
    ...
    if ($cp346 -lt 0 -or $cp347 -le $cp346 -or $cp348 -le $cp347 -or $cp349 -le $cp348 -or $cp350 -le $cp349 -or $cp351 -le $cp350 -or $cp352 -le $cp351 -or $cp353 -le $cp352 -or $cp354 -le $cp353 -or $cp355 -le $cp354 -or $cp356 -le $cp355 -or $cp357 -le $cp356 -or $numerical -le $cp357) {
        throw "Binding must execute CP346 then CP347 then CP348 then CP349 then CP350 then CP351 then CP352 then CP353 then CP354 then CP355 then CP356 then CP357 before numerical coupling"
    }
...
Assert-Contains -Path $cp347PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp357_lifecycle_evidence' -Description "CP347 cumulative non-direct firewall"
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 295' -Description "CP347 script total"
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| executable script records \| 295 \|' -Description "generated CP347 script total"
