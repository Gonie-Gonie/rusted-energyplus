...
if ($cp369BindingIndexForCp370 -lt 0 -or $cp370BindingIndex -le $cp369BindingIndexForCp370 -or
    $cp370NumericalIndex -le $cp370BindingIndex) {
...
Assert-Contains -Path $cp370ArbitraryAssertions -Pattern 'assert_numerical_nonfeed\(' -Description "CP370 terminal numerical nonfeed"
...
foreach ($historical in 334..369) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp370_lifecycle_evidence' -Description "historical CP370 firewall"
}
...
Assert-Cp370TextContains -Text $cp370InventoryText -Pattern 'script_count = 308' -Description "script total"
...
if ([regex]::Matches($cp370InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp370InventoryText, '(?m)^classification = "internal"$').Count -ne 68) {
    throw "CP370 inventory must be exactly 240 public and 68 internal scripts"
}
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| executable script records \| 308 \|' -Description "CP370 generated total"
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| public scripts \| 240 \|' -Description "CP370 generated public"
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| internal scripts \| 68 \|' -Description "CP370 generated internal"
