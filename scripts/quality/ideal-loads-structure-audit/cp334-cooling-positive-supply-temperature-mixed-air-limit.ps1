...
# Binding must be CP333 -> CP334 -> CP335 -> CP336 -> the unchanged numerical DTO with
...
if (
    ...
    $numericalBindingIndexForCp334 -le $cp336BindingIndexForCp334
) {
    throw "Binding must retain exact CP333 -> CP334 -> CP335 -> CP336 -> numerical Calc order"
}
...
if (
    ...
    -not $cp336BindingCallForCp334.Success
) {
    throw "Binding must retain complete CP333, CP334, CP335, and CP336 exact release calls"
}
...
if (
    ...
    $numericalBindingIndexForCp334 -lt $cp336BindingCallEndForCp334
) {
...
$postCp336BeforeNumericalForCp334 = $cp334BindingText.Substring(
    $cp336BindingCallEndForCp334,
    $numericalBindingIndexForCp334 - $cp336BindingCallEndForCp334
)
$postCp336BeforeNumericalCodeForCp334 =
    [regex]::Replace($postCp336BeforeNumericalForCp334, '(?m)//.*$', '')
if ($postCp336BeforeNumericalCodeForCp334 -match '(?<![A-Za-z0-9_])(?:\b[A-Za-z_][A-Za-z0-9_:]*|\.[A-Za-z_][A-Za-z0-9_]*)!?\s*\(') {
...
Assert-Contains -Path $cp334PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp336_lifecycle_evidence' -Description "non-direct CP334 through CP336 evidence rejection"
