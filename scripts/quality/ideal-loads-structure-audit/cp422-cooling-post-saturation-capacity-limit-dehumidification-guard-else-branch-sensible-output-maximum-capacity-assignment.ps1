...
foreach ($file in $audits) { if ($file.BaseName -notmatch '^cp(?<number>\d+)-') {continue}; $number=[int]$Matches['number']; if ($number -ge 334 -and $number -le 422) {Assert-Contains -Path $file.FullName -Pattern 'non_direct_runtime_rejects_cp316_through_cp425_lifecycle_evidence' -Description 'historical non-direct firewall'}; if ($number -ge 337 -and $number -le 422) {Assert-Contains -Path $file.FullName -Pattern 'script_count = 363' -Description 'historical script count'}; if ($number -ge 367 -and $number -le 422) {Assert-Contains -Path $file.FullName -Pattern 'Count -ne 123' -Description 'historical classification count'}; if ($number -ge 335 -and $number -le 422) {Assert-Contains -Path $file.FullName -Pattern ([regex]::Escape('\| 363 \|')) -Description 'historical generated total';Assert-Contains -Path $file.FullName -Pattern ([regex]::Escape('\| 123 \|')) -Description 'historical generated internal total'} }
...
$inventory=Read-RepoText -Path 'specs\script_inventory.toml';foreach ($pattern in @('script_count = 363','dev_command_count = 238','unused_script_count = 0','unreachable_count = 0')) {Assert-Cp422Text -Text $inventory -Pattern $pattern -Description 'inventory count'};if ([regex]::Matches($inventory,'(?m)^classification = "public"\r?$').Count -ne 240 -or [regex]::Matches($inventory,'(?m)^classification = "internal"\r?$').Count -ne 123) {throw 'CP422 inventory classification drift'}
