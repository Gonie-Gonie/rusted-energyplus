...
foreach ($test in @('scheduled_binding_advances_cp422_after_cp421_before_unchanged_coupling','cp422_adapter_accepts_only_the_cp421_snapshot','cp422_is_preserved_before_cp423_in_the_current_116_snapshot_binding')) { Assert-Contains -Path $adapterTests -Pattern $test -Description 'binding regression' }
...
foreach ($file in $audits) { if ($file.BaseName -notmatch '^cp(?<number>\d+)-') {continue}; $number=[int]$Matches['number']; if ($number -ge 334 -and $number -le 422) {Assert-Contains -Path $file.FullName -Pattern 'non_direct_runtime_rejects_cp316_through_cp426_lifecycle_evidence' -Description 'historical non-direct firewall'}; if ($number -ge 337 -and $number -le 422) {Assert-Contains -Path $file.FullName -Pattern 'script_count = 364' -Description 'historical script count'}; if ($number -ge 367 -and $number -le 422) {Assert-Contains -Path $file.FullName -Pattern 'Count -ne 124' -Description 'historical classification count'}; if ($number -ge 335 -and $number -le 422) {Assert-Contains -Path $file.FullName -Pattern ([regex]::Escape('\| 364 \|')) -Description 'historical generated total';Assert-Contains -Path $file.FullName -Pattern ([regex]::Escape('\| 124 \|')) -Description 'historical generated internal total'} }
...
$inventory=Read-RepoText -Path 'specs\script_inventory.toml';foreach ($pattern in @('script_count = 364','dev_command_count = 238','unused_script_count = 0','unreachable_count = 0')) {Assert-Cp422Text -Text $inventory -Pattern $pattern -Description 'inventory count'};if ([regex]::Matches($inventory,'(?m)^classification = "public"\r?$').Count -ne 240 -or [regex]::Matches($inventory,'(?m)^classification = "internal"\r?$').Count -ne 124) {throw 'CP422 inventory classification drift'}
