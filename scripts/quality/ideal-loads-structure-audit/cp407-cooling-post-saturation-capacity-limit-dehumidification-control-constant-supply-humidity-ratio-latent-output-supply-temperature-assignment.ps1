...
) -Description 'CP406-to-CP407-to-numerical binding order'
...
) -Description 'CP406-to-CP407 scheduled output order'
$bindingText = Read-RepoText -Path $binding
if ([regex]::Matches($bindingText,"\bcalculation_$predecessorStem\b").Count -ne 3 -or [regex]::Matches($bindingText,"\bcalculation_$stem\b").Count -ne 2) {
...
Assert-PatternsInOrder -Path $pipelineRoot -Patterns @("$predecessorStem::\s*validate_direct_lifecycle","$stem::\s*validate_direct_lifecycle") -Description 'pipeline CP406-to-CP407 order'
...
foreach ($file in $audits) {
    ...
    if ($number -ge 334 -and $number -le 406) { Assert-Contains -Path $file.FullName -Pattern 'non_direct_runtime_rejects_cp316_through_cp407_lifecycle_evidence' -Description 'historical non-direct firewall' }
    if ($number -ge 337 -and $number -le 406) { Assert-Contains -Path $file.FullName -Pattern 'script_count = 345' -Description 'historical script count' }
    if ($number -ge 335 -and $number -le 406) {
        Assert-Contains -Path $file.FullName -Pattern '345[^\r\n]*generated' -Description 'historical generated total'
        Assert-Contains -Path $file.FullName -Pattern '105[^\r\n]*generated' -Description 'historical generated internal total'
    }
    if ($number -ge 367 -and $number -le 406) {
        Assert-Contains -Path $file.FullName -Pattern 'Count -ne 105' -Description 'historical classification count'
        Assert-Contains -Path $file.FullName -Pattern '240 public and 105 internal' -Description 'historical classification phrase'
    }
}
...
foreach ($pattern in @('\$cp407Call\s*=','CP406-to-CP407','CP407-to-numerical')) { Assert-Contains -Path $cp345 -Pattern $pattern -Description 'CP345 terminal chain' }
foreach ($file in @($audits | Where-Object { $_.BaseName -match '^cp(?<number>399|400|401|402|403|404|405|406)-' })) { Assert-Contains -Path $file.FullName -Pattern 'latent_output_supply_temperature_assignment\\s\*' -Description 'recent CP407 binding order' }
...
foreach ($pattern in @('script_count = 345','dev_command_count = 238','unused_script_count = 0','unreachable_count = 0')) { Assert-Cp407Text -Text $inventory -Pattern $pattern -Description 'inventory count' }
if ([regex]::Matches($inventory,'(?m)^classification = "public"$').Count -ne 240 -or [regex]::Matches($inventory,'(?m)^classification = "internal"$').Count -ne 105) { throw 'CP407 inventory classification drift' }
