...
foreach ($file in $audits) {
    ...
    if ($number -ge 334 -and $number -le 406) { Assert-Contains -Path $file.FullName -Pattern 'non_direct_runtime_rejects_cp316_through_cp435_lifecycle_evidence' -Description 'historical non-direct firewall' }
    if ($number -ge 337 -and $number -le 406) { Assert-Contains -Path $file.FullName -Pattern 'script_count = 373' -Description 'historical script count' }
...
foreach ($pattern in @('script_count = 373','dev_command_count = 238','unused_script_count = 0','unreachable_count = 0')) { Assert-Cp407Text -Text $inventory -Pattern $pattern -Description 'inventory count' }
if ([regex]::Matches($inventory,'(?m)^classification = "public"$').Count -ne 240 -or [regex]::Matches($inventory,'(?m)^classification = "internal"$').Count -ne 133) { throw 'CP407 inventory classification drift' }
