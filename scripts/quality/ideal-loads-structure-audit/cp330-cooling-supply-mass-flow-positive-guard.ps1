...
# Binding order is CP329 -> CP330 -> CP331 -> CP332 -> CP333 -> unchanged numerical DTO.
...
if (
    ...
    $numericalBindingIndexForCp330 -le $cp333BindingIndexForCp330
) {
    throw "Binding must retain exact CP329 -> CP330 -> CP331 -> CP332 -> CP333 -> numerical Calc order"
}
...
if (
    ...
    $numericalBindingIndexForCp330 -lt $cp333BindingCallEndForCp330
) {
...
$postCp333BeforeNumericalForCp330 = $cp330BindingText.Substring(
    $cp333BindingCallEndForCp330,
    $numericalBindingIndexForCp330 - $cp333BindingCallEndForCp330
)
$postCp333BeforeNumericalCodeForCp330 =
    [regex]::Replace($postCp333BeforeNumericalForCp330, '(?m)//.*$', '')
if ($postCp333BeforeNumericalCodeForCp330 -match '(?<![A-Za-z0-9_])(?:\b[A-Za-z_][A-Za-z0-9_:]*|\.[A-Za-z_][A-Za-z0-9_]*)!?\s*\(') {
