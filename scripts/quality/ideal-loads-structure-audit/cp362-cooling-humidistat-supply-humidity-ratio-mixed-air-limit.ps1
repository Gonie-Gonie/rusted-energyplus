...
if ($cp361BindingIndexForCp362 -lt 0 -or $cp362BindingIndex -le $cp361BindingIndexForCp362 -or $cp363BindingIndexForCp362 -le $cp362BindingIndex -or $cp362NumericalIndex -le $cp363BindingIndexForCp362) {
    throw "Binding must execute CP361 then CP362 then CP363 before numerical coupling"
}
...
Assert-Contains -Path $cp362PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp363_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 301' -Description "CP362 script total"
