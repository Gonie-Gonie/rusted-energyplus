...
$cp375BindingIndexForCp362 = $cp362BindingText.IndexOf("let calculation_cooling_supply_humidity_ratio_humidification_supply_humidity_ratio_maximum_assignment ="); $cp362NumericalIndex = $cp362BindingText.IndexOf("let coupling = complete_direct_zone_purchased_air_coupling(")
if ($cp361BindingIndexForCp362 -lt 0 -or $cp362BindingIndex -le $cp361BindingIndexForCp362 -or $cp363BindingIndexForCp362 -le $cp362BindingIndex -or $cp364BindingIndexForCp362 -le $cp363BindingIndexForCp362 -or $cp365BindingIndexForCp362 -le $cp364BindingIndexForCp362 -or $cp366BindingIndexForCp362 -le $cp365BindingIndexForCp362 -or $cp367BindingIndexForCp362 -le $cp366BindingIndexForCp362 -or $cp368BindingIndexForCp362 -le $cp367BindingIndexForCp362 -or
    ...
    $cp362NumericalIndex -le $cp375BindingIndexForCp362) {
...
Assert-Contains -Path $cp362PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp375_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 334..361) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp375_lifecycle_evidence' -Description "historical CP363 firewall"
}
foreach ($historical in 335..361) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern ([regex]::Escape('\| executable script records \| 313 \|')) -Description "historical current generated total"
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern ([regex]::Escape('\| internal scripts \| 73 \|')) -Description "historical current generated internal"
}
foreach ($historical in 337..361) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'script_count = 313' -Description "historical current script inventory total"
}
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 313' -Description "CP362 script total"
