...
Assert-Contains -Path $cp362PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp397_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 335..361) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern ([regex]::Escape('\| executable script records \| 335 \|')) -Description "historical current generated total"
Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern ([regex]::Escape('\| internal scripts \| 95 \|')) -Description "historical current generated internal"
}
foreach ($historical in 337..361) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'script_count = 335' -Description "historical current script inventory total"
}
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 335' -Description "CP362 script total"
