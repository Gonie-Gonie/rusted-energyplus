...
if ($cp361BindingIndexForCp362 -lt 0 -or $cp362BindingIndex -le $cp361BindingIndexForCp362 -or $cp362NumericalIndex -le $cp362BindingIndex) {
    throw "Binding must execute CP361 then CP362 before numerical coupling"
}
...
Assert-Contains -Path $cp362PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp362_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Contains -Path $cp362ArbitraryAssertions -Pattern 'super::super::super::super::super::assert_numerical_nonfeed\(runtime, results\)' -Description "CP362 numerical nonfeed ownership"
foreach ($field in @(
        "predecessor_resulting_supply_humidity_ratio_for_dehumidification",
        "mixed_air_humidity_ratio",
        "supply_humidity_ratio_for_dehumidification_before_mixed_air_limit",
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 300' -Description "CP362 script total"
