...
foreach ($file in @(Get-ChildItem -LiteralPath $auditRoot -Filter 'cp*.ps1' -File)) {
    ...
    if ($number -ge 334 -and $number -le 402) {
        Assert-Contains -Path $file.FullName -Pattern 'non_direct_runtime_rejects_cp316_through_cp420_lifecycle_evidence' -Description "historical CP403 non-direct firewall"
    }
    if ($number -ge 337 -and $number -le 402) {
        Assert-Contains -Path $file.FullName -Pattern 'script_count = 358' -Description "historical current script count"
    }
    if ($number -ge 335 -and $number -le 402) {
        Assert-Contains -Path $file.FullName -Pattern '\\\| 358 \\\|' -Description "historical generated script count"
        Assert-Contains -Path $file.FullName -Pattern '\\\| 118 \\\|' -Description "historical generated internal count"
    }
    if ($number -ge 367 -and $number -le 402) {
        Assert-Contains -Path $file.FullName -Pattern 'Count -ne 118' -Description "historical internal classification count"
    }
}
$cleanupAudits = @(
...
if ([regex]::Matches($inventory, '(?m)^classification = "public"$').Count -ne 240 -or
    ...
    throw "CP403 inventory classification drift; expected 240 public and 118 internal"
}
