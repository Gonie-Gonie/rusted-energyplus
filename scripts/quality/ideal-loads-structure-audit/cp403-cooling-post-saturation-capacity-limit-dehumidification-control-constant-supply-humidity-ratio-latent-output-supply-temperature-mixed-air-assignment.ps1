...
Assert-Contains -Path $release -Pattern 'completed_direct_.*latent_output_guard_is_consistent' -Description "recursive CP402 completion"
...
Assert-PatternsInOrder -Path $binding -Patterns @(
    ...
    'let\s+calculation_cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_latent_output_supply_temperature_mixed_air_limit\s*=',
...
Assert-PatternsInOrder -Path $scheduledOutput -Patterns @(
    ...
    'pub\s+calculation_cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_latent_output_supply_temperature_mixed_air_limit\s*:',
    'pub\s+coupling\s*:'
...
foreach ($file in @(Get-ChildItem -LiteralPath $auditRoot -Filter 'cp*.ps1' -File)) {
    ...
    if ($number -ge 334 -and $number -le 402) {
        Assert-Contains -Path $file.FullName -Pattern 'non_direct_runtime_rejects_cp316_through_cp408_lifecycle_evidence' -Description "historical CP403 non-direct firewall"
    }
    if ($number -ge 337 -and $number -le 402) {
        Assert-Contains -Path $file.FullName -Pattern 'script_count = 346' -Description "historical current script count"
    }
    if ($number -ge 335 -and $number -le 402) {
        Assert-Contains -Path $file.FullName -Pattern '\\\| 346 \\\|' -Description "historical generated script count"
        Assert-Contains -Path $file.FullName -Pattern '\\\| 106 \\\|' -Description "historical generated internal count"
    }
    if ($number -ge 367 -and $number -le 402) {
        Assert-Contains -Path $file.FullName -Pattern 'Count -ne 106' -Description "historical internal classification count"
    }
}
$cleanupAudits = @(
...
Assert-Contains -Path $cp345Audit -Pattern 'CP408-to-numerical' -Description "CP345 CP408 terminal interval"
...
if ([regex]::Matches($inventory, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($inventory, '(?m)^classification = "internal"$').Count -ne 106) {
    throw "CP403 inventory classification drift; expected 240 public and 106 internal"
}
...
Assert-Contains -Path 'docs\src\generated\script-index.md' -Pattern '\| 346 \|' -Description "generated script total"
Assert-Contains -Path 'docs\src\generated\script-index.md' -Pattern '\| public scripts \| 240 \|' -Description "generated public total"
Assert-Contains -Path 'docs\src\generated\script-index.md' -Pattern '\| 106 \|' -Description "generated internal total"
...
Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\cp345-cooling-positive-supply-post-capacity-limit-humidity-ratio-mixed-air-assignment.ps1" -Pattern '\$cp408Call\s*=' -Description "CP345 CP408 call capture"
