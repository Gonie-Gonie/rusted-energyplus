...
Assert-PatternsInOrder -Path $binding -Patterns @(
    ...
    'let\s+coupling\s*='
) -Description "CP402-to-CP403-to-CP404-to-CP405-to-CP406-to-CP407 binding order"
...
foreach ($file in @(Get-ChildItem -LiteralPath $auditRoot -Filter 'cp*.ps1' -File)) {
    ...
    if ($number -ge 334 -and $number -le 402) {
        Assert-Contains -Path $file.FullName -Pattern 'non_direct_runtime_rejects_cp316_through_cp407_lifecycle_evidence' -Description "historical CP403 non-direct firewall"
    }
...
Assert-Contains -Path $cp345Audit -Pattern 'CP407-to-numerical' -Description "CP345 CP407 terminal interval"
...
if ([regex]::Matches($inventory, '(?m)^classification = "public"$').Count -ne 240 -or
    ...
    throw "CP403 inventory classification drift; expected 240 public and 105 internal"
}
