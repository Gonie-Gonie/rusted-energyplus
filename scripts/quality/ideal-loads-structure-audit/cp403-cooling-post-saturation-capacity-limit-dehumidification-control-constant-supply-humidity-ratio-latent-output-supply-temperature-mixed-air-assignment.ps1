...
foreach ($file in @(Get-ChildItem -LiteralPath $auditRoot -Filter 'cp*.ps1' -File)) {
    ...
    if ($number -ge 334 -and $number -le 402) {
        Assert-Contains -Path $file.FullName -Pattern 'non_direct_runtime_rejects_cp316_through_cp416_lifecycle_evidence' -Description "historical CP403 non-direct firewall"
    }
    if ($number -ge 337 -and $number -le 402) {
        Assert-Contains -Path $file.FullName -Pattern 'script_count = 354' -Description "historical current script count"
    }
    if ($number -ge 335 -and $number -le 402) {
        Assert-Contains -Path $file.FullName -Pattern '\\\| 354 \\\|' -Description "historical generated script count"
        Assert-Contains -Path $file.FullName -Pattern '\\\| 114 \\\|' -Description "historical generated internal count"
    }
    if ($number -ge 367 -and $number -le 402) {
        Assert-Contains -Path $file.FullName -Pattern 'Count -ne 114' -Description "historical internal classification count"
    }
}
$cleanupAudits = @(
...
Assert-Contains -Path $cp345Audit -Pattern 'CP408-to-CP409' -Description "CP345 CP408-to-CP409 interval"; Assert-Contains -Path $cp345Audit -Pattern 'CP414-to-numerical' -Description "CP345 CP413 terminal interval"
...
if ([regex]::Matches($inventory, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($inventory, '(?m)^classification = "internal"$').Count -ne 114) {
