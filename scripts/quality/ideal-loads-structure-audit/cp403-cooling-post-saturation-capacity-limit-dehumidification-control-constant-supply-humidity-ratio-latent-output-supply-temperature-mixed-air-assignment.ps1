...
foreach ($file in @(Get-ChildItem -LiteralPath $auditRoot -Filter 'cp*.ps1' -File)) {
    ...
    if ($number -ge 334 -and $number -le 402) {
        Assert-Contains -Path $file.FullName -Pattern 'non_direct_runtime_rejects_cp316_through_cp435_lifecycle_evidence' -Description "historical CP403 non-direct firewall"
    }
