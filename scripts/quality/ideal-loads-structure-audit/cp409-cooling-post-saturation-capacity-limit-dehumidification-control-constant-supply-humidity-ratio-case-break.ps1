...
) -Description 'CP408-to-CP410-to-numerical binding order'
...
foreach ($file in $audits) {
    ...
    if ($number -ge 334 -and $number -le 408) { Assert-Contains -Path $file.FullName -Pattern 'non_direct_runtime_rejects_cp316_through_cp410_lifecycle_evidence' -Description 'historical non-direct firewall' }
    if ($number -ge 337 -and $number -le 408) { Assert-Contains -Path $file.FullName -Pattern 'script_count = 348' -Description 'historical script count' }
    if ($number -ge 367 -and $number -le 408) { Assert-Contains -Path $file.FullName -Pattern 'Count -ne 108' -Description 'historical classification count' }
}
$cleanup = @(
...
foreach ($pattern in @('\$cp409Call\s*=','CP408-to-CP409','CP410-to-numerical')) { Assert-Contains -Path $cp345 -Pattern $pattern -Description 'CP345 terminal chain' }
...
if ([regex]::Matches($inventory,'(?m)^classification = "public"\r?$').Count -ne 240 -or
    [regex]::Matches($inventory,'(?m)^classification = "internal"\r?$').Count -ne 108) { throw 'CP409 inventory classification drift' }
