...
foreach ($file in $audits) {
    ...
    if ($number -ge 334 -and $number -le 408) { Assert-Contains -Path $file.FullName -Pattern 'non_direct_runtime_rejects_cp316_through_cp436_lifecycle_evidence' -Description 'historical non-direct firewall' }
    if ($number -ge 337 -and $number -le 408) { Assert-Contains -Path $file.FullName -Pattern 'script_count = 374' -Description 'historical script count' }
    if ($number -ge 367 -and $number -le 408) { Assert-Contains -Path $file.FullName -Pattern 'Count -ne 134' -Description 'historical classification count' }
}
$cleanup = @(
...
if ([regex]::Matches($inventory,'(?m)^classification = "public"\r?$').Count -ne 240 -or
    [regex]::Matches($inventory,'(?m)^classification = "internal"\r?$').Count -ne 134) { throw 'CP409 inventory classification drift' }
