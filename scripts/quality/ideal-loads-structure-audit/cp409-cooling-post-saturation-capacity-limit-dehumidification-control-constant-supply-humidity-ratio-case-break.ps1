...
Assert-Contains -Path $prefixValidation -Pattern 'cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_latent_output_supply_temperature_mixed_air_limit_committed_latest_snapshot_is_consistent\s*\(' -Description 'bounded CP408 committed predecessor proof'
Assert-NotContains -Path $prefixValidation -Pattern 'completed_direct_cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_latent_output_supply_temperature_mixed_air_limit_is_consistent\s*\(' -Description 'recursive CP408 predecessor completion'
...
) -Description 'CP408-to-CP414-to-CP415 binding order'
...
foreach ($file in $audits) {
    ...
    if ($number -ge 334 -and $number -le 408) { Assert-Contains -Path $file.FullName -Pattern 'non_direct_runtime_rejects_cp316_through_cp431_lifecycle_evidence' -Description 'historical non-direct firewall' }
    if ($number -ge 337 -and $number -le 408) { Assert-Contains -Path $file.FullName -Pattern 'script_count = 369' -Description 'historical script count' }
    if ($number -ge 367 -and $number -le 408) { Assert-Contains -Path $file.FullName -Pattern 'Count -ne 129' -Description 'historical classification count' }
}
$cleanup = @(
...
foreach ($pattern in @('\$cp409Call\s*=','CP408-to-CP409','CP414-to-CP415')) { Assert-Contains -Path $cp345 -Pattern $pattern -Description 'CP345 terminal chain' }
...
if ([regex]::Matches($inventory,'(?m)^classification = "public"\r?$').Count -ne 240 -or
    [regex]::Matches($inventory,'(?m)^classification = "internal"\r?$').Count -ne 129) { throw 'CP409 inventory classification drift' }
