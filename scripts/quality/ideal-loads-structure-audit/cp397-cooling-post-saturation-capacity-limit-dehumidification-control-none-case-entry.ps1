# CP397 maps PurchasedAirManager.cc physical source line 2290 only.
& {
$stem = "cooling_post_saturation_capacity_limit_dehumidification_control_none_case_entry"
$predecessorStem = "cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_case_break"
$typeStem = "PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationControlNoneCaseEntry"
...
$coupledTests = "crates\ep_runtime\src\ideal_loads\test_coupled_runtime_cp397.rs"
...
$audit = "scripts\quality\ideal-loads-structure-audit\cp397-cooling-post-saturation-capacity-limit-dehumidification-control-none-case-entry.ps1"
$site = "enter-purchased-air-post-saturation-capacity-limit-dehumidification-control-none-case"
$numericFields = @(
...
$localBool = "dehumidification_control_none_case_entered"
$stateFields = @(
    "system", "transition_count", "inactive_transition_count",
    "dehumidification_control_none_case_entry_count", "predecessor_route_counts",
    "source_site_execution_count", "latest"
)

function Assert-Cp397Text {
    param([string]$Text, [string]$Pattern, [string]$Description)
    if ($Text -notmatch $Pattern) { throw "CP397 $Description missing '$Pattern'" }
}

function Get-Cp397BraceBlock {
    param([string]$Text, [string]$AnchorPattern, [string]$Description)
    $anchors = [regex]::Matches($Text, $AnchorPattern)
    if ($anchors.Count -ne 1) { throw "CP397 $Description expected one anchor, found $($anchors.Count)" }
    $open = $Text.IndexOf("{", $anchors[0].Index)
    if ($open -lt 0) { throw "CP397 $Description opening brace missing" }
    $depth = 0
    ...
    throw "CP397 $Description closing brace missing"
}
...
$required = @(
    ...
    $snapshotJson, $snapshotJsonTests, $cp396Assertions, $cp397Assertions, $audit
)
foreach ($file in $required) {
    Assert-FileExists -Path $file -Description "CP397 implementation/audit file"
    Assert-LineLimit -Path $file -Limit 500 -Description "CP397 bounded file"
}
foreach ($directory in @($root, $coupledRoot, $pipelineDir)) {
    foreach ($file in Get-ChildItem -LiteralPath $directory -Recurse -File -Filter "*.rs") {
        Assert-LineLimit -Path $file.FullName -Limit 500 -Description "CP397 bounded recursive file"
    }
}
Assert-FileExists -Path $arbitraryRoot -Description "CP397 arbitrary-run test root"
Assert-LineLimit -Path $arbitraryRoot -Limit 1200 -Description "CP397 arbitrary-run root cap"

if ((Get-FileHash -Algorithm SHA256 -LiteralPath $source).Hash -cne $hash) { throw "CP397 PurchasedAirManager.cc SHA-256 drift" }
...
if ($lines[2289].Trim() -cne 'case HumControl::None:' -or
    ...
    throw "CP397 source label, CP398 boundary, executable successor, or dynamic continuation drift"
}
Assert-Contains -Path $module -Pattern 'PurchasedAirManager\.cc:2290' -Description "mapped None label"
Assert-Contains -Path $module -Pattern 'PurchasedAirManager\.cc:2294' -Description "first excluded executable"
Assert-Contains -Path $module -Pattern 'Line 2291.*next sibling case-label control boundary|line 2291.*next sibling case-label control boundary' -Description "CP398 control boundary"
...
Assert-ExactStringArray -Path $module `
    -Name 'PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_CONTROL_NONE_CASE_ENTRY_SOURCE_ORDER' `
    -Expected @($site) -Description "exact sole None entry site"

$moduleText = Read-RepoText -Path $module
$snapshotStruct = Get-Cp397BraceBlock -Text $moduleText -AnchorPattern "pub\s+struct\s+$($typeStem)Snapshot\s*" -Description "snapshot declaration"
...
if ($actualNumericFields.Count -ne 6) { throw "CP397 snapshot must expose exactly six Option<f64> fields" }
for ($index = 0; $index -lt $numericFields.Count; $index += 1) {
    if ($actualNumericFields[$index] -cne $numericFields[$index]) { throw "CP397 numeric field order drift at $index" }
}
Assert-PatternsInOrder -Path $module -Patterns @(
    ...
    'pub\s+predecessor_cp396_resulting_supply_humidity_ratio\s*:',
    'pub\s+predecessor_cp396_resulting_supply_enthalpy_j_per_kg\s*:',
    'pub\s+predecessor_cp396_resulting_supply_temperature_c\s*:',
...
if ([regex]::Matches($snapshotStruct, "pub\s+$localBool\s*:\s*bool").Count -ne 1) { throw "CP397 must add exactly one local None-entry boolean" }

$stateStruct = Get-Cp397BraceBlock -Text (Read-RepoText -Path $state) -AnchorPattern "pub\s+struct\s+$($typeStem)RuntimeState\s*" -Description "runtime state declaration"
[string[]]$actualStateFields = @([regex]::Matches($stateStruct, 'pub\s+(?<field>[A-Za-z0-9_]+)\s*:') | ForEach-Object { $_.Groups['field'].Value })
if ($actualStateFields.Count -ne $stateFields.Count) { throw "CP397 runtime state public field count drift" }
for ($index = 0; $index -lt $stateFields.Count; $index += 1) {
    if ($actualStateFields[$index] -cne $stateFields[$index]) { throw "CP397 runtime state field order drift at $index" }
}
...
foreach ($pattern in @(
        ...
        'index\s*>=\s*3',
        'checked_selected_sum\(&state\.predecessor_route_counts,\s*&\[20,\s*24,\s*27\]\)',
    ...
    )) { Assert-Cp397Text -Text $core -Pattern $pattern -Description "route/algebra contract" }
Assert-Contains -Path $accounting -Pattern '(?s)if\s+!route\.active.*?inactive_transition_count\s*\+=\s*1;.*?return;.*?none_case_entry_count\s*\+=\s*1;.*?source_site_execution_count\s*\+=' -Description "inactive versus active accounting"
Assert-Contains -Path $transition -Pattern "$localBool\s*:\s*route\.active" -Description "route-owned None entry flag"
foreach ($pair in @(
        @('predecessor_cp396_resulting_supply_humidity_ratio', 'resulting_supply_humidity_ratio'),
        @('predecessor_cp396_resulting_supply_enthalpy_j_per_kg', 'resulting_supply_enthalpy_j_per_kg'),
        @('predecessor_cp396_resulting_supply_temperature_c', 'resulting_supply_temperature_c')
...
Assert-Contains -Path $release -Pattern 'HumidistatCaseBreakSnapshot as Predecessor' -Description "exact CP396 predecessor type"
Assert-Contains -Path $release -Pattern 'predecessor_cp396:\s*Predecessor' -Description "predecessor-only public argument"
Assert-Contains -Path $release -Pattern 'humidistat_case_break_snapshot_is_exact_direct_release\(predecessor_cp396\)' -Description "recursive CP396 direct release"
...
foreach ($pattern in @(
    ...
    )) { Assert-Contains -Path $prefix -Pattern $pattern -Description "bounded recursively complete CP396 prefix admission" }
Assert-NotContains -Path $release -Pattern 'predecessor_cp395|HumidistatSupplyHumidityRatioAssignmentSnapshot as Predecessor|ActiveInput|latest_numerical|supply_temperature_c:\s*f64|supply_enthalpy_j_per_kg:\s*f64' -Description "older or scalar predecessor substitution"
...
foreach ($pattern in @(
        'cp397_boundary_and_single_none_case_entry_site_are_exact',
        'cp397_preserves_thirty_routes_and_enters_none_exactly_three_times',
        ...
        'malformed_or_wrong_identity_cp396_predecessor_rejects_without_mutation',
        'every_active_counter_overflow_rejects_before_mutation',
        'inactive_counter_overflow_rejects_before_mutation',
        'direct_release_enters_none_label_and_retains_lifecycle_metadata'
    )) { Assert-Cp397Text -Text $testText -Pattern $pattern -Description "non-vacuous core test" }

$cp396BindingName = "calculation_$predecessorStem"
$cp397BindingName = "calculation_$stem"
Assert-PatternsInOrder -Path $binding -Patterns @(
    ...
    'let\s+coupling\s*='
) -Description "CP396-to-CP397-to-unchanged-numerical binding order"
...
if ($numericalIndex -lt 0) { throw "CP397 unchanged numerical coupling anchor missing" }
$dto = Get-Cp397BraceBlock -Text $bindingText.Substring($numericalIndex) -AnchorPattern 'DirectZonePurchasedAirCouplingInput\s*\{' -Description "numerical DTO"
if ($dto -match [regex]::Escape($stem) -or $dto -match '\bcp397\b' -or $dto -match $localBool) { throw "CP397 evidence unexpectedly feeds DirectZonePurchasedAirCouplingInput" }
Assert-Contains -Path $bindingTests -Pattern 'binding_places_cp397_after_cp396_before_unchanged_numerical_coupling' -Description "binding preservation test"
Assert-PatternsInOrder -Path $scheduledOutput -Patterns @("pub\s+$([regex]::Escape($cp396BindingName))\s*:", "pub\s+$([regex]::Escape($cp397BindingName))\s*:", 'pub\s+coupling\s*:') -Description "scheduled output order"
...
Assert-PatternsInOrder -Path $pipelineRoot -Patterns @("$($predecessorStem)::\s*validate_direct_lifecycle", "$($stem)::\s*validate_direct_lifecycle") -Description "pipeline CP396-to-CP397 validation order"
Assert-Contains -Path $pipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp440_lifecycle_evidence' -Description "cumulative non-direct firewall"
foreach ($pattern in @('public_cp397_validator_depends_only_on_cp396_and_accepts_direct_active_routes', 'ep_run_cp397_rejects_missing_cp396_predecessor_evidence', 'ep_run_cp397_links_exactly_to_cp396_and_rejects_corruption')) { Assert-Contains -Path $pipelineValidationTests -Pattern $pattern -Description "pipeline validation regression" }
foreach ($pattern in @('cp397_preserves_cp396_terminal_carriers_and_tracks_direct_none_entry', 'cp397_rejects_cp396_carrier_bit_drift_and_route_drift', 'cp397_validation_remains_independent_of_numerical_output_state')) { Assert-Contains -Path $coupledTests -Pattern $pattern -Description "coupled validation regression" }
...
foreach ($registration in @(
        ...
        [PSCustomObject]@{ Path = $scheduledOutput; Pattern = "pub $($cp397BindingName):" },
...
if ($jsonNumbers.Count -ne 6 -or $ieeeSidecars.Count -ne 6) { throw "CP397 serialization must expose exactly six numeric projections and IEEE sidecars" }
for ($index = 0; $index -lt $numericFields.Count; $index += 1) {
    ...
    if ($jsonNumbers[$index].Groups['field'].Value -cne $expected -or $jsonNumbers[$index].Groups['value'].Value -cne $expected -or
        $ieeeSidecars[$index].Groups['field'].Value -cne $expected -or $ieeeSidecars[$index].Groups['value'].Value -cne $expected) { throw "CP397 numeric/IEEE serialization order drift at '$expected'" }
}
...
Assert-Contains -Path $cp396Assertions -Pattern '#\[path\s*=\s*"cp397_assertions\.rs"\]' -Description "arbitrary assertion chain"
foreach ($pattern in @('CP397_KEY', 'PurchasedAirManager\.cc:2290', 'PurchasedAirManager\.cc:2294', $site, 'dehumidification_control_none_case_entry_count', 'source_site_execution_count', 'predecessor_cp396_resulting_supply_', 'resulting_supply_temperature_c', '_ieee_bits', 'assert_non_direct')) { Assert-Contains -Path $cp397Assertions -Pattern $pattern -Description "arbitrary CP397 assertion" }

$algorithmText = Read-RepoText -Path "specs\algorithm_ledger.toml"
$capabilityText = Read-RepoText -Path "specs\capabilities.toml"
$algorithmClaims = [regex]::Matches($algorithmText, '(?m)^\s*"CP397 supersedes only CP396[^"\r\n]+",\s*$')
$capabilityClaims = [regex]::Matches($capabilityText, '(?m)^\s*"CP397 additionally requires[^"\r\n]+",\s*$')
if ($algorithmClaims.Count -ne 2 -or $capabilityClaims.Count -ne 2) { throw "CP397 must add exactly two algorithm and two capability claims" }
foreach ($claim in @($algorithmClaims + $capabilityClaims)) {
    foreach ($pattern in @(
            $commit, $hash, 'physical.*line 2290', 'case HumControl::None:', $site,
            'line 2291', 'CP398', 'line 2294', 'line 2313', 'thirty routes', '20, 24, and 27',
            'twenty-seven', 'eleven public|Public routes are exactly', 'nineteen routes remain private',
            'T397=T396', 'E397=R\[20\]\+R\[24\]\+R\[27\]', 'I397=T397-E397', '30/27/3/3',
            'sole immediate predecessor', 'exactly six `Option<f64>`', 'six authoritative IEEE sidecars',
            $numericFields[0], $numericFields[5], $localBool, 'without a selector reread',
            'CP396-to-CP397-to-unchanged-numerical', 'DirectZonePurchasedAirCouplingInput',
            '32 algorithms', '293 routines', '58 state-mapped', '235 source-mapped', '170 required',
            '335 total', '240 public', '95 internal', 'zero unused', 'zero unreachable',
...
Assert-Contains -Path "docs\src\generated\algorithm-ledger.md" -Pattern 'CP397 supersedes only CP396' -Description "generated algorithm addendum"
Assert-Contains -Path "docs\src\generated\capability-index.md" -Pattern 'CP397 additionally requires' -Description "generated capability addendum"

foreach ($historical in 334..396) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp440_lifecycle_evidence' -Description "historical non-direct firewall"
}
foreach ($historical in 335..396) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern ([regex]::Escape('\| 378 \|')) -Description "historical generated total"
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern ([regex]::Escape('\| 138 \|')) -Description "historical generated internal"
}
foreach ($historical in 337..396) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'script_count = 378' -Description "historical inventory total"
}
foreach ($historical in 367..396) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'Count -ne 138' -Description "historical internal count"
...
foreach ($historical in @('cp326-cooling-supply-mass-flow-limit-body.ps1') + @(329..344 | ForEach-Object { (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$($_)-*.ps1").Name })) {
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$historical" -Pattern "advance_$stem" -Description "historical CP397 helper whitelist"
}
...
foreach ($pattern in @('\$cp397Call\s*=', 'CP396-to-CP397', 'CP397-to-CP398', 'CP400-to-CP401', 'CP401-to-CP402', 'CP402-to-CP403', 'CP403-to-CP404')) { Assert-Contains -Path $cp345Audit -Pattern $pattern -Description "CP345 terminal chain" }
...
$cp396AuditIndex = $master.IndexOf("cp396-cooling-post-saturation-capacity-limit-dehumidification-control-humidistat-case-break.ps1")
$cp397AuditIndex = $master.IndexOf((Split-Path -Leaf $audit))
$completionIndex = $master.IndexOf('Write-Host "IdealLoads structure audit complete."')
if ($cp396AuditIndex -lt 0 -or $cp397AuditIndex -le $cp396AuditIndex -or $completionIndex -le $cp397AuditIndex) { throw "Master CP397 registration order drift" }
$inventory = Read-RepoText -Path "specs\script_inventory.toml"
foreach ($pattern in @('script_count = 378', 'dev_command_count = 238', 'unused_script_count = 0', 'unreachable_count = 0')) { Assert-Cp397Text -Text $inventory -Pattern $pattern -Description "inventory" }
if ([regex]::Matches($inventory, '(?m)^classification = "public"$').Count -ne 240 -or [regex]::Matches($inventory, '(?m)^classification = "internal"$').Count -ne 138) { throw "CP397 inventory classification drift" }
$auditLeaf = Split-Path -Leaf $audit
Assert-Cp397Text -Text $inventory -Pattern ('(?s)path = "scripts/quality/ideal-loads-structure-audit/' + [regex]::Escape($auditLeaf) + '".*?callers = \["scripts/quality/ideal-loads-structure-audit\.ps1"\]') -Description "inventory record/caller"
foreach ($pattern in @('\| 378 \|', '\| public scripts \| 240 \|', '\| 138 \|', '\| scripts without callers \| 0 \|')) { Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern $pattern -Description "generated inventory" }
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern ([regex]::Escape("scripts/quality/ideal-loads-structure-audit/$auditLeaf")) -Description "generated CP397 audit record"
