...
foreach ($historical in 334..396) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp399_lifecycle_evidence' -Description "historical non-direct firewall"
}
...
foreach ($historical in 367..396) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'Count -ne 97' -Description "historical internal count"
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern '240 public and 97 internal' -Description "historical classification"
}
...
foreach ($pattern in @('\$cp397Call\s*=', 'CP396-to-CP397', 'CP397-to-CP398', 'CP399-to-numerical')) { Assert-Contains -Path $cp345Audit -Pattern $pattern -Description "CP345 terminal chain" }
...
foreach ($pattern in @('script_count = 337', 'dev_command_count = 238', 'unused_script_count = 0', 'unreachable_count = 0')) { Assert-Cp397Text -Text $inventory -Pattern $pattern -Description "inventory" }
if ([regex]::Matches($inventory, '(?m)^classification = "public"$').Count -ne 240 -or [regex]::Matches($inventory, '(?m)^classification = "internal"$').Count -ne 97) { throw "CP397 inventory classification drift" }
