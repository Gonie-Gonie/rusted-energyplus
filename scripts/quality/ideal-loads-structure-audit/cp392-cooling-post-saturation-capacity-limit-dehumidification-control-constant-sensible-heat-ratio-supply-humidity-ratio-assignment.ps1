...
Assert-Contains -Path $release -Pattern 'overdrying_limit_snapshot_is_exact_direct_release\(predecessor_cp391\)' -Description "exact CP391 direct-release evidence"
Assert-Contains -Path $prefix -Pattern 'cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_overdrying_limit_committed_latest_snapshot_is_consistent\s*\(' -Description "bounded CP391 committed predecessor proof"
Assert-NotContains -Path $prefix -Pattern 'completed_direct_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_overdrying_limit_is_consistent\s*\(' -Description "recursive CP391 predecessor completion"
...
foreach ($pattern in @('script_count = 378', 'dev_command_count = 238', 'unused_script_count = 0', 'unreachable_count = 0')) { Assert-Cp392Text -Text $inventory -Pattern $pattern -Description "inventory" }
if ([regex]::Matches($inventory, '(?m)^classification = "public"$').Count -ne 240 -or [regex]::Matches($inventory, '(?m)^classification = "internal"$').Count -ne 138) { throw "CP392 inventory must be exactly 240 public and 137 internal scripts" }
