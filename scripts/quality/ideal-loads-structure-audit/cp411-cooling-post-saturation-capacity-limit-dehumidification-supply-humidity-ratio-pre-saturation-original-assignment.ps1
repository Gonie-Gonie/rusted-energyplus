...
foreach ($file in $audits) {
    ...
    if ($number -ge 334 -and $number -le 410) { Assert-Contains -Path $file.FullName -Pattern 'non_direct_runtime_rejects_cp316_through_cp417_lifecycle_evidence' -Description 'historical non-direct firewall' }
    if ($number -ge 337 -and $number -le 410) { Assert-Contains -Path $file.FullName -Pattern 'script_count = 355' -Description 'historical script count' }
    if ($number -ge 367 -and $number -le 410) { Assert-Contains -Path $file.FullName -Pattern 'Count -ne 115' -Description 'historical classification count' }
...
foreach ($pattern in @('script_count = 355','dev_command_count = 238','unused_script_count = 0','unreachable_count = 0')) { Assert-Cp411Text -Text $inventory -Pattern $pattern -Description 'inventory count' }
if ([regex]::Matches($inventory,'(?m)^classification = "public"\r?$').Count -ne 240 -or [regex]::Matches($inventory,'(?m)^classification = "internal"\r?$').Count -ne 115) { throw 'CP411 inventory classification drift' }
