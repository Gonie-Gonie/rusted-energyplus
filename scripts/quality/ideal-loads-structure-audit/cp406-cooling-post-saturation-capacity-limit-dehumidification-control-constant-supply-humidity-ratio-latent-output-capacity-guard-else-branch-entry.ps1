...
) -Description "CP405-to-CP406-to-CP407-to-CP408-to-CP409-to-numerical binding order"
...
foreach ($file in $audits) {
    ...
    if ($number -ge 334 -and $number -le 405) {
        Assert-Contains -Path $file.FullName -Pattern 'non_direct_runtime_rejects_cp316_through_cp409_lifecycle_evidence' -Description "historical non-direct firewall"
    }
...
foreach ($pattern in @('\$cp406Call\s*=', '\$cp409Call\s*=', 'CP405-to-CP406', 'CP408-to-CP409', 'CP409-to-numerical')) {
