...
foreach ($file in $audits) {
    ...
    if ($number -ge 334 -and $number -le 405) {
        Assert-Contains -Path $file.FullName -Pattern 'non_direct_runtime_rejects_cp316_through_cp429_lifecycle_evidence' -Description "historical non-direct firewall"
    }
    if ($number -ge 337 -and $number -le 405) {
        Assert-Contains -Path $file.FullName -Pattern 'script_count = 367' -Description "historical current script count"
    }
    if ($number -ge 335 -and $number -le 405) {
        Assert-Contains -Path $file.FullName -Pattern ([regex]::Escape('\| 367 \|')) -Description "historical generated script count"
        Assert-Contains -Path $file.FullName -Pattern ([regex]::Escape('\| 127 \|')) -Description "historical generated internal count"
    }
    if ($number -ge 367 -and $number -le 405) {
        Assert-Contains -Path $file.FullName -Pattern 'Count -ne 127' -Description "historical internal classification count"
