...
foreach ($file in $audits) {
    ...
    if ($number -ge 334 -and $number -le 405) {
        Assert-Contains -Path $file.FullName -Pattern 'non_direct_runtime_rejects_cp316_through_cp419_lifecycle_evidence' -Description "historical non-direct firewall"
    }
    ...
    if ($number -ge 367 -and $number -le 405) {
        ...
        Assert-Contains -Path $file.FullName -Pattern '240 public and 117 internal' -Description "historical public/internal classification phrase"
    }
}
$cleanupAudits = @(
