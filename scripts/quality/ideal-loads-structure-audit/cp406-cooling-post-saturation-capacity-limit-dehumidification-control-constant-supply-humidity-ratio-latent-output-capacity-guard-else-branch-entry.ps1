...
foreach ($file in $audits) {
    ...
    if ($number -ge 334 -and $number -le 405) {
        Assert-Contains -Path $file.FullName -Pattern 'non_direct_runtime_rejects_cp316_through_cp438_lifecycle_evidence' -Description "historical non-direct firewall"
    }
    if ($number -ge 337 -and $number -le 405) {
        Assert-Contains -Path $file.FullName -Pattern 'script_count = 376' -Description "historical current script count"
    }
    if ($number -ge 335 -and $number -le 405) {
        Assert-Contains -Path $file.FullName -Pattern ([regex]::Escape('\| 376 \|')) -Description "historical generated script count"
        Assert-Contains -Path $file.FullName -Pattern ([regex]::Escape('\| 136 \|')) -Description "historical generated internal count"
    }
    if ($number -ge 367 -and $number -le 405) {
        Assert-Contains -Path $file.FullName -Pattern 'Count -ne 136' -Description "historical internal classification count"
        Assert-Contains -Path $file.FullName -Pattern '240 public and 136 internal' -Description "historical public/internal classification phrase"
    }
}
$cleanupAudits = @(
