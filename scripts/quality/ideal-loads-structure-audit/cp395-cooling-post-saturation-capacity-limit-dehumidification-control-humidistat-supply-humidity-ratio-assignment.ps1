...
Assert-Contains -Path $pipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp400_lifecycle_evidence' -Description "cumulative non-direct firewall test"
...
foreach ($historical in 334..394) { $file = (Get-ChildItem "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name; Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp400_lifecycle_evidence' -Description "historical firewall" }
foreach ($historical in 335..394) { $file = (Get-ChildItem "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name; Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern ([regex]::Escape('\| executable script records \| 338 \|')) -Description "historical generated total"; Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern ([regex]::Escape('\| internal scripts \| 98 \|')) -Description "historical generated internal" }
foreach ($historical in 337..394) { $file = (Get-ChildItem "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name; Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'script_count = 338' -Description "historical inventory total" }
foreach ($historical in 367..394) { $file = (Get-ChildItem "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name; Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'Count -ne 98' -Description "historical internal count"; Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern '240 public and 98 internal' -Description "historical classification" }
...
foreach ($pattern in @('\$cp395Call\s*=', 'CP394-to-CP395', 'CP396-to-CP397', 'CP397-to-CP398', 'CP400-to-numerical')) { Assert-Contains -Path $cp345Audit -Pattern $pattern -Description "CP345 terminal chain" }; Assert-LineLimit -Path $cp345Audit -Limit 1200 -Description "CP345 structural cap"
...
foreach ($pattern in @('script_count = 338', 'dev_command_count = 238', 'unused_script_count = 0', 'unreachable_count = 0')) { Assert-Cp395Text -Text $inventory -Pattern $pattern -Description "inventory" }
if ([regex]::Matches($inventory, '(?m)^classification = "public"$').Count -ne 240 -or [regex]::Matches($inventory, '(?m)^classification = "internal"$').Count -ne 98) { throw "CP395 inventory classification drift" }
