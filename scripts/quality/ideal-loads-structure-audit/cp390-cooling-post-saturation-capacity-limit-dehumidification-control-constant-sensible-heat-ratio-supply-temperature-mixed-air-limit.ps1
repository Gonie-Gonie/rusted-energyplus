...
foreach ($pattern in @('\$cp389Call\s*=', '\$cp390Call\s*=', '\$cp391Call\s*=', '\$cp392Call\s*=', 'CP389-to-CP390', 'CP390-to-CP391', 'CP391-to-CP392', 'CP395-to-numerical')) {
...
if ([regex]::Matches($inventory, '(?m)^classification = "public"$').Count -ne 240 -or
    ...
    throw "CP390 inventory must be exactly 240 public and 93 internal scripts"
}
