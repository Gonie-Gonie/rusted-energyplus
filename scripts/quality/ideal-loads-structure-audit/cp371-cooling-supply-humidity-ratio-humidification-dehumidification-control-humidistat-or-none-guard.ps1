...
if ($cp370BindingIndexForCp371 -lt 0 -or $cp371BindingIndex -le $cp370BindingIndexForCp371 -or $cp371NumericalIndex -le $cp371BindingIndex) {
...
Assert-Contains -Path $cp371ArbitraryAssertions -Pattern 'assert_numerical_nonfeed\(' -Description "CP371 terminal numerical nonfeed"
Assert-Contains -Path $cp371PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp371_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 334..370) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp371_lifecycle_evidence' -Description "historical CP371 firewall"
}
...
if ($cp370AuditIndexForCp371 -lt 0 -or $cp371AuditIndex -le $cp370AuditIndexForCp371 -or $cp371CompletionIndex -le $cp371AuditIndex) {
...
Assert-Cp371TextContains -Text $cp371InventoryText -Pattern 'script_count = 309' -Description "script total"
...
if ([regex]::Matches($cp371InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp371InventoryText, '(?m)^classification = "internal"$').Count -ne 69) {
    throw "CP371 inventory must be exactly 240 public and 69 internal scripts"
}
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| executable script records \| 309 \|' -Description "CP371 generated total"
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| public scripts \| 240 \|' -Description "CP371 generated public"
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| internal scripts \| 69 \|' -Description "CP371 generated internal"
