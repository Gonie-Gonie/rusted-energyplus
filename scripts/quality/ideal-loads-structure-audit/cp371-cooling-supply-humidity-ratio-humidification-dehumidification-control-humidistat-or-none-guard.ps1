...
if ($cp370BindingIndexForCp371 -lt 0 -or $cp371BindingIndex -le $cp370BindingIndexForCp371 -or
    ...
    $cp376BindingIndexForCp371 -le $cp375BindingIndexForCp371 -or $cp377BindingIndexForCp371 -le $cp376BindingIndexForCp371 -or $cp378BindingIndexForCp371 -le $cp377BindingIndexForCp371 -or $cp371NumericalIndex -le $cp378BindingIndexForCp371) {
...
Assert-Contains -Path $cp371PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp378_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 334..370) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp378_lifecycle_evidence' -Description "historical CP371 firewall"
}
...
Assert-Cp371TextContains -Text $cp371InventoryText -Pattern 'script_count = 316' -Description "script total"
...
if ([regex]::Matches($cp371InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp371InventoryText, '(?m)^classification = "internal"$').Count -ne 76) {
    throw "CP371 inventory must be exactly 240 public and 76 internal scripts"
}
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| executable script records \| 316 \|' -Description "CP371 generated total"
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| public scripts \| 240 \|' -Description "CP371 generated public"
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| internal scripts \| 76 \|' -Description "CP371 generated internal"
