...
$cp376BindingIndexForCp371 = $cp371BindingText.IndexOf("let calculation_cooling_supply_humidity_ratio_pre_saturation_original_assignment ="); $cp377BindingIndexForCp371 = $cp371BindingText.IndexOf("let calculation_cooling_supply_humidity_ratio_saturation_assignment =")
...
if ($cp370BindingIndexForCp371 -lt 0 -or $cp371BindingIndex -le $cp370BindingIndexForCp371 -or
    ...
    $cp376BindingIndexForCp371 -le $cp375BindingIndexForCp371 -or $cp377BindingIndexForCp371 -le $cp376BindingIndexForCp371 -or $cp371NumericalIndex -le $cp377BindingIndexForCp371) {
...
Assert-Contains -Path $cp371PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp377_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 334..370) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp377_lifecycle_evidence' -Description "historical CP371 firewall"
}
...
Assert-Cp371TextContains -Text $cp371InventoryText -Pattern 'script_count = 315' -Description "script total"
...
if ([regex]::Matches($cp371InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp371InventoryText, '(?m)^classification = "internal"$').Count -ne 75) {
    throw "CP371 inventory must be exactly 240 public and 75 internal scripts"
}
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| executable script records \| 315 \|' -Description "CP371 generated total"
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| public scripts \| 240 \|' -Description "CP371 generated public"
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| internal scripts \| 75 \|' -Description "CP371 generated internal"
