...
Assert-PatternsInOrder -Path $scheduledOutput -Patterns @(
    ...
    'pub\s+coupling:'
) -Description "CP392-to-CP393-to-CP394-to-numerical output order"
...
foreach ($pattern in @('script_count = 332', 'dev_command_count = 238', 'unused_script_count = 0', 'unreachable_count = 0')) { Assert-Cp393Text -Text $inventory -Pattern $pattern -Description "inventory" }
if ([regex]::Matches($inventory, '(?m)^classification = "public"$').Count -ne 240 -or [regex]::Matches($inventory, '(?m)^classification = "internal"$').Count -ne 92) { throw "CP393 inventory must be exactly 240 public and 92 internal scripts" }
