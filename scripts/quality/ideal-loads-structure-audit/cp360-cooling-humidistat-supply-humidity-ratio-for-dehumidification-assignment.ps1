...
if (
    ...
    $cp360NumericalIndex -le $cp369BindingIndexForCp360) {
    throw "Binding must execute CP359 through CP368 before numerical coupling"
}
...
Assert-Contains -Path $cp360PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp369_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 307' -Description "CP360 script total"
