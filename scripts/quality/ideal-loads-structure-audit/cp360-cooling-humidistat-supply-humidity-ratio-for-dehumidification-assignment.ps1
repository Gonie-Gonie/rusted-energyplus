...
if (
    ...
    $cp360NumericalIndex -le $cp366BindingIndex
) {
    throw "Binding must execute CP359 through CP366 before numerical coupling"
}
...
Assert-Contains -Path $cp360PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp366_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 304' -Description "CP360 script total"
