...
function Assert-Cp354BindingContract {
    ...
    if ($cp353 -lt 0 -or $cp354 -le $cp353 -or $numerical -le $cp354) {
        throw "Binding must execute CP353 then CP354 before numerical coupling"
    }
...
Assert-Contains -Path $cp354PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp354_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 334..353) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp354_lifecycle_evidence' -Description "historical CP354 firewall"
}
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 292' -Description "CP354 script total"
