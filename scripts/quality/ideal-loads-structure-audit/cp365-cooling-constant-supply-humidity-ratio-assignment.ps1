...
if ($cp364BindingIndexForCp365 -lt 0 -or $cp365BindingIndex -le $cp364BindingIndexForCp365 -or $cp366BindingIndexForCp365 -le $cp365BindingIndex -or $cp367BindingIndexForCp365 -le $cp366BindingIndexForCp365 -or $cp368BindingIndexForCp365 -le $cp367BindingIndexForCp365 -or
    ...
    $cp376BindingIndexForCp365 -le $cp375BindingIndexForCp365 -or $cp377BindingIndexForCp365 -le $cp376BindingIndexForCp365 -or $cp378BindingIndexForCp365 -le $cp377BindingIndexForCp365 -or $cp365NumericalIndex -le $cp378BindingIndexForCp365) {
...
Assert-Contains -Path $cp365PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp378_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 334..364) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp378_lifecycle_evidence' -Description "historical CP365 firewall"
}
