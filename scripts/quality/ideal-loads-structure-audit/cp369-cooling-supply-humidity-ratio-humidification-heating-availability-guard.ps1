...
foreach ($historical in 334..368) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp390_lifecycle_evidence' -Description "historical CP369 firewall"
}
foreach ($historical in 326..333) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-NotContains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp390_lifecycle_evidence' -Description "out-of-range CP369 firewall token"
}
...
Assert-Cp369TextContains -Text $cp369InventoryText -Pattern 'script_count = 328' -Description "script total"
...
[regex]::Matches($cp369InventoryText, '(?m)^classification = "internal"$').Count -ne 88) {
throw "CP369 inventory must be exactly 240 public and 88 internal scripts"
}
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| executable script records \| 328 \|' -Description "CP369 generated total"
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| public scripts \| 240 \|' -Description "CP369 generated public"
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| internal scripts \| 88 \|' -Description "CP369 generated internal"
