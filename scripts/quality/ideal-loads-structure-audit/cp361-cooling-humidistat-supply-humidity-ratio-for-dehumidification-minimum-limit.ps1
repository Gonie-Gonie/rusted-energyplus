...
Assert-Contains -Path $cp361PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp367_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($number in 335..360) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$number-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern ([regex]::Escape('\| executable script records \| 305 \|')) -Description "historical current generated total"
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern ([regex]::Escape('\| internal scripts \| 65 \|')) -Description "historical current generated internal"
}
foreach ($number in 337..360) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$number-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'script_count = 305' -Description "historical current script inventory total"
}
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 305' -Description "CP361 script total"
