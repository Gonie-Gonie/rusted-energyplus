...
if ($cp360BindingIndexForCp361 -lt 0 -or $cp361BindingIndex -le $cp360BindingIndexForCp361 -or $cp362BindingIndexForCp361 -le $cp361BindingIndex -or $cp363BindingIndexForCp361 -le $cp362BindingIndexForCp361 -or $cp364BindingIndexForCp361 -le $cp363BindingIndexForCp361 -or $cp365BindingIndexForCp361 -le $cp364BindingIndexForCp361 -or $cp366BindingIndexForCp361 -le $cp365BindingIndexForCp361 -or $cp367BindingIndexForCp361 -le $cp366BindingIndexForCp361 -or $cp368BindingIndexForCp361 -le $cp367BindingIndexForCp361 -or
    ...
    $cp361NumericalIndex -le $cp370BindingIndexForCp361) {
...
Assert-Contains -Path $cp361PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp370_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($number in 334..360) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$number-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp370_lifecycle_evidence' -Description "historical CP363 firewall"
}
foreach ($number in 335..360) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$number-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern ([regex]::Escape('\| executable script records \| 308 \|')) -Description "historical current generated total"
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern ([regex]::Escape('\| internal scripts \| 68 \|')) -Description "historical current generated internal"
}
foreach ($number in 337..360) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$number-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'script_count = 308' -Description "historical current script inventory total"
}
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 308' -Description "CP361 script total"
