...
$cp377BindingIndex = $cp377BindingText.IndexOf("let calculation_${cp377Stem} ="); $cp378BindingIndex = $cp377BindingText.IndexOf("let calculation_cooling_supply_humidity_ratio_saturation_limit_assignment ="); $cp379BindingIndex = $cp377BindingText.IndexOf("let calculation_cooling_supply_enthalpy_post_saturation_assignment =")
...
if ($cp376BindingIndexForCp377 -lt 0 -or $cp377BindingIndex -le $cp376BindingIndexForCp377 -or
    $cp378BindingIndex -le $cp377BindingIndex -or $cp379BindingIndex -le $cp378BindingIndex -or $cp377NumericalIndex -le $cp379BindingIndex) {
...
Assert-Contains -Path $cp377PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp379_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\cp345-cooling-positive-supply-post-capacity-limit-humidity-ratio-mixed-air-assignment.ps1" -Pattern 'CP379-to-numerical' -Description "CP345 terminal interval"
Assert-LineLimit -Path "scripts\quality\ideal-loads-structure-audit\cp345-cooling-positive-supply-post-capacity-limit-humidity-ratio-mixed-air-assignment.ps1" -Limit 1200 -Description "CP345 historical audit"
...
Assert-Cp377TextContains -Text $cp377InventoryText -Pattern 'script_count = 317' -Description "script total"
...
if ([regex]::Matches($cp377InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp377InventoryText, '(?m)^classification = "internal"$').Count -ne 77) {
    throw "CP377 inventory must be exactly 240 public and 77 internal scripts"
}
