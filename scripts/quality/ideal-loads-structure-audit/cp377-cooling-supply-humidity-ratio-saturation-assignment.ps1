...
Assert-Contains -Path $cp377PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp434_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Cp377TextContains -Text $cp377InventoryText -Pattern 'script_count = 372' -Description "script total"
...
[regex]::Matches($cp377InventoryText, '(?m)^classification = "internal"$').Count -ne 132) {
throw "CP377 inventory must be exactly 240 public and 122 internal scripts"
}
