...
Assert-Contains -Path $cp377PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp388_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Cp377TextContains -Text $cp377InventoryText -Pattern 'script_count = 326' -Description "script total"
...
if ([regex]::Matches($cp377InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp377InventoryText, '(?m)^classification = "internal"$').Count -ne 86) {
    throw "CP377 inventory must be exactly 240 public and 86 internal scripts"
}
