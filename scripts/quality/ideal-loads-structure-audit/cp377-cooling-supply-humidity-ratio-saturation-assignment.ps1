...
Assert-Contains -Path $cp377PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp428_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Cp377TextContains -Text $cp377InventoryText -Pattern 'script_count = 366' -Description "script total"
...
[regex]::Matches($cp377InventoryText, '(?m)^classification = "internal"$').Count -ne 126) {
