...
Assert-Contains -Path $cp377PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp437_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Cp377TextContains -Text $cp377InventoryText -Pattern 'script_count = 375' -Description "script total"
...
[regex]::Matches($cp377InventoryText, '(?m)^classification = "internal"$').Count -ne 135) {
throw "CP377 inventory must be exactly 240 public and 135 internal scripts"
}
