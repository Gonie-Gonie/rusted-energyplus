...
if ($cp376BindingIndexForCp377 -lt 0 -or $cp377BindingIndex -le $cp376BindingIndexForCp377 -or
    $cp378BindingIndex -le $cp377BindingIndex -or $cp379BindingIndex -le $cp378BindingIndex -or $cp380BindingIndex -le $cp379BindingIndex -or $cp381BindingIndex -le $cp380BindingIndex -or $cp382BindingIndex -le $cp381BindingIndex -or $cp383BindingIndex -le $cp382BindingIndex -or $cp377NumericalIndex -le $cp383BindingIndex) {
...
Assert-Contains -Path $cp377PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp383_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Cp377TextContains -Text $cp377InventoryText -Pattern 'script_count = 321' -Description "script total"
...
if ([regex]::Matches($cp377InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp377InventoryText, '(?m)^classification = "internal"$').Count -ne 81) {
    throw "CP377 inventory must be exactly 240 public and 81 internal scripts"
}
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| executable script records \| 321 \|' -Description "generated total"
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| public scripts \| 240 \|' -Description "generated public"
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| internal scripts \| 81 \|' -Description "generated internal"
