...
Assert-Contains -Path $adapterTests -Pattern 'cp421_is_preserved_before_cp422_in_the_current_121_snapshot_binding' -Description 'current 121-snapshot binding regression'
...
$inventory=Read-RepoText -Path 'specs\script_inventory.toml';foreach ($pattern in @('script_count = 368','dev_command_count = 238','unused_script_count = 0','unreachable_count = 0')) {Assert-Cp421Text -Text $inventory -Pattern $pattern -Description 'inventory count'};if ([regex]::Matches($inventory,'(?m)^classification = "public"\r?$').Count -ne 240 -or [regex]::Matches($inventory,'(?m)^classification = "internal"\r?$').Count -ne 128) {throw 'CP421 inventory classification drift'}
