...
Assert-Contains -Path $adapterTests -Pattern 'cp421_is_preserved_before_cp422_in_the_current_131_snapshot_binding' -Description 'current 123-snapshot binding regression'
...
foreach ($file in $audits) { if ($file.BaseName -notmatch '^cp(?<number>\d+)-') {continue}; $number=[int]$Matches['number']; if ($number -ge 334 -and $number -le 421) {Assert-Contains -Path $file.FullName -Pattern 'non_direct_runtime_rejects_cp316_through_cp440_lifecycle_evidence' -Description 'historical non-direct firewall'}; if ($number -ge 337 -and $number -le 421) {Assert-Contains -Path $file.FullName -Pattern 'script_count = 378' -Description 'historical script count'}; if ($number -ge 367 -and $number -le 421) {Assert-Contains -Path $file.FullName -Pattern 'Count -ne 138' -Description 'historical classification count'}; if ($number -ge 335 -and $number -le 421) {Assert-Contains -Path $file.FullName -Pattern ([regex]::Escape('\| 378 \|')) -Description 'historical generated total';Assert-Contains -Path $file.FullName -Pattern ([regex]::Escape('\| 138 \|')) -Description 'historical generated internal total'} }
...
if ((Get-Content -LiteralPath 'scripts\quality\ideal-loads-structure-audit.ps1').Count -gt 4203) {throw 'IdealLoads master audit line cap exceeded after CP421 registration'}
...
$inventory=Read-RepoText -Path 'specs\script_inventory.toml';foreach ($pattern in @('script_count = 378','dev_command_count = 238','unused_script_count = 0','unreachable_count = 0')) {Assert-Cp421Text -Text $inventory -Pattern $pattern -Description 'inventory count'};if ([regex]::Matches($inventory,'(?m)^classification = "public"\r?$').Count -ne 240 -or [regex]::Matches($inventory,'(?m)^classification = "internal"\r?$').Count -ne 138) {throw 'CP421 inventory classification drift'}
