...
if ($cp362BindingIndexForCp363 -lt 0 -or $cp363BindingIndex -le $cp362BindingIndexForCp363 -or $cp364BindingIndexForCp363 -le $cp363BindingIndex -or $cp363NumericalIndex -le $cp364BindingIndexForCp363) {
    throw "Binding must execute CP362 then CP363 then CP364 before numerical coupling"
}
...
Assert-Contains -Path $cp363PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp364_lifecycle_evidence' -Description "cumulative non-direct firewall"
