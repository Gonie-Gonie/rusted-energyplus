...
if ($cp362BindingIndexForCp363 -lt 0 -or $cp363BindingIndex -le $cp362BindingIndexForCp363 -or $cp363NumericalIndex -le $cp363BindingIndex) {
    throw "Binding must execute CP362 then CP363 before numerical coupling"
}
...
Assert-Contains -Path $cp363PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp363_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Contains -Path $cp363ArbitraryAssertions -Pattern 'assert_numerical_nonfeed\(runtime, results\)' -Description "CP363 terminal numerical nonfeed"
...
foreach ($historical in 337..362) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'script_count = 301' -Description "historical inventory total"
}
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 301' -Description "script total"
