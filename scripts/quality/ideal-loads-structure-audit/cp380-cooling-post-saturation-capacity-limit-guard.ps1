...
Assert-Contains -Path $cp380PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp409_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
if ([regex]::Matches($cp380InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp380InventoryText, '(?m)^classification = "internal"$').Count -ne 107) {
    throw "CP380 inventory must be exactly 240 public and 106 internal scripts"
}
