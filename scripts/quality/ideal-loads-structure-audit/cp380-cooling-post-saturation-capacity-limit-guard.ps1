...
Assert-Contains -Path $cp380PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp418_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
if ([regex]::Matches($cp380InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp380InventoryText, '(?m)^classification = "internal"$').Count -ne 116) {
    throw "CP380 inventory must be exactly 240 public and 116 internal scripts"
}
