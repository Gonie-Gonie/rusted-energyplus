...
Assert-Contains -Path $cp380PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp433_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
if ([regex]::Matches($cp380InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp380InventoryText, '(?m)^classification = "internal"$').Count -ne 131) {
