...
Assert-PatternsInOrder -Path $binding -Patterns @(
    ...
    'let\s+coupling\s*='
) -Description "CP403-to-CP404-to-CP405-to-CP406-to-CP407-to-CP408-to-CP411-to-numerical binding order"
...
foreach ($file in @(Get-ChildItem -LiteralPath $auditRoot -Filter 'cp*.ps1' -File)) {
    ...
    if ($number -ge 334 -and $number -le 403) {
        Assert-Contains -Path $file.FullName -Pattern 'non_direct_runtime_rejects_cp316_through_cp411_lifecycle_evidence' -Description "historical CP404 non-direct firewall"
    }
...
foreach ($pattern in @('\$cp404Call\s*=', '\$cp405Call\s*=', '\$cp409Call\s*=', 'CP403-to-CP404', 'CP404-to-CP405', 'CP408-to-CP409', 'CP411-to-numerical')) {
