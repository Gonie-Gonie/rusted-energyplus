...
foreach ($file in @(Get-ChildItem -LiteralPath $auditRoot -Filter 'cp*.ps1' -File)) {
    ...
    if ($number -ge 334 -and $number -le 403) {
        Assert-Contains -Path $file.FullName -Pattern 'non_direct_runtime_rejects_cp316_through_cp440_lifecycle_evidence' -Description "historical CP404 non-direct firewall"
    }
    if ($number -ge 337 -and $number -le 403) {
        Assert-Contains -Path $file.FullName -Pattern 'script_count = 378' -Description "historical current script count"
    }
    if ($number -ge 335 -and $number -le 403) {
        Assert-Contains -Path $file.FullName -Pattern ([regex]::Escape('\| 378 \|')) -Description "historical generated script count"
        Assert-Contains -Path $file.FullName -Pattern ([regex]::Escape('\| 138 \|')) -Description "historical generated internal count"
    }
    if ($number -ge 367 -and $number -le 403) {
        Assert-Contains -Path $file.FullName -Pattern 'Count -ne 138' -Description "historical internal classification count"
    }
}
...
if ([regex]::Matches($inventory, '(?m)^classification = "public"$').Count -ne 240 -or
    ...
    throw "CP404 inventory classification drift; expected 240 public and 137 internal"
}
