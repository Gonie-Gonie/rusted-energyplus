...
Assert-Contains -Path $cp352PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp359_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 334..351) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp359_lifecycle_evidence' -Description "historical CP357 firewall"
}
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 297' -Description "CP352 script total"
