...
foreach ($test in @(
        ...
        "non_direct_runtime_rejects_cp316_through_cp439_lifecycle_evidence"
    )) {
...
foreach ($historical in 334..366) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp439_lifecycle_evidence' -Description "historical CP367 firewall"
}
foreach ($historical in 326..333) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-NotContains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp439_lifecycle_evidence' -Description "out-of-range CP367 firewall token"
}
...
foreach ($historical in 326..334) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-NotContains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern ([regex]::Escape('\| 377 \|')) -Description "out-of-range generated-total token"
    Assert-NotContains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern ([regex]::Escape('\| 137 \|')) -Description "out-of-range generated-internal token"
}
...
foreach ($historical in 326..336) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-NotContains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'script_count = 377' -Description "out-of-range inventory-total token"
}
...
Assert-Cp367TextContains -Text $cp367InventoryText -Pattern 'script_count = 377' -Description "script total"
...
[regex]::Matches($cp367InventoryText, '(?m)^classification = "internal"$').Count -ne 137) {
...
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| 377 \|' -Description "CP367 generated total"
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| public scripts \| 240 \|' -Description "CP367 generated public"
Assert-Contains -Path "docs\src\generated\script-index.md" -Pattern '\| 137 \|' -Description "CP367 generated internal"
