...
function Assert-Cp351BindingContract {
    ...
    if ($cp350 -lt 0 -or $cp351 -le $cp350 -or $cp352 -le $cp351 -or $numerical -le $cp352) {
        throw "Binding must execute CP350 then CP351 then CP352 before numerical coupling"
    }
    ...
    if ($dto -match '(?i)cp35[01]|sensible_output_assignment|total_output_assignment') {
        throw "CP350/CP351 evidence must not enter DirectZonePurchasedAirCouplingInput"
    }
}
...
Assert-Contains -Path $cp351PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp352_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 334..350) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp352_lifecycle_evidence' -Description "historical CP351 firewall"
}
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 290' -Description "CP351 script total"
