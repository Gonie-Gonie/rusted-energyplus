...
function Assert-Cp351BindingContract {
    ...
    if ($cp350 -lt 0 -or $cp351 -le $cp350 -or $cp352 -le $cp351 -or $cp353 -le $cp352 -or $numerical -le $cp353) {
        throw "Binding must execute CP350 then CP351 then CP352 then CP353 before numerical coupling"
    }
...
Assert-Contains -Path $cp351PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp353_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 291' -Description "CP351 script total"
