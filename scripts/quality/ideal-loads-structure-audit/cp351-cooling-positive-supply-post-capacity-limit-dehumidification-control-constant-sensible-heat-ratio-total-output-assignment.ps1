...
function Assert-Cp351BindingContract {
    ...
    if ($cp350 -lt 0 -or $cp351 -le $cp350 -or $cp352 -le $cp351 -or $cp353 -le $cp352 -or $cp354 -le $cp353 -or $cp355 -le $cp354 -or $cp356 -le $cp355 -or $cp357 -le $cp356 -or $numerical -le $cp357) {
        throw "Binding must execute CP350 then CP351 then CP352 then CP353 then CP354 then CP355 then CP356 then CP357 before numerical coupling"
    }
...
Assert-Contains -Path $cp351PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp357_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 295' -Description "CP351 script total"
