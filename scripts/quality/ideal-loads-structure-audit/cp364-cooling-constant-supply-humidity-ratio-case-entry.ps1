...
if ($cp363BindingIndexForCp364 -lt 0 -or $cp364BindingIndex -le $cp363BindingIndexForCp364 -or $cp365BindingIndexForCp364 -le $cp364BindingIndex -or $cp366BindingIndexForCp364 -le $cp365BindingIndexForCp364 -or $cp367BindingIndexForCp364 -le $cp366BindingIndexForCp364 -or $cp368BindingIndexForCp364 -le $cp367BindingIndexForCp364 -or
    ...
    $cp364NumericalIndex -le $cp374BindingIndexForCp364) {
...
Assert-Contains -Path $cp364PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp374_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Contains -Path "crates\ep_run\tests\arbitrary_run_ideal_loads\cp374_assertions.rs" -Pattern 'assert_numerical_nonfeed' -Description "CP374 terminal numerical nonfeed"
...
foreach ($historical in 334..363) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp374_lifecycle_evidence' -Description "historical CP364 firewall"
}
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 312' -Description "script total"
