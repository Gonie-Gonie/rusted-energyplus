...
if ($cp363BindingIndexForCp364 -lt 0 -or $cp364BindingIndex -le $cp363BindingIndexForCp364 -or $cp364NumericalIndex -le $cp364BindingIndex) {
    throw "Binding must execute CP363 then CP364 before numerical coupling"
}
...
Assert-Contains -Path $cp364PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp364_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Contains -Path $cp364ArbitraryAssertions -Pattern 'assert_numerical_nonfeed\(runtime, results\)' -Description "CP364 terminal numerical nonfeed"
...
foreach ($historical in 334..363) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp364_lifecycle_evidence' -Description "historical CP364 firewall"
}
foreach ($historical in 335..363) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern ([regex]::Escape('\| executable script records \| 302 \|')) -Description "historical generated total"
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern ([regex]::Escape('\| internal scripts \| 62 \|')) -Description "historical generated internal"
}
foreach ($historical in 337..363) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'script_count = 302' -Description "historical inventory total"
}
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 302' -Description "script total"
