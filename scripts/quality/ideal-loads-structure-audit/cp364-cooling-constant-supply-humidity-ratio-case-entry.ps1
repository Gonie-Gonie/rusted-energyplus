...
Assert-Contains -Path $cp364PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp406_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 334..363) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp406_lifecycle_evidence' -Description "historical CP364 firewall"
}
...
Assert-Contains -Path "specs\script_inventory.toml" -Pattern 'script_count = 344' -Description "script total"
