...
Assert-Contains -Path $cp384PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp417_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($pattern in @(
        'script_count = 355', 'dev_command_count = 238',
...
if ([regex]::Matches($cp384InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp384InventoryText, '(?m)^classification = "internal"$').Count -ne 115) {
    throw "CP384 inventory must be exactly 240 public and 113 internal scripts"
}
