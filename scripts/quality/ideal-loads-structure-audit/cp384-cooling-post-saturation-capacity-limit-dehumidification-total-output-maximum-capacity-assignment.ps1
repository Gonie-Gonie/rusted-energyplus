...
Assert-Contains -Path $cp384PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp421_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
if ([regex]::Matches($cp384InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp384InventoryText, '(?m)^classification = "internal"$').Count -ne 119) {
    throw "CP384 inventory must be exactly 240 public and 119 internal scripts"
}
