...
Assert-Contains -Path $cp381PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp387_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
Assert-Contains -Path $cp345Audit -Pattern 'CP386-to-CP387' -Description "CP345 CP387 predecessor interval"; Assert-Contains -Path $cp345Audit -Pattern 'CP387-to-numerical' -Description "CP345 terminal interval"
...
if ([regex]::Matches($cp381InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp381InventoryText, '(?m)^classification = "internal"$').Count -ne 85) {
    throw "CP381 inventory must be exactly 240 public and 85 internal scripts"
}
...
foreach ($pattern in @('\| executable script records \| 325 \|', '\| public scripts \| 240 \|', '\| internal scripts \| 85 \|', '\| scripts without callers \| 0 \|')) {
