...
Assert-Contains -Path $cp381PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp386_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
if ([regex]::Matches($cp381InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp381InventoryText, '(?m)^classification = "internal"$').Count -ne 84) {
    throw "CP381 inventory must be exactly 240 public and 84 internal scripts"
}
...
foreach ($pattern in @('\| executable script records \| 324 \|', '\| public scripts \| 240 \|', '\| internal scripts \| 84 \|', '\| scripts without callers \| 0 \|')) {
