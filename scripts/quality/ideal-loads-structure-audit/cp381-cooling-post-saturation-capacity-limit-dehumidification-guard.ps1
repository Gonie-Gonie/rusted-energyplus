...
# CP380 -> CP381 -> CP382 -> CP383 -> unchanged numerical placement.
...
$cp382BindingIndex = $cp381BindingText.IndexOf("let calculation_cooling_post_saturation_capacity_limit_dehumidification_total_output_assignment ="); $cp383BindingIndex = $cp381BindingText.IndexOf("let calculation_cooling_post_saturation_capacity_limit_dehumidification_total_output_guard =")
...
if ($cp380BindingIndexForCp381 -lt 0 -or $cp381BindingIndex -le $cp380BindingIndexForCp381 -or
    $cp382BindingIndex -le $cp381BindingIndex -or $cp383BindingIndex -le $cp382BindingIndex -or $cp381NumericalIndex -le $cp383BindingIndex) {
    throw "Binding must execute CP380, CP381, CP382, CP383, then unchanged numerical coupling"
}
...
Assert-Contains -Path $cp381PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp383_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 334..380) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp383_lifecycle_evidence' -Description "historical firewall"
}
...
foreach ($historical in 367..380) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'Count -ne 81' -Description "historical internal classification count"
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern '240 public and 81 internal' -Description "historical classification diagnostic"
}
...
Assert-Contains -Path $cp345Audit -Pattern 'CP383-to-numerical' -Description "CP345 terminal interval"
...
foreach ($pattern in @('script_count = 321', 'dev_command_count = 238', 'unused_script_count = 0', 'unreachable_count = 0')) {
...
if ([regex]::Matches($cp381InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp381InventoryText, '(?m)^classification = "internal"$').Count -ne 81) {
    throw "CP381 inventory must be exactly 240 public and 81 internal scripts"
}
...
foreach ($pattern in @('\| executable script records \| 321 \|', '\| public scripts \| 240 \|', '\| internal scripts \| 81 \|', '\| scripts without callers \| 0 \|')) {
