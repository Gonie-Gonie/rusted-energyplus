...
if ($cp380BindingIndexForCp381 -lt 0 -or $cp381BindingIndex -le $cp380BindingIndexForCp381 -or
    $cp382BindingIndex -le $cp381BindingIndex -or $cp383BindingIndex -le $cp382BindingIndex -or $cp384BindingIndexForCp381 -le $cp383BindingIndex -or $cp381NumericalIndex -le $cp384BindingIndexForCp381) {
...
Assert-Contains -Path $cp381PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp384_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
if ([regex]::Matches($cp381InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp381InventoryText, '(?m)^classification = "internal"$').Count -ne 82) {
    throw "CP381 inventory must be exactly 240 public and 82 internal scripts"
}
