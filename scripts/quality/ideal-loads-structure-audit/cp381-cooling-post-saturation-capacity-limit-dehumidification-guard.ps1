...
Assert-Contains -Path $cp381PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp410_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 367..380) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'Count -ne 108' -Description "historical internal classification count"
...
if ([regex]::Matches($cp381InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp381InventoryText, '(?m)^classification = "internal"$').Count -ne 108) {
...
foreach ($pattern in @('\| 348 \|', '\| public scripts \| 240 \|', '\| 108 \|', '\| scripts without callers \| 0 \|')) {
