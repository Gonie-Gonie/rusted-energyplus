...
$cp381Required = @(
    ...
    $cp381SnapshotSerialization, $cp380Assertions, $cp381Assertions,
    $cp382Assertions, $cp383Assertions, $cp381Audit
)
...
# CP380 -> CP381 -> CP382 -> CP383 -> CP384 -> unchanged numerical placement.
...
$cp382BindingIndex = $cp381BindingText.IndexOf("let calculation_cooling_post_saturation_capacity_limit_dehumidification_total_output_assignment ="); $cp383BindingIndex = $cp381BindingText.IndexOf("let calculation_cooling_post_saturation_capacity_limit_dehumidification_total_output_guard ="); $cp384BindingIndexForCp381 = $cp381BindingText.IndexOf("let calculation_cooling_post_saturation_capacity_limit_dehumidification_total_output_maximum_capacity_assignment ="); $cp385BindingIndexForCp381 = $cp381BindingText.IndexOf("let calculation_cooling_post_saturation_capacity_limit_dehumidification_total_output_supply_enthalpy_assignment =")
...
if ($cp380BindingIndexForCp381 -lt 0 -or $cp381BindingIndex -le $cp380BindingIndexForCp381 -or
    $cp382BindingIndex -le $cp381BindingIndex -or $cp383BindingIndex -le $cp382BindingIndex -or $cp384BindingIndexForCp381 -le $cp383BindingIndex -or $cp385BindingIndexForCp381 -le $cp384BindingIndexForCp381 -or $cp381NumericalIndex -le $cp385BindingIndexForCp381) {
    throw "Binding must execute CP380, CP381, CP382, CP383, CP384, then unchanged numerical coupling"
}
...
Assert-Contains -Path $cp381PipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp440_lifecycle_evidence' -Description "cumulative non-direct firewall"
...
foreach ($historical in 334..380) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp440_lifecycle_evidence' -Description "historical firewall"
}
...
foreach ($historical in 367..380) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'Count -ne 138' -Description "historical internal classification count"
...
Assert-Contains -Path $cp345Audit -Pattern 'CP381-to-CP382' -Description "CP345 successor interval"
Assert-Contains -Path $cp345Audit -Pattern 'CP382-to-CP383' -Description "CP345 CP383 successor interval"
...
if ([regex]::Matches($cp381InventoryText, '(?m)^classification = "public"$').Count -ne 240 -or
    [regex]::Matches($cp381InventoryText, '(?m)^classification = "internal"$').Count -ne 138) {
    throw "CP381 inventory must be exactly 240 public and 136 internal scripts"
}
...
foreach ($pattern in @('\| 378 \|', '\| public scripts \| 240 \|', '\| 138 \|', '\| scripts without callers \| 0 \|')) {
