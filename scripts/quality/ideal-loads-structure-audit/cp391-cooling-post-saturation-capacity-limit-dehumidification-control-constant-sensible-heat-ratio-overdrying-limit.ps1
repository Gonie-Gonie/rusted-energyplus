...
if ([regex]::Matches($bindingText.Substring($cp390Index, $cp391Index - $cp390Index), [regex]::Escape($cp390BindingName)).Count -ne 1 -or
    ...
    throw "Binding CP390-to-CP391-to-CP392-to-CP393-to-numerical intervals are not exact"
}
...
foreach ($pattern in @('\$cp390Call\s*=', '\$cp391Call\s*=', '\$cp392Call\s*=', 'CP390-to-CP391', 'CP391-to-CP392', 'CP393-to-numerical')) { Assert-Contains -Path $cp345Audit -Pattern $pattern -Description "strict CP345 terminal ordering" }
...
foreach ($pattern in @('script_count = 331', 'dev_command_count = 238', 'unused_script_count = 0', 'unreachable_count = 0')) { Assert-Cp391Text -Text $inventory -Pattern $pattern -Description "inventory" }
if ([regex]::Matches($inventory, '(?m)^classification = "public"$').Count -ne 240 -or [regex]::Matches($inventory, '(?m)^classification = "internal"$').Count -ne 91) { throw "CP391 inventory must be exactly 240 public and 91 internal scripts" }
