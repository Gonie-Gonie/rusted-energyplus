...
Assert-Contains -Path $prefix -Pattern 'cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_supply_temperature_mixed_air_limit_committed_latest_snapshot_is_consistent\s*\(' -Description "bounded CP390 committed predecessor proof"
Assert-NotContains -Path $prefix -Pattern 'completed_direct_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_supply_temperature_mixed_air_limit_is_consistent\s*\(' -Description "recursive CP390 predecessor completion"
Assert-Contains -Path $prefix -Pattern 'supply_temperature_mixed_air_limit_latest_witness' -Description "CP390 private latest witness"
Assert-Contains -Path $prefix -Pattern 'supply_temperature_mixed_air_limit_snapshots_match_bit_exact' -Description "CP390 retained/witness bit corroboration"
Assert-Contains -Path $release -Pattern 'supply_temperature_mixed_air_limit_snapshot_is_exact_direct_release\(predecessor_cp390\)' -Description "exact CP390 direct-release evidence"
...
Assert-Contains -Path $pipelineRoot -Pattern 'non_direct_runtime_rejects_cp316_through_cp440_lifecycle_evidence' -Description "cumulative non-direct firewall test"
...
foreach ($historical in 334..390) {
    ...
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'non_direct_runtime_rejects_cp316_through_cp440_lifecycle_evidence' -Description "historical cumulative firewall"
}
...
foreach ($historical in 367..390) {
    $file = (Get-ChildItem -LiteralPath "scripts\quality\ideal-loads-structure-audit" -Filter "cp$historical-*.ps1").Name
    Assert-Contains -Path "scripts\quality\ideal-loads-structure-audit\$file" -Pattern 'Count -ne 138' -Description "historical internal classification"
...
foreach ($pattern in @('script_count = 378', 'dev_command_count = 238', 'unused_script_count = 0', 'unreachable_count = 0')) { Assert-Cp391Text -Text $inventory -Pattern $pattern -Description "inventory" }
if ([regex]::Matches($inventory, '(?m)^classification = "public"$').Count -ne 240 -or [regex]::Matches($inventory, '(?m)^classification = "internal"$').Count -ne 138) { throw "CP391 inventory must be exactly 240 public and 137 internal scripts" }
