...
# Binding order is CP323 -> CP324 -> the pre-existing numerical DTO.
...
if (
    ...
    $numericalBindingIndexForCp324 -le $cp324BindingIndex
) {
    throw "Binding must retain exact CP323 -> CP324 -> numerical Calc order"
}
...
if (-not $cp323BindingCallForCp324.Success -or -not $cp324BindingCall.Success) {
    throw "Binding must retain complete CP323 and CP324 exact release calls"
}
...
if (
    ...
    $numericalBindingIndexForCp324 -lt $cp324BindingCallEnd
) {
    throw "CP323 and CP324 exact release calls must complete in source order before numerical Calc"
}
...
$postCp324BeforeNumerical = $cp324BindingText.Substring(
    $cp324BindingCallEnd,
    $numericalBindingIndexForCp324 - $cp324BindingCallEnd
)
if ($postCp324BeforeNumerical -match 'MaxCoolMassFlowRate|VerySmallMassFlow|CalcPurchAirMixedAir|(?i)(?:ems|psychrometric|diagnostic|node_service)\s*\(') {
    throw "No line-2161-or-later or live EMS behavior may execute after CP324 and before numerical Calc"
}
