...
$script = Join-Path $RepoRoot "tools\docs\validate_script_inventory.py"
if (-not (Test-Path -LiteralPath $script -PathType Leaf)) {
    throw "Missing script inventory validator: $script"
}

& $python $script --repo-root $RepoRoot
if ($LASTEXITCODE -ne 0) {
    throw "Script inventory check failed with exit code $LASTEXITCODE"
}
