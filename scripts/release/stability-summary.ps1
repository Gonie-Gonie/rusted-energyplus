...
$script = Join-Path $RepoRoot "tools\reporting\stability_summary.py"
if (-not (Test-Path -LiteralPath $script -PathType Leaf)) {
    throw "Missing stability summary generator: $script"
}

& $python $script --repo-root $RepoRoot --version $Version
if ($LASTEXITCODE -ne 0) {
    throw "Stability summary generation failed with exit code $LASTEXITCODE"
}
