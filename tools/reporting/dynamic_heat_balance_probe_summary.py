...
LANES = (
    ...
    ProbeLane(
        lane="all-ctf-third-order-frozen-hconv-weather-storage-balance-surfconv-frozen-refair-current-lw-converged-inside-ctf-out-hist-scriptf-flat-hconv-reeval2-warmup-min20-iter20",
        summary_path=Path(
            ".runtime/official-dynamic-diagnostic-all-ctf-third-order-frozen-hconv-weather-storage-balance-surfconv-frozen-refair-current-lw-converged-inside-ctf-out-hist-scriptf-flat-warmup-min20-surface-iter20-hconv-reeval2"
        )
        / ORACLE_VERSION
        / CASE_ID
        / "compare/compare-summary.json",
...
def floor_ctf_max_sample_driver_row(summary: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        ...
        "inside_conduction_signed_delta_w": inside_solve.get(
            "inside_conduction_signed_delta_w"
        ),
        "inside_current_signed_delta_w": inside_solve.get(
            "inside_current_signed_delta_w"
        ),
        "inside_current_outside_term_signed_delta_w": inside_solve.get(
            "inside_current_outside_term_signed_delta_w"
        ),
        "inside_current_inside_term_signed_delta_w": inside_solve.get(
            "inside_current_inside_term_signed_delta_w"
        ),
        "inside_current_delta_w": inside_solve.get("inside_current_delta_w"),
        "inside_current_cancellation_delta_w": inside_solve.get(
            "inside_current_cancellation_delta_w"
        ),
        "inside_history_signed_delta_w": inside_solve.get(
    ...
    for lane in summary["lanes"]:
        ...
        lines.append(
            "| {lane} | {sample} | {in_temp} | {ref_air_temp} | {numerator} | {tracked} | {coverage} | {untracked} | {history_share} | {ref_air_share} | {ref_hconv_share} | {ref_temp_share} | {lw_share} | {outside_share} | {history} | {history_signed} | {current_signed} | {current_out_signed} | {current_in_signed} | {current_cancel} | {conduction_signed} | {ref_air} | {ref_air_signed} | {ref_air_cancel} | {ref_hconv} | {ref_hconv_signed} | {ref_temp} | {ref_temp_signed} | {lw} | {history_temp} | {history_flux} | {slots} | {slot1_total} | {slot1_tin} | {slot1_equiv} | {slot2_total} | {slot2_tin} | {slot2_equiv} |".format(
                ...
                ref_air_signed=fmt_signed_number(
                    driver.get("reference_air_source_signed_delta_w")
                ),
                ref_air_cancel=fmt_signed_number(
                    driver.get("reference_air_source_cancellation_delta_w")
                ),
                ref_hconv=fmt_number(
                    driver.get("reference_air_coefficient_source_delta_w")
                ),
                ref_hconv_signed=fmt_signed_number(
                    driver.get("reference_air_coefficient_source_signed_delta_w")
                ),
                ref_temp=fmt_number(
                    driver.get("reference_air_temperature_source_delta_w")
                ),
                ref_temp_signed=fmt_signed_number(
                    driver.get("reference_air_temperature_source_signed_delta_w")
                ),
                history=fmt_number(driver.get("inside_history_delta_w")),
                history_signed=fmt_signed_number(
                    driver.get("inside_history_signed_delta_w")
                ),
                current_signed=fmt_signed_number(
                    driver.get("inside_current_signed_delta_w")
                ),
                current_out_signed=fmt_signed_number(
                    driver.get("inside_current_outside_term_signed_delta_w")
                ),
                current_in_signed=fmt_signed_number(
                    driver.get("inside_current_inside_term_signed_delta_w")
                ),
                current_cancel=fmt_signed_number(
                    driver.get("inside_current_cancellation_delta_w")
                ),
                conduction_signed=fmt_signed_number(
                    driver.get("inside_conduction_signed_delta_w")
                ),
