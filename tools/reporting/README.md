...
- `v026_dynamic_idf_inventory.py` inventories IDF-backed case manifests through
  ...
  case a target domain and role so official ExampleFile heat-balance work stays
  separated from HVAC, plant, and static intake gaps. It is a planning aid for
  the v0.26-example dynamic-conformance expansion target, not a release claim
  artifact.
- `dynamic_heat_balance_probe_summary.py` summarizes the official dynamic
  ...
  with three and five surface passes plus an EnergyPlus initial CTF-history seed
  fork, DOE-2 exterior-convection and grey interior-longwave forks, and the
  combined DOE-2/interior-longwave fork,
  combined
