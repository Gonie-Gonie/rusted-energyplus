...
use ep_model::{MaterialDefinition, TypedModel, WindowComplexGapMaterial};
use ep_raw_model::{RawModel, RawObject, RawValue, load_epjson_file};

pub(crate) const CASE_ID: &str = "window_material_gap_001";
pub(crate) const USAGE: &str = "usage: eplus-rs compare window-material-gap <input.epJSON> <eplusout.eio> [--tolerance exact|near]";

const OUTPUT_CONSTRUCTIONS_OBJECT_TYPE: &str = "Output:Constructions";
const MATERIAL_DETAILS_HEADER_MARKER: &str = "! <Material Details>";
const GAP_ROUGHNESS: &str = "Rough";
...
impl NumericToleranceMode {
    ...
    const fn policy_label(self) -> &'static str {
        match self {
            Self::Exact => {
                "energyplus-26.1-window-material-gap-material-details-4R-normalized-exact"
            }
...
#[derive(Clone, Debug)]
struct WindowMaterialGapDefinition {
    material_name: String,
    fields: WindowComplexGapMaterial,
}
...
#[derive(Debug)]
struct WindowMaterialGapComparison {
    definitions: Vec<WindowMaterialGapDefinition>,
...
pub(crate) fn run_compare_window_material_gap(args: &[String]) -> i32 {
    ...
    if window_material_gap_definitions(&model).is_empty() {
        eprintln!("no WindowMaterial:Gap objects are available for comparison");
        return 1;
    }
    ...
    let comparison =
        match compare_window_material_gap(&raw_model, &model, &eio_contents, tolerance_mode) {
...
fn parse_tolerance_mode(args: &[String]) -> Result<NumericToleranceMode, String> {
    match args {
        ...
        [flag, value] if flag == "--tolerance" => match value.as_str() {
            ...
            _ => Err(format!(
                "unsupported WindowMaterial:Gap tolerance mode: {value}"
            )),
        },
        _ => Err("unsupported WindowMaterial:Gap compare options".to_string()),
    }
}

fn compare_window_material_gap(
    raw_model: &RawModel,
    model: &TypedModel,
    eio_contents: &str,
    tolerance_mode: NumericToleranceMode,
) -> Result<WindowMaterialGapComparison, String> {
    ...
    if forbidden_window_shape.total_rows() != 0 {
        ...
        record_first_divergence(
            &mut first_divergence,
            format!(
                "forbidden specialized window report evidence observed: WindowMaterial:Gap headers {}, WindowMaterial:Gap rows {}, glazing headers {}, glazing rows {}, WindowConstruction headers {}, WindowConstruction rows {}",
                forbidden_window_shape.gap_header_rows,
                forbidden_window_shape.gap_data_rows,
    ...
    Ok(WindowMaterialGapComparison {
        definitions,
...
fn window_material_gap_definitions(model: &TypedModel) -> Vec<WindowMaterialGapDefinition> {
    model
        .materials
        .iter()
        .filter_map(|material| {
            let MaterialDefinition::WindowComplexGap(fields) = material.definition else {
                return None;
            };
            Some(WindowMaterialGapDefinition {
...
fn expected_numeric_fields(
    definition: &WindowMaterialGapDefinition,
    ...
    Some([
        ...
        ("thermal_absorptance", 0.0, observed.thermal_absorptance),
...
fn generic_row_matches(
    ...
    normalized_material_name(&observed.material_name)
        ...
        && observed.roughness == GAP_ROUGHNESS
...
fn record_generic_divergence(
    ...
    if observed.roughness != GAP_ROUGHNESS {
        record_first_divergence(
            first_divergence,
            format!(
                "{prefix} field roughness expected {GAP_ROUGHNESS} observed {}",
                observed.roughness
            ),
        );
        return;
...
fn forbidden_window_table_shape(eio_contents: &str) -> ForbiddenWindowTableShape {
    ...
    for line in eio_contents.lines() {
        ...
        if trimmed.starts_with("! <WindowMaterial:Gap>,") {
            shape.gap_header_rows += 1;
        }
        if trimmed.starts_with("WindowMaterial:Gap,") {
            shape.gap_data_rows += 1;
        }
...
fn render_comparison(comparison: &WindowMaterialGapComparison) {
    let indices = indices_by_material_name(&comparison.oracle_material_details);
    let oracle_gap_rows = comparison
        .definitions
        .iter()
        .map(|definition| {
            indices
    ...
    println!("Window Material Gap Comparison");
    ...
    println!("  gas_state_reporting_claim: false");
    println!("  pressure_reporting_claim: false");
    println!("  deflection_reporting_claim: false");
    println!("  support_pillar_reporting_claim: false");
    println!("  construction_use_claim: false");
    println!("  specialized_gap_claim: false");
    ...
    println!("  oracle_window_material_gap_rows: {oracle_gap_rows}");
    println!(
    ...
    println!(
        "  specialized_gap_header_rows: {}",
        comparison.forbidden_window_shape.gap_header_rows
    );
    println!(
        "  specialized_gap_rows: {}",
        comparison.forbidden_window_shape.gap_data_rows
    );
    println!(
...
#[cfg(test)]
#[path = "window_material_gap_tests.rs"]
mod tests;
