...
use ep_model::{MaterialDefinition, TypedModel, WindowComplexShadeMaterial};
use ep_raw_model::{RawModel, RawObject, RawValue, load_epjson_file};

pub(crate) const CASE_ID: &str = "window_material_complex_shade_001";
pub(crate) const USAGE: &str = "usage: eplus-rs compare window-material-complex-shade <input.epJSON> <eplusout.eio> [--tolerance exact|near]";
...
const COMPLEX_SHADE_ROUGHNESS: &str = "Rough";
const SOURCE_IDF_MATERIAL_ORDER: [&str; 7] = [
    "Z FULL DEFAULTS",
    "Y BSDF CUSTOM BASE",
...
#[derive(Clone, Debug)]
struct WindowMaterialComplexShadeDefinition {
    material_name: String,
    fields: WindowComplexShadeMaterial,
}
...
struct ForbiddenWindowTableShape {
    complex_shade_header_rows: usize,
    complex_shade_data_rows: usize,
...
impl ForbiddenWindowTableShape {
    const fn total_rows(self) -> usize {
        self.complex_shade_header_rows
            + self.complex_shade_data_rows
...
#[derive(Debug)]
struct WindowMaterialComplexShadeComparison {
    definitions: Vec<WindowMaterialComplexShadeDefinition>,
...
pub(crate) fn run_compare_window_material_complex_shade(args: &[String]) -> i32 {
    ...
    if window_material_complex_shade_definitions(&model).is_empty() {
        eprintln!("no WindowMaterial:ComplexShade objects are available for comparison");
        return 1;
    }
    ...
    let comparison = match compare_window_material_complex_shade(
        &raw_model,
        &model,
        &eio_contents,
        tolerance_mode,
...
fn parse_tolerance_mode(args: &[String]) -> Result<NumericToleranceMode, String> {
    match args {
        ...
        [flag, value] if flag == "--tolerance" => match value.as_str() {
            ...
            _ => Err(format!(
                "unsupported WindowMaterial:ComplexShade tolerance mode: {value}"
            )),
        },
        _ => Err("unsupported WindowMaterial:ComplexShade compare options".to_string()),
    }
}

fn compare_window_material_complex_shade(
    raw_model: &RawModel,
    model: &TypedModel,
    eio_contents: &str,
    tolerance_mode: NumericToleranceMode,
) -> Result<WindowMaterialComplexShadeComparison, String> {
    ...
    let expected_data_rows = if report_requests.materials {
        SOURCE_IDF_MATERIAL_ORDER.len()
    } else {
        0
    };
    let mut passed = true;
    ...
    let definition_names = definitions
        ...
        .collect::<Vec<_>>();
    let expected_definition_names = SOURCE_IDF_MATERIAL_ORDER
        .iter()
        .map(|name| normalized_material_name(name))
        .collect::<Vec<_>>();
    if definition_names != expected_definition_names {
        passed = false;
        record_first_divergence(
            &mut first_divergence,
            format!(
                "typed complex-shade definitions expected source fixture order {expected_definition_names:?} observed {definition_names:?}"
            ),
        );
    }
    ...
    if material_details_shape.data_rows != expected_data_rows {
        passed = false;
        record_first_divergence(
            &mut first_divergence,
            format!(
                "Material Details rows expected {expected_data_rows} observed {}",
    ...
    if forbidden_window_shape.total_rows() != 0 {
        ...
        record_first_divergence(
            &mut first_divergence,
            format!(
                "forbidden specialized window report evidence observed: WindowMaterial:ComplexShade headers {}, WindowMaterial:ComplexShade rows {}, glazing headers {}, glazing rows {}, WindowConstruction headers {}, WindowConstruction rows {}",
                forbidden_window_shape.complex_shade_header_rows,
                forbidden_window_shape.complex_shade_data_rows,
    ...
    let expected_oracle_order = if report_requests.materials {
        expected_definition_names.clone()
    } else {
        Vec::new()
    };
    let observed_oracle_order = oracle_material_details
        .iter()
        .map(|row| normalized_material_name(&row.material_name))
        .collect::<Vec<_>>();
    if observed_oracle_order != expected_oracle_order {
        passed = false;
        record_first_divergence(
            &mut first_divergence,
            format!(
                "Material Details source fixture order expected {expected_oracle_order:?} observed {observed_oracle_order:?}"
            ),
        );
    }
    ...
    Ok(WindowMaterialComplexShadeComparison {
        definitions,
...
fn window_material_complex_shade_definitions(
    model: &TypedModel,
) -> Vec<WindowMaterialComplexShadeDefinition> {
    let mut definitions = model
        .materials
        .iter()
        .filter_map(|material| {
            let MaterialDefinition::WindowComplexShade(fields) = material.definition else {
                return None;
            };
            Some(WindowMaterialComplexShadeDefinition {
    ...
    definitions.sort_by(|left, right| {
        source_order_rank(&left.material_name)
            .cmp(&source_order_rank(&right.material_name))
            .then_with(|| {
                normalized_material_name(&left.material_name)
                    .cmp(&normalized_material_name(&right.material_name))
            })
    });
    definitions
}

fn source_order_rank(name: &str) -> usize {
    let normalized = normalized_material_name(name);
    SOURCE_IDF_MATERIAL_ORDER
        .iter()
        .position(|expected| normalized_material_name(expected) == normalized)
        .unwrap_or(SOURCE_IDF_MATERIAL_ORDER.len())
}
...
fn expected_numeric_fields(
    definition: &WindowMaterialComplexShadeDefinition,
    ...
    Some([
        ...
        (
            "thickness_m",
            super::energyplus_round_sig_digits_positive(definition.fields.thickness_m, 4)?,
            observed.thickness_m,
        ),
        (
            "conductivity_w_per_m_k",
            super::energyplus_round_sig_digits_positive(
                definition.fields.conductivity_w_per_m_k,
                3,
            )?,
        ...
        (
            "thermal_absorptance",
            super::energyplus_round_sig_digits_positive(definition.fields.thermal_absorptance, 4)?,
            observed.thermal_absorptance,
        ),
        ("solar_absorptance", 0.0, observed.solar_absorptance),
        ("visible_absorptance", 0.0, observed.visible_absorptance),
    ])
}
...
fn generic_row_matches(
    ...
    normalized_material_name(&observed.material_name)
        ...
        && observed.roughness == COMPLEX_SHADE_ROUGHNESS
...
fn record_generic_divergence(
    ...
    if observed.roughness != COMPLEX_SHADE_ROUGHNESS {
        record_first_divergence(
            first_divergence,
            format!(
                "{prefix} field roughness expected {COMPLEX_SHADE_ROUGHNESS} observed {}",
                observed.roughness
            ),
        );
        return;
...
fn forbidden_window_table_shape(eio_contents: &str) -> ForbiddenWindowTableShape {
    ...
    for line in eio_contents.lines() {
        ...
        if trimmed.starts_with("! <WindowMaterial:ComplexShade>,") {
            shape.complex_shade_header_rows += 1;
        }
        if trimmed.starts_with("WindowMaterial:ComplexShade,") {
            shape.complex_shade_data_rows += 1;
        }
...
fn render_comparison(comparison: &WindowMaterialComplexShadeComparison) {
    let indices = indices_by_material_name(&comparison.oracle_material_details);
    let oracle_complex_shade_rows = comparison
        .definitions
        .iter()
        .map(|definition| {
            indices
    ...
    println!("Window Material Complex Shade Comparison");
    ...
    println!("  layer_type_reporting_claim: false");
    println!("  infrared_reporting_claim: false");
    println!("  front_emissivity_reporting_claim: false");
    println!("  opening_reporting_claim: false");
    println!("  slat_reporting_claim: false");
    println!("  complex_fenestration_construction_claim: false");
    println!("  specialized_complex_shade_claim: false");
    ...
    println!("  oracle_window_material_complex_shade_rows: {oracle_complex_shade_rows}");
    println!(
    ...
    println!(
        "  specialized_complex_shade_header_rows: {}",
        comparison.forbidden_window_shape.complex_shade_header_rows
    );
    println!(
        "  specialized_complex_shade_rows: {}",
        comparison.forbidden_window_shape.complex_shade_data_rows
    );
    println!(
...
#[cfg(test)]
#[path = "window_material_complex_shade_tests.rs"]
mod tests;
