...
use super::super::cooling_supply_humidity_ratio_humidification_moisture_demand_assignment_validation::{
    expected_snapshot, validate_lifecycle,
};

#[test]
fn cp372_lifecycle_matches_cp371_and_does_not_feed_numerical_result() {
    let fixture = validator_fixture();
    assert!(fixture.is_some(), "CP372 validator fixture");
...
#[test]
fn cp372_direct_counters_and_latest_corruption_are_rejected() {
    let fixture = validator_fixture();
    assert!(fixture.is_some(), "CP372 corruption fixture");
    let Some((model, output, lifecycle, predecessor)) = fixture else {
        return;
    };
    ...
    assert!(latest.state.latest.is_some(), "CP372 latest");
    let Some(latest_snapshot) = latest.state.latest.as_mut() else {
        return;
    };
    latest_snapshot.zone_humidifying_setpoint_moisture_demand_read = true;
    assert_eq!(
    ...
    assert!(
        ...
        "CP371 predecessor latest"
    );
    let Some(predecessor_latest_snapshot) = predecessor_latest.state.latest.as_mut() else {
        return;
    };
    predecessor_latest_snapshot.dehumidification_control_type_first_read = true;
    assert_eq!(
...
fn set_humidistat_assignment_one(lifecycle: &mut Lifecycle) {
    lifecycle
        .state
        .dehumidification_control_humidistat_moisture_demand_assignment_count = 1;
}

fn set_none_assignment_one(lifecycle: &mut Lifecycle) {
    lifecycle
        .state
        .dehumidification_control_none_moisture_demand_assignment_count = 1;
}
...
fn set_assignment_one(lifecycle: &mut Lifecycle) {
    lifecycle
        .state
        .humidification_moisture_demand_assignment_count = 1;
}
...
fn set_read_one(lifecycle: &mut Lifecycle) {
    lifecycle
        .state
        .zone_humidifying_setpoint_moisture_demand_read_count = 1;
}

fn set_write_one(lifecycle: &mut Lifecycle) {
    lifecycle
        .state
        .zone_humidifying_setpoint_moisture_demand_assignment_count = 1;
}
...
fn validator_fixture() -> Option<(
    ...
    .ok()?;
    let lifecycle = purchased_air_calc_cooling_supply_humidity_ratio_humidification_moisture_demand_assignment_lifecycle_summary(
        &runtime,
        output.initialization.system,
    )
    .ok()?;
    let predecessor = purchased_air_calc_cooling_supply_humidity_ratio_humidification_dehumidification_control_humidistat_or_none_guard_lifecycle_summary(
        &runtime,
        output.initialization.system,
    )
    .ok()?;
