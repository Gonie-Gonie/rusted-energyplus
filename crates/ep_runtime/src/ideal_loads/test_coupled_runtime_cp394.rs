//! Non-vacuous CP394 coupled-runtime integration tests.

use super::*;
use crate::ideal_loads::{
    DirectZonePurchasedAirScheduledCouplingInput, DirectZonePurchasedAirScheduledCouplingOutput,
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationControlConstantSensibleHeatRatioCaseBreakLifecycleSummary as Predecessor,
...
use super::super::cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_case_entry_validation::{
    snapshot_matches_release, validate_lifecycle,
};

#[test]
fn cp394_preserves_cp393_terminal_carriers_and_skips_humidistat_entry_on_direct_routes() {
    for (limit, maximum_capacity_w, availability) in [
        ...
        let snapshot = output.calculation_cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_case_entry;
        let cp393 = output.calculation_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_case_break;
        ...
        assert_eq!(
            ...
            cp393.dehumidification_control_constant_sensible_heat_ratio_case_exited_via_break,
        );
        for (left, right) in [
            (
                snapshot.predecessor_cp393_resulting_supply_humidity_ratio,
                cp393.resulting_supply_humidity_ratio,
            ),
            (
                snapshot.predecessor_cp393_resulting_supply_enthalpy_j_per_kg,
                cp393.resulting_supply_enthalpy_j_per_kg,
            ),
            (
                snapshot.predecessor_cp393_resulting_supply_temperature_c,
                cp393.resulting_supply_temperature_c,
            ),
            (
...
#[test]
fn cp394_rejects_cp393_carrier_bit_drift_and_route_drift() {
    ...
    let value = predecessor
        ...
        .as_mut()
        .expect("CP393 retained enthalpy");
...
#[test]
fn cp394_validation_remains_independent_of_numerical_output_state() {
    let (model, mut output, lifecycle, predecessor) =
        validator_fixture(IdealLoadsLimit::LimitCapacity, 0.020, 500.0, 1.0, 2);
    let binding = bind_direct_zone_purchased_air_model(&model).expect("CP394 binding");
    output
        .coupling
        .purchased_air
        .calculation
...
fn validator_fixture(
    ...
    let cache = precompute_schedule_cache(&model.typed, 1).expect("CP394 schedule cache");
    let binding = bind_direct_zone_purchased_air_model(&model).expect("CP394 direct binding");
    ...
    for step in 0..steps {
        ...
        latest = Some(
            ...
            .expect("CP394 coupling"),
        );
    }
    let output = latest.expect("at least one CP394 step");
    let system = output.initialization.system;
    let lifecycle = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_case_entry_lifecycle_summary(&runtime, system).expect("CP394 lifecycle");
    let predecessor = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_case_break_lifecycle_summary(&runtime, system).expect("CP393 lifecycle");
...
fn latest_violation() -> Error {
    Error::CalcCoolingPostSaturationCapacityLimitDehumidificationControlHumidistatCaseEntryLifecycleInvariant {
