//! Non-vacuous CP387 coupled-runtime integration tests.
...
use crate::ideal_loads::{
    ...
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationControlSwitchLifecycleSummary as Predecessor,
...
use super::super::cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_cp_air_assignment_validation::{
    snapshot_matches_release, validate_lifecycle,
};

#[test]
fn cp387_completes_as_inactive_for_every_direct_none_selector_route() {
    for (limit, maximum_capacity_w, expected_cp386_active) in [
        ...
        assert_eq!(
            ...
            expected_cp386_active
        );
        assert_eq!(
            cp387.predecessor_dehumidification_control_type,
            expected_cp386_active.then_some(DehumidificationControlType::None)
        );
        assert!(!cp387.dehumidification_control_constant_sensible_heat_ratio_case_entered);
        assert!(
            !cp387.dehumidification_control_constant_sensible_heat_ratio_cp_air_assignment_executed
        );
        assert!(!cp387.mixed_air_humidity_ratio_read);
        assert!(cp387.mixed_air_humidity_ratio.is_none());
        assert!(!cp387.psychrometric_cp_air_evaluated);
        assert!(cp387.psychrometric_cp_air_result_j_per_kg_k.is_none());
        assert!(!cp387.cp_air_assigned);
        assert!(cp387.cp_air_j_per_kg_k.is_none());
        assert_eq!(
            cp387
...
#[test]
fn cp387_rejects_one_bit_drift_in_cp386_retained_enthalpy() {
    ...
    let value = predecessor
        ...
        .as_mut()
        .expect("CP386 retained enthalpy");
...
#[test]
fn cp387_rejects_predecessor_route_count_drift() {
...
#[test]
fn cp387_replay_accumulates_inactive_counts_and_keeps_latest_lineage() {
    let (model, output, lifecycle, predecessor) =
...
#[test]
fn cp387_is_evidence_only_and_does_not_consume_load_result_node_report_or_feedback() {
    let (model, output, lifecycle, predecessor) =
        validator_fixture(IdealLoadsLimit::LimitCapacity, 0.020, 500.0, 1);
...
fn validator_fixture(
    ...
    let cache = precompute_schedule_cache(&model.typed, 1).expect("CP387 schedule cache");
    let binding = bind_direct_zone_purchased_air_model(&model).expect("CP387 direct binding");
    ...
    for step in 0..steps {
        ...
        latest = Some(
            ...
            .expect("CP387 coupling"),
        );
    }
    let output = latest.expect("CP387 fixture requires at least one step");
    let system = output.initialization.system;
    let lifecycle = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_cp_air_assignment_lifecycle_summary(&runtime, system).expect("CP387 lifecycle");
    let predecessor = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_switch_lifecycle_summary(&runtime, system).expect("CP386 lifecycle");
