//! Non-vacuous CP411 coupled-runtime integration and numerical-firewall tests.
...
use super::super::cooling_post_saturation_capacity_limit_dehumidification_supply_humidity_ratio_pre_saturation_original_assignment_validation::{
    snapshot_matches_release, validate_lifecycle,
};

#[test]
fn cp411_flattened_route_contract_has_18_active_routes_and_four_public_active_routes() {
    ...
    let active_public = public_predecessor_indices
        ...
        .sum::<usize>();
    assert_eq!((total, 18, 18, active_public), (36, 18, 18, 4));
}

#[test]
fn cp411_copies_the_cp410_humidity_ratio_on_active_routes_and_skips_inactive_routes() {
    ...
    for (limit, humidity_ratio, maximum_capacity_w, availability) in [
        ...
        let active = cp410.predecessor_dehumidification_control_switch_dispatched;
        ...
        if active {
            for value in [
                ...
                assert_eq!(
                    ...
                    cp410.resulting_supply_humidity_ratio.map(f64::to_bits),
                );
            }
            saw_assignment = true;
        } else {
...
#[test]
fn cp411_validation_rejects_changed_assignment_lineage_and_non_direct_routes() {
    ...
    let latest = lifecycle
        ...
        .latest
        .as_mut()
        .expect("CP411 latest snapshot");
    latest.assigned_supply_humidity_ratio_original = latest
    ...
    assert_eq!(
        validate(&model, &output, &lifecycle, &predecessor),
        Err(Error::CalcCoolingPostSaturationCapacityLimitDehumidificationSupplyHumidityRatioPreSaturationOriginalAssignmentLifecycleInvariant {
...
#[test]
fn cp411_evidence_does_not_feed_or_replace_numerical_coupling_dto() {
...
fn validator_fixture(
    ...
    let cache = precompute_schedule_cache(&model.typed, 1).expect("CP411 schedule cache");
    let binding = bind_direct_zone_purchased_air_model(&model).expect("CP411 direct binding");
    ...
    .expect("CP411 coupling");
    let system = output.initialization.system;
    let lifecycle = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_supply_humidity_ratio_pre_saturation_original_assignment_lifecycle_summary(
        &runtime,
        system,
    )
    .expect("CP411 lifecycle");
    let predecessor = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_default_case_break_lifecycle_summary(
        &runtime,
        system,
    )
