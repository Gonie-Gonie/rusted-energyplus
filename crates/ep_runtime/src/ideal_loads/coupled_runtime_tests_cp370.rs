...
use crate::ideal_loads::{
    ...
    PurchasedAirCalcCoolingSupplyHumidityRatioHumidificationControlHumidistatGuardLifecycleSummary as Lifecycle,
    PurchasedAirCalcCoolingSupplyHumidityRatioHumidificationHeatingAvailabilityGuardLifecycleSummary as PredecessorLifecycle,
...
use super::super::cooling_supply_humidity_ratio_humidification_control_humidistat_guard_validation::{
    expected_snapshot, validate_lifecycle,
};

#[test]
fn cp370_lifecycle_matches_cp369_and_does_not_feed_numerical_result() {
    let fixture = validator_fixture();
    assert!(fixture.is_some(), "CP370 validator fixture");
    ...
    assert_eq!(state.heating_on_guard_false_fallthrough_count, 0);
    ...
    assert_eq!(
        snapshot,
        expected_snapshot(output.calculation_cooling_supply_humidity_ratio_humidification_heating_availability_guard)
    );
...
#[test]
fn cp370_guard_count_and_latest_corruption_are_rejected() {
    let fixture = validator_fixture();
    assert!(fixture.is_some(), "CP370 corruption fixture");
    let Some((model, output, lifecycle, predecessor)) = fixture else {
        return;
    };
    for (field, mutate, expected, actual) in [
        (
            "humidification_control_type_read_count",
            set_read_zero as fn(&mut Lifecycle),
            1,
            0,
        ),
        (
            "humidification_control_type_humidistat_comparison_count",
            set_comparison_zero as fn(&mut Lifecycle),
            1,
            0,
        ),
        (
        ...
        (
            ...
            set_false_zero as fn(&mut Lifecycle),
            1,
            0,
        ),
        (
        ...
        assert_eq!(
            ...
            Err(Error::CalcCoolingSupplyHumidityRatioHumidificationControlHumidistatGuardLifecycleInvariant {
                field,
                expected,
                actual,
            })
...
fn set_read_zero(lifecycle: &mut Lifecycle) {
    lifecycle.state.humidification_control_type_read_count = 0;
}
fn set_comparison_zero(lifecycle: &mut Lifecycle) {
    lifecycle
        .state
...
fn set_false_zero(lifecycle: &mut Lifecycle) {
    lifecycle
        .state
...
fn validator_fixture() -> Option<(
    ...
    .ok()?;
    let lifecycle = purchased_air_calc_cooling_supply_humidity_ratio_humidification_control_humidistat_guard_lifecycle_summary(
        &runtime,
...
fn validate(
    ...
    let binding = bind_direct_zone_purchased_air_model(model).expect("CP370 binding");
