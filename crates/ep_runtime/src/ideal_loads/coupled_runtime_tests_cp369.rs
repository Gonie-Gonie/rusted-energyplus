...
use crate::ideal_loads::{
    ...
    PurchasedAirCalcCoolingDefaultSupplyHumidityRatioCaseBreakLifecycleSummary as PredecessorLifecycle,
    PurchasedAirCalcCoolingSupplyHumidityRatioHumidificationHeatingAvailabilityGuardLifecycleSummary as Lifecycle,
...
use super::super::cooling_supply_humidity_ratio_humidification_heating_availability_guard_validation::{
    expected_snapshot, validate_lifecycle,
};

#[test]
fn cp369_lifecycle_matches_cp368_and_does_not_feed_numerical_result() {
    let fixture = validator_fixture();
    assert!(fixture.is_some(), "CP369 validator fixture");
    ...
    assert_eq!(
        snapshot,
        expected_snapshot(output.calculation_cooling_default_supply_humidity_ratio_case_break)
    );
...
#[test]
fn cp369_guard_count_and_latest_corruption_are_rejected() {
    let fixture = validator_fixture();
    assert!(fixture.is_some(), "CP369 corruption fixture");
    let Some((model, output, lifecycle, predecessor)) = fixture else {
        return;
    };
    for (field, mutate, expected, actual) in [
        ...
        (
            ...
            set_body_zero as fn(&mut Lifecycle),
            1,
            0,
        ),
        (
        ...
        assert_eq!(
            ...
            Err(Error::CalcCoolingSupplyHumidityRatioHumidificationHeatingAvailabilityGuardLifecycleInvariant {
                field,
                expected,
                actual,
            })
...
fn set_read_zero(lifecycle: &mut Lifecycle) {
    lifecycle.state.heating_on_read_count = 0;
}
fn set_body_zero(lifecycle: &mut Lifecycle) {
...
fn validator_fixture() -> Option<(
    ...
    .ok()?;
    let lifecycle = purchased_air_calc_cooling_supply_humidity_ratio_humidification_heating_availability_guard_lifecycle_summary(
        &runtime,
...
fn validate(
    ...
    let binding = bind_direct_zone_purchased_air_model(model).expect("CP369 binding");
