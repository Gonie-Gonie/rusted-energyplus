//! Non-vacuous CP392 coupled-runtime integration tests.

use super::*;
use crate::ideal_loads::{
    DirectZonePurchasedAirScheduledCouplingInput, DirectZonePurchasedAirScheduledCouplingOutput,
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationControlConstantSensibleHeatRatioOverdryingLimitLifecycleSummary as Predecessor,
...
use super::super::cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_supply_humidity_ratio_assignment_validation::{
    snapshot_matches_release, validate_lifecycle,
};

#[test]
fn cp392_preserves_cp391_enthalpy_temperature_and_nulls_source_locals_on_direct_routes() {
    for (limit, maximum_capacity_w, availability) in [
        ...
        let cp391 = output.calculation_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_overdrying_limit;
        assert_eq!(
            snapshot
                .predecessor_cp391_resulting_supply_enthalpy_j_per_kg
                .map(f64::to_bits),
            cp391.resulting_supply_enthalpy_j_per_kg.map(f64::to_bits)
        );
        assert_eq!(
            snapshot
                .predecessor_cp391_resulting_supply_temperature_c
                .map(f64::to_bits),
            cp391.resulting_supply_temperature_c.map(f64::to_bits)
        );
        assert_eq!(
            snapshot
...
#[test]
fn cp392_rejects_one_bit_drift_in_cp391_retained_result() {
    ...
    let value = predecessor
        ...
        .resulting_supply_enthalpy_j_per_kg
        .as_mut()
        .expect("CP391 retained enthalpy");
...
#[test]
fn cp392_rejects_cp391_source_order_corruption() {
    let (model, output, lifecycle, mut predecessor) =
        validator_fixture(IdealLoadsLimit::LimitCapacity, 0.020, 500.0, 1.0, 1);
    predecessor
        .state
        ...
        .as_mut()
        .expect("CP391 latest")
        .source_order = &["forged-cp391-source-order"];
    assert_eq!(
        validate(&model, &output, &lifecycle, &predecessor, 1),
        Err(latest_violation())
    );
...
#[test]
fn cp392_rejects_route_drift_and_remains_evidence_only() {
    let (model, output, mut lifecycle, predecessor) =
        validator_fixture(IdealLoadsLimit::LimitCapacity, 0.020, 500.0, 1.0, 2);
    let mut output = output;
    let binding = bind_direct_zone_purchased_air_model(&model).expect("CP392 binding");
    output
        .coupling
        .prediction
        .predicted_loads
...
fn validator_fixture(
    ...
    let cache = precompute_schedule_cache(&model.typed, 1).expect("CP392 schedule cache");
    let binding = bind_direct_zone_purchased_air_model(&model).expect("CP392 direct binding");
    ...
    for step in 0..steps {
        ...
        latest = Some(
            ...
            .expect("CP392 coupling"),
        );
    }
    let output = latest.expect("at least one CP392 step");
    let system = output.initialization.system;
    let lifecycle = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_supply_humidity_ratio_assignment_lifecycle_summary(&runtime, system).expect("CP392 lifecycle");
    let predecessor = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_overdrying_limit_lifecycle_summary(&runtime, system).expect("CP391 lifecycle");
