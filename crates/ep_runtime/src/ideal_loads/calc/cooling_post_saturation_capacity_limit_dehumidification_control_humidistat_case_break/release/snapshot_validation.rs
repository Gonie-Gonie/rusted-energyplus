//! Exact CP396 compressed snapshot, route, and binary64 validation.
...
pub(in crate::ideal_loads) fn cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_case_break_snapshot_is_exact_direct_release(
    snapshot: Snapshot,
) -> bool {
...
pub(in crate::ideal_loads::calc) fn snapshots_match_bit_exact(
    ...
    let values_match = compare_clear!(predecessor_cp395_resulting_supply_humidity_ratio)
        && compare_clear!(predecessor_cp395_resulting_supply_enthalpy_j_per_kg)
        && compare_clear!(predecessor_cp395_resulting_supply_temperature_c)
