//! Exact CP424 prefix, route, marker, and bitwise validation.

use super::prefix::predecessor_cp423_snapshot;
use super::super::{
    PURCHASED_AIR_CALC_COOLING_SUPPLY_MASS_FLOW_POSITIVE_GUARD_ELSE_BRANCH_ENTRY_FIRST_EXCLUDED_SOURCE as EXCLUDED,
    PURCHASED_AIR_CALC_COOLING_SUPPLY_MASS_FLOW_POSITIVE_GUARD_ELSE_BRANCH_ENTRY_SOURCE as SOURCE,
    PURCHASED_AIR_CALC_COOLING_SUPPLY_MASS_FLOW_POSITIVE_GUARD_ELSE_BRANCH_ENTRY_SOURCE_ORDER as ORDER,
...
pub(super) fn snapshot_is_exact(snapshot: Snapshot) -> bool {
    snapshot_route(snapshot).is_some()
        && crate::ideal_loads::cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_sensible_output_supply_temperature_assignment_snapshot_is_exact(
            predecessor_cp423_snapshot(snapshot),
        )
}

pub(in crate::ideal_loads::calc) fn snapshot_route(snapshot: Snapshot) -> Option<Route> {
    let predecessor = predecessor_cp423_snapshot(snapshot);
...
pub(super) fn retained_route_matches_snapshot_bounded(snapshot: Snapshot, route: Route) -> bool {
    let predecessor = predecessor_cp423_snapshot(snapshot);
    let predecessor_route = crate::ideal_loads::calc::PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationGuardElseBranchSensibleOutputSupplyTemperatureAssignmentRetainedRoute {
        logical_index: route.logical_index,
        active: route.active,
        assignment_executed: route.assignment_executed,
    };
    crate::ideal_loads::calc::cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_sensible_output_supply_temperature_assignment_retained_route_matches_snapshot_bounded(
        predecessor,
        predecessor_route,
...
pub(super) fn prefix_and_local_shape_match(
    ...
    crate::ideal_loads::cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_sensible_output_supply_temperature_assignment_snapshots_match_bit_exact(
        predecessor_cp423_snapshot(snapshot),
        predecessor,
    ) && local_shape_is_exact(snapshot, predecessor, route)
}
...
fn local_shape_is_exact(
    ...
    snapshot.source == SOURCE
        ...
        && snapshot.cooling_supply_mass_flow_positive_guard_else_branch_entered == route.entered
        && route.entered == predecessor.positive_guard_false_fallthrough_skipped
