//! CP424 bounded state and CP423-accounting validation.
...
use super::super::transition::PurchasedAirCalcCoolingSupplyMassFlowPositiveGuardElseBranchEntryRetainedRoute as Route;
use super::snapshot_validation::{retained_route_matches_snapshot_bounded, snapshots_match_bit_exact};
use crate::ideal_loads::{
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationGuardElseBranchSensibleOutputSupplyTemperatureAssignmentRuntimeState as PredecessorState,
...
pub(super) fn pending_state_is_consistent(
    ...
    state.system == unit.system
        ...
        && state.transition_count.checked_add(1)
            == Some(unit.calc_cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_sensible_output_supply_temperature_assignment.transition_count)
}
...
pub(super) fn state_counts_are_consistent(state: &State) -> bool {
    ...
    let Some(entries) = checked_sum(&state.positive_supply_mass_flow_guard_else_branch_entry_route_counts) else { return false; };
    let Some(inactive) = transitions.checked_sub(entries) else { return false; };
    for index in 0..36 {
        let expected = if index == 2 { state.predecessor_route_counts[index] } else { 0 };
        if state.positive_supply_mass_flow_guard_else_branch_entry_route_counts[index] != expected {
            return false;
        }
    }
    ...
    state.transition_count == transitions
        ...
        && state.positive_supply_mass_flow_guard_else_branch_entry_count == entries
        && state.source_site_execution_count == entries
        && state.cp423_supply_humidity_ratio_state_owner_count == w_owners
        && state.unchanged_supply_humidity_ratio_preservation_count == w_owners
        && state.cp423_supply_enthalpy_state_owner_count == h_owners
        && state.unchanged_supply_enthalpy_preservation_count == h_owners
        && state.cp423_supply_temperature_state_owner_count == t_owners
