...
#[test]
fn cp417_boundary_and_four_sites_are_exact() {
    assert_eq!(
        super::PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_SUPPLY_ENTHALPY_ASSIGNMENT_SOURCE,
        "EnergyPlus 26.1 PurchasedAirManager.cc:2321",
    );
    assert_eq!(
        super::PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_SUPPLY_ENTHALPY_ASSIGNMENT_FIRST_EXCLUDED_SOURCE,
        "EnergyPlus 26.1 PurchasedAirManager.cc:2330",
    );
    assert_eq!(
        super::PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_SUPPLY_ENTHALPY_ASSIGNMENT_SOURCE_ORDER,
        &[
...
#[test]
fn exhaustive_54_outcomes_and_seven_route_partitions_are_exact() {
    ...
    for route in routes {
        ...
        for &body_entered in outcomes {
            ...
            let cp417_route = predecessor_route(cp416).expect("valid CP417 route");
            let snapshot = advance(&mut state, cp416).expect("valid CP417 outcome");
            let index = cp417_route.logical_index;

            expected_predecessor[index] += 1;
            if cp417_route.predecessor_guard_false_fallthrough {
                expected_guard_false[index] += 1;
            }
            if cp417_route.predecessor_guard_body_entered {
                expected_guard_body[index] += 1;
            }
            if cp417_route.predecessor_saturation_temperature_assignment_executed {
                expected_saturation_assignment[index] += 1;
            }
            if cp417_route.predecessor_saturation_temperature_mixed_air_limit_executed {
                expected_mixed_air_limit[index] += 1;
            }
            if cp417_route.predecessor_supply_humidity_ratio_assignment_executed {
                expected_humidity_assignment[index] += 1;
            }
            if cp417_route.active {
            ...
            assert_snapshot_matches_predecessor(snapshot, cp416, cp417_route.active);
            assert!(cooling_post_saturation_capacity_limit_dehumidification_supply_enthalpy_assignment_snapshot_is_exact(snapshot));
    ...
    assert_eq!(state.supply_enthalpy_assignment_count, 18);
    assert_eq!(state.source_site_execution_count, 72);
    assert_eq!(state.cp416_supply_humidity_ratio_state_owner_count, 36);
    assert_eq!(state.unchanged_supply_humidity_ratio_preservation_count, 36);
    assert_eq!(state.cp416_supply_enthalpy_state_owner_count, 41);
    assert_eq!(state.unchanged_supply_enthalpy_preservation_count, 23);
    assert_eq!(state.cp416_supply_temperature_state_owner_count, 51);
...
#[test]
fn public_route_transition_preserves_enthalpy_edge_bit_semantics() {
    ...
    let results = cases.map(|(enthalpy, mixed_air_temperature)| {
        ...
        let temperature = predecessor
            ...
            .expect("temperature");
        let humidity_ratio = predecessor
            .resulting_supply_humidity_ratio
            .expect("humidity ratio");
        let snapshot = advance(&mut State::new(predecessor.system), predecessor).expect("CP417");
        ...
        (temperature, humidity_ratio, expected)
    });
    ...
    assert_eq!(results[0].2.to_bits(), results[1].2.to_bits());
    assert_eq!(results[1].2.to_bits(), results[2].2.to_bits());
    assert!(results[3].2.is_nan());
    assert!(results[4].2.is_nan() && results[7].2.is_nan());
    assert_eq!(results[5].2, f64::INFINITY);
    assert!(results[6].2.is_finite());
...
fn inactive_routes_do_not_read_or_evaluate_operands() {
    ...
    let snapshot = advance(&mut State::new(cp416.system), cp416).expect("inactive CP417");

    assert!(!snapshot.cp416_retained_supply_temperature_owned_read);
    assert!(!snapshot.supply_temperature_for_enthalpy_read);
    assert!(snapshot.supply_temperature_for_enthalpy_c.is_none());
    assert!(!snapshot.cp416_retained_supply_humidity_ratio_owned_read);
    assert!(!snapshot.supply_humidity_ratio_for_enthalpy_read);
    assert!(snapshot.supply_humidity_ratio_for_enthalpy.is_none());
...
fn snapshot_corruption_is_rejected() {
    ...
    let snapshot = advance(&mut State::new(predecessor.system), predecessor).expect("active CP417");
    assert!(cooling_post_saturation_capacity_limit_dehumidification_supply_enthalpy_assignment_snapshot_is_exact(snapshot));

    let mut value_corrupted = snapshot;
    value_corrupted.assigned_supply_enthalpy_j_per_kg = Some(f64::from_bits(
        snapshot
            .assigned_supply_enthalpy_j_per_kg
            .expect("assigned enthalpy")
            .to_bits()
            ^ 1,
    ));
    assert!(!cooling_post_saturation_capacity_limit_dehumidification_supply_enthalpy_assignment_snapshot_is_exact(value_corrupted));
...
fn active_counter_overflow_is_transactional() {
    ...
    let setters: &[fn(&mut State, usize)] = &[
        ...
        |state, _| state.supply_enthalpy_assignment_count = usize::MAX,
        |state, index| state.supply_enthalpy_assignment_route_counts[index] = usize::MAX,
        |state, _| state.source_site_execution_count = usize::MAX,
        |state, _| state.cp416_supply_humidity_ratio_state_owner_count = usize::MAX,
        |state, _| state.unchanged_supply_humidity_ratio_preservation_count = usize::MAX,
        |state, _| state.cp416_supply_enthalpy_state_owner_count = usize::MAX,
        |state, _| state.cp416_supply_temperature_state_owner_count = usize::MAX,
        |state, _| state.unchanged_supply_temperature_preservation_count = usize::MAX,
        |state, _| state.cp417_psychrometric_supply_enthalpy_state_owner_count = usize::MAX,
        |state, _| state.cp416_retained_supply_temperature_owned_read_count = usize::MAX,
        |state, _| state.supply_temperature_for_enthalpy_read_count = usize::MAX,
        |state, _| state.cp416_retained_supply_humidity_ratio_owned_read_count = usize::MAX,
        |state, _| state.supply_humidity_ratio_for_enthalpy_read_count = usize::MAX,
        |state, _| state.psychrometric_supply_enthalpy_evaluation_count = usize::MAX,
        |state, _| state.supply_enthalpy_assignment_write_count = usize::MAX,
    ];
...
fn assert_snapshot_matches_predecessor(snapshot: Snapshot, predecessor: Predecessor, active: bool) {
    ...
    if active {
        ...
        let humidity_ratio = predecessor
            .resulting_supply_humidity_ratio
            .expect("active humidity-ratio owner");
        let expected = energyplus_psy_h_fn_tdb_w(temperature, humidity_ratio);
        assert_eq!(
            snapshot.supply_temperature_for_enthalpy_c.map(f64::to_bits),
            Some(temperature.to_bits()),
        );
        assert_eq!(
            snapshot.supply_humidity_ratio_for_enthalpy.map(f64::to_bits),
...
fn active_fixture_with_operands(enthalpy: f64, mixed_air_temperature_c: f64) -> Predecessor {
    ...
    advance_cp416(&mut Cp416State::new(cp415.system), cp415).expect("CP416")
}
