//! Pure CP418-to-CP419 not-dehumidifying `CpAir` assignment.
...
use crate::ideal_loads::calc::PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationGuardElseBranchEntryCommittedRoute as PredecessorRoute;
use crate::psychrometrics::energyplus_psy_cp_air_fn_w;

mod accounting;
use accounting::{increment_counts, next_transition_fits};

/// One retained CP419 route over the lossless 36-wide CP418 partition.
...
pub(in crate::ideal_loads::calc) fn predecessor_route(
    ...
    let route = crate::ideal_loads::calc::cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_entry_snapshot_route(predecessor)?;
...
pub(in crate::ideal_loads::calc) fn cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_cp_air_assignment_route_from_validated_predecessor(
    predecessor: Predecessor,
...
#[allow(dead_code)]
pub(in crate::ideal_loads::calc) fn advance_cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_cp_air_assignment_state(
    state: &mut State,
    predecessor: Predecessor,
    active_input: Option<PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationGuardElseBranchCpAirAssignmentActiveInput>,
) -> Option<Snapshot> {
...
pub(in crate::ideal_loads::calc) fn advance_cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_cp_air_assignment_state_with_validated_route(
    state: &mut State,
    predecessor: Predecessor,
    route: RetainedRoute,
    active_input: Option<PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationGuardElseBranchCpAirAssignmentActiveInput>,
    ...
    let snapshot = Snapshot {
        ...
        predecessor_cp418_resulting_supply_humidity_ratio: predecessor.resulting_supply_humidity_ratio,
        predecessor_cp418_resulting_supply_enthalpy_j_per_kg: predecessor.resulting_supply_enthalpy_j_per_kg,
        predecessor_cp418_resulting_supply_temperature_c: predecessor.resulting_supply_temperature_c,
        predecessor_post_saturation_capacity_limit_dehumidification_guard_else_branch_entered: predecessor.post_saturation_capacity_limit_dehumidification_guard_else_branch_entered,
        post_saturation_capacity_limit_dehumidification_guard_else_branch_cp_air_assignment_executed: route.active,
        cp329_retained_mixed_air_humidity_ratio_owned_read: route.active,
        mixed_air_humidity_ratio_for_cp_air_read: route.active,
        mixed_air_humidity_ratio_for_cp_air: prepared.mixed_air_humidity_ratio,
...
fn route_matches_predecessor(route: RetainedRoute, predecessor: Predecessor) -> bool {
    ...
    route.logical_index < 36
        ...
        && route.active
            == predecessor
                .post_saturation_capacity_limit_dehumidification_guard_else_branch_entered
        && route.active == ELSE_ENTRY_LOGICAL_INDICES.contains(&route.logical_index)
}
