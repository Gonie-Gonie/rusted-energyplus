//! Persistent CP404 runtime-state and latest-witness validation.
...
#[allow(dead_code)]
pub(in crate::ideal_loads) fn cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_latent_output_supply_humidity_ratio_assignment_latest_metadata_is_consistent(
    unit: &PurchasedAirUnitRuntimeState,
...
fn counts_are_exact(state: &State) -> bool {
    ...
    let partition_exact = (0..30).all(|index| {
        ...
        if active {
            ...
            state.predecessor_guard_false_fallthrough_route_counts[index] == 0
                && state.supply_humidity_ratio_assignment_route_counts[index] == 0
        }
    });
    transition == Some(state.transition_count)
        ...
        && assignment == Some(state.supply_humidity_ratio_assignment_count)
        && partition_exact
        && state
            .supply_humidity_ratio_assignment_count
            .checked_mul(4)
            == Some(state.source_site_execution_count)
        && humidity_owner == Some(state.cp403_supply_humidity_ratio_state_owner_count)
        && state.cp403_supply_humidity_ratio_state_owner_count
            == state.unchanged_supply_humidity_ratio_preservation_count
        && enthalpy_owner == Some(state.cp403_supply_enthalpy_state_owner_count)
        && state.cp403_supply_enthalpy_state_owner_count
            == state.unchanged_supply_enthalpy_preservation_count
        && temperature_owner == Some(state.cp403_supply_temperature_state_owner_count)
        ...
        .into_iter()
        .all(|count| count == state.supply_humidity_ratio_assignment_count)
}
