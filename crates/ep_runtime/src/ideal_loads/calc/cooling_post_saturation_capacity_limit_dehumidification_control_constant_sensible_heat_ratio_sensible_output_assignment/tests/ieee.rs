...
fn signed_zero_nan_and_infinity_are_preserved_without_local_gates() {
    ...
    for ratio in [
        ...
        assert_eq!(
            snapshot.calculated_cooling_sensible_output_w.map(f64::to_bits),
...
fn cp387_cp_air_and_cp385_enthalpy_are_retained_but_not_operands() {
    let low = fixtures::chain(3, 1, true, Some(D::ConstantSensibleHeatRatio), 1, 99.0, 50_000.0, 0.004);
    let high = fixtures::chain(3, 1, true, Some(D::ConstantSensibleHeatRatio), 1, 99.0, 60_000.0, 0.020);
    ...
    assert_ne!(
        ...
        high_snapshot.predecessor_cp_air_j_per_kg_k.map(f64::to_bits),
    );
    assert_ne!(
        low_snapshot.resulting_supply_enthalpy_j_per_kg.map(f64::to_bits),
        high_snapshot.resulting_supply_enthalpy_j_per_kg.map(f64::to_bits),
    );
    assert_eq!(
    ...
    for (chain, snapshot) in [(low, low_snapshot), (high, high_snapshot)] {
        ...
        assert_eq!(
            ...
            chain.cp385.resulting_supply_enthalpy_j_per_kg.map(f64::to_bits),
        );
    }
}
