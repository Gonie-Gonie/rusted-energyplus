//! CP398 passive binary64 carrier tests.
...
fn compressed_snapshot_preserves_arbitrary_carrier_bits_without_numeric_gates() {
    ...
    assert!(
        crate::ideal_loads::calc::cooling_post_saturation_capacity_limit_dehumidification_control_none_case_entry_snapshot_is_exact(
            predecessor,
        )
    );
    ...
    .expect("arbitrary-carrier CP398");
...
fn set_recursive_carriers(
    snapshot: &mut crate::ideal_loads::PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationControlNoneCaseEntrySnapshot,
    humidity: f64,
    enthalpy: f64,
    temperature: f64,
) {
    snapshot.predecessor_cp396_resulting_supply_humidity_ratio = Some(humidity);
    snapshot.predecessor_cp396_resulting_supply_enthalpy_j_per_kg = Some(enthalpy);
    snapshot.predecessor_cp396_resulting_supply_temperature_c = Some(temperature);
