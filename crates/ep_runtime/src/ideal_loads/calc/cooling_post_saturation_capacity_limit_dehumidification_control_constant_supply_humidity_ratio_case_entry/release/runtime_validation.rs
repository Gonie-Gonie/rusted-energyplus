//! Persistent CP398 runtime-state validation.
...
pub(super) fn call_order_is_pending(
    ...
    unit.calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_case_entry
        ...
        && unit.calc_cooling_post_saturation_capacity_limit_dehumidification_control_none_case_entry.transition_count
            == ordinal
}
...
pub(super) fn prepare_next_transition(
    ...
    let snapshot = advance_cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_case_entry_state(
        &mut next,
        predecessor,
    )?;
    Some((next, snapshot))
...
pub(super) fn prepared_completed_state_is_consistent(
    ...
    let predecessor = &unit.calc_cooling_post_saturation_capacity_limit_dehumidification_control_none_case_entry;
...
#[allow(dead_code)]
pub(in crate::ideal_loads) fn cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_case_entry_latest_metadata_is_consistent(
    unit: &PurchasedAirUnitRuntimeState,
...
fn predecessor_counts_match(state: &State, predecessor: &PredecessorState) -> bool {
    state.predecessor_route_counts == predecessor.predecessor_route_counts
        ...
        && checked_selected_sum(&predecessor.predecessor_route_counts, &[20, 24, 27])
            == Some(predecessor.dehumidification_control_none_case_entry_count)
        && checked_selected_sum(
            &predecessor.predecessor_route_counts,
            &[20, 21, 24, 25, 27, 29],
        ) == Some(state.dehumidification_control_constant_supply_humidity_ratio_case_entry_count)
}
...
fn state_is_consistent(
    ...
    let active_total = checked_selected_sum(
    ...
    let expected_sites = entries.checked_mul(
        PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_CONTROL_CONSTANT_SUPPLY_HUMIDITY_RATIO_CASE_ENTRY_SOURCE_ORDER.len(),
    );
