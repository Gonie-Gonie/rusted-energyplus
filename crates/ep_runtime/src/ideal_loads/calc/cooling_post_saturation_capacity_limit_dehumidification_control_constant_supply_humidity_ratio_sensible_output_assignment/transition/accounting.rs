//! Exact CP400 route, retained-state, and eight-site accounting.
...
use crate::ideal_loads::PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_CONTROL_CONSTANT_SUPPLY_HUMIDITY_RATIO_SENSIBLE_OUTPUT_ASSIGNMENT_SOURCE_ORDER;
...
pub(super) fn increment_counts(state: &mut State, route: RetainedRoute) {
    ...
    if predecessor_has_supply_humidity_ratio(route.predecessor_index) {
        state.cp399_supply_humidity_ratio_state_owner_count += 1;
    ...
    state.dehumidification_control_constant_supply_humidity_ratio_sensible_output_assignment_count += 1;
    state.source_site_execution_count += PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_CONTROL_CONSTANT_SUPPLY_HUMIDITY_RATIO_SENSIBLE_OUTPUT_ASSIGNMENT_SOURCE_ORDER.len();
