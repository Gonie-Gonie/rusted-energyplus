//! Persistent CP400 runtime-state validation.
...
pub(super) fn call_order_is_pending(
    ...
    unit.calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_sensible_output_assignment
        ...
        && unit.calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_cp_air_assignment.transition_count == ordinal
}
...
pub(super) fn prepare_next_transition(
    ...
    let snapshot = advance_cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_sensible_output_assignment_state(
        &mut next,
        predecessor,
        active_owners,
    )?;
...
#[allow(dead_code)]
pub(in crate::ideal_loads) fn cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_sensible_output_assignment_latest_metadata_is_consistent(
    unit: &PurchasedAirUnitRuntimeState,
...
fn state_is_consistent(
    ...
    if state.system != expected_system
        ...
        || humidity != Some(state.cp399_supply_humidity_ratio_state_owner_count)
        || humidity != Some(state.unchanged_supply_humidity_ratio_preservation_count)
        || enthalpy != Some(state.cp399_supply_enthalpy_state_owner_count)
        || enthalpy != Some(state.unchanged_supply_enthalpy_preservation_count)
        || temperature != Some(state.cp399_supply_temperature_state_owner_count)
