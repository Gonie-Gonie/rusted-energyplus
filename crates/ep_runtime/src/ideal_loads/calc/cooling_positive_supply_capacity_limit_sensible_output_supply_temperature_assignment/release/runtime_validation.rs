//! Persistent CP343 runtime-state validation.

use ep_model::IdealLoadsAirSystemId;

use super::super::PurchasedAirCalcCoolingPositiveSupplyCapacityLimitSensibleOutputSupplyTemperatureAssignmentRetainedInput;
...
pub(in crate::ideal_loads::calc) fn next_supply_temperature_assignment_transition_fits(
    ...
    if !predecessor.capacity_limit_sensible_output_supply_enthalpy_assignment_executed {
        return false;
    }
    state
        .capacity_limit_sensible_output_supply_temperature_assignment_count
...
fn state_is_consistent(
    state: &PurchasedAirCalcCoolingPositiveSupplyCapacityLimitSensibleOutputSupplyTemperatureAssignmentRuntimeState,
    witness: Option<
        PurchasedAirCalcCoolingPositiveSupplyCapacityLimitSensibleOutputSupplyTemperatureAssignmentSnapshot,
    >,
    ...
    let assigned = state.capacity_limit_sensible_output_supply_temperature_assignment_count;
    let counters_match = state.system == expected_system
        ...
        && state.supply_enthalpy_for_dry_bulb_inversion_read_count == assigned
        && state.supply_humidity_ratio_for_dry_bulb_inversion_read_count == assigned
        && state.psychrometric_supply_temperature_evaluation_count == assigned
        && state.supply_temperature_assignment_write_count == assigned
