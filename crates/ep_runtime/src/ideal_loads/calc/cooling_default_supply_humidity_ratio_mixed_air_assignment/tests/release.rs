...
fn public_direct_none_is_numeric_lazy_and_records_only_a_completed_skip() {
    for minimum in [
        ...
        let completed = completed_cp366_case(minimum);
        assert!(completed.is_some(), "CP366 direct fixture must remain lazy");
        ...
        assert!(result.is_ok(), "CP367 must not read any humidity owner");
        ...
        let Some(state) = runtime.units.get(&system.id).map(|unit| {
...
#[test]
fn private_csh_delegates_cp366_and_still_skips_the_untyped_default() {
    for minimum in [-0.0, 0.0, 0.0077] {
        let completed = completed_cp366_case(minimum);
        assert!(completed.is_some(), "finite CP366 fixture must complete");
        let Some((mut runtime, system, predecessor)) = completed else {
            return;
        };
        let direct =
        ...
        assert!(private.is_some(), "canonical private CP366 must reconstruct");
...
fn corruption_replay_and_nonzero_source_counter_reject_transactionally() {
    let Some((mut runtime, system, mut predecessor)) = completed_cp366_case(0.0077) else {
        return;
    };
    ...
    assert!(matches!(
        result,
        Err(
            super::super::PurchasedAirCalcCoolingDefaultSupplyHumidityRatioMixedAirAssignmentError::
                CoolingConstantSupplyHumidityRatioCaseBreakSnapshotMismatch { .. }
        )
    ));
    assert_eq!(runtime, before);

    let Some((mut runtime, system, predecessor)) = completed_cp366_case(0.0077) else {
        return;
    };
    assert!(
        advance_direct_no_oa_calc_cooling_default_supply_humidity_ratio_mixed_air_assignment(
