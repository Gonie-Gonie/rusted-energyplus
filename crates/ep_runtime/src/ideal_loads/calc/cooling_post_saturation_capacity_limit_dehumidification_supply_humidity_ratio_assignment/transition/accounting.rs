//! CP416 checked counter preparation and mutation.
...
pub(super) fn increment_counts(state: &mut State, predecessor: Predecessor, route: RetainedRoute) {
    ...
    if predecessor.resulting_supply_humidity_ratio.is_some() {
        state.cp415_supply_humidity_ratio_state_owner_count += 1;
        if !route.active {
            state.unchanged_supply_humidity_ratio_preservation_count += 1;
        }
    }
    if predecessor.resulting_supply_enthalpy_j_per_kg.is_some() {
        state.cp415_supply_enthalpy_state_owner_count += 1;
        state.unchanged_supply_enthalpy_preservation_count += 1;
    }
    if predecessor.resulting_supply_temperature_c.is_some() {
        state.cp415_supply_temperature_state_owner_count += 1;
    ...
    state.cp416_psychrometric_supply_humidity_ratio_state_owner_count += 1;
    state.cp415_retained_supply_temperature_owned_read_count += 1;
    state.supply_temperature_for_humidity_ratio_inversion_read_count += 1;
    state.cp415_retained_supply_enthalpy_owned_read_count += 1;
