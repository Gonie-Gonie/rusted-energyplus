//! Pure CP415-to-CP416 supply-humidity-ratio psychrometric assignment.
...
/// One retained CP416 route over the lossless 36-wide CP415 partition.
...
pub(in crate::ideal_loads::calc) fn predecessor_route(
    ...
    let route = crate::ideal_loads::calc::cooling_post_saturation_capacity_limit_dehumidification_supply_temperature_saturation_mixed_air_limit_snapshot_route(predecessor)?;
    Some(RetainedRoute {
        ...
        predecessor_saturation_temperature_assignment_executed: route
            .predecessor_assignment_executed,
        predecessor_saturation_temperature_mixed_air_limit_executed: route.active,
        active: route.active,
    })
}

pub(in crate::ideal_loads::calc) fn advance_cooling_post_saturation_capacity_limit_dehumidification_supply_humidity_ratio_assignment_state(
    state: &mut State,
    predecessor: Predecessor,
) -> Option<Snapshot> {
    ...
    let psychrometric_supply_humidity_ratio =
        operands.map(|(temperature, enthalpy)| energyplus_psy_w_fn_tdb_h(temperature, enthalpy));
    let resulting_supply_humidity_ratio =
        psychrometric_supply_humidity_ratio.or(predecessor.resulting_supply_humidity_ratio);
    ...
    let snapshot = Snapshot {
        ...
        predecessor_cp415_resulting_supply_humidity_ratio: predecessor.resulting_supply_humidity_ratio,
        predecessor_cp415_resulting_supply_enthalpy_j_per_kg: predecessor.resulting_supply_enthalpy_j_per_kg,
        predecessor_cp415_resulting_supply_temperature_c: predecessor.resulting_supply_temperature_c,
        post_saturation_capacity_limit_dehumidification_supply_humidity_ratio_assignment_executed: route.active,
        cp415_retained_supply_humidity_ratio_state_owned: predecessor.resulting_supply_humidity_ratio.is_some(),
        cp415_retained_supply_enthalpy_state_owned: predecessor.resulting_supply_enthalpy_j_per_kg.is_some(),
        cp415_retained_supply_temperature_state_owned: predecessor.resulting_supply_temperature_c.is_some(),
        cp415_retained_supply_temperature_owned_read: route.active,
        supply_temperature_for_humidity_ratio_inversion_read: route.active,
        supply_temperature_c: operands.map(|value| value.0),
        cp415_retained_supply_enthalpy_owned_read: route.active,
        supply_enthalpy_for_humidity_ratio_inversion_read: route.active,
        supply_enthalpy_j_per_kg: operands.map(|value| value.1),
