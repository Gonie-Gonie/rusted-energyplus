//! Persistent CP403 runtime-state and latest-witness validation.
...
use crate::ideal_loads::{
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationControlConstantSupplyHumidityRatioLatentOutputGuardRuntimeState as PredecessorState,
...
#[allow(dead_code)]
pub(in crate::ideal_loads) fn cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_latent_output_supply_temperature_mixed_air_assignment_latest_metadata_is_consistent(
    unit: &PurchasedAirUnitRuntimeState,
...
fn counts_are_exact(state: &State) -> bool {
    ...
    let humidity_owner = checked_selected_sum(
        &state.predecessor_route_counts,
        &[18, 19, 22, 23, 26, 28],
    );
    ...
    let partition_exact = (0..30).all(|index| {
        ...
        if active {
            ...
            state.predecessor_guard_false_fallthrough_route_counts[index] == 0
                && state.supply_temperature_mixed_air_assignment_route_counts[index] == 0
        }
    });
    transition == Some(state.transition_count)
        ...
        && assignment == Some(state.supply_temperature_mixed_air_assignment_count)
        && partition_exact
        && state
            .supply_temperature_mixed_air_assignment_count
            .checked_mul(2)
            == Some(state.source_site_execution_count)
        && humidity_owner == Some(state.cp402_supply_humidity_ratio_state_owner_count)
        && state.cp402_supply_humidity_ratio_state_owner_count
            == state.unchanged_supply_humidity_ratio_preservation_count
        && enthalpy_owner == Some(state.cp402_supply_enthalpy_state_owner_count)
        && state.cp402_supply_enthalpy_state_owner_count
            == state.unchanged_supply_enthalpy_preservation_count
        && temperature_owner == Some(state.cp402_supply_temperature_state_owner_count)
        && state
            .cp402_supply_temperature_state_owner_count
            .checked_sub(state.supply_temperature_mixed_air_assignment_count)
        ...
        .into_iter()
        .all(|count| count == state.supply_temperature_mixed_air_assignment_count)
}
