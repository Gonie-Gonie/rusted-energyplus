use super::completed_cp351_case;
...
fn public_none_and_inherited_routes_are_complete_null_skips() {
    for (demand, availability, capacity, expected) in [
        ...
        let completed = completed_cp351_case(demand, availability, capacity);
...
fn assert_complete_null_skip(
    snapshot:
        crate::ideal_loads::PurchasedAirCalcCoolingPositiveSupplyPostCapacityLimitDehumidificationControlConstantSensibleHeatRatioSupplyEnthalpyAssignmentSnapshot,
) {
    assert!(
        !snapshot
