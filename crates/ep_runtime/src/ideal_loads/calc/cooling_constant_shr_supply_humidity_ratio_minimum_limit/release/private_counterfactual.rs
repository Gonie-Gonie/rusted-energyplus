//! Canonical private-active CP355 reconstruction.
...
use crate::ideal_loads::calc::cooling_constant_shr_supply_humidity_ratio_overdrying_limit::private_active_counterfactual_from_direct_release as cp354_private_active_counterfactual_from_direct_release;
use crate::ideal_loads::{PurchasedAirRuntimeState, PurchasedAirUnitRuntimeState};

/// Rebuilds the exact private constant-SHR CP355 counterfactual from the
/// recursively validated private CP354 predecessor and same-call owners.
pub(in crate::ideal_loads::calc) fn private_active_counterfactual_from_direct_release(
    ...
    let retained = unit
        ...
        .latest?;
    let witness = runtime
        .cooling_constant_shr_supply_humidity_ratio_minimum_limit_latest_witness(system.id)?;
    if system.id != direct.system
        ...
        || !super::completed_direct_cooling_constant_shr_supply_humidity_ratio_minimum_limit_is_consistent(
            runtime,
            unit,
            system,
            direct,
    ...
    let direct_cp354 = unit
        .calc_cooling_constant_shr_supply_humidity_ratio_overdrying_limit
        .latest?;
    let private_cp354 = cp354_private_active_counterfactual_from_direct_release(
        runtime,
        unit,
        system,
        direct_cp354,
    )?;
    let operands = active_operands_from_retained_owners(runtime, unit, system, private_cp354)?;
    let mut state = State::new(system.id);
    let counterfactual = advance(&mut state, private_cp354, Some(operands))?;
    route_independent_identity_matches(direct, counterfactual).then_some(counterfactual)
}

/// Proves a supplied CP355 witness is the bit-exact canonical private-active
