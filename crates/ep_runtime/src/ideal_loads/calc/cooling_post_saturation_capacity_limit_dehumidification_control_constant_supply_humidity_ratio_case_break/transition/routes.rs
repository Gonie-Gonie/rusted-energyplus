//! Exact CP408 route preservation and compressed CP409 route reconstruction.
...
pub(in crate::ideal_loads::calc) fn predecessor_route(
    ...
    let route = crate::ideal_loads::calc::cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_latent_output_supply_temperature_mixed_air_limit_snapshot_route(predecessor)?;
    Some(RetainedRoute {
        predecessor_index: route.predecessor_index,
        predecessor_guard_false_fallthrough: route.active,
        predecessor_maximum_capacity_assignment_executed: route
            .predecessor_maximum_capacity_assignment_executed,
        active: route.predecessor_guard_evaluated,
    })
}
...
fn inactive_route(snapshot: Snapshot, predecessor_index: usize) -> Option<RetainedRoute> {
    ...
    local_route(snapshot, predecessor_index)
}

fn local_route(snapshot: Snapshot, predecessor_index: usize) -> Option<RetainedRoute> {
    let constant_shr = matches!(predecessor_index, 18 | 22 | 28);
    let humidistat = matches!(predecessor_index, 19 | 23 | 26);
    let shared = predecessor_index_is_active(predecessor_index);
    ...
    Some(RetainedRoute {
        predecessor_index,
        predecessor_guard_false_fallthrough: guard_false,
        predecessor_maximum_capacity_assignment_executed: maximum,
        active: shared,
    })
}
...
/// Stable flattened ordering of the 36 CP408/CP409 logical routes.
#[cfg(test)]
...
pub(in crate::ideal_loads::calc) const fn predecessor_has_supply_humidity_ratio(
    ...
    route.active || matches!(route.predecessor_index, 18 | 19 | 22 | 23 | 26 | 28)
}
