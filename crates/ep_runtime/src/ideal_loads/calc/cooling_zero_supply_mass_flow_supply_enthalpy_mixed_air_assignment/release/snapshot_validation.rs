//! Exact CP425 prefix, route, assignment, and bitwise validation.

use super::prefix::predecessor_cp424_snapshot;
use super::super::{
    PURCHASED_AIR_CALC_COOLING_ZERO_SUPPLY_MASS_FLOW_SUPPLY_ENTHALPY_MIXED_AIR_ASSIGNMENT_FIRST_EXCLUDED_SOURCE as EXCLUDED,
    PURCHASED_AIR_CALC_COOLING_ZERO_SUPPLY_MASS_FLOW_SUPPLY_ENTHALPY_MIXED_AIR_ASSIGNMENT_SOURCE as SOURCE,
    PURCHASED_AIR_CALC_COOLING_ZERO_SUPPLY_MASS_FLOW_SUPPLY_ENTHALPY_MIXED_AIR_ASSIGNMENT_SOURCE_ORDER as ORDER,
...
pub(super) fn snapshot_is_exact(snapshot: Snapshot) -> bool {
    snapshot_route(snapshot).is_some()
        && crate::ideal_loads::cooling_supply_mass_flow_positive_guard_else_branch_entry_snapshot_is_exact(
            predecessor_cp424_snapshot(snapshot),
        )
}

pub(in crate::ideal_loads::calc) fn snapshot_route(snapshot: Snapshot) -> Option<Route> {
    let predecessor = predecessor_cp424_snapshot(snapshot);
    let predecessor_route =
        crate::ideal_loads::calc::cooling_supply_mass_flow_positive_guard_else_branch_entry_snapshot_route(
            predecessor,
        )?;
    let route =
        cooling_zero_supply_mass_flow_supply_enthalpy_mixed_air_assignment_route_from_committed_predecessor(
...
pub(super) fn retained_route_matches_snapshot_bounded(snapshot: Snapshot, route: Route) -> bool {
    let predecessor = predecessor_cp424_snapshot(snapshot);
    let predecessor_route =
        crate::ideal_loads::calc::PurchasedAirCalcCoolingSupplyMassFlowPositiveGuardElseBranchEntryRetainedRoute {
...
fn local_shape_is_exact(
    ...
    let assigned = snapshot.assigned_supply_enthalpy_from_mixed_air_j_per_kg;
    let expected_enthalpy = if route.assignment_executed {
        assigned
    } else {
    ...
    snapshot.source == SOURCE
        ...
        && route.assignment_executed == route.predecessor_entered
        ...
        && snapshot.cp329_retained_mixed_air_enthalpy_owned_read == route.assignment_executed
        && snapshot.mixed_air_enthalpy_for_zero_supply_mass_flow_supply_enthalpy_read
            == route.assignment_executed
        ...
        && same(snapshot.resulting_supply_enthalpy_j_per_kg, expected_enthalpy)
        && same(
