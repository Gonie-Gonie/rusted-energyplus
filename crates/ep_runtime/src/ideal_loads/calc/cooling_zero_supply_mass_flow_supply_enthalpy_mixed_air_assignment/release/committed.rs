//! Sealed CP425 route and mixed-air-enthalpy capability for CP426.
...
/// Returns CP425's bounded committed route without recursive derivation.
pub(in crate::ideal_loads::calc) fn cooling_zero_supply_mass_flow_supply_enthalpy_mixed_air_assignment_committed_latest_route(
    ...
    let predecessor = predecessor_cp424_snapshot(latest);
    ...
    let assigned_enthalpy_is_exact = if route.assignment_executed {
        let committed = cooling_mixed_air_call_committed_latest_mixed_air_enthalpy(
            unit,
            cp329_witness?,
        )?;
        [
            latest.mixed_air_enthalpy_for_zero_supply_mass_flow_supply_enthalpy_j_per_kg,
            latest.assigned_supply_enthalpy_from_mixed_air_j_per_kg,
...
mod tests {
    #[test]
    fn cp425_committed_route_hot_path_is_bounded() {
        ...
        assert_eq!(
            hot.matches("positive_guard_else_branch_entry_committed_latest_route(")
                .count(),
            1,
        );
        assert_eq!(
            hot.matches("mixed_air_call_committed_latest_mixed_air_enthalpy(")
                .count(),
            1,
        );
    }
