//! Persistent CP392 runtime-state validation.
...
pub(super) fn call_order_is_pending(
    ...
    unit.calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_supply_humidity_ratio_assignment
        ...
        && unit.calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_overdrying_limit.transition_count
            == ordinal
}
...
pub(super) fn pending_state_is_consistent(
    ...
    let state = &unit.calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_supply_humidity_ratio_assignment;
...
pub(super) fn prepare_next_transition(
    ...
    let snapshot = advance_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_supply_humidity_ratio_assignment_state(
        &mut next,
        predecessor,
    )?;
    Some((next, snapshot))
...
pub(super) fn prepared_completed_state_is_consistent(
    ...
    let predecessor = &unit.calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_overdrying_limit;
...
pub(in crate::ideal_loads::calc) fn committed_latest_snapshot_is_consistent(
    ...
    system.id == unit.system
        ...
        && super::snapshot_validation::cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_supply_humidity_ratio_assignment_snapshot_is_exact_direct_release(
            witness,
        )
}

#[allow(dead_code)]
pub(in crate::ideal_loads) fn cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_supply_humidity_ratio_assignment_latest_metadata_is_consistent(
    unit: &PurchasedAirUnitRuntimeState,
...
fn predecessor_counts_match(state: &State, predecessor: &PredecessorState) -> bool {
    state.predecessor_route_counts == predecessor.predecessor_route_counts
        && state.transition_count == predecessor.transition_count
        && state.dehumidification_control_constant_sensible_heat_ratio_supply_humidity_ratio_assignment_count
            == predecessor
...
fn state_is_consistent(
    ...
    if state.system != expected_system
        ...
        || temperature_total != Some(state.cp391_supply_temperature_state_owner_count)
        || temperature_total != Some(state.unchanged_supply_temperature_preservation_count)
        || enthalpy_total != Some(state.cp391_supply_enthalpy_state_owner_count)
