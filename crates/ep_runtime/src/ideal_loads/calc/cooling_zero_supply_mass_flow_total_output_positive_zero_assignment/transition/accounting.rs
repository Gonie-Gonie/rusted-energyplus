//! Transactional CP429 route and source-site accounting.
...
pub(super) fn next_transition_fits(state: &State, predecessor: Predecessor, route: Route) -> bool {
    ...
    if route.assignment_executed {
        state
            ...
            && state.cp429_cooling_total_output_state_owner_count.checked_add(1).is_some()
            && state.cooling_total_output_assignment_write_count.checked_add(1).is_some()
    } else {
...
pub(super) fn increment_counts(state: &mut State, predecessor: Predecessor, route: Route) {
    ...
    if predecessor.resulting_supply_humidity_ratio.is_some() {
        state.cp428_supply_humidity_ratio_state_owner_count += 1;
        state.unchanged_supply_humidity_ratio_preservation_count += 1;
    }
    if predecessor.resulting_supply_enthalpy_j_per_kg.is_some() {
        state.cp428_supply_enthalpy_state_owner_count += 1;
        state.unchanged_supply_enthalpy_preservation_count += 1;
    }
    if predecessor.resulting_supply_temperature_c.is_some() {
        state.cp428_supply_temperature_state_owner_count += 1;
        state.unchanged_supply_temperature_preservation_count += 1;
    }
    if route.assignment_executed {
        state.zero_supply_mass_flow_total_output_positive_zero_assignment_count += 1;
        state.zero_supply_mass_flow_total_output_positive_zero_assignment_route_counts[index] += 1;
        state.source_site_execution_count += 1;
        state.cp429_cooling_total_output_state_owner_count += 1;
