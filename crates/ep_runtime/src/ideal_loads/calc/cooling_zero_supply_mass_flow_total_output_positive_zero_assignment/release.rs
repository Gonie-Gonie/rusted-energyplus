//! Release-bound CP429 zero-flow total-output positive-zero assignment.
...
use super::transition::{
    ...
    advance_cooling_zero_supply_mass_flow_total_output_positive_zero_assignment_state_with_validated_route as advance_with_route,
    cooling_zero_supply_mass_flow_total_output_positive_zero_assignment_route_from_committed_predecessor,
};
use crate::ideal_loads::calc::cooling_zero_supply_mass_flow_sensible_output_positive_zero_assignment_committed_latest_route;
use crate::ideal_loads::{
    PurchasedAirCalcCoolingZeroSupplyMassFlowSensibleOutputPositiveZeroAssignmentSnapshot as Predecessor,
...
#[must_use]
pub fn cooling_zero_supply_mass_flow_total_output_positive_zero_assignment_predecessor_cp428_snapshot(
    snapshot: Snapshot,
) -> Predecessor {
    prefix::predecessor_cp428_snapshot(snapshot)
}

/// Executes CP429 for the exact direct no-OA release route.
pub fn advance_direct_no_oa_calc_cooling_zero_supply_mass_flow_total_output_positive_zero_assignment(
    runtime: &mut PurchasedAirRuntimeState,
    system: &IdealLoadsAirSystem,
    predecessor_cp428: Predecessor,
) -> Result<
    Snapshot,
    PurchasedAirCalcCoolingZeroSupplyMassFlowTotalOutputPositiveZeroAssignmentError,
> {
    use PurchasedAirCalcCoolingZeroSupplyMassFlowTotalOutputPositiveZeroAssignmentError as Error;

    let selected = predecessor_cp428.system;
    let unit = runtime
        .units
        .get(&selected)
    ...
    if unit.controlled_zone != Some(predecessor_cp428.controlled_zone)
        || unit
    ...
    let predecessor_witness = runtime
        .cooling_zero_supply_mass_flow_sensible_output_positive_zero_assignment_latest_witness(
            selected,
        )
        .ok_or_else(|| predecessor_mismatch(selected))?;
    if !cooling_zero_supply_mass_flow_sensible_output_positive_zero_assignment_snapshots_match_bit_exact(
        predecessor_cp428,
        predecessor_witness,
    ) {
        return Err(predecessor_mismatch(selected));
    }
    let mixed_air_witness = if predecessor_cp428
    ...
    if predecessor_cp428.parent_call_ordinal != unit.init_call_count {
        return Err(call_order_error(unit, selected));
    }

    let state = &unit.calc_cooling_zero_supply_mass_flow_total_output_positive_zero_assignment;
    ...
    let post_route = next_state
        .latest_route
        .ok_or(Error::RuntimeStateInvariantViolation { system: selected })?;
    if !prefix_and_local_shape_match(snapshot, predecessor_cp428, post_route)
    ...
    let unit = runtime
        ...
        .get_mut(&selected)
        .ok_or(Error::UnknownSystem { system: selected })?;
    unit.calc_cooling_zero_supply_mass_flow_total_output_positive_zero_assignment = next_state;
    runtime
        .set_cooling_zero_supply_mass_flow_total_output_positive_zero_assignment_latest_witness(
            selected,
            Some(snapshot),
        );
    Ok(snapshot)
...
#[allow(dead_code)]
pub(in crate::ideal_loads) fn private_cooling_zero_supply_mass_flow_total_output_positive_zero_assignment_characterization(
...
#[allow(dead_code)]
pub(in crate::ideal_loads) fn cooling_zero_supply_mass_flow_total_output_positive_zero_assignment_snapshot_is_exact(
    snapshot: Snapshot,
) -> bool {
...
#[allow(dead_code)]
pub(in crate::ideal_loads) fn cooling_zero_supply_mass_flow_total_output_positive_zero_assignment_snapshots_match_bit_exact(
    left: Snapshot,
    right: Snapshot,
) -> bool {
...
fn predecessor_mismatch(
    ...
    PurchasedAirCalcCoolingZeroSupplyMassFlowTotalOutputPositiveZeroAssignmentError::
        CoolingZeroSupplyMassFlowSensibleOutputPositiveZeroAssignmentSnapshotMismatch { system }
}
