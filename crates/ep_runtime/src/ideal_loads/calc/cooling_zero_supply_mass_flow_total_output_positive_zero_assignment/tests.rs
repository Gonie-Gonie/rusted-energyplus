//! CP429 boundary, exhaustive route, forgery, preservation, and overflow tests.

mod committed_seal;
mod schema_prefix;

use super::transition::{
    PurchasedAirCalcCoolingZeroSupplyMassFlowTotalOutputPositiveZeroAssignmentRetainedRoute as Route,
    cooling_zero_supply_mass_flow_total_output_positive_zero_assignment_route_from_committed_predecessor as successor_route,
};
use super::{
    PurchasedAirCalcCoolingZeroSupplyMassFlowTotalOutputPositiveZeroAssignmentRuntimeState as State,
    advance_cooling_zero_supply_mass_flow_total_output_positive_zero_assignment_state as advance,
    advance_cooling_zero_supply_mass_flow_total_output_positive_zero_assignment_state_with_validated_route as advance_validated,
};
use crate::ideal_loads::calc::{
    cooling_zero_supply_mass_flow_sensible_output_positive_zero_assignment_snapshot_route as cp428_route,
    cp428_all_snapshots_for_successor_tests,
};
use crate::ideal_loads::PurchasedAirCalcCoolingZeroSupplyMassFlowSensibleOutputPositiveZeroAssignmentSnapshot as Predecessor;

#[test]
fn cp429_boundary_and_single_site_are_exact() {
    assert_eq!(
        super::PURCHASED_AIR_CALC_COOLING_ZERO_SUPPLY_MASS_FLOW_TOTAL_OUTPUT_POSITIVE_ZERO_ASSIGNMENT_SOURCE,
        "EnergyPlus 26.1 PurchasedAirManager.cc:2344",
    );
    assert_eq!(
        super::PURCHASED_AIR_CALC_COOLING_ZERO_SUPPLY_MASS_FLOW_TOTAL_OUTPUT_POSITIVE_ZERO_ASSIGNMENT_FIRST_EXCLUDED_SOURCE,
        "EnergyPlus 26.1 PurchasedAirManager.cc:2348",
    );
    assert_eq!(
        super::PURCHASED_AIR_CALC_COOLING_ZERO_SUPPLY_MASS_FLOW_TOTAL_OUTPUT_POSITIVE_ZERO_ASSIGNMENT_SOURCE_ORDER,
        &["assign-local-cooling-total-output-positive-zero-for-zero-supply-mass-flow"],
    );
    let excluded_structural_context = [
        (2345usize, "}"),
        (2347usize, "} else { // Heating or no-load case"),
    ];
    assert_eq!(excluded_structural_context[0], (2345, "}"));
    assert_eq!(
        excluded_structural_context[1],
        (2347, "} else { // Heating or no-load case"),
    );
}

#[test]
fn exhaustive_59_routes_assign_only_index_two_and_preserve_every_route() {
    let predecessors = cp428_all_snapshots_for_successor_tests();
    ...
    for predecessor in predecessors {
        let predecessor_route = cp428_route(predecessor).expect("CP428 route");
        let route = successor_route(predecessor, predecessor_route).expect("CP429 route");
        let snapshot = advance_validated(&mut state, predecessor, route).expect("CP429");
    ...
    assert_eq!(state.cp428_supply_humidity_ratio_state_owner_count, 37);
    assert_eq!(state.unchanged_supply_humidity_ratio_preservation_count, 37);
    assert_eq!(state.cp428_supply_enthalpy_state_owner_count, 42);
    assert_eq!(state.unchanged_supply_enthalpy_preservation_count, 42);
    assert_eq!(state.cp428_supply_temperature_state_owner_count, 57);
    assert_eq!(state.unchanged_supply_temperature_preservation_count, 57);
    assert_eq!(state.cp429_cooling_total_output_state_owner_count, 1);
    assert_eq!(state.cooling_total_output_assignment_write_count, 1);
...
fn pure_transition_writes_only_exact_positive_zero_on_the_active_route() {
    ...
    assert_eq!(snapshot.assigned_cooling_total_output_w.map(f64::to_bits), Some(0));
    assert_ne!(snapshot.assigned_cooling_total_output_w.map(f64::to_bits), Some((-0.0_f64).to_bits()));
    let mut forged = snapshot;
    forged.assigned_cooling_total_output_w = Some(-0.0_f64);
    assert!(
        !super::cooling_zero_supply_mass_flow_total_output_positive_zero_assignment_snapshot_is_exact(
            forged,
        )
    );
    assert!(
        !super::cooling_zero_supply_mass_flow_total_output_positive_zero_assignment_snapshots_match_bit_exact(
            snapshot,
            forged,
        )
    );

    let inactive = cp428_all_snapshots_for_successor_tests()
        .into_iter()
        .find(|snapshot| !route_for(*snapshot).assignment_executed)
        .expect("inactive");
    let snapshot = advance(&mut State::new(inactive.system), inactive).expect("inactive CP429");
...
fn every_incremented_counter_overflow_is_transactional() {
    ...
    scalar!(zero_supply_mass_flow_total_output_positive_zero_assignment_count);
    scalar!(source_site_execution_count);
    scalar!(cp429_cooling_total_output_state_owner_count);
    scalar!(cooling_total_output_assignment_write_count);
    ...
    inactive_scalar!(cp428_supply_humidity_ratio_state_owner_count);
    inactive_scalar!(unchanged_supply_humidity_ratio_preservation_count);
    inactive_scalar!(cp428_supply_enthalpy_state_owner_count);
    inactive_scalar!(unchanged_supply_enthalpy_preservation_count);
    inactive_scalar!(cp428_supply_temperature_state_owner_count);
...
#[test]
fn cp429_cp428_and_cp329_hot_paths_are_statically_bounded_and_lazy() {
    ...
    assert_eq!(
        hot.matches("sensible_output_positive_zero_assignment_committed_latest_route(")
            .count(),
        1
    );
    assert_eq!(
    ...
    let owner_index = hot.find("if predecessor_cp428").expect("witness guard");
    let owner_call = hot.find("cooling_mixed_air_call_latest_witness").expect("witness call");
    assert!(owner_call > owner_index);
}

#[test]
fn cp429_pending_transition_advances_from_cp428_not_from_itself() {
    ...
    assert_eq!(
        pending
            .matches("calc_cooling_zero_supply_mass_flow_sensible_output_positive_zero_assignment")
            .count(),
        1,
    );
    assert_eq!(
...
pub(super) fn active_predecessor() -> Predecessor {
    cp428_all_snapshots_for_successor_tests()
        .into_iter()
        .find(|snapshot| snapshot.cooling_zero_supply_mass_flow_sensible_output_positive_zero_assignment_executed)
        .expect("CP429 active predecessor")
}

pub(in crate::ideal_loads::calc) fn cp429_all_snapshots_for_successor_tests() -> Vec<
    crate::ideal_loads::PurchasedAirCalcCoolingZeroSupplyMassFlowTotalOutputPositiveZeroAssignmentSnapshot,
> {
    cp428_all_snapshots_for_successor_tests()
        .into_iter()
        .map(|predecessor| {
            ...
            advance_validated(&mut State::new(predecessor.system), predecessor, route)
                .expect("CP429 successor fixture")
        })
        .collect()
}
...
pub(super) fn route_for(predecessor: Predecessor) -> Route {
    successor_route(
        predecessor,
        cp428_route(predecessor).expect("CP428 route"),
    )
    .expect("CP429 route")
}
