//! Sealed CP429 route and positive-zero total-output carrier for CP430.
...
/// Returns CP429's bounded committed route without recursive derivation.
pub(in crate::ideal_loads::calc) fn cooling_zero_supply_mass_flow_total_output_positive_zero_assignment_committed_latest_route(
    ...
    let predecessor = predecessor_cp428_snapshot(latest);
    ...
    let assigned_total_output_is_exact = if route.assignment_executed {
        latest
            .assigned_cooling_total_output_w
            .is_some_and(|value| value.to_bits() == 0.0_f64.to_bits())
    } else {
        latest.assigned_cooling_total_output_w.is_none()
    };
    (state.system == unit.system
        ...
        && assigned_total_output_is_exact
...
mod tests {
    #[test]
    fn cp429_committed_route_hot_path_is_bounded() {
