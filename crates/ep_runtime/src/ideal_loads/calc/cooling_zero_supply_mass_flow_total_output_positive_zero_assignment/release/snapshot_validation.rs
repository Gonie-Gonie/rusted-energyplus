//! Exact CP429 prefix, route, assignment, and bitwise validation.

use super::prefix::predecessor_cp428_snapshot;
use super::super::{
...
pub(super) fn snapshot_is_exact(snapshot: Snapshot) -> bool {
    snapshot_route(snapshot).is_some()
        && crate::ideal_loads::cooling_zero_supply_mass_flow_sensible_output_positive_zero_assignment_snapshot_is_exact(
            predecessor_cp428_snapshot(snapshot),
        )
}

pub(super) fn snapshot_route(snapshot: Snapshot) -> Option<Route> {
    let predecessor = predecessor_cp428_snapshot(snapshot);
    let predecessor_route =
        crate::ideal_loads::calc::cooling_zero_supply_mass_flow_sensible_output_positive_zero_assignment_snapshot_route(
            predecessor,
        )?;
    let route =
        cooling_zero_supply_mass_flow_total_output_positive_zero_assignment_route_from_committed_predecessor(
...
pub(super) fn retained_route_matches_snapshot_bounded(snapshot: Snapshot, route: Route) -> bool {
    let predecessor = predecessor_cp428_snapshot(snapshot);
    let predecessor_route =
        crate::ideal_loads::calc::PurchasedAirCalcCoolingZeroSupplyMassFlowSensibleOutputPositiveZeroAssignmentRetainedRoute {
...
fn local_shape_is_exact(
    ...
    let assigned = snapshot.assigned_cooling_total_output_w;
    snapshot.source == SOURCE
        ...
        && snapshot.cooling_total_output_positive_zero_assignment_performed
            == route.assignment_executed
        && route.assignment_executed == assigned.is_some()
        && assigned.is_none_or(|value| value.to_bits() == 0)
        && same(
