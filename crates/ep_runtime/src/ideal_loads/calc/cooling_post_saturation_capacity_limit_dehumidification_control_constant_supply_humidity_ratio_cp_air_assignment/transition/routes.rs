//! Exact CP398 route preservation and compressed CP399 reconstruction.
...
pub(in crate::ideal_loads::calc) fn predecessor_route(predecessor: Predecessor) -> Option<RetainedRoute> {
    let route = crate::ideal_loads::calc::cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_case_entry_snapshot_route(predecessor)?;
    Some(RetainedRoute {
...
pub(in crate::ideal_loads::calc) fn compressed_snapshot_route(snapshot: Snapshot) -> Option<RetainedRoute> {
    let route = predecessor_route(predecessor_snapshot(snapshot))?;
    (snapshot.predecessor_dehumidification_control_none_or_constant_supply_humidity_ratio_shared_case_entered == route.active)
        .then_some(route)
}

pub(in crate::ideal_loads::calc) fn predecessor_snapshot(snapshot: Snapshot) -> Predecessor {
    use crate::ideal_loads::{
        PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_CONTROL_CONSTANT_SUPPLY_HUMIDITY_RATIO_CASE_ENTRY_FIRST_EXCLUDED_SOURCE as EXCLUDED,
        PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_CONTROL_CONSTANT_SUPPLY_HUMIDITY_RATIO_CASE_ENTRY_SOURCE as SOURCE,
        PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_CONTROL_CONSTANT_SUPPLY_HUMIDITY_RATIO_CASE_ENTRY_SOURCE_ORDER as ORDER,
    };
    Predecessor {
        source: SOURCE,
        ...
        dehumidification_control_none_or_constant_supply_humidity_ratio_shared_case_entered: snapshot.predecessor_dehumidification_control_none_or_constant_supply_humidity_ratio_shared_case_entered,
        resulting_supply_humidity_ratio: snapshot.predecessor_cp398_resulting_supply_humidity_ratio,
        resulting_supply_enthalpy_j_per_kg: snapshot.predecessor_cp398_resulting_supply_enthalpy_j_per_kg,
        resulting_supply_temperature_c: snapshot.predecessor_cp398_resulting_supply_temperature_c,
    }
}
...
pub(in crate::ideal_loads::calc) const fn predecessor_has_supply_humidity_ratio(index: usize) -> bool {
