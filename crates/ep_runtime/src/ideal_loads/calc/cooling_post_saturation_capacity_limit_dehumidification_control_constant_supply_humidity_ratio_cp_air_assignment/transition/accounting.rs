//! Exact CP399 route and three-site accounting.

use super::State;
use super::routes::RetainedRoute;
use crate::ideal_loads::PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_CONTROL_CONSTANT_SUPPLY_HUMIDITY_RATIO_CP_AIR_ASSIGNMENT_SOURCE_ORDER;

pub(super) fn next_transition_fits(state: &State, route: RetainedRoute) -> bool {
    if state.transition_count.checked_add(1).is_none()
        || state.predecessor_route_counts[route.predecessor_index].checked_add(1).is_none()
    {
        return false;
    }
    if !route.active {
    ...
    state.dehumidification_control_constant_supply_humidity_ratio_cp_air_assignment_count.checked_add(1).is_some()
        ...
        && state.mixed_air_humidity_ratio_read_count.checked_add(1).is_some()
        && state.psychrometric_cp_air_evaluation_count.checked_add(1).is_some()
...
pub(super) fn increment_counts(state: &mut State, route: RetainedRoute) {
    ...
    state.source_site_execution_count += PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_CONTROL_CONSTANT_SUPPLY_HUMIDITY_RATIO_CP_AIR_ASSIGNMENT_SOURCE_ORDER.len();
