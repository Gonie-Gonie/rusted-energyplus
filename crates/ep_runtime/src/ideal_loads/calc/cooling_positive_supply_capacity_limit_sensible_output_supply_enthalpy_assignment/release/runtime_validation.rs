//! Persistent CP342 runtime-state validation.
...
use super::super::PurchasedAirCalcCoolingPositiveSupplyCapacityLimitSensibleOutputSupplyEnthalpyAssignmentRetainedInput;
    ...
    state_is_consistent(state, witness, snapshot.system)
        ...
        && state.capacity_limit_sensible_output_supply_enthalpy_assignment_count
            == predecessor
                .capacity_limit_sensible_output_maximum_capacity_assignment_count
        && state.capacity_limit_sensible_output_supply_enthalpy_assignment_count
            == guard.capacity_limit_sensible_output_adjustment_body_entry_count
        && state
            .capacity_limit_sensible_output_guard_false_fallthrough_count
            .checked_add(
                state.capacity_limit_sensible_output_supply_enthalpy_assignment_count,
            )
            == Some(guard.capacity_limit_sensible_output_guard_evaluation_count)
        && state
            .latest
...
fn state_is_consistent(
    ...
    let counters_match = state.system == expected_system
        ...
        && state.mixed_air_enthalpy_read_count == assigned
        && state.cooling_sensible_output_read_count == assigned
        && state.supply_mass_flow_rate_read_count == assigned
        && state.specific_cooling_output_calculation_count == assigned
        && state.supply_enthalpy_calculation_count == assigned
        && state.supply_enthalpy_assignment_write_count == assigned
