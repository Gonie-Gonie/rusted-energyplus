//! Transactional CP402 route, owner, and conditional-site accounting.
...
pub(super) fn increment_counts(state: &mut State, route: RetainedRoute) {
    ...
    if predecessor_has_supply_humidity_ratio(index) {
        state.cp401_supply_humidity_ratio_state_owner_count += 1;
    ...
    state.dehumidification_control_none_or_constant_supply_humidity_ratio_latent_output_capacity_guard_evaluation_count += 1;
    ...
    if route.body_entered {
        ...
        state.cooling_latent_output_greater_than_or_equal_to_maximum_total_cooling_capacity_count += 1;
        state.dehumidification_control_none_or_constant_supply_humidity_ratio_latent_output_capacity_adjustment_body_entry_count += 1;
    } else {
        state.guard_false_fallthrough_route_counts[index] += 1;
        state.dehumidification_control_none_or_constant_supply_humidity_ratio_latent_output_capacity_guard_false_fallthrough_count += 1;
    }
}
