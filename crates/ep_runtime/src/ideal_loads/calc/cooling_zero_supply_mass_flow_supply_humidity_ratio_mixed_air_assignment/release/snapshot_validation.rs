//! Exact CP426 prefix, route, assignment, and bitwise validation.
...
pub(super) fn snapshot_is_exact(snapshot: Snapshot) -> bool {
    snapshot_route(snapshot).is_some()
        && crate::ideal_loads::cooling_zero_supply_mass_flow_supply_enthalpy_mixed_air_assignment_snapshot_is_exact(
            predecessor_cp425_snapshot(snapshot),
        )
}

pub(in crate::ideal_loads::calc) fn snapshot_route(snapshot: Snapshot) -> Option<Route> {
    let predecessor = predecessor_cp425_snapshot(snapshot);
    let predecessor_route =
        crate::ideal_loads::calc::cooling_zero_supply_mass_flow_supply_enthalpy_mixed_air_assignment_snapshot_route(
            predecessor,
        )?;
    let route =
        cooling_zero_supply_mass_flow_supply_humidity_ratio_mixed_air_assignment_route_from_committed_predecessor(
...
pub(super) fn retained_route_matches_snapshot_bounded(snapshot: Snapshot, route: Route) -> bool {
    let predecessor = predecessor_cp425_snapshot(snapshot);
    let predecessor_route =
        crate::ideal_loads::calc::PurchasedAirCalcCoolingZeroSupplyMassFlowSupplyEnthalpyMixedAirAssignmentRetainedRoute {
...
fn local_shape_is_exact(
    ...
    let assigned = snapshot.assigned_supply_humidity_ratio_from_mixed_air;
    let expected_humidity_ratio = if route.assignment_executed {
        assigned
    } else {
    ...
    snapshot.source == SOURCE
        ...
        && snapshot.cp329_retained_mixed_air_humidity_ratio_for_zero_supply_mass_flow_supply_humidity_ratio_assignment_owned_read == route.assignment_executed
