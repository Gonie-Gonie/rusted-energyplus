//! Persistent CP334 runtime-state validation.
...
use super::super::PurchasedAirCalcCoolingPositiveSupplyTemperatureMixedAirLimitRetainedRoute;
...
pub(in crate::ideal_loads::calc) fn pending_supply_temperature_mixed_air_limit_state_is_consistent(
    unit: &PurchasedAirUnitRuntimeState,
    predecessor: PurchasedAirCalcCoolingPositiveSupplyTemperatureMinimumLimitSnapshot,
    witness: Option<PurchasedAirCalcCoolingPositiveSupplyTemperatureMixedAirLimitSnapshot>,
) -> bool {
    let state = &unit.calc_cooling_positive_supply_temperature_mixed_air_limit;
    let predecessor_state = &unit.calc_cooling_positive_supply_temperature_minimum_limit;
    state_is_consistent(state, witness, predecessor.system)
        ...
        && state
            ...
            == Some(predecessor_state.supply_temperature_minimum_limit_count)
}

pub(in crate::ideal_loads::calc) fn next_supply_temperature_mixed_air_limit_transition_fits(
    unit: &PurchasedAirUnitRuntimeState,
    predecessor: PurchasedAirCalcCoolingPositiveSupplyTemperatureMinimumLimitSnapshot,
) -> bool {
    let state = &unit.calc_cooling_positive_supply_temperature_mixed_air_limit;
    ...
    state
        ...
        && state
            ...
            .checked_add(
                super::super::PURCHASED_AIR_CALC_COOLING_POSITIVE_SUPPLY_TEMPERATURE_MIXED_AIR_LIMIT_SOURCE_ORDER
                    .len(),
            )
            .is_some()
        && state
...
pub(super) fn completed_supply_temperature_mixed_air_limit_state_is_consistent(
    ...
    let state = &unit.calc_cooling_positive_supply_temperature_mixed_air_limit;
    let predecessor = &unit.calc_cooling_positive_supply_temperature_minimum_limit;
...
fn state_is_consistent(
    ...
    let Some(route_partition) = state
        ...
        .and_then(|count| count.checked_add(state.supply_temperature_mixed_air_limit_count))
    else {
        return false;
    };
    ...
    let active = state.supply_temperature_mixed_air_limit_count;
    let counters_match = state.system == expected_system
        ...
        && state.supply_temperature_for_minimum_read_count == active
        && state.mixed_air_temperature_for_minimum_read_count == active
        && state.source_shaped_two_argument_minimum_evaluation_count == active
        && state.supply_temperature_assignment_count == active
        && state.witnessed_positive_guard_false_fallthrough_skip_count
            == state.positive_guard_false_fallthrough_skip_count
        && state.witnessed_supply_temperature_mixed_air_limit_count == active;
    if !counters_match {
        return false;
    }
