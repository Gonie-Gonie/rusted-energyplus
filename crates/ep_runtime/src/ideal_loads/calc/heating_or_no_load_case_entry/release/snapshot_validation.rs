//! Exact CP430 prefix, route, marker, and bitwise validation.
...
pub(super) fn snapshot_is_exact(snapshot: Snapshot) -> bool {
    snapshot_route(snapshot).is_some()
        && crate::ideal_loads::cooling_zero_supply_mass_flow_total_output_positive_zero_assignment_snapshot_is_exact(
            predecessor_cp429_snapshot(snapshot),
        )
}
pub(super) fn snapshot_route(snapshot: Snapshot) -> Option<Route> {
    let predecessor = predecessor_cp429_snapshot(snapshot);
...
pub(super) fn retained_route_matches_snapshot_bounded(
    ...
    let predecessor = predecessor_cp429_snapshot(snapshot);
    let predecessor_route = crate::ideal_loads::calc::PurchasedAirCalcCoolingZeroSupplyMassFlowTotalOutputPositiveZeroAssignmentRetainedRoute {
...
fn local_shape_is_exact(
    ...
    snapshot.source == SOURCE
        ...
        && snapshot.heating_or_no_load_case_entered == route.entered
        && route.entered == predecessor.non_cooling_skipped
        && same(
