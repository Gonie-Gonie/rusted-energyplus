//! Persistent CP411 runtime-state validation.
...
pub(super) fn calc_state_identities_match(
    ...
    unit.system == system
        ...
        && unit.calc_cooling_post_saturation_capacity_limit_dehumidification_control_default_case_break.system == system
...
pub(super) fn call_order_is_pending(
    ...
    unit.calc_cooling_post_saturation_capacity_limit_dehumidification_supply_humidity_ratio_pre_saturation_original_assignment
        .transition_count.checked_add(1) == Some(ordinal)
        && unit.calc_entry.call_count == ordinal
        && unit.calc_cooling_post_saturation_capacity_limit_dehumidification_control_default_case_break.transition_count == ordinal
}
...
pub(super) fn pending_state_is_consistent(
    ...
    let state = &unit.calc_cooling_post_saturation_capacity_limit_dehumidification_supply_humidity_ratio_pre_saturation_original_assignment;
...
pub(super) fn prepare_next_transition(
    ...
    let snapshot = advance_cooling_post_saturation_capacity_limit_dehumidification_supply_humidity_ratio_pre_saturation_original_assignment_state(&mut next, predecessor)?;
    Some((next, snapshot))
}
...
#[allow(dead_code)]
pub(in crate::ideal_loads) fn cooling_post_saturation_capacity_limit_dehumidification_supply_humidity_ratio_pre_saturation_original_assignment_latest_metadata_is_consistent(
    unit: &PurchasedAirUnitRuntimeState,
    ...
    state.transition_count == expected_transition_count
        ...
        && predecessor_counts_match(state, &unit.calc_cooling_post_saturation_capacity_limit_dehumidification_control_default_case_break)
}
...
fn predecessor_counts_match(state: &State, predecessor: &PredecessorState) -> bool {
    state.transition_count == predecessor.transition_count
        ...
        && predecessor.dehumidification_control_default_case_break_count == 0
        && predecessor.source_site_execution_count == 0
}
...
fn counts_are_exact(state: &State) -> bool {
    ...
    let assignment_total =
        checked_sum(&state.supply_humidity_ratio_pre_saturation_original_assignment_route_counts);
    ...
    let assignment_partition_exact = (0..30).all(|index| {
        if matches!(index, 18..=29) {
            state.supply_humidity_ratio_pre_saturation_original_assignment_route_counts[index]
                == state.predecessor_route_counts[index]
        } else {
            state.supply_humidity_ratio_pre_saturation_original_assignment_route_counts[index] == 0
        }
    });
    let transition_partition = state
        .inactive_transition_count
        .checked_add(state.supply_humidity_ratio_pre_saturation_original_assignment_count);
    predecessor_total == Some(state.transition_count)
        ...
        && assignment_total
            == Some(state.supply_humidity_ratio_pre_saturation_original_assignment_count)
        ...
        && state.unchanged_supply_humidity_ratio_preservation_count
            == state.cp410_supply_humidity_ratio_state_owner_count
        && Some(state.cp410_supply_enthalpy_state_owner_count) == enthalpy_total
        && state.unchanged_supply_enthalpy_preservation_count
            == state.cp410_supply_enthalpy_state_owner_count
        && Some(state.cp410_supply_temperature_state_owner_count) == temperature_total
