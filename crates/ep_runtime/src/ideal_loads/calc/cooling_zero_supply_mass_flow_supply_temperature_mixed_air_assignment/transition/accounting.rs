//! Transactional CP427 route and source-site accounting.
...
pub(super) fn next_transition_fits(state: &State, predecessor: Predecessor, route: Route) -> bool {
    ...
    let preserved = [
        (
            ...
            state.cp426_supply_humidity_ratio_state_owner_count,
            state.unchanged_supply_humidity_ratio_preservation_count,
        ),
        (
            predecessor.resulting_supply_enthalpy_j_per_kg.is_some(),
            state.cp426_supply_enthalpy_state_owner_count,
            state.unchanged_supply_enthalpy_preservation_count,
        ),
        (
            predecessor.resulting_supply_temperature_c.is_some(),
            state.cp426_supply_temperature_state_owner_count,
    ...
    if route.assignment_executed {
        state
            ...
            && state.cp427_supply_temperature_state_owner_count.checked_add(1).is_some()
            && state.cp329_retained_mixed_air_temperature_for_zero_supply_mass_flow_supply_temperature_assignment_owned_read_count.checked_add(1).is_some()
            && state.mixed_air_temperature_for_zero_supply_mass_flow_supply_temperature_assignment_read_count.checked_add(1).is_some()
            && state.supply_temperature_assignment_write_count.checked_add(1).is_some()
    } else {
...
pub(super) fn increment_counts(state: &mut State, predecessor: Predecessor, route: Route) {
    ...
    if predecessor.resulting_supply_enthalpy_j_per_kg.is_some() {
        state.cp426_supply_enthalpy_state_owner_count += 1;
        state.unchanged_supply_enthalpy_preservation_count += 1;
    }
    if predecessor.resulting_supply_temperature_c.is_some() {
        state.cp426_supply_temperature_state_owner_count += 1;
        state.unchanged_supply_temperature_preservation_count += 1;
    }
    if route.assignment_executed {
        state.zero_supply_mass_flow_supply_temperature_mixed_air_assignment_count += 1;
        state.zero_supply_mass_flow_supply_temperature_mixed_air_assignment_route_counts[index] += 1;
        state.source_site_execution_count += 2;
        state.cp427_supply_temperature_state_owner_count += 1;
        state.cp329_retained_mixed_air_temperature_for_zero_supply_mass_flow_supply_temperature_assignment_owned_read_count += 1;
        state.mixed_air_temperature_for_zero_supply_mass_flow_supply_temperature_assignment_read_count += 1;
