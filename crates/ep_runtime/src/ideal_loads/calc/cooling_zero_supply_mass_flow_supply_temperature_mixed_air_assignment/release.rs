//! Release-bound CP427 zero-flow supply-temperature assignment.
...
use super::transition::{
    ...
    advance_cooling_zero_supply_mass_flow_supply_temperature_mixed_air_assignment_state_with_validated_route as advance_with_route,
...
pub use error::PurchasedAirCalcCoolingZeroSupplyMassFlowSupplyTemperatureMixedAirAssignmentError;
pub(in crate::ideal_loads::calc) use committed::cooling_zero_supply_mass_flow_supply_temperature_mixed_air_assignment_committed_latest_route;
pub(in crate::ideal_loads::calc) use snapshot_validation::{
    retained_route_matches_snapshot_bounded as cooling_zero_supply_mass_flow_supply_temperature_mixed_air_assignment_retained_route_matches_snapshot_bounded,
    snapshot_route as cooling_zero_supply_mass_flow_supply_temperature_mixed_air_assignment_snapshot_route,
};
...
/// Losslessly reconstructs the CP426 predecessor retained in CP427.
#[must_use]
pub fn cooling_zero_supply_mass_flow_supply_temperature_mixed_air_assignment_predecessor_cp426_snapshot(
    snapshot: Snapshot,
) -> Predecessor {
    prefix::predecessor_cp426_snapshot(snapshot)
}

/// Executes CP427 for the exact direct no-OA release route.
pub fn advance_direct_no_oa_calc_cooling_zero_supply_mass_flow_supply_temperature_mixed_air_assignment(
    runtime: &mut PurchasedAirRuntimeState,
    system: &IdealLoadsAirSystem,
    predecessor_cp426: Predecessor,
) -> Result<
    Snapshot,
    PurchasedAirCalcCoolingZeroSupplyMassFlowSupplyTemperatureMixedAirAssignmentError,
> {
    use PurchasedAirCalcCoolingZeroSupplyMassFlowSupplyTemperatureMixedAirAssignmentError as Error;

    let selected = predecessor_cp426.system;
    let unit = runtime
        .units
        .get(&selected)
        .ok_or(Error::UnknownSystem { system: selected })?;
    let witness = runtime
        .cooling_zero_supply_mass_flow_supply_temperature_mixed_air_assignment_latest_witness(
            selected,
        );
    ...
    if unit.controlled_zone != Some(predecessor_cp426.controlled_zone)
        || unit
    ...
    let mixed_air_witness = if predecessor_cp426
        .cooling_zero_supply_mass_flow_supply_humidity_ratio_mixed_air_assignment_executed
    {
        Some(
            runtime
                ...
                .ok_or(Error::MixedAirTemperatureOwnerUnavailableOrInconsistent {
                    system: selected,
                })?,
        )
    } else {
    ...
    let mixed_air_temperature_c = if route.assignment_executed {
        Some(
            cooling_mixed_air_call_committed_latest_mixed_air_temperature(
                unit,
                mixed_air_witness.ok_or(
                    Error::MixedAirTemperatureOwnerUnavailableOrInconsistent {
                        system: selected,
                    },
                )?,
            )
    ...
    if predecessor_cp426.parent_call_ordinal != unit.init_call_count {
        return Err(call_order_error(unit, selected));
    }

    let state = &unit.calc_cooling_zero_supply_mass_flow_supply_temperature_mixed_air_assignment;
    ...
    let post_route = next_state
        .latest_route
        .ok_or(Error::RuntimeStateInvariantViolation { system: selected })?;
    if !prefix_and_local_shape_match(snapshot, predecessor_cp426, post_route)
    ...
    let unit = runtime
        ...
        .get_mut(&selected)
        .ok_or(Error::UnknownSystem { system: selected })?;
    unit.calc_cooling_zero_supply_mass_flow_supply_temperature_mixed_air_assignment = next_state;
    runtime
        .set_cooling_zero_supply_mass_flow_supply_temperature_mixed_air_assignment_latest_witness(
            selected,
            Some(snapshot),
        );
    Ok(snapshot)
...
#[allow(dead_code)]
pub(in crate::ideal_loads) fn private_cooling_zero_supply_mass_flow_supply_temperature_mixed_air_assignment_characterization(
    predecessor: Predecessor,
    mixed_air_temperature_c: Option<f64>,
) -> Option<Snapshot> {
    let mut state = State::new(predecessor.system);
    advance(&mut state, predecessor, mixed_air_temperature_c)
}

#[allow(dead_code)]
pub(in crate::ideal_loads) fn cooling_zero_supply_mass_flow_supply_temperature_mixed_air_assignment_snapshot_is_exact(
    snapshot: Snapshot,
) -> bool {
...
#[allow(dead_code)]
pub(in crate::ideal_loads) fn cooling_zero_supply_mass_flow_supply_temperature_mixed_air_assignment_snapshots_match_bit_exact(
    left: Snapshot,
    right: Snapshot,
) -> bool {
...
fn predecessor_mismatch(
    ...
    PurchasedAirCalcCoolingZeroSupplyMassFlowSupplyTemperatureMixedAirAssignmentError::
        CoolingZeroSupplyMassFlowSupplyHumidityRatioMixedAirAssignmentSnapshotMismatch { system }
}
