//! Pure CP426-to-CP427 zero-flow supply-temperature assignment.
...
use crate::ideal_loads::calc::PurchasedAirCalcCoolingZeroSupplyMassFlowSupplyHumidityRatioMixedAirAssignmentRetainedRoute as PredecessorRoute;
...
/// One retained CP427 route over the exact 36-wide CP426 partition.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(in crate::ideal_loads::calc) struct PurchasedAirCalcCoolingZeroSupplyMassFlowSupplyTemperatureMixedAirAssignmentRetainedRoute {
...
use PurchasedAirCalcCoolingZeroSupplyMassFlowSupplyTemperatureMixedAirAssignmentRetainedRoute as Route;

pub(in crate::ideal_loads::calc) fn cooling_zero_supply_mass_flow_supply_temperature_mixed_air_assignment_route_from_committed_predecessor(
...
pub(in crate::ideal_loads::calc) fn advance_cooling_zero_supply_mass_flow_supply_temperature_mixed_air_assignment_state(
    state: &mut State,
    predecessor: Predecessor,
    mixed_air_temperature_c: Option<f64>,
) -> Option<Snapshot> {
    let predecessor_route =
        crate::ideal_loads::calc::cooling_zero_supply_mass_flow_supply_humidity_ratio_mixed_air_assignment_snapshot_route(predecessor)?;
    let route = cooling_zero_supply_mass_flow_supply_temperature_mixed_air_assignment_route_from_committed_predecessor(
        predecessor,
        predecessor_route,
    )?;
    advance_cooling_zero_supply_mass_flow_supply_temperature_mixed_air_assignment_state_with_validated_route(
        state,
        predecessor,
        route,
...
pub(in crate::ideal_loads::calc) fn advance_cooling_zero_supply_mass_flow_supply_temperature_mixed_air_assignment_state_with_validated_route(
    state: &mut State,
    predecessor: Predecessor,
    route: Route,
    mixed_air_temperature_c: Option<f64>,
    ...
    if state.system != predecessor.system
        || route.assignment_executed != mixed_air_temperature_c.is_some()
        || !route_matches_predecessor(predecessor, route)
        || !next_transition_fits(state, predecessor, route)
    {
        return None;
    }
    let snapshot = snapshot::build_snapshot(predecessor, route, mixed_air_temperature_c);
