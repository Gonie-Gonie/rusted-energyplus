//! CP427 bounded state and CP426-accounting validation.
...
use super::super::transition::PurchasedAirCalcCoolingZeroSupplyMassFlowSupplyTemperatureMixedAirAssignmentRetainedRoute as Route;
use super::snapshot_validation::{retained_route_matches_snapshot_bounded, snapshots_match_bit_exact};
use crate::ideal_loads::{
    PurchasedAirCalcCoolingZeroSupplyMassFlowSupplyHumidityRatioMixedAirAssignmentRuntimeState as PredecessorState,
...
pub(super) fn state_counts_are_consistent(state: &State) -> bool {
    ...
    for index in 0..36 {
        ...
        if state.zero_supply_mass_flow_supply_temperature_mixed_air_assignment_route_counts[index]
            != expected
        {
            return false;
        }
    ...
    state.transition_count == transitions
        ...
        && state.cp426_supply_humidity_ratio_state_owner_count == w_owners
        && state.unchanged_supply_humidity_ratio_preservation_count == w_owners
        && state.cp426_supply_enthalpy_state_owner_count == h_owners
        && state.unchanged_supply_enthalpy_preservation_count == h_owners
        && state.cp426_supply_temperature_state_owner_count == t_owners
        && state.unchanged_supply_temperature_preservation_count == t_owners
        && state.cp427_supply_temperature_state_owner_count == assignments
        && state.cp329_retained_mixed_air_temperature_for_zero_supply_mass_flow_supply_temperature_assignment_owned_read_count
            == assignments
