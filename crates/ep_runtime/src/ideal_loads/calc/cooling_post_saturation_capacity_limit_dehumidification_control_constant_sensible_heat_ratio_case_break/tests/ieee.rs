//! CP393 control-only binary64 carrier tests.
...
fn compressed_snapshot_preserves_arbitrary_carrier_bits_without_numeric_gates() {
    ...
    let mut snapshot = advance_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_case_break_state(
        &mut state,
        chain.cp392,
    )
    .expect("active CP393");
    ...
    for (predecessor, resulting, bits) in [
        (
            &mut snapshot.predecessor_cp392_resulting_supply_humidity_ratio,
            &mut snapshot.resulting_supply_humidity_ratio,
            0x7ff8_0000_0000_00a5,
        ),
        (
            &mut snapshot.predecessor_cp392_resulting_supply_enthalpy_j_per_kg,
            &mut snapshot.resulting_supply_enthalpy_j_per_kg,
            f64::NEG_INFINITY.to_bits(),
        ),
        (
            &mut snapshot.predecessor_cp392_resulting_supply_temperature_c,
    ...
    assert!(cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_case_break_snapshot_is_exact(snapshot));
}
...
fn binary64_snapshot_comparison_distinguishes_nan_payloads() {
    ...
    forged.resulting_supply_humidity_ratio = Some(f64::from_bits(0x7ff8_0000_0000_00a5));
    assert!(!cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_case_break_snapshots_match_bit_exact(snapshot, forged));
}
