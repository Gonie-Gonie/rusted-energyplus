//! CP393 exhaustive route and carrier-preservation tests.
...
#[test]
fn cp393_preserves_thirty_routes_and_breaks_exactly_three() {
    let chains = fixtures::all_chains();
    let mut state = State::new(chains[0].cp392.system);
    let snapshots: Vec<_> = chains
        .iter()
        .map(|chain| advance(&mut state, chain.cp392).expect("CP393 route"))
        .collect();
    ...
    for (index, (chain, snapshot)) in chains.into_iter().zip(snapshots).enumerate() {
        ...
        assert_bits_eq(
            ...
            chain.cp392.resulting_supply_humidity_ratio,
        );
        assert_bits_eq(
        ...
        assert_bits_eq(
            ...
            chain.cp392.resulting_supply_enthalpy_j_per_kg,
        );
        assert_bits_eq(
        ...
        assert_bits_eq(
            ...
            chain.cp392.resulting_supply_temperature_c,
        );
        assert_bits_eq(
...
fn exact(snapshot: Snapshot) -> bool {
    cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_case_break_snapshot_is_exact(snapshot)
}
