//! Pure CP411-to-CP412 saturation-humidity-ratio assignment.
...
/// Current-timestep operand not retained by CP411.
#[derive(Clone, Copy, Debug, PartialEq)]
pub(in crate::ideal_loads::calc) struct PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationSupplyHumidityRatioSaturationAssignmentActiveInput
{
    /// Exact active outdoor-barometric-pressure bits.
    pub outdoor_barometric_pressure_pa: f64,
}

pub(in crate::ideal_loads::calc) fn advance_cooling_post_saturation_capacity_limit_dehumidification_supply_humidity_ratio_saturation_assignment_state(
    state: &mut State,
    predecessor: Predecessor,
    input: Option<PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationSupplyHumidityRatioSaturationAssignmentActiveInput>,
) -> Option<Snapshot> {
    ...
    let (temperature, pressure, evaluated) = if active {
        let input = input?;
        let temperature = predecessor.resulting_supply_temperature_c?;
        let pressure = input.outdoor_barometric_pressure_pa;
        (
            Some(temperature),
            Some(pressure),
    ...
    let snapshot = Snapshot {
        ...
        assigned_supply_humidity_ratio_original: predecessor
