//! Release-bound CP422 sensible-output maximum-capacity assignment.
...
use super::transition::{
    ...
    advance_cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_sensible_output_maximum_capacity_assignment_state_with_validated_route as advance_with_route,
...
#[must_use]
pub fn cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_sensible_output_maximum_capacity_assignment_predecessor_cp421_snapshot(
    snapshot: Snapshot,
) -> Predecessor {
    prefix::predecessor_cp421_snapshot(snapshot)
}

/// Executes CP422 for the exact direct no-OA release route.
pub fn advance_direct_no_oa_calc_cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_sensible_output_maximum_capacity_assignment(
    runtime: &mut PurchasedAirRuntimeState,
    system: &IdealLoadsAirSystem,
    predecessor_cp421: Predecessor,
) -> Result<Snapshot, PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationGuardElseBranchSensibleOutputMaximumCapacityAssignmentError> {
    use PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationGuardElseBranchSensibleOutputMaximumCapacityAssignmentError as Error;

    let selected = predecessor_cp421.system;
    let unit = runtime
        .units
        .get(&selected)
        .ok_or(Error::UnknownSystem { system: selected })?;
    let witness = runtime.cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_sensible_output_maximum_capacity_assignment_latest_witness(selected);
    ...
    if unit.controlled_zone != Some(predecessor_cp421.controlled_zone)
        || unit
    ...
    let (committed_route, committed_output, committed_capacity) =
    ...
    if predecessor_cp421.parent_call_ordinal != unit.init_call_count {
        return Err(call_order_error(unit, selected));
    }
    let input = if route.active {
        Some(ActiveInput {
            preexisting_cooling_sensible_output_w: committed_output.ok_or(
                Error::FormulaOperandOwnersUnavailableOrInconsistent { system: selected },
            )?,
            maximum_total_cooling_capacity_w: committed_capacity.ok_or(
                Error::FormulaOperandOwnersUnavailableOrInconsistent { system: selected },
            )?,
            cp421_retained_maximum_total_cooling_capacity_owned_read: true,
        })
    } else {
        if committed_output.is_some() || committed_capacity.is_some() {
            return Err(Error::RuntimeStateInvariantViolation { system: selected });
        }
        None
    };
    ...
    let snapshot = advance_with_route(&mut next_state, predecessor_cp421, route, input)
        .ok_or(Error::RuntimeStateInvariantViolation { system: selected })?;
    let post_transition_route = next_state
        .latest_route
        .ok_or(Error::RuntimeStateInvariantViolation { system: selected })?;
    if !prefix_and_local_shape_match(snapshot, predecessor_cp421, post_transition_route)
    ...
    let unit = runtime
        ...
        .get_mut(&selected)
        .ok_or(Error::UnknownSystem { system: selected })?;
    unit.calc_cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_sensible_output_maximum_capacity_assignment = next_state;
    runtime.set_cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_sensible_output_maximum_capacity_assignment_latest_witness(
        selected,
        Some(snapshot),
    );
    Ok(snapshot)
...
#[allow(dead_code)]
pub(in crate::ideal_loads) fn private_cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_sensible_output_maximum_capacity_assignment_characterization(
...
#[allow(dead_code)]
pub(in crate::ideal_loads) fn cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_sensible_output_maximum_capacity_assignment_snapshot_is_exact(
    snapshot: Snapshot,
) -> bool {
...
#[allow(dead_code)]
pub(in crate::ideal_loads) fn cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_sensible_output_maximum_capacity_assignment_snapshots_match_bit_exact(
    left: Snapshot,
    right: Snapshot,
) -> bool {
...
fn predecessor_mismatch(
    ...
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationGuardElseBranchSensibleOutputMaximumCapacityAssignmentError::CoolingPostSaturationCapacityLimitDehumidificationGuardElseBranchSensibleOutputGuardSnapshotMismatch { system }
}
...
fn call_order_error(
    ...
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationGuardElseBranchSensibleOutputMaximumCapacityAssignmentError::PredecessorCallOrder {
        system,
        init_call_count: unit.init_call_count,
        calculation_entry_call_count: unit.calc_entry.call_count,
        predecessor_transition_count: unit.calc_cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_sensible_output_guard.transition_count,
        transition_count: unit.calc_cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_sensible_output_maximum_capacity_assignment.transition_count,
    }
}
