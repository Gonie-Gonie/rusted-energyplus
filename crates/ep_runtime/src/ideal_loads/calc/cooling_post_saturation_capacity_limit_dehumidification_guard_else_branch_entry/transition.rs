//! Pure CP417-to-CP418 not-dehumidifying else-branch entry.
...
use crate::ideal_loads::calc::PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationSupplyEnthalpyAssignmentCommittedRoute as PredecessorRoute;

mod accounting;
use accounting::{increment_counts, next_transition_fits};

/// One retained CP418 route over the lossless 36-wide CP417 partition.
...
pub(in crate::ideal_loads::calc) fn predecessor_route(
    ...
    let route = crate::ideal_loads::calc::cooling_post_saturation_capacity_limit_dehumidification_supply_enthalpy_assignment_snapshot_route(predecessor)?;
...
pub(in crate::ideal_loads::calc) fn cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_entry_route_from_validated_predecessor(
    predecessor: Predecessor,
    ...
    let route = RetainedRoute {
        ...
        predecessor_supply_enthalpy_assignment_executed: route.active,
        active: predecessor.predecessor_dehumidification_guard_false_fallthrough,
    };
    route_matches_predecessor(route, predecessor).then_some(route)
}

#[allow(dead_code)]
pub(in crate::ideal_loads::calc) fn advance_cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_entry_state(
    state: &mut State,
    predecessor: Predecessor,
) -> Option<Snapshot> {
...
pub(in crate::ideal_loads::calc) fn advance_cooling_post_saturation_capacity_limit_dehumidification_guard_else_branch_entry_state_with_validated_route(
    state: &mut State,
    predecessor: Predecessor,
    route: RetainedRoute,
) -> Option<Snapshot> {
    ...
    let snapshot = Snapshot {
        ...
        post_saturation_capacity_limit_dehumidification_guard_else_branch_entered: route.active,
    };
...
fn route_matches_predecessor(route: RetainedRoute, predecessor: Predecessor) -> bool {
    ...
    route.logical_index < 36
        ...
        && route.active == predecessor.predecessor_dehumidification_guard_false_fallthrough
