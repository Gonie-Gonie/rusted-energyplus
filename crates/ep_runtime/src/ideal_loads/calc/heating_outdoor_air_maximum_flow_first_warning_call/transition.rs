//! Pure CP438-to-CP439 first-warning-call-site transition.
...
use crate::ideal_loads::calc::PurchasedAirCalcHeatingOutdoorAirMaximumFlowFirstWarningCounterIncrementRetainedRoute as PredecessorRoute;
use crate::ideal_loads::PurchasedAirCalcHeatingOutdoorAirMaximumFlowFirstWarningCounterIncrementSnapshot as Predecessor;
...
/// One retained CP439 route over CP438's exact 36-wide base partition.
#[allow(missing_docs)]
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(in crate::ideal_loads::calc) struct PurchasedAirCalcHeatingOutdoorAirMaximumFlowFirstWarningCallRetainedRoute {
...
use PurchasedAirCalcHeatingOutdoorAirMaximumFlowFirstWarningCallRetainedRoute as Route;

pub(in crate::ideal_loads::calc) fn heating_outdoor_air_maximum_flow_first_warning_call_route_from_committed_predecessor(
    ...
    if predecessor_route.logical_index >= 36
        ...
        || predecessor
            ...
            != predecessor_route.counter_increment_executed
    {
        return None;
    }
    Some(Route {
        ...
        predecessor_counter_increment_executed: predecessor_route.counter_increment_executed,
        first_warning_call_site_reached: predecessor_route.counter_increment_executed,
    })
}

pub(in crate::ideal_loads::calc) fn advance_heating_outdoor_air_maximum_flow_first_warning_call_state(
    state: &mut State,
    predecessor: Predecessor,
) -> Option<Snapshot> {
    let predecessor_route = crate::ideal_loads::calc::heating_outdoor_air_maximum_flow_first_warning_counter_increment_snapshot_route(predecessor)?;
    let route = heating_outdoor_air_maximum_flow_first_warning_call_route_from_committed_predecessor(
        predecessor,
        predecessor_route,
    )?;
    advance_heating_outdoor_air_maximum_flow_first_warning_call_state_with_validated_route(
        state,
        predecessor,
        predecessor_route,
        route,
...
pub(in crate::ideal_loads::calc) fn advance_heating_outdoor_air_maximum_flow_first_warning_call_state_with_validated_route(
    state: &mut State,
    predecessor: Predecessor,
    predecessor_route: PredecessorRoute,
    route: Route,
