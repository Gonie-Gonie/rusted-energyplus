...
use super::transition::{
    ...
    advance_heating_outdoor_air_maximum_flow_first_warning_call_state_with_validated_route as advance_validated,
    heating_outdoor_air_maximum_flow_first_warning_call_route_from_committed_predecessor as successor_route,
};
...
use super::{
    ...
    heating_outdoor_air_maximum_flow_first_warning_call_snapshot_is_exact,
};
use crate::ideal_loads::calc::{
    PurchasedAirCalcHeatingOutdoorAirMaximumFlowFirstWarningCounterIncrementRetainedRoute as PredecessorRoute,
    cp438_all_snapshots_for_successor_tests, cp438_fixture_unit_for_successor_tests,
    heating_outdoor_air_maximum_flow_first_warning_counter_increment_snapshot_route as predecessor_route,
};
...
#[test]
fn cp439_boundary_maps_complete_warning_call_and_excludes_continue_call() {
    assert_eq!(
        PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_FIRST_WARNING_CALL_SOURCE,
        "EnergyPlus 26.1 PurchasedAirManager.cc:2366-2370"
    );
    assert_eq!(
        PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_FIRST_WARNING_CALL_FIRST_EXCLUDED_SOURCE,
        "EnergyPlus 26.1 PurchasedAirManager.cc:2371"
    );
    assert_eq!(
        PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_FIRST_WARNING_CALL_SOURCE_ORDER,
        &["reach-heating-outdoor-air-maximum-flow-first-warning-call-site"]
    );
}
...
fn exhaustive_67_routes_preserve_20_47_visibility_and_reach_three_private_calls() {
    ...
    let mut expected = [[0usize; 36]; 8];

    for predecessor in predecessors {
        let predecessor_route = route_for(predecessor);
        let route = successor_route(predecessor, predecessor_route).expect("CP439 route");
        let is_public = is_public_route(predecessor, route);
        public += usize::from(is_public);
        public_calls += usize::from(is_public && route.first_warning_call_site_reached);
        private_calls += usize::from(!is_public && route.first_warning_call_site_reached);
        record(&mut expected, route);
        let snapshot = advance(&mut state, predecessor).expect("CP439 snapshot");
        assert!(
            heating_outdoor_air_maximum_flow_first_warning_counter_increment_snapshots_match_bit_exact(
                heating_outdoor_air_maximum_flow_first_warning_call_predecessor_cp438_snapshot(
                    snapshot,
        ...
        assert!(heating_outdoor_air_maximum_flow_first_warning_call_snapshot_is_exact(snapshot));
    }
...
#[test]
fn active_call_requires_cp438_increment_and_sealed_counter_one_without_mutation() {
    let predecessor = cp438_all_snapshots_for_successor_tests()
        .into_iter()
        .find(|snapshot| {
            snapshot.heating_outdoor_air_maximum_flow_first_warning_counter_increment_executed
        })
        .expect("active CP438 increment");
    assert_eq!(
        predecessor.assigned_outdoor_air_flow_maximum_heating_output_error_count,
        Some(1)
    );
    let mut state = State::new(predecessor.system);
    let snapshot = advance(&mut state, predecessor).expect("CP439 call");
    assert!(snapshot.heating_outdoor_air_maximum_flow_first_warning_call_site_reached);
    assert_eq!(
...
#[test]
fn cp439_new_state_has_eight_zeroed_width_36_arrays_and_no_service_state() {
...
#[test]
fn cp439_transition_contains_no_sink_format_service_or_counter_mutation() {
...
#[allow(dead_code)]
pub(in crate::ideal_loads::calc) fn cp439_all_snapshots_for_successor_tests()
-> Vec<super::PurchasedAirCalcHeatingOutdoorAirMaximumFlowFirstWarningCallSnapshot> {
    cp438_all_snapshots_for_successor_tests()
        .into_iter()
        .map(|predecessor| {
            let mut state = State::new(predecessor.system);
            advance(&mut state, predecessor).expect("CP439 snapshot")
        })
        .collect()
}
...
pub(in crate::ideal_loads::calc) fn cp439_fixture_unit_for_successor_tests() -> (
    ...
    let (mut unit, predecessor, predecessor_route, _) = cp438_fixture_unit_for_successor_tests();
    let route = successor_route(predecessor, predecessor_route).expect("CP439 route");
    let mut state = State::new(predecessor.system);
    let snapshot = advance_validated(&mut state, predecessor, predecessor_route, route)
        .expect("CP439");
    unit.calc_heating_outdoor_air_maximum_flow_first_warning_call = state;
    (unit, snapshot, route)
}

fn route_for(predecessor: Predecessor) -> PredecessorRoute {
    predecessor_route(predecessor).expect("CP438 route")
}
...
fn record(expected: &mut [[usize; 36]; 8], route: Route) {
    ...
    expected[7][route.logical_index] += usize::from(route.first_warning_call_site_reached);
}
