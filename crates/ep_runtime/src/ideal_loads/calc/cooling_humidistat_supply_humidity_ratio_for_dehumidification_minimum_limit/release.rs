//! Release-bound CP361 Humidistat local humidity-ratio minimum limit.
...
use crate::ideal_loads::calc::cooling_humidistat_supply_humidity_ratio_for_dehumidification_assignment::completed_direct_cooling_humidistat_supply_humidity_ratio_for_dehumidification_assignment_is_consistent;
use crate::ideal_loads::calc::cooling_humidistat_supply_humidity_ratio_for_dehumidification_minimum_limit::transition::predecessor_snapshots_match_bit_exact;
...
// CP362 may consume this explicitly parameterized bridge. Keep it available
// while CP361 is the source-order frontier.
#[allow(dead_code)]
...
pub(in crate::ideal_loads) use snapshot_validation::cooling_humidistat_supply_humidity_ratio_for_dehumidification_minimum_limit_snapshot_is_exact_direct_release;
#[cfg(test)]
pub(in crate::ideal_loads::calc) use snapshot_validation::snapshots_match_bit_exact as snapshots_match_bit_exact_for_test;

pub(in crate::ideal_loads::calc) fn completed_direct_cooling_humidistat_supply_humidity_ratio_for_dehumidification_minimum_limit_is_consistent(
...
/// Fail-closed CP361 public release error.
#[allow(missing_docs)]
#[derive(Clone, Copy, Debug, PartialEq)]
pub enum PurchasedAirCalcCoolingHumidistatSupplyHumidityRatioForDehumidificationMinimumLimitError {
    UnknownSystem {
    ...
    CoolingHumidistatSupplyHumidityRatioForDehumidificationAssignmentSnapshotMismatch {
        system: IdealLoadsAirSystemId,
    },
...
/// Executes CP361 for the exact direct no-OA release route.
///
/// Selector `None` completes the switch before line 2231. The four CP361
/// source sites and every numeric evidence field therefore remain complete
/// skips; this wrapper accepts no numerical operands and does not validate
/// the unexecuted typed minimum-cooling humidity-ratio field.
pub fn advance_direct_no_oa_calc_cooling_humidistat_supply_humidity_ratio_for_dehumidification_minimum_limit(
    runtime: &mut PurchasedAirRuntimeState,
    system: &IdealLoadsAirSystem,
    predecessor_cp360: Predecessor,
) -> Result<
    Snapshot,
    PurchasedAirCalcCoolingHumidistatSupplyHumidityRatioForDehumidificationMinimumLimitError,
> {
    let selected = predecessor_cp360.system;
    ...
    if system.id != selected {
        return Err(
            PurchasedAirCalcCoolingHumidistatSupplyHumidityRatioForDehumidificationMinimumLimitError::SystemIdentityMismatch {
                expected: selected,
                actual: system.id,
            },
        );
    ...
    if !classify_no_oa_sensible_subset(system).is_supported() {
        return Err(
            PurchasedAirCalcCoolingHumidistatSupplyHumidityRatioForDehumidificationMinimumLimitError::SystemOutsideDirectSubset {
                system: selected,
            },
        );
    }
    if system.dehumidification_control_type != DehumidificationControlType::None {
        return Err(
            PurchasedAirCalcCoolingHumidistatSupplyHumidityRatioForDehumidificationMinimumLimitError::DehumidificationControlTypeOutsideDirectSubset {
                system: selected,
    ...
    {
        return Err(
            PurchasedAirCalcCoolingHumidistatSupplyHumidityRatioForDehumidificationMinimumLimitError::InitializationNotReady {
                system: selected,
            },
        );
    }
    ...
    if predecessor_cp360.controlled_zone != controlled_zone
        || !predecessor_snapshots_match_bit_exact(retained_predecessor, predecessor_cp360)
        || !predecessor_witness.is_some_and(|witness| {
            predecessor_snapshots_match_bit_exact(witness, predecessor_cp360)
        })
    {
        return Err(predecessor_mismatch(selected));
    }
    if !cooling_humidistat_supply_humidity_ratio_for_dehumidification_assignment_snapshot_is_exact_direct_release(
        predecessor_cp360,
    ) {
        return Err(
            PurchasedAirCalcCoolingHumidistatSupplyHumidityRatioForDehumidificationMinimumLimitError::PredecessorOutsideDirectSubset {
                system: selected,
            },
        );
    }
    ...
    {
        return Err(
            PurchasedAirCalcCoolingHumidistatSupplyHumidityRatioForDehumidificationMinimumLimitError::RuntimeStateInvariantViolation {
                system: selected,
            },
        );
    }
    ...
    debug_assert!(runtime.units.get(&selected).is_some_and(|unit| {
        completed_direct_cooling_humidistat_supply_humidity_ratio_for_dehumidification_minimum_limit_is_consistent(
            ...
            system,
            snapshot,
            runtime.cooling_humidistat_supply_humidity_ratio_for_dehumidification_minimum_limit_latest_witness(selected),
        )
    }));
    Ok(snapshot)
}
