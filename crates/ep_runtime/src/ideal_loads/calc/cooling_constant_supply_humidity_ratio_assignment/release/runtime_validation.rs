//! Persistent CP365 runtime-state validation.
...
pub(super) fn pending_state_is_consistent(
    ...
    let state = &unit.calc_cooling_constant_supply_humidity_ratio_assignment;
    let prior = &unit.calc_cooling_constant_supply_humidity_ratio_case_entry;
    state_is_consistent(state, witness, predecessor.system, selector)
        ...
        && pending_count(
            ...
            route == Route::DehumidificationControlConstantSupplyHumidityRatioAssigned,
        ) == Some(prior.dehumidification_control_constant_supply_humidity_ratio_case_entry_count)
}
...
pub(super) fn completed_state_is_consistent(
    ...
    state_is_consistent(state, witness, snapshot.system, selector)
        ...
        && state.dehumidification_control_constant_supply_humidity_ratio_assignment_count
            == prior.dehumidification_control_constant_supply_humidity_ratio_case_entry_count
        && state
            .latest
            .is_some_and(|latest| snapshots_match_bit_exact(latest, snapshot))
}

pub(in crate::ideal_loads) fn cooling_constant_supply_humidity_ratio_assignment_latest_metadata_is_consistent(
    state: &State,
...
fn state_is_consistent(
    ...
    let csh = state.dehumidification_control_constant_supply_humidity_ratio_assignment_count;
    let selector_partition_matches = c0
        ...
        && csh
            == usize::from(selector == DehumidificationControlType::ConstantSupplyHumidityRatio)
                * selected;
    let Some(expected_source_sites) = csh.checked_mul(
    ...
    let counters_match = state.system == expected_system
        ...
        && state.minimum_cooling_supply_air_humidity_ratio_read_count == csh
        && state.supply_humidity_ratio_assignment_count == csh
