//! Persistent CP366 runtime-state validation.
...
use crate::ideal_loads::calc::cooling_constant_supply_humidity_ratio_case_break::transition::{
...
pub(super) fn pending_state_is_consistent(
    ...
    let state = &unit.calc_cooling_constant_supply_humidity_ratio_case_break;
    let prior = &unit.calc_cooling_constant_supply_humidity_ratio_assignment;
    state_is_consistent(state, witness, predecessor.system, selector)
        ...
        && pending_count(
            ...
            route == Route::DehumidificationControlConstantSupplyHumidityRatioCaseBreak,
        ) == Some(prior.dehumidification_control_constant_supply_humidity_ratio_assignment_count)
}
...
pub(super) fn completed_state_is_consistent(
    ...
    state_is_consistent(state, witness, snapshot.system, selector)
        ...
        && state.dehumidification_control_constant_supply_humidity_ratio_case_break_count
            == prior.dehumidification_control_constant_supply_humidity_ratio_assignment_count
        && state
            .latest
            .is_some_and(|latest| snapshots_match_exact(latest, snapshot))
}

pub(in crate::ideal_loads) fn cooling_constant_supply_humidity_ratio_case_break_latest_metadata_is_consistent(
    state: &State,
...
fn state_is_consistent(
    ...
    let csh = state.dehumidification_control_constant_supply_humidity_ratio_case_break_count;
