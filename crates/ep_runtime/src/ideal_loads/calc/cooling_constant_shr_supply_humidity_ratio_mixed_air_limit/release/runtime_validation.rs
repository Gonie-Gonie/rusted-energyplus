//! Persistent CP356 runtime-state validation.
...
pub(super) fn pending_state_is_consistent(
    ...
    let state = &unit.calc_cooling_constant_shr_supply_humidity_ratio_mixed_air_limit;
    let prior = &unit.calc_cooling_constant_shr_supply_humidity_ratio_minimum_limit;
...
pub(in crate::ideal_loads::calc) fn next_transition_fits(
    state: &State,
    predecessor: Predecessor,
    active_operands: Option<ActiveOperands>,
) -> bool {
    predecessor_route(predecessor).is_some_and(|route| {
        pure_next_transition_fits(state, route)
            && input_fits_route(route, predecessor, active_operands)
    })
}
