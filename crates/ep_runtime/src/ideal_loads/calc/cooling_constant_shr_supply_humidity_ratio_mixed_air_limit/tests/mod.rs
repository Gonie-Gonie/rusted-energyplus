...
use super::{
    ...
    PurchasedAirCalcCoolingConstantShrSupplyHumidityRatioMixedAirLimitActiveOperands as ActiveOperands,
...
use crate::ideal_loads::calc::cooling_constant_shr_supply_humidity_ratio_minimum_limit::{
    completed_cp354_case,
    private_active_counterfactual_from_direct_release as private_active_cp355_from_direct,
};
...
pub(in crate::ideal_loads::calc) fn completed_cp355_case(
    ...
    let (mut runtime, system, cp354) =
        completed_cp354_case(cooling_demand_w, overall_availability, capacity_limit)?;
    let cp355 = advance_direct_no_oa_calc_cooling_constant_shr_supply_humidity_ratio_minimum_limit(
        &mut runtime,
        &system,
        cp354,
    )
    .ok()?;
    Some((runtime, system, cp355))
}

pub(super) fn private_active_cp355_predecessor(
    runtime: &PurchasedAirRuntimeState,
    system: &ep_model::IdealLoadsAirSystem,
    direct: Predecessor,
) -> Option<Predecessor> {
    let unit = runtime.units.get(&system.id)?;
    private_active_cp355_from_direct(runtime, unit, system, direct)
}
...
fn predecessor(route: Route, ordinal: usize) -> Predecessor {
    ...
    let before = 0.006;
    let minimum = 0.008;
    let maximum = source_shaped_two_argument_maximum(before, minimum);
    Predecessor {
        source:
            PURCHASED_AIR_CALC_COOLING_CONSTANT_SHR_SUPPLY_HUMIDITY_RATIO_MINIMUM_LIMIT_SOURCE,
        ...
        supply_humidity_ratio_for_minimum_limit_maximum_read: route == Q,
        supply_humidity_ratio_before_minimum_limit: (route == Q).then_some(before),
        minimum_cooling_supply_air_humidity_ratio_for_maximum_read: route == Q,
        minimum_cooling_supply_air_humidity_ratio: (route == Q).then_some(minimum),
        source_shaped_two_argument_maximum_evaluated: route == Q,
        maximum_supply_humidity_ratio: (route == Q).then_some(maximum),
        supply_humidity_ratio_assignment_performed: route == Q,
        assigned_supply_humidity_ratio: (route == Q).then_some(maximum),
        resulting_supply_humidity_ratio: (route == Q).then_some(maximum),
    }
}

const fn operands(mixed_air: f64) -> ActiveOperands {
    ActiveOperands {
        mixed_air_humidity_ratio: mixed_air,
    }
}
...
fn source_boundary_four_sites_and_seven_route_algebra_are_exact() {
    ...
    for (index, route) in [U, N, P, C0, Q, H, CSH].into_iter().enumerate() {
        let active = (route == Q).then_some(operands(0.007));
...
#[test]
fn active_transition_uses_only_cp355_left_and_cp329_right() {
...
fn source_shaped_minimum_preserves_right_bias_and_ieee_bits() {
    ...
    let second_nan = f64::from_bits(0x7ff8_0000_0000_00c7);
    assert_eq!(
        source_shaped_two_argument_minimum(left_nan, second_nan).to_bits(),
        second_nan.to_bits()
    );
}

#[test]
fn pure_active_transition_has_no_numeric_gate() {
    let mut predecessor = predecessor(Q, 1);
    predecessor.resulting_supply_humidity_ratio = Some(f64::from_bits(0x7ff8_0000_0000_0011));
    predecessor.assigned_supply_humidity_ratio = predecessor.resulting_supply_humidity_ratio;
    predecessor.maximum_supply_humidity_ratio = predecessor.resulting_supply_humidity_ratio;
    let right = f64::from_bits(0x7ff8_0000_0000_0022);
    let mut state = State::new(predecessor.system);
    let snapshot = advance(&mut state, predecessor, Some(operands(right))).unwrap();
    assert_eq!(
        snapshot.resulting_supply_humidity_ratio.unwrap().to_bits(),
        right.to_bits()
    );
}
...
fn bit_exact_snapshot_matching_and_active_overflow_are_transactional() {
    let mut state = State::new(IdealLoadsAirSystemId(7));
    let snapshot = advance(&mut state, predecessor(Q, 1), Some(operands(0.007))).unwrap();
    let mut forged = snapshot;
    forged.mixed_air_humidity_ratio = Some(f64::from_bits(0x7ff8_0000_0000_0011));
    ...
    macro_rules! reject_overflow {
        ($field:ident) => {{
            ...
            assert!(advance(&mut state, predecessor(Q, 1), Some(operands(0.007))).is_none());
    ...
    reject_overflow!(supply_humidity_ratio_for_mixed_air_limit_minimum_read_count);
    reject_overflow!(mixed_air_humidity_ratio_for_minimum_read_count);
