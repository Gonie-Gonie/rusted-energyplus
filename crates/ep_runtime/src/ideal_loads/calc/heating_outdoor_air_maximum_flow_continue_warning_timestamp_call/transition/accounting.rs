//! Transactional CP441 route and continue-warning timestamp call-site accounting.
...
pub(super) fn next_transition_fits(state: &State, predecessor: Predecessor, route: Route) -> bool {
    ...
    let arrays = [
        ...
        state.heating_outdoor_air_maximum_flow_continue_warning_timestamp_call_route_counts[index],
    ];
    if arrays
        .into_iter()
        .any(|count| count.checked_add(1).is_none())
    {
        return false;
    }
    let preserved = [
...
pub(super) fn increment_counts(state: &mut State, predecessor: Predecessor, route: Route) {
    ...
    if predecessor.resulting_supply_humidity_ratio.is_some() {
        state.cp440_supply_humidity_ratio_state_owner_count += 1;
        state.unchanged_supply_humidity_ratio_preservation_count += 1;
    }
    if predecessor.resulting_supply_enthalpy_j_per_kg.is_some() {
        state.cp440_supply_enthalpy_state_owner_count += 1;
        state.unchanged_supply_enthalpy_preservation_count += 1;
    }
    if predecessor.resulting_supply_temperature_c.is_some() {
        state.cp440_supply_temperature_state_owner_count += 1;
        state.unchanged_supply_temperature_preservation_count += 1;
    }
    if route.continue_warning_timestamp_call_site_reached {
        state.heating_outdoor_air_maximum_flow_continue_warning_timestamp_call_site_count += 1;
        state.source_site_execution_count += 1;
        state.cp440_outdoor_air_flow_maximum_heating_output_error_count_state_owner_count += 1;
