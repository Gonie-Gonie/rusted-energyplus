//! Exact CP441 snapshot and retained-route validation.
...
use crate::ideal_loads::calc::{
    ...
    heating_outdoor_air_maximum_flow_continue_warning_call_snapshot_route,
    heating_outdoor_air_maximum_flow_continue_warning_timestamp_call_route_from_committed_predecessor,
};
use crate::ideal_loads::{
    PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_CONTINUE_WARNING_TIMESTAMP_CALL_FIRST_EXCLUDED_SOURCE as EXCLUDED,
    PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_CONTINUE_WARNING_TIMESTAMP_CALL_SOURCE as SOURCE,
    PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_CONTINUE_WARNING_TIMESTAMP_CALL_SOURCE_ORDER as ORDER,
...
pub(super) fn snapshot_is_exact(snapshot: Snapshot) -> bool {
    let predecessor = predecessor_cp440_snapshot(snapshot);
    let Some(predecessor_route) =
        heating_outdoor_air_maximum_flow_continue_warning_call_snapshot_route(predecessor)
    else {
        return false;
    };
    let Some(route) =
    ...
    heating_outdoor_air_maximum_flow_continue_warning_call_snapshot_is_exact(predecessor)
...
pub(super) fn prefix_and_local_shape_match(
    ...
    snapshot.source == SOURCE
        ...
        && heating_outdoor_air_maximum_flow_continue_warning_call_snapshots_match_bit_exact(
            predecessor_cp440_snapshot(snapshot),
            predecessor,
        )
        && heating_outdoor_air_maximum_flow_continue_warning_timestamp_call_route_from_committed_predecessor(
            predecessor,
        ...
        && snapshot.heating_outdoor_air_maximum_flow_first_warning_call_site_reached
            == route.predecessor_first_warning_call_site_reached
        && snapshot.heating_outdoor_air_maximum_flow_continue_warning_call_site_reached
            == route.predecessor_continue_warning_call_site_reached
...
pub(super) fn snapshots_match_bit_exact(left: Snapshot, right: Snapshot) -> bool {
    heating_outdoor_air_maximum_flow_continue_warning_call_snapshots_match_bit_exact(
        predecessor_cp440_snapshot(left),
        predecessor_cp440_snapshot(right),
    ) && left.heating_outdoor_air_maximum_flow_continue_warning_timestamp_call_site_reached
        == right.heating_outdoor_air_maximum_flow_continue_warning_timestamp_call_site_reached
}
