//! Persistent CP441 state and CP440-accounting validation.
...
pub(super) fn state_counts_are_consistent(state: &State) -> bool {
    ...
    let Some(continue_calls) = checked_sum(&state.predecessor_continue_warning_call_route_counts)
    else {
        return false;
    };
    let Some(timestamp_calls) = checked_sum(
        &state.heating_outdoor_air_maximum_flow_continue_warning_timestamp_call_route_counts,
    ) else {
        return false;
    };
    for index in 0..36 {
        ...
        let continue_call = state.predecessor_continue_warning_call_route_counts[index];
        let timestamp_call = state
            .heating_outdoor_air_maximum_flow_continue_warning_timestamp_call_route_counts[index];
        ...
        if !predecessor_exact
            ...
            || timestamp_call != continue_call
        {
            return false;
        }
    }
    let Some(inactive) = transitions.checked_sub(timestamp_calls) else {
        return false;
    };
    state.transition_count == transitions
        ...
        && timestamp_calls == continue_calls
        ...
        && state.heating_outdoor_air_maximum_flow_continue_warning_timestamp_call_site_count
            == timestamp_calls
        && state.source_site_execution_count == timestamp_calls
