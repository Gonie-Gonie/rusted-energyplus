//! Exact CP396 route preservation and compressed CP397 route reconstruction.
...
pub(in crate::ideal_loads::calc) fn predecessor_route(
    ...
    let route = crate::ideal_loads::calc::cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_case_break_snapshot_route(predecessor)?;
    Some(RetainedRoute {
...
pub(in crate::ideal_loads::calc) fn compressed_snapshot_route(
    ...
    let constant_shr = predecessor_index_is_constant_shr(index);
    let humidistat = predecessor_index_is_humidistat(index);
    ...
    if snapshot
        ...
        || snapshot.predecessor_dehumidification_control_humidistat_case_exited_via_break
            != humidistat
        || snapshot.dehumidification_control_none_case_entered != active
    {
        return None;
    }
    Some(RetainedRoute {
...
pub(in crate::ideal_loads::calc) const fn predecessor_index_is_active(index: usize) -> bool {
    matches!(index, 20 | 24 | 27)
}

const fn predecessor_index_is_constant_shr(index: usize) -> bool {
    matches!(index, 18 | 22 | 28)
}

const fn predecessor_index_is_humidistat(index: usize) -> bool {
