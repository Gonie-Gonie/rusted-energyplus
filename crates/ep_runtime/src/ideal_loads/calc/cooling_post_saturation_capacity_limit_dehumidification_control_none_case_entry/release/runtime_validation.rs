//! Persistent CP397 runtime-state validation.
...
pub(super) fn calc_state_identities_match(
    ...
    unit.system == system
        ...
        && unit.calc_cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_case_break.system == system
        && unit.calc_cooling_post_saturation_capacity_limit_dehumidification_control_none_case_entry.system == system
}
...
pub(super) fn call_order_is_pending(
    ...
    unit.calc_cooling_post_saturation_capacity_limit_dehumidification_control_none_case_entry
        ...
        && unit.calc_cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_case_break.transition_count
            == ordinal
}
...
pub(super) fn prepare_next_transition(
    ...
    let snapshot = advance_cooling_post_saturation_capacity_limit_dehumidification_control_none_case_entry_state(
        &mut next,
        predecessor,
    )?;
    Some((next, snapshot))
...
pub(super) fn prepared_completed_state_is_consistent(
    ...
    let predecessor = &unit.calc_cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_case_break;
...
#[allow(dead_code)]
pub(in crate::ideal_loads) fn cooling_post_saturation_capacity_limit_dehumidification_control_none_case_entry_latest_metadata_is_consistent(
    unit: &PurchasedAirUnitRuntimeState,
...
fn predecessor_counts_match(state: &State, predecessor: &PredecessorState) -> bool {
    state.predecessor_route_counts == predecessor.predecessor_route_counts
        && state.transition_count == predecessor.transition_count
        && checked_selected_sum(&predecessor.predecessor_route_counts, &[20, 24, 27])
            == Some(state.dehumidification_control_none_case_entry_count)
}
...
fn state_is_consistent(
    ...
    let active_total = checked_selected_sum(&state.predecessor_route_counts, &[20, 24, 27]);
    let entries = state.dehumidification_control_none_case_entry_count;
    let expected_sites = entries.checked_mul(
        PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_CONTROL_NONE_CASE_ENTRY_SOURCE_ORDER.len(),
    );
