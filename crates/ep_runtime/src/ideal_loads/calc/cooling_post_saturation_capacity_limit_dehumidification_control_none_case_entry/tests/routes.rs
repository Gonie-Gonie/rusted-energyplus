//! CP397 exhaustive route, case-entry, and carrier tests.
...
#[test]
fn cp397_preserves_thirty_routes_and_enters_none_exactly_three_times() {
    let chains = fixtures::all_chains();
    let mut state = State::new(chains[0].cp396.system);
    let snapshots: Vec<_> = chains
        .iter()
        .map(|chain| advance(&mut state, chain.cp396).expect("CP397 route"))
        .collect();
    ...
    for (index, (chain, snapshot)) in chains.into_iter().zip(snapshots).enumerate() {
        ...
        assert_eq!(snapshot.dehumidification_control_none_case_entered, none);
        assert_eq!(
