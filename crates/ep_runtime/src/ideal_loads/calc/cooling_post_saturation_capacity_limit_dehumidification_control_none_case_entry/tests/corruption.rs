//! CP397 predecessor, route, case-entry, and carrier corruption rejection.

use super::*;
use ep_model::DehumidificationControlType as D;

type Snapshot = PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationControlNoneCaseEntrySnapshot;
...
fn active_case() -> (fixtures::Chain, Snapshot) {
    ...
    let mut state = State::new(chain.cp396.system);
    let snapshot = advance(&mut state, chain.cp396).expect("active CP397");
    (chain, snapshot)
}

#[test]
fn malformed_or_wrong_identity_cp396_predecessor_rejects_without_mutation() {
    let chain = active_case().0;
    let mut bad_source = chain.cp396;
    bad_source.source = "forged CP396 source";
    let mut bad_excluded = chain.cp396;
    bad_excluded.first_excluded_source = "forged CP396 excluded source";
    let mut bad_order = chain.cp396;
    bad_order.source_order = &["forged CP396 source order"];
    let mut bad_carrier = chain.cp396;
    bad_carrier.resulting_supply_enthalpy_j_per_kg =
        flip(bad_carrier.resulting_supply_enthalpy_j_per_kg);
    let mut bad_system = chain.cp396;
...
fn active_control_entry_and_carrier_corruptions_are_rejected() {
    ...
    let mutations: &[fn(&mut Snapshot)] = &[
        ...
        |value| value.dehumidification_control_none_case_entered = false,
        |value| {
            value.predecessor_cp396_resulting_supply_humidity_ratio = Some(0.008)
        },
        |value| {
            value.predecessor_cp396_resulting_supply_enthalpy_j_per_kg =
                flip(value.predecessor_cp396_resulting_supply_enthalpy_j_per_kg)
        },
        |value| {
            value.predecessor_cp396_resulting_supply_temperature_c =
                flip(value.predecessor_cp396_resulting_supply_temperature_c)
        },
...
fn inactive_routes_reject_forged_entries_and_carrier_presence() {
    ...
    let mut retained_state = State::new(retained.cp396.system);
    let retained_snapshot = advance(&mut retained_state, retained.cp396).expect("retained-W CP397");
    assert_corruptions_rejected(
        retained_snapshot,
        &[|value| value.dehumidification_control_none_case_entered = true],
    );

    let empty = fixtures::chain(3, 1, true, Some(D::None), 1, 0.7, 18.0, 1.0);
    let mut empty_state = State::new(empty.cp396.system);
    let empty_snapshot = advance(&mut empty_state, empty.cp396).expect("empty-W CP397");
    assert_corruptions_rejected(
        empty_snapshot,
        &[
            |value| value.predecessor_cp396_resulting_supply_humidity_ratio = Some(0.008),
            |value| value.resulting_supply_humidity_ratio = Some(0.008),
        ],
    );
}
