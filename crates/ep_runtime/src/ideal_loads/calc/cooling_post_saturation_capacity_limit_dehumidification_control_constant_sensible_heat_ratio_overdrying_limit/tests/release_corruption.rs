//! CP391 public-release fail-closed and transactional tests.
...
use crate::ideal_loads::{
    ...
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationControlConstantSensibleHeatRatioOverdryingLimitError as Error,
...
fn identity_subset_and_initialization_failures_preserve_cp391_state_and_witness() {
    let (runtime, system, predecessor) = completed_cp390_case();
...
fn supplied_latest_and_witness_predecessor_corruption_is_transactional() {
    ...
    forged_argument.source = "forged CP390 source";
    ...
    latest_runtime
        ...
        .as_mut()
        .expect("CP390 latest")
        .source = "forged CP390 latest source";
    assert_rejected_transactionally(&mut latest_runtime, &system, predecessor, |error| {
        matches!(
                error,
                Error::CoolingPostSaturationCapacityLimitDehumidificationControlConstantSensibleHeatRatioSupplyTemperatureMixedAirLimitSnapshotMismatch { .. }
            )
    });
    ...
    let mut witness = witness_runtime
        ...
        .expect("CP390 witness");
    witness.source = "forged CP390 witness source";
    witness_runtime.set_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_supply_temperature_mixed_air_limit_latest_witness(
        system.id,
        witness,
    );
...
fn call_order_and_replay_failures_preserve_cp391_state_and_witness() {
    ...
    let released = advance_direct_no_oa_calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_overdrying_limit(
        &mut replay_runtime,
        &system,
        predecessor,
    )
    .expect("first CP391 release");
    assert!(cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_overdrying_limit_snapshot_is_exact_direct_release(
        released,
    ));
...
fn public_release_counter_overflow_preserves_cp391_state_and_witness() {
    let (mut runtime, system, predecessor) = completed_cp390_case();
    runtime
        .units
        .get_mut(&system.id)
        .expect("known unit")
...
fn completed_cp390_case() -> (PurchasedAirRuntimeState, IdealLoadsAirSystem, Cp390) {
...
fn assert_rejected_transactionally(
    ...
    let before_state = runtime
        ...
        .calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_overdrying_limit
        .clone();
    let before_witness = runtime.cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_overdrying_limit_latest_witness(selected);

    let error = advance_direct_no_oa_calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_overdrying_limit(
        runtime,
        system,
        predecessor,
    )
    .expect_err("CP391 release must fail");
    assert!(expected(&error), "unexpected CP391 error: {error:?}");
    ...
    let after_witness = runtime.cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_overdrying_limit_latest_witness(selected);
...
fn assert_witness_unchanged(
    ...
    assert_eq!(
        before.is_some(),
        after.is_some(),
        "CP391 witness presence changed after rejected release"
    );
    if let (Some(before), Some(after)) = (before, after) {
        assert!(cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_overdrying_limit_snapshots_match_bit_exact(
            before,
            after,
        ));
    }
