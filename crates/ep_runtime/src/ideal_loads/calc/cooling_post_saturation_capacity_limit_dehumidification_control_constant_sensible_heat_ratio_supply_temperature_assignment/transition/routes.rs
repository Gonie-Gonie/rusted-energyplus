//! Exact CP388 route refinement for CP389.

use ep_model::DehumidificationControlType as D;

use crate::ideal_loads::PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationControlConstantSensibleHeatRatioSensibleOutputAssignmentSnapshot as Predecessor;
...
pub(in crate::ideal_loads::calc) fn predecessor_route(
    ...
    if !crate::ideal_loads::calc::cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_sensible_output_assignment_snapshot_is_exact(predecessor) {
        return None;
    }
    ...
    if stage < 3 {
        return retained(3 + lineage * 3 + stage, false);
    }
    ...
    retained(index, selector == D::ConstantSensibleHeatRatio)
}

const fn retained(predecessor_index: usize, active: bool) -> Option<RetainedRoute> {
    Some(RetainedRoute {
        predecessor_index,
        active,
    })
