...
use crate::ideal_loads::calc::{
    ...
    PurchasedAirCalcCoolingSupplyHumidityRatioHumidificationSupplyHumidityRatioForHumidificationMaximumLimitActiveOperands as Cp374Operands,
    PurchasedAirCalcCoolingSupplyHumidityRatioHumidificationSupplyHumidityRatioForHumidificationMaximumLimitRuntimeState as Cp374State,
    ...
    advance_cooling_supply_humidity_ratio_humidification_supply_humidity_ratio_for_humidification_maximum_limit_state as advance_cp374,
};
...
#[test]
fn cp375_source_boundary_and_dependency_order_are_exact() {
    assert_eq!(
        PURCHASED_AIR_CALC_COOLING_SUPPLY_HUMIDITY_RATIO_HUMIDIFICATION_SUPPLY_HUMIDITY_RATIO_MAXIMUM_ASSIGNMENT_SOURCE,
...
fn active_cp374(
    ...
    local_before_limit: f64,
    maximum_limit: f64,
) -> Predecessor {
    cp374(
        selector,
        HumidificationControlType::Humidistat,
        Some(local_before_limit),
        Some(Cp373Operands {
            supply_mass_flow_rate_kg_per_s: 1.0,
            zone_node_humidity_ratio: 0.0,
        }),
        Some(Cp374Operands {
            maximum_heating_supply_air_humidity_ratio: maximum_limit,
        }),
    )
}

fn skipped_cp374() -> Predecessor {
    cp374(
...
fn cp374(
    ...
    demand: Option<f64>,
    cp373_operands: Option<Cp373Operands>,
    cp374_operands: Option<Cp374Operands>,
) -> Predecessor {
    cp374_from_cp369(
...
fn cp374_from_cp369(
    ...
    let cp373 = advance_cp373(&mut cp373_state, cp372, cp373_operands)
        .expect("valid CP373 fixture");
    let mut cp374_state = Cp374State::new(cp373.system);
    advance_cp374(&mut cp374_state, cp373, cp374_operands).expect("valid CP374 fixture")
}
...
fn active_cp369(selector: DehumidificationControlType) -> Cp369Snapshot {
    Cp369Snapshot {
        ...
        predecessor_dehumidification_control_none_case_completed_skip: selector == DehumidificationControlType::None,
        predecessor_dehumidification_control_constant_sensible_heat_ratio_case_completed_skip: selector == DehumidificationControlType::ConstantSensibleHeatRatio,
        predecessor_dehumidification_control_humidistat_case_completed_skip: selector == DehumidificationControlType::Humidistat,
        predecessor_dehumidification_control_constant_supply_humidity_ratio_case_completed_skip: selector == DehumidificationControlType::ConstantSupplyHumidityRatio,
        predecessor_dehumidification_control_default_supply_humidity_ratio_case_exited_via_break: false,
        dehumidification_control_none_case_completed_skip: selector == DehumidificationControlType::None,
        dehumidification_control_constant_sensible_heat_ratio_case_completed_skip: selector == DehumidificationControlType::ConstantSensibleHeatRatio,
        dehumidification_control_humidistat_case_completed_skip: selector == DehumidificationControlType::Humidistat,
        dehumidification_control_constant_supply_humidity_ratio_case_completed_skip: selector == DehumidificationControlType::ConstantSupplyHumidityRatio,
