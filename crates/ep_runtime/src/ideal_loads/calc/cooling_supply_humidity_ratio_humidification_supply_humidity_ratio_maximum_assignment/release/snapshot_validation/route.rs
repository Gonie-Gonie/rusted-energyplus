//! CP375 source provenance and eight-route structural validation.
...
pub(super) fn structural_route(snapshot: Snapshot) -> Option<Route> {
    ...
    let route = match cooling_supply_humidity_ratio_humidification_dehumidification_control_humidistat_or_none_guard_snapshot_route(
        ...
        Cp371Route::DehumidificationControlHumidistatBodyEntered => Route::DehumidificationControlHumidistatSupplyHumidityRatioMaximumAssignmentExecuted,
        Cp371Route::DehumidificationControlNoneBodyEntered => Route::DehumidificationControlNoneSupplyHumidityRatioMaximumAssignmentExecuted,
        Cp371Route::DehumidificationControlGuardFalseFallthrough => Route::DehumidificationControlGuardFalseFallthrough,
    };
    let predecessor_h = snapshot.predecessor_dehumidification_control_humidistat_supply_humidity_ratio_for_humidification_maximum_limit_executed;
    let predecessor_n = snapshot.predecessor_dehumidification_control_none_supply_humidity_ratio_for_humidification_maximum_limit_executed;
    let local_h = snapshot.dehumidification_control_humidistat_supply_humidity_ratio_maximum_assignment_executed;
    let local_n = snapshot.dehumidification_control_none_supply_humidity_ratio_maximum_assignment_executed;
    let flags_match = match route {
        Route::DehumidificationControlHumidistatSupplyHumidityRatioMaximumAssignmentExecuted => predecessor_h && !predecessor_n && local_h && !local_n,
        Route::DehumidificationControlNoneSupplyHumidityRatioMaximumAssignmentExecuted => !predecessor_h && predecessor_n && !local_h && local_n,
