//! Persistent CP440 state and CP439-accounting validation.
...
pub(super) fn state_counts_are_consistent(state: &State) -> bool {
    ...
    let Some(first_calls) = checked_sum(&state.predecessor_first_warning_call_route_counts) else {
        return false;
    };
    let Some(continue_calls) =
    ...
    for index in 0..36 {
        ...
        let first_call = state.predecessor_first_warning_call_route_counts[index];
        let continue_call =
        ...
        if !predecessor_exact
            ...
            || first_call != increment
            || continue_call != first_call
        {
            return false;
        }
    }
    let Some(inactive) = transitions.checked_sub(continue_calls) else {
        return false;
    };
