//! Exact CP393 route preservation and compressed CP394 reconstruction.

use crate::ideal_loads::{
    PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_CONTROL_CONSTANT_SENSIBLE_HEAT_RATIO_CASE_BREAK_FIRST_EXCLUDED_SOURCE as PREDECESSOR_EXCLUDED,
    PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_CONTROL_CONSTANT_SENSIBLE_HEAT_RATIO_CASE_BREAK_SOURCE as PREDECESSOR_SOURCE,
    PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_CONTROL_CONSTANT_SENSIBLE_HEAT_RATIO_CASE_BREAK_SOURCE_ORDER as PREDECESSOR_ORDER,
...
pub(in crate::ideal_loads::calc) fn predecessor_route(
    ...
    let route = crate::ideal_loads::calc::cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_case_break_snapshot_route(predecessor)?;
    Some(RetainedRoute {
...
pub(in crate::ideal_loads::calc) fn compressed_snapshot_route(
    ...
    let route = crate::ideal_loads::calc::cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_case_break_snapshot_route(predecessor)?;
    let active = predecessor_index_is_active(route.predecessor_index);
    if snapshot.dehumidification_control_humidistat_case_entered != active {
        return None;
    }
    Some(RetainedRoute {
        predecessor_index: route.predecessor_index,
...
fn predecessor_snapshot(snapshot: Snapshot) -> Predecessor {
    let predecessor_break = snapshot
    ...
    Predecessor {
        ...
        predecessor_dehumidification_control_constant_sensible_heat_ratio_supply_humidity_ratio_assignment_executed: predecessor_break,
        predecessor_cp392_resulting_supply_humidity_ratio: snapshot.predecessor_cp393_resulting_supply_humidity_ratio,
        predecessor_cp392_resulting_supply_enthalpy_j_per_kg: snapshot.predecessor_cp393_resulting_supply_enthalpy_j_per_kg,
        predecessor_cp392_resulting_supply_temperature_c: snapshot.predecessor_cp393_resulting_supply_temperature_c,
        dehumidification_control_constant_sensible_heat_ratio_case_exited_via_break: predecessor_break,
        resulting_supply_humidity_ratio: snapshot.predecessor_cp393_resulting_supply_humidity_ratio,
        resulting_supply_enthalpy_j_per_kg: snapshot.predecessor_cp393_resulting_supply_enthalpy_j_per_kg,
        resulting_supply_temperature_c: snapshot.predecessor_cp393_resulting_supply_temperature_c,
    }
}
