//! CP394 public-release lifecycle and transactional tests.
...
#[test]
fn direct_release_skips_humidistat_entry_and_retains_lifecycle_metadata() {
    let (mut runtime, system, predecessor) = completed_cp393_case();
    let snapshot = advance_direct_no_oa_calc_cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_case_entry(
        &mut runtime,
        &system,
        predecessor,
    )
    .expect("CP394 direct release");
    assert!(cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_case_entry_snapshot_is_exact_direct_release(snapshot));
    assert!(!snapshot.dehumidification_control_humidistat_case_entered);
    let summary = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_case_entry_lifecycle_summary(
        &runtime,
        system.id,
    )
...
fn identity_predecessor_replay_and_overflow_fail_transactionally() {
    let (runtime, system, predecessor) = completed_cp393_case();
    ...
    forged.source = "forged CP393 source";
    ...
    assert!(advance_direct_no_oa_calc_cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_case_entry(
        &mut replay_runtime,
        &system,
        predecessor,
    )
...
fn completed_cp393_case() -> (PurchasedAirRuntimeState, IdealLoadsAirSystem, Cp393) {
