//! Exact CP405 snapshot, direct-route, and binary64 validation.
...
pub(in crate::ideal_loads) fn cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_latent_output_maximum_capacity_assignment_snapshot_is_exact_direct_release(
    snapshot: Snapshot,
) -> bool {
    snapshot_route(snapshot).is_some_and(|route| {
        predecessor_index_is_public(route.predecessor_index)
            && crate::ideal_loads::cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_latent_output_supply_humidity_ratio_assignment_snapshot_is_exact_direct_release(
...
pub(in crate::ideal_loads::calc) fn snapshots_match_bit_exact(
    ...
    let values_match = compare_clear!(predecessor_cp397_resulting_supply_humidity_ratio)
        ...
        && compare_clear!(predecessor_cp404_supply_temperature_c)
        && compare_clear!(predecessor_cp404_supply_enthalpy_j_per_kg)
        && compare_clear!(predecessor_cp404_psychrometric_supply_humidity_ratio)
        && compare_clear!(predecessor_cp404_assigned_supply_humidity_ratio)
        && compare_clear!(predecessor_cp404_resulting_supply_humidity_ratio)
        && compare_clear!(predecessor_cp404_resulting_supply_enthalpy_j_per_kg)
        && compare_clear!(predecessor_cp404_resulting_supply_temperature_c)
        && compare_clear!(preexisting_cooling_latent_output_w)
        && compare_clear!(maximum_total_cooling_capacity_w)
        && compare_clear!(assigned_cooling_latent_output_w)
        && compare_clear!(resulting_cooling_latent_output_w)
