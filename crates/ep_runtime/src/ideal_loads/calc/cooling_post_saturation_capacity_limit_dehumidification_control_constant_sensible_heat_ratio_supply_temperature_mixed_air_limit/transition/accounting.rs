//! Exact CP390 route, retained-state, and source-site accounting.

use super::State;
use super::routes::{RetainedRoute, predecessor_has_supply_temperature};
use crate::ideal_loads::PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_CONTROL_CONSTANT_SENSIBLE_HEAT_RATIO_SUPPLY_TEMPERATURE_MIXED_AIR_LIMIT_SOURCE_ORDER;
...
pub(super) fn increment_counts(state: &mut State, route: RetainedRoute) {
    ...
    if predecessor_has_supply_temperature(route.predecessor_index) {
        state.cp389_supply_temperature_state_owner_count += 1;
    }
    if !route.active {
    ...
    state.mixed_air_temperature_bit_corroboration_count += 1;
