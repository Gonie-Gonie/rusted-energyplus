//! CP390 public-release fail-closed and transactional tests.
...
use crate::ideal_loads::{
    ...
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationControlConstantSensibleHeatRatioSupplyTemperatureMixedAirLimitError as Error,
...
#[test]
fn identity_subset_and_initialization_failures_preserve_cp390_state_and_witness() {
    let (runtime, system, predecessor) = completed_cp389_case();
...
fn supplied_latest_and_witness_predecessor_corruption_is_transactional() {
    ...
    forged_argument.source = "forged CP389 source";
    ...
    latest_runtime
        ...
        .as_mut()
        .expect("CP389 latest")
        .source = "forged CP389 latest source";
    assert_rejected_transactionally(&mut latest_runtime, &system, predecessor, |error| {
        matches!(
                error,
                Error::CoolingPostSaturationCapacityLimitDehumidificationControlConstantSensibleHeatRatioSupplyTemperatureAssignmentSnapshotMismatch { .. }
            )
    });
    ...
    let mut witness = witness_runtime
        ...
        .expect("CP389 witness");
    witness.source = "forged CP389 witness source";
    witness_runtime.set_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_supply_temperature_assignment_latest_witness(
        system.id,
        witness,
    );
...
#[test]
fn call_order_and_replay_failures_preserve_cp390_state_and_witness() {
    let (runtime, system, predecessor) = completed_cp389_case();
    ...
    let released = advance_direct_no_oa_calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_supply_temperature_mixed_air_limit(
        &mut replay_runtime,
        &system,
        predecessor,
    )
    .expect("first CP390 release");
    assert!(cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_supply_temperature_mixed_air_limit_snapshot_is_exact_direct_release(
        released,
    ));
...
fn completed_cp389_case() -> (PurchasedAirRuntimeState, IdealLoadsAirSystem, Cp389) {
...
fn assert_rejected_transactionally(
    ...
    let before_state = runtime
        ...
        .clone();
    let before_witness = runtime.cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_supply_temperature_mixed_air_limit_latest_witness(selected);

    let error = advance_direct_no_oa_calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_supply_temperature_mixed_air_limit(
        runtime,
        system,
        predecessor,
    )
    .expect_err("CP390 release must fail");
    assert!(expected(&error), "unexpected CP390 error: {error:?}");
    ...
    let after_witness = runtime.cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_supply_temperature_mixed_air_limit_latest_witness(selected);
...
fn assert_witness_unchanged(
    ...
    assert_eq!(
        before.is_some(),
        after.is_some(),
        "CP390 witness presence changed after rejected release"
    );
