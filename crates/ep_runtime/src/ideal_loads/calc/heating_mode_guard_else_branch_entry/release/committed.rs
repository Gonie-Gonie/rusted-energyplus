//! Sealed bounded CP433 route capability for CP434.
...
/// Returns CP433's committed route without recursive snapshot-route replay.
pub(in crate::ideal_loads::calc) fn heating_mode_guard_else_branch_entry_committed_latest_route(
    ...
    let predecessor = predecessor_cp432_snapshot(latest);
    ...
    (state.system == unit.system
        ...
        && predecessor_counts_match(state, &unit.calc_heating_operating_mode_heat_assignment))
    .then_some(route)
}
