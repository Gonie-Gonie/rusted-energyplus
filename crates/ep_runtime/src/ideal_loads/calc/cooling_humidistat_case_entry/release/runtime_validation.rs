//! Persistent CP358 runtime-state validation.
...
use crate::ideal_loads::calc::cooling_humidistat_case_entry::transition::{
    next_transition_fits as pure_next_transition_fits, predecessor_route,
};
use crate::ideal_loads::{
    PurchasedAirCalcCoolingConstantShrCaseBreakSnapshot as Predecessor,
...
pub(super) fn call_order_is_pending(
    ...
    unit.calc_cooling_humidistat_case_entry
        ...
        && unit.calc_cooling_constant_shr_case_break.transition_count == ordinal
}
...
pub(super) fn pending_state_is_consistent(
    ...
    let state = &unit.calc_cooling_humidistat_case_entry;
    let prior = &unit.calc_cooling_constant_shr_case_break;
    state_is_consistent(state, witness, predecessor.system, selector)
        ...
        ) == Some(prior.dehumidification_control_constant_sensible_heat_ratio_case_break_count)
        && pending_count(
...
pub(super) fn completed_state_is_consistent(
    ...
    state_is_consistent(state, witness, snapshot.system, selector)
        ...
        && state.dehumidification_control_constant_sensible_heat_ratio_case_completed_skip_count
            == prior.dehumidification_control_constant_sensible_heat_ratio_case_break_count
        && state.dehumidification_control_humidistat_case_entry_count
...
fn state_is_consistent(
    ...
    let Some(recursively_witnessed) = checked_sum(&[
        ...
        state.witnessed_dehumidification_control_humidistat_case_entry_count,
        state
            .witnessed_dehumidification_control_constant_supply_humidity_ratio_case_selected_skip_count,
    ]) else {
        return false;
    };
    let none_completed = state.dehumidification_control_none_case_completed_skip_count;
    let constant_shr_completed =
        state.dehumidification_control_constant_sensible_heat_ratio_case_completed_skip_count;
    let humidistat_entry = state.dehumidification_control_humidistat_case_entry_count;
    let later_case_skip =
        state.dehumidification_control_constant_supply_humidity_ratio_case_selected_skip_count;
    let Some(completed_before_entry) = none_completed.checked_add(constant_shr_completed) else {
        return false;
    };
    let Some(at_or_after_entry) = humidistat_entry.checked_add(later_case_skip) else {
        return false;
    };
    let Some(control_flow_partition) = completed_before_entry.checked_add(at_or_after_entry) else {
        return false;
    };
    ...
    let counters_match = state.system == expected_system
        ...
        && state.source_site_execution_count == humidistat_entry
