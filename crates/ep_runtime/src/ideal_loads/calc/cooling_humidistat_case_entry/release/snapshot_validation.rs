//! Exact CP358 snapshot validation.
...
pub(in crate::ideal_loads) fn cooling_humidistat_case_entry_snapshot_is_exact_direct_release(
    snapshot: Snapshot,
) -> bool {
...
pub(super) fn snapshot_route(snapshot: Snapshot) -> Option<Route> {
    ...
    let local_count = usize::from(snapshot.dehumidification_control_none_case_completed_skip)
        ...
        + usize::from(snapshot.dehumidification_control_humidistat_case_entered)
        + usize::from(
            snapshot.dehumidification_control_constant_supply_humidity_ratio_case_selected_skip,
        );
...
fn provenance_is_exact(snapshot: Snapshot) -> bool {
    snapshot.source == PURCHASED_AIR_CALC_COOLING_HUMIDISTAT_CASE_ENTRY_SOURCE
        && snapshot.first_excluded_source
            == PURCHASED_AIR_CALC_COOLING_HUMIDISTAT_CASE_ENTRY_FIRST_EXCLUDED_SOURCE
        && snapshot.source_order == PURCHASED_AIR_CALC_COOLING_HUMIDISTAT_CASE_ENTRY_SOURCE_ORDER
}
