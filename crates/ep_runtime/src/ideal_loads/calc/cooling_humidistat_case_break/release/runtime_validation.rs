//! Persistent CP363 runtime-state validation.
...
use crate::ideal_loads::calc::cooling_humidistat_case_break::transition::{
...
pub(super) fn pending_state_is_consistent(
    ...
    let state = &unit.calc_cooling_humidistat_case_break;
    let prior = &unit.calc_cooling_humidistat_supply_humidity_ratio_mixed_air_limit;
...
pub(in crate::ideal_loads) fn cooling_humidistat_case_break_latest_metadata_is_consistent(
    state: &State,
...
fn state_is_consistent(
    ...
    let earlier_case_skip =
        state.dehumidification_control_constant_sensible_heat_ratio_case_completed_skip_count;
    let break_flow = state.dehumidification_control_humidistat_case_break_count;
    let later_case_skip =
        state.dehumidification_control_constant_supply_humidity_ratio_case_selected_skip_count;
    let Some(after_none_case) = checked_sum(&[earlier_case_skip, break_flow, later_case_skip])
    else {
        return false;
    };
    let Some(control_flow_partition) = completed_skip.checked_add(after_none_case)
    else {
        return false;
    };
    let selected_counts_match = state.dehumidification_control_none_case_completed_skip_count
        ...
        && state.dehumidification_control_constant_sensible_heat_ratio_case_completed_skip_count
            == usize::from(selector == DehumidificationControlType::ConstantSensibleHeatRatio) * selected
        && break_flow
            == usize::from(selector == DehumidificationControlType::Humidistat)
                * selected
        && state.dehumidification_control_constant_supply_humidity_ratio_case_selected_skip_count
