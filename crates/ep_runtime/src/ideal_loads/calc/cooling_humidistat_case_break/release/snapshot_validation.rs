//! Exact CP363 snapshot validation.
...
pub(in crate::ideal_loads) fn cooling_humidistat_case_break_snapshot_is_exact_direct_release(
    snapshot: Snapshot,
) -> bool {
...
pub(super) fn snapshot_route(snapshot: Snapshot) -> Option<Route> {
    ...
    let local_count = usize::from(snapshot.dehumidification_control_none_case_completed_skip)
        + usize::from(snapshot.dehumidification_control_constant_sensible_heat_ratio_case_completed_skip)
        + usize::from(
            snapshot.dehumidification_control_humidistat_case_exited_via_break,
        )
        + usize::from(
    ...
    match snapshot.predecessor_dehumidification_control_type? {
        ...
        DehumidificationControlType::ConstantSensibleHeatRatio
            if snapshot.predecessor_dehumidification_control_constant_sensible_heat_ratio_case_completed_skip
                && snapshot.dehumidification_control_constant_sensible_heat_ratio_case_completed_skip =>
        {
...
fn provenance_is_exact(snapshot: Snapshot) -> bool {
    snapshot.source == PURCHASED_AIR_CALC_COOLING_HUMIDISTAT_CASE_BREAK_SOURCE
