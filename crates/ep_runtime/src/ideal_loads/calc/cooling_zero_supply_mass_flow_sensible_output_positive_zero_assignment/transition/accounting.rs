//! Transactional CP428 route and source-site accounting.
...
pub(super) fn next_transition_fits(state: &State, predecessor: Predecessor, route: Route) -> bool {
    ...
    if route.assignment_executed {
        state
            ...
            .checked_add(1)
            .is_some()
            && state.zero_supply_mass_flow_sensible_output_positive_zero_assignment_route_counts[index]
                .checked_add(1)
                .is_some()
            && state.source_site_execution_count.checked_add(1).is_some()
            && state.cp428_cooling_sensible_output_state_owner_count.checked_add(1).is_some()
            && state.cooling_sensible_output_assignment_write_count.checked_add(1).is_some()
    } else {
...
pub(super) fn increment_counts(state: &mut State, predecessor: Predecessor, route: Route) {
    ...
    if route.assignment_executed {
        state.zero_supply_mass_flow_sensible_output_positive_zero_assignment_count += 1;
        state.zero_supply_mass_flow_sensible_output_positive_zero_assignment_route_counts[index] += 1;
        state.source_site_execution_count += 1;
        state.cp428_cooling_sensible_output_state_owner_count += 1;
