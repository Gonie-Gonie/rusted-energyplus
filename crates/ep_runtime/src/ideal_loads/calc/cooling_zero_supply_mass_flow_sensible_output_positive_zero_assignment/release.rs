//! Release-bound CP428 zero-flow sensible-output positive-zero assignment.
...
use super::transition::{
    ...
    advance_cooling_zero_supply_mass_flow_sensible_output_positive_zero_assignment_state_with_validated_route as advance_with_route,
...
/// Losslessly reconstructs the CP427 predecessor retained in CP428.
#[must_use]
pub fn cooling_zero_supply_mass_flow_sensible_output_positive_zero_assignment_predecessor_cp427_snapshot(
    snapshot: Snapshot,
) -> Predecessor {
    prefix::predecessor_cp427_snapshot(snapshot)
}

/// Executes CP428 for the exact direct no-OA release route.
pub fn advance_direct_no_oa_calc_cooling_zero_supply_mass_flow_sensible_output_positive_zero_assignment(
    runtime: &mut PurchasedAirRuntimeState,
    system: &IdealLoadsAirSystem,
    predecessor_cp427: Predecessor,
) -> Result<
    Snapshot,
    PurchasedAirCalcCoolingZeroSupplyMassFlowSensibleOutputPositiveZeroAssignmentError,
> {
    use PurchasedAirCalcCoolingZeroSupplyMassFlowSensibleOutputPositiveZeroAssignmentError as Error;

    let selected = predecessor_cp427.system;
    let unit = runtime
        .units
        .get(&selected)
    ...
    if unit.controlled_zone != Some(predecessor_cp427.controlled_zone)
        || unit
    ...
    let mixed_air_witness = if predecessor_cp427
    ...
    if predecessor_cp427.parent_call_ordinal != unit.init_call_count {
        return Err(call_order_error(unit, selected));
    }

    let state = &unit.calc_cooling_zero_supply_mass_flow_sensible_output_positive_zero_assignment;
    ...
    let post_route = next_state
        .latest_route
        .ok_or(Error::RuntimeStateInvariantViolation { system: selected })?;
    if !prefix_and_local_shape_match(snapshot, predecessor_cp427, post_route)
    ...
    let unit = runtime
        ...
        .get_mut(&selected)
        .ok_or(Error::UnknownSystem { system: selected })?;
    unit.calc_cooling_zero_supply_mass_flow_sensible_output_positive_zero_assignment = next_state;
    runtime
        .set_cooling_zero_supply_mass_flow_sensible_output_positive_zero_assignment_latest_witness(
            selected,
            Some(snapshot),
        );
    Ok(snapshot)
...
#[allow(dead_code)]
pub(in crate::ideal_loads) fn private_cooling_zero_supply_mass_flow_sensible_output_positive_zero_assignment_characterization(
...
#[allow(dead_code)]
pub(in crate::ideal_loads) fn cooling_zero_supply_mass_flow_sensible_output_positive_zero_assignment_snapshot_is_exact(
    snapshot: Snapshot,
) -> bool {
...
#[allow(dead_code)]
pub(in crate::ideal_loads) fn cooling_zero_supply_mass_flow_sensible_output_positive_zero_assignment_snapshots_match_bit_exact(
    left: Snapshot,
    right: Snapshot,
) -> bool {
...
fn predecessor_mismatch(
    ...
    PurchasedAirCalcCoolingZeroSupplyMassFlowSensibleOutputPositiveZeroAssignmentError::
        CoolingZeroSupplyMassFlowSupplyTemperatureMixedAirAssignmentSnapshotMismatch { system }
}
