//! Exact CP428 prefix, route, assignment, and bitwise validation.
...
pub(super) fn snapshot_is_exact(snapshot: Snapshot) -> bool {
    snapshot_route(snapshot).is_some()
        && crate::ideal_loads::cooling_zero_supply_mass_flow_supply_temperature_mixed_air_assignment_snapshot_is_exact(
            predecessor_cp427_snapshot(snapshot),
        )
}

pub(in crate::ideal_loads::calc) fn snapshot_route(snapshot: Snapshot) -> Option<Route> {
    let predecessor = predecessor_cp427_snapshot(snapshot);
    let predecessor_route =
        crate::ideal_loads::calc::cooling_zero_supply_mass_flow_supply_temperature_mixed_air_assignment_snapshot_route(
            predecessor,
        )?;
    let route =
        cooling_zero_supply_mass_flow_sensible_output_positive_zero_assignment_route_from_committed_predecessor(
...
pub(in crate::ideal_loads::calc) fn retained_route_matches_snapshot_bounded(
    ...
    let predecessor = predecessor_cp427_snapshot(snapshot);
    let predecessor_route =
        crate::ideal_loads::calc::PurchasedAirCalcCoolingZeroSupplyMassFlowSupplyTemperatureMixedAirAssignmentRetainedRoute {
...
fn local_shape_is_exact(
    ...
    let assigned = snapshot.assigned_cooling_sensible_output_w;
    snapshot.source == SOURCE
        ...
        && snapshot.cooling_sensible_output_positive_zero_assignment_performed
            == route.assignment_executed
