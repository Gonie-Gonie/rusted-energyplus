...
pub(in crate::ideal_loads::calc) fn advance_cooling_supply_humidity_ratio_humidification_dehumidification_control_humidistat_or_none_guard_state(
    ...
    let second_is_none =
        second_control.map(|control| control == DehumidificationControlType::None);
    let body_entered =
        first_is_humidistat == Some(true) || second_is_none == Some(true);
    ...
    if !next_transition_fits(state, predecessor, evaluate, first_is_humidistat, second_is_none, source_sites) {
        return None;
    }
    ...
    state.dehumidification_control_constant_sensible_heat_ratio_case_completed_skip_count +=
        usize::from(predecessor.dehumidification_control_constant_sensible_heat_ratio_case_completed_skip);
    ...
    state.dehumidification_control_constant_supply_humidity_ratio_case_completed_skip_count +=
        usize::from(predecessor.dehumidification_control_constant_supply_humidity_ratio_case_completed_skip);
...
fn route(
    ...
    match predecessor {
        ...
        PredecessorRoute::HumidificationControlBodyEntered
            if first_is_humidistat == Some(true) =>
        {
...
fn next_transition_fits(
    ...
    let increments = [
        ...
        (state.unit_off_skip_count, usize::from(predecessor.unit_off_skipped)),
        (state.non_cooling_skip_count, usize::from(predecessor.non_cooling_skipped)),
        (state.positive_guard_false_fallthrough_skip_count, usize::from(predecessor.positive_guard_false_fallthrough_skipped)),
        (state.dehumidification_control_none_case_completed_skip_count, usize::from(predecessor.dehumidification_control_none_case_completed_skip)),
        (state.dehumidification_control_constant_sensible_heat_ratio_case_completed_skip_count, usize::from(predecessor.dehumidification_control_constant_sensible_heat_ratio_case_completed_skip)),
        (state.dehumidification_control_humidistat_case_completed_skip_count, usize::from(predecessor.dehumidification_control_humidistat_case_completed_skip)),
        (state.dehumidification_control_constant_supply_humidity_ratio_case_completed_skip_count, usize::from(predecessor.dehumidification_control_constant_supply_humidity_ratio_case_completed_skip)),
        (state.heating_on_read_count, usize::from(predecessor.predecessor_heating_on_read)),
        (state.heating_on_body_entry_count, usize::from(predecessor.predecessor_cooling_supply_humidity_ratio_humidification_body_entered)),
        (state.heating_on_guard_false_fallthrough_count, usize::from(predecessor.predecessor_heating_on_guard_false_fallthrough)),
        (state.humidification_control_type_read_count, usize::from(predecessor.humidification_control_type_read)),
        (state.humidification_control_type_humidistat_comparison_count, usize::from(predecessor.humidification_control_type_humidistat.is_some())),
        (state.humidification_control_body_entry_count, usize::from(predecessor.humidification_control_body_entered)),
        (state.humidification_control_guard_false_fallthrough_count, usize::from(predecessor.humidification_control_guard_false_fallthrough)),
        (state.dehumidification_control_type_first_read_count, usize::from(evaluate)),
        (state.dehumidification_control_type_humidistat_comparison_count, usize::from(evaluate)),
        (state.dehumidification_control_type_humidistat_match_count, usize::from(first_is_humidistat == Some(true))),
        (state.dehumidification_control_type_second_read_count, usize::from(first_is_humidistat == Some(false))),
        (state.dehumidification_control_type_none_comparison_count, usize::from(first_is_humidistat == Some(false))),
        (state.dehumidification_control_type_none_match_count, usize::from(second_is_none == Some(true))),
        (state.dehumidification_control_body_entry_count, usize::from(first_is_humidistat == Some(true) || second_is_none == Some(true))),
        (state.dehumidification_control_guard_false_fallthrough_count, usize::from(evaluate && first_is_humidistat != Some(true) && second_is_none != Some(true))),
        (state.source_site_execution_count, source_sites),
    ];
    increments
        .into_iter()
