//! Pure CP321-to-CP322 source-characterization transition.
...
use crate::ideal_loads::PurchasedAirCalcCoolingCapacityZeroFlowResetSnapshot;

type Winner = (Operand, f64);

pub(in crate::ideal_loads::calc) fn advance_cooling_supply_mass_flow_maximum_state(
    state: &mut PurchasedAirCalcCoolingSupplyMassFlowMaximumRuntimeState,
    predecessor: PurchasedAirCalcCoolingCapacityZeroFlowResetSnapshot,
    input: PurchasedAirCalcCoolingSupplyMassFlowMaximumInput,
) -> PurchasedAirCalcCoolingSupplyMassFlowMaximumSnapshot {
    let cooling = predecessor.cooling_body_entered;
    let outdoor_air = cooling.then_some(input.outdoor_air_mass_flow_rate_kg_per_s);
    ...
    let route = if predecessor.unit_off_skipped {
        ...
        PurchasedAirCalcCoolingSupplyMassFlowMaximumRetainedRoute::UnitOff
    } else if predecessor.non_cooling_skipped {
        state.non_cooling_skip_count += 1;
        PurchasedAirCalcCoolingSupplyMassFlowMaximumRetainedRoute::NonCooling
    } else {
        ...
        PurchasedAirCalcCoolingSupplyMassFlowMaximumRetainedRoute::CoolingMaximumAssigned
    };

    let snapshot = PurchasedAirCalcCoolingSupplyMassFlowMaximumSnapshot {
