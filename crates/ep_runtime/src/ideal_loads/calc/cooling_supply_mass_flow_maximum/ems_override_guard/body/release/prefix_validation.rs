...
use super::runtime_validation::{
    completed_guard_state_is_consistent, completed_supply_maximum_state_is_consistent,
};
...
pub(super) fn completed_direct_prefix_through_ems_override_guard_is_consistent(
    runtime: &PurchasedAirRuntimeState,
    unit: &PurchasedAirUnitRuntimeState,
    system: &IdealLoadsAirSystem,
    guard: PurchasedAirCalcCoolingSupplyMassFlowEmsOverrideGuardSnapshot,
    guard_witness: Option<PurchasedAirCalcCoolingSupplyMassFlowEmsOverrideGuardSnapshot>,
) -> bool {
    let Some(economizer_guard) = unit.calc_cooling_economizer_guard.latest else {
        return false;
    };
    let Some(condition) = unit.calc_cooling_economizer_condition.latest else {
        return false;
    };
    let Some(economizer_body) = unit.calc_cooling_economizer_body.latest else {
        return false;
    };
    ...
    crate::ideal_loads::calc::cooling_economizer_condition::
        ...
        && completed_supply_maximum_state_is_consistent(
            unit,
            maximum,
            runtime.cooling_supply_mass_flow_maximum_latest_witness(system.id),
        )
        && guard_links_to_maximum(guard, maximum)
...
fn guard_links_to_maximum(
    guard: PurchasedAirCalcCoolingSupplyMassFlowEmsOverrideGuardSnapshot,
    maximum: PurchasedAirCalcCoolingSupplyMassFlowMaximumSnapshot,
) -> bool {
    guard.system == maximum.system
        && guard.parent_call_ordinal == maximum.parent_call_ordinal
        && guard.controlled_zone == maximum.controlled_zone
        && guard.unit_body_entered == maximum.unit_body_entered
        && guard.predecessor_cooling_body_entered == maximum.cooling_body_entered
        && guard.unit_off_skipped == maximum.unit_off_skipped
        && guard.non_cooling_skipped == maximum.non_cooling_skipped
        && guard.cooling_body_entered == maximum.cooling_body_entered
}
