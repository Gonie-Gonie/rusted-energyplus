//! Persistent CP368 runtime-state validation.
...
pub(super) fn pending_state_is_consistent(
    ...
    let state = &unit.calc_cooling_default_supply_humidity_ratio_case_break;
    let prior = &unit.calc_cooling_default_supply_humidity_ratio_mixed_air_assignment;
    state_is_consistent(state, witness, predecessor.system, selector)
        ...
        && pending_count(
            ...
            route == Route::DehumidificationControlConstantSupplyHumidityRatioCaseCompletedSkip,
        ) == Some(
...
pub(in crate::ideal_loads) fn cooling_default_supply_humidity_ratio_case_break_latest_metadata_is_consistent(
    state: &State,
...
mod tests {
    ...
    fn latest_route_counter_transfer_is_rejected_without_state_mutation() {
        let system = IdealLoadsAirSystemId(368);
        let latest = Snapshot {
            source:
                PURCHASED_AIR_CALC_COOLING_DEFAULT_SUPPLY_HUMIDITY_RATIO_CASE_BREAK_SOURCE,
