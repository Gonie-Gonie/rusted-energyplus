//! Canonical private CP368 default-case-break CSH-skip reconstruction.
...
use super::super::{
    ...
    advance_cooling_default_supply_humidity_ratio_case_break_state as advance,
};
...
use crate::ideal_loads::calc::cooling_default_supply_humidity_ratio_mixed_air_assignment::private_default_supply_humidity_ratio_mixed_air_assignment_csh_counterfactual_from_direct_release as cp367_private_csh_skip_from_direct_release;
use crate::ideal_loads::{PurchasedAirRuntimeState, PurchasedAirUnitRuntimeState};

/// Rebuilds private CP368 by recursively rebuilding canonical private CP367
/// and recording only the numeric-free default-case-break skip.
#[allow(dead_code)]
pub(in crate::ideal_loads::calc) fn private_default_supply_humidity_ratio_case_break_csh_counterfactual_from_direct_release(
    ...
    if system.id != direct.system
        ...
        || !super::completed_direct_cooling_default_supply_humidity_ratio_case_break_is_consistent(
            runtime,
            unit,
            system,
            direct,
    ...
    let direct_cp367 = unit
        .calc_cooling_default_supply_humidity_ratio_mixed_air_assignment
        .latest?;
    let private_cp367 =
        cp367_private_csh_skip_from_direct_release(runtime, unit, system, direct_cp367)?;
    let mut state = State::new(system.id);
    let counterfactual = advance(&mut state, private_cp367)?;
    (snapshot_route(counterfactual)
        == Some(Route::DehumidificationControlConstantSupplyHumidityRatioCaseCompletedSkip)
        && case_break_links_to_predecessor(counterfactual, private_cp367)
        && active_lineage_is_exact(runtime, unit, system, private_cp367, counterfactual)
        && !counterfactual
...
/// Proves that a supplied private CP368 witness is the exact canonical
/// numeric-free characterization of the retained direct release.
#[allow(dead_code)]
pub(in crate::ideal_loads::calc) fn private_default_supply_humidity_ratio_case_break_csh_counterfactual_links_to_direct_release(
    runtime: &PurchasedAirRuntimeState,
    unit: &PurchasedAirUnitRuntimeState,
    system: &IdealLoadsAirSystem,
    direct: Snapshot,
