...
#[test]
fn source_boundary_one_site_and_seven_route_zero_execution_algebra_are_exact() {
    assert_eq!(
        PURCHASED_AIR_CALC_COOLING_DEFAULT_SUPPLY_HUMIDITY_RATIO_CASE_BREAK_SOURCE,
    ...
    for (index, route) in [U, N, P, C0, Q, H, CSH].into_iter().enumerate() {
        let snapshot =
            advance(&mut state, predecessor(route, index + 1)).expect("CP368 pure transition");
        assert!(
...
fn provenance_selector_and_one_hot_corruption_are_transactional() {
    ...
    forged.dehumidification_control_default_supply_humidity_ratio_mixed_air_assignment_executed =
        true;
    rejects_without_mutation(forged);
}
...
fn predecessor(route: Route, ordinal: usize) -> Predecessor {
    ...
    Predecessor {
        ...
        dehumidification_control_constant_supply_humidity_ratio_case_completed_skip: route == CSH,
