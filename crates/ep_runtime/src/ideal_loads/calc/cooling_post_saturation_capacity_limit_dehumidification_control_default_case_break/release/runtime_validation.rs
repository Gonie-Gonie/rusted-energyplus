//! Persistent CP410 runtime-state validation.

use ep_model::IdealLoadsAirSystemId;

use super::super::transition::routes::{RetainedRoute, predecessor_index_is_split};
use super::super::{
...
pub(super) fn call_order_is_pending(
    ...
    unit.calc_cooling_post_saturation_capacity_limit_dehumidification_control_default_case_break
        ...
        && unit.calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_case_break.transition_count
            == ordinal
}
...
pub(super) fn prepare_next_transition(
    ...
    let snapshot = advance_cooling_post_saturation_capacity_limit_dehumidification_control_default_case_break_state(
        &mut next,
        predecessor,
    )?;
    Some((next, snapshot))
...
pub(super) fn prepared_completed_state_is_consistent(
    ...
    let predecessor = &unit.calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_case_break;
...
#[allow(dead_code)]
pub(in crate::ideal_loads) fn cooling_post_saturation_capacity_limit_dehumidification_control_default_case_break_latest_metadata_is_consistent(
    unit: &PurchasedAirUnitRuntimeState,
...
pub(in crate::ideal_loads::calc) fn committed_latest_snapshot_is_consistent(
    ...
    state.system == unit.system
        ...
        && super::snapshot_validation::cooling_post_saturation_capacity_limit_dehumidification_control_default_case_break_snapshot_is_exact_direct_release(
            witness,
        )
}

fn predecessor_counts_match(state: &State, predecessor: &PredecessorState) -> bool {
    let predecessor_breaks = state
...
fn counts_are_exact(state: &State) -> bool {
    ...
    let predecessor_breaks = state
        .predecessor_guard_false_fallthrough_count
        .checked_add(state.predecessor_maximum_capacity_assignment_count);
    let selected_total = checked_selected_sum(&state.predecessor_route_counts);
    ...
    predecessor_total == Some(state.transition_count)
        ...
        && selected_total == predecessor_breaks
        && state.inactive_transition_count == state.transition_count
        && state.dehumidification_control_default_case_break_count == 0
        && state
