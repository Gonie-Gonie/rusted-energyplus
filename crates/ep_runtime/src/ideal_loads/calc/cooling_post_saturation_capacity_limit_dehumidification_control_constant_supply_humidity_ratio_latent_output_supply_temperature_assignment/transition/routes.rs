//! Exact CP406 route preservation and compact CP407 route reconstruction.
...
pub(in crate::ideal_loads::calc) fn predecessor_route(
    ...
    let route = crate::ideal_loads::calc::cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_latent_output_capacity_guard_else_branch_entry_snapshot_route(predecessor)?;
    ...
    if assignment_executed != (route.guard_evaluated && !route.assignment_executed) {
        return None;
    }
    Some(RetainedRoute {
        predecessor_index: route.predecessor_index,
        predecessor_guard_evaluated: route.guard_evaluated,
        predecessor_maximum_capacity_assignment_executed: route.assignment_executed,
        assignment_executed,
    })
}
...
pub(in crate::ideal_loads::calc) fn compressed_snapshot_route(
    ...
    let maximum_assignment = snapshot
    ...
    if active {
        if guard_false == maximum_assignment
            || else_entered != guard_false
            || assignment != else_entered
        {
            return None;
        }
    } else if guard_false || maximum_assignment || else_entered || assignment {
        return None;
    }
    Some(RetainedRoute {
        predecessor_index,
        predecessor_guard_evaluated: active,
        predecessor_maximum_capacity_assignment_executed: maximum_assignment,
...
fn inactive_route(snapshot: Snapshot, index: usize) -> Option<usize> {
    if snapshot.predecessor_supply_enthalpy_assignment_executed != matches!(index, 18..=29)
        || snapshot.predecessor_dehumidification_control_type_read != matches!(index, 18..=29)
        || snapshot.predecessor_dehumidification_control_switch_dispatched
            != matches!(index, 18..=29)
        || snapshot.predecessor_dehumidification_control_type.is_some() != matches!(index, 18..=29)
...
/// Stable flattened ordering of the 36 CP406/CP407 logical routes.
#[cfg(test)]
...
pub(in crate::ideal_loads::calc) const fn resulting_has_supply_humidity_ratio(
    route: RetainedRoute,
) -> bool {
    route.assignment_executed || predecessor_has_supply_humidity_ratio(route)
}

pub(in crate::ideal_loads::calc) const fn resulting_has_supply_enthalpy(index: usize) -> bool {
    predecessor_has_supply_enthalpy(index)
}

pub(in crate::ideal_loads::calc) const fn resulting_has_supply_temperature(index: usize) -> bool {
