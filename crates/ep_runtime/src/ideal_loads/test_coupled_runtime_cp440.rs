//! CP440 coupled-runtime marker, accounting, binding, and no-feed contracts.

use crate::ideal_loads::PurchasedAirCalcHeatingOutdoorAirMaximumFlowContinueWarningCallRuntimeState;
use ep_model::IdealLoadsAirSystemId;

#[test]
fn cp440_contract_locks_routes_schema_json_and_current_binding() {
    ...
    let source = include_str!("calc/heating_outdoor_air_maximum_flow_continue_warning_call.rs");
    let snapshot = source
        .split_once(
    ...
    assert!(
        snapshot.contains("heating_outdoor_air_maximum_flow_continue_warning_call_site_reached")
    );
    let serializer = include_str!(
        "../../../ep_run/src/pipeline/purchased_air_heating_outdoor_air_maximum_flow_continue_warning_call/serialization/snapshot.rs"
    );
    let serializer_production = serializer
        .split_once("#[cfg(test)]")
        .map_or(serializer, |(production, _)| production);
    assert!(serializer_production.contains("cp439_snapshot_json(predecessor)"));
    assert_eq!(serializer_production.matches("target.insert(").count(), 4);
    assert!(
        serializer_production
            .contains("heating_outdoor_air_maximum_flow_continue_warning_call_site_reached")
    );
...
#[test]
fn cp440_new_state_has_nine_zeroed_lossless_route_partitions() {
    let state = PurchasedAirCalcHeatingOutdoorAirMaximumFlowContinueWarningCallRuntimeState::new(
...
#[test]
fn cp440_is_ordered_after_cp439_and_does_not_feed_numerics_or_services() {
    ...
    assert!(cp439 < cp440 && cp440 < coupling);
    assert!(!binding[cp440..coupling].contains("DirectZonePurchasedAirCouplingInput {"));
    let validator = include_str!(
        "coupled_runtime/heating_outdoor_air_maximum_flow_continue_warning_call_validation.rs"
    );
    for required in [
