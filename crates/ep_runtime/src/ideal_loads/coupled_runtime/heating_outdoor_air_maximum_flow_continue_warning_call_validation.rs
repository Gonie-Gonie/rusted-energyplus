//! Cheap coupled validation for CP440 heating continue-warning call-site evidence.

#[rustfmt::skip]
use crate::ideal_loads::{DirectZonePurchasedAirModelBinding, DirectZonePurchasedAirScheduledCouplingOutput, PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_CONTINUE_WARNING_CALL_FIRST_EXCLUDED_SOURCE, PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_CONTINUE_WARNING_CALL_SOURCE, PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_CONTINUE_WARNING_CALL_SOURCE_ORDER, PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_FIRST_WARNING_CALL_FIRST_EXCLUDED_SOURCE, PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_FIRST_WARNING_CALL_SOURCE, PurchasedAirCalcHeatingOutdoorAirMaximumFlowContinueWarningCallLifecycleSummary as Lifecycle, PurchasedAirCalcHeatingOutdoorAirMaximumFlowContinueWarningCallRuntimeState as State, PurchasedAirCalcHeatingOutdoorAirMaximumFlowContinueWarningCallSnapshot as Snapshot, PurchasedAirCalcHeatingOutdoorAirMaximumFlowFirstWarningCallLifecycleSummary as PredecessorLifecycle, PurchasedAirCalcHeatingOutdoorAirMaximumFlowFirstWarningCallRuntimeState as PredecessorState, PurchasedAirCalcHeatingOutdoorAirMaximumFlowFirstWarningCallSnapshot as PredecessorSnapshot, heating_outdoor_air_maximum_flow_continue_warning_call_predecessor_cp439_snapshot, heating_outdoor_air_maximum_flow_continue_warning_call_snapshot_is_exact, heating_outdoor_air_maximum_flow_continue_warning_call_snapshots_match_bit_exact, heating_outdoor_air_maximum_flow_first_warning_call_snapshots_match_bit_exact};
...
pub(in crate::ideal_loads) fn snapshot_matches_release(
    ...
    let predecessor = output.calculation_heating_outdoor_air_maximum_flow_first_warning_call;
    let snapshot = output.calculation_heating_outdoor_air_maximum_flow_continue_warning_call;
    snapshot.system == binding.ideal_loads_air_system
        ...
        && heating_outdoor_air_maximum_flow_continue_warning_call_snapshot_is_exact(snapshot)
        && local_public_skip_matches(snapshot, predecessor)
}

pub(in crate::ideal_loads) fn validate_lifecycle(
    lifecycle: &Lifecycle,
    predecessor_cp439: &PredecessorLifecycle,
    timestep_count: usize,
    latest_output: &DirectZonePurchasedAirScheduledCouplingOutput,
    binding: &DirectZonePurchasedAirModelBinding<'_>,
) -> Result<(), Error> {
    let state = &lifecycle.state;
    let predecessor = &predecessor_cp439.state;
    if lifecycle.source
        ...
        || predecessor_cp439.source
            != PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_FIRST_WARNING_CALL_SOURCE
        || predecessor_cp439.first_excluded_source
            != PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_FIRST_WARNING_CALL_FIRST_EXCLUDED_SOURCE
        || PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_CONTINUE_WARNING_CALL_SOURCE_ORDER.len()
            != 1
    ...
    if !heating_outdoor_air_maximum_flow_continue_warning_call_snapshots_match_bit_exact(
        latest,
        latest_output.calculation_heating_outdoor_air_maximum_flow_continue_warning_call,
...
fn validate_counts(
    ...
    for (field, expected, actual) in [
        ...
        (
            ...
            state.cp439_supply_humidity_ratio_state_owner_count,
        ),
        (
        ...
        (
            ...
            state.cp439_supply_enthalpy_state_owner_count,
        ),
        (
        ...
        (
            ...
            state.cp439_supply_temperature_state_owner_count,
        ),
        (
        ...
        (
            "predecessor_first_warning_call_count",
            predecessor.heating_outdoor_air_maximum_flow_first_warning_call_site_count,
            calls,
        ),
        ("public_continue_warning_call_count", 0, calls),
    ] {
...
fn violation(field: &'static str, expected: usize, actual: usize) -> Error {
    Error::CalcHeatingOutdoorAirMaximumFlowContinueWarningCallLifecycleInvariant {
        field,
        expected,
        actual,
    }
...
mod tests {
    #[test]
    fn validator_uses_nine_routes_one_marker_site_and_keeps_services_out() {
        let source =
            include_str!("heating_outdoor_air_maximum_flow_continue_warning_call_validation.rs");
        let production = source
            .split_once("#[cfg(test)]")
            .map_or(source, |(production, _)| production);
        for required in [
            "predecessor_cp439_snapshot",
            "continue_warning_call_route_alias",
            "public_continue_warning_call_count",
            "warning_counter_preservation_count",
        ] {
