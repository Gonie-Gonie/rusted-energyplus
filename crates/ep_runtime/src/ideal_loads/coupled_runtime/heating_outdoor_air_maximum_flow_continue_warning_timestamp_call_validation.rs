//! Cheap coupled validation for CP441 heating continue-warning timestamp call-site evidence.

#[rustfmt::skip]
use crate::ideal_loads::{DirectZonePurchasedAirModelBinding, DirectZonePurchasedAirScheduledCouplingOutput, PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_CONTINUE_WARNING_TIMESTAMP_CALL_FIRST_EXCLUDED_SOURCE, PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_CONTINUE_WARNING_TIMESTAMP_CALL_SOURCE, PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_CONTINUE_WARNING_TIMESTAMP_CALL_SOURCE_ORDER, PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_CONTINUE_WARNING_CALL_FIRST_EXCLUDED_SOURCE, PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_CONTINUE_WARNING_CALL_SOURCE, PurchasedAirCalcHeatingOutdoorAirMaximumFlowContinueWarningTimestampCallLifecycleSummary as Lifecycle, PurchasedAirCalcHeatingOutdoorAirMaximumFlowContinueWarningTimestampCallRuntimeState as State, PurchasedAirCalcHeatingOutdoorAirMaximumFlowContinueWarningTimestampCallSnapshot as Snapshot, PurchasedAirCalcHeatingOutdoorAirMaximumFlowContinueWarningCallLifecycleSummary as PredecessorLifecycle, PurchasedAirCalcHeatingOutdoorAirMaximumFlowContinueWarningCallRuntimeState as PredecessorState, PurchasedAirCalcHeatingOutdoorAirMaximumFlowContinueWarningCallSnapshot as PredecessorSnapshot, heating_outdoor_air_maximum_flow_continue_warning_timestamp_call_predecessor_cp440_snapshot, heating_outdoor_air_maximum_flow_continue_warning_timestamp_call_snapshot_is_exact, heating_outdoor_air_maximum_flow_continue_warning_timestamp_call_snapshots_match_bit_exact, heating_outdoor_air_maximum_flow_continue_warning_call_snapshots_match_bit_exact};
...
pub(in crate::ideal_loads) fn snapshot_matches_release(
    ...
    let predecessor = output.calculation_heating_outdoor_air_maximum_flow_continue_warning_call;
    let snapshot =
    ...
    snapshot.system == binding.ideal_loads_air_system
        ...
        && heating_outdoor_air_maximum_flow_continue_warning_timestamp_call_snapshot_is_exact(snapshot)
        && local_public_skip_matches(snapshot, predecessor)
}

pub(in crate::ideal_loads) fn validate_lifecycle(
    lifecycle: &Lifecycle,
    predecessor_cp440: &PredecessorLifecycle,
    timestep_count: usize,
    latest_output: &DirectZonePurchasedAirScheduledCouplingOutput,
    binding: &DirectZonePurchasedAirModelBinding<'_>,
) -> Result<(), Error> {
    let state = &lifecycle.state;
    let predecessor = &predecessor_cp440.state;
    if lifecycle.source
        != PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_CONTINUE_WARNING_TIMESTAMP_CALL_SOURCE
        || lifecycle.first_excluded_source
            != PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_CONTINUE_WARNING_TIMESTAMP_CALL_FIRST_EXCLUDED_SOURCE
        || predecessor_cp440.source
            != PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_CONTINUE_WARNING_CALL_SOURCE
        || predecessor_cp440.first_excluded_source
            != PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_CONTINUE_WARNING_CALL_FIRST_EXCLUDED_SOURCE
        || PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_CONTINUE_WARNING_TIMESTAMP_CALL_SOURCE_ORDER.len()
            != 1
    ...
    if !heating_outdoor_air_maximum_flow_continue_warning_timestamp_call_snapshots_match_bit_exact(
        latest,
        latest_output.calculation_heating_outdoor_air_maximum_flow_continue_warning_timestamp_call,
...
fn validate_counts(
    ...
    for (field, expected, actual) in [
        ...
        ("public_continue_warning_timestamp_call_count", 0, calls),
    ] {
...
fn violation(field: &'static str, expected: usize, actual: usize) -> Error {
    Error::CalcHeatingOutdoorAirMaximumFlowContinueWarningTimestampCallLifecycleInvariant {
        field,
        expected,
        actual,
    }
...
mod tests {
    #[test]
    fn validator_uses_ten_routes_one_marker_site_and_keeps_services_out() {
        let source = include_str!(
            "heating_outdoor_air_maximum_flow_continue_warning_timestamp_call_validation.rs"
        );
        let production = source
            .split_once("#[cfg(test)]")
            .map_or(source, |(production, _)| production);
        for required in [
            "predecessor_cp440_snapshot",
            "continue_warning_timestamp_call_route_alias",
