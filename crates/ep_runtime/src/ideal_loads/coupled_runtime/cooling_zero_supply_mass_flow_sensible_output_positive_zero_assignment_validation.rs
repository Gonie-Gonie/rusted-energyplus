//! Cheap coupled validation for CP428 zero-flow sensible-output evidence.
...
pub(in crate::ideal_loads) fn snapshot_matches_release(
    ...
    snapshot.system == binding.ideal_loads_air_system
        ...
        && cooling_zero_supply_mass_flow_supply_temperature_mixed_air_assignment_snapshots_match_bit_exact(
            cooling_zero_supply_mass_flow_sensible_output_positive_zero_assignment_predecessor_cp427_snapshot(snapshot),
            predecessor,
        )
...
pub(in crate::ideal_loads) fn validate_lifecycle(
    lifecycle: &Lifecycle,
    predecessor_cp427: &PredecessorLifecycle,
    timestep_count: usize,
    latest_output: &DirectZonePurchasedAirScheduledCouplingOutput,
    binding: &DirectZonePurchasedAirModelBinding<'_>,
) -> Result<(), Error> {
    let state = &lifecycle.state;
    let predecessor = &predecessor_cp427.state;
    if lifecycle.source
        != PURCHASED_AIR_CALC_COOLING_ZERO_SUPPLY_MASS_FLOW_SENSIBLE_OUTPUT_POSITIVE_ZERO_ASSIGNMENT_SOURCE
        || lifecycle.first_excluded_source
            != PURCHASED_AIR_CALC_COOLING_ZERO_SUPPLY_MASS_FLOW_SENSIBLE_OUTPUT_POSITIVE_ZERO_ASSIGNMENT_FIRST_EXCLUDED_SOURCE
        || predecessor_cp427.source
            != PURCHASED_AIR_CALC_COOLING_ZERO_SUPPLY_MASS_FLOW_SUPPLY_TEMPERATURE_MIXED_AIR_ASSIGNMENT_SOURCE
        || predecessor_cp427.first_excluded_source
            != PURCHASED_AIR_CALC_COOLING_ZERO_SUPPLY_MASS_FLOW_SUPPLY_TEMPERATURE_MIXED_AIR_ASSIGNMENT_FIRST_EXCLUDED_SOURCE
        || PURCHASED_AIR_CALC_COOLING_ZERO_SUPPLY_MASS_FLOW_SENSIBLE_OUTPUT_POSITIVE_ZERO_ASSIGNMENT_SOURCE_ORDER.len()
            != 1
...
fn validate_counts(
    state: &State,
    predecessor: &crate::ideal_loads::PurchasedAirCalcCoolingZeroSupplyMassFlowSupplyTemperatureMixedAirAssignmentRuntimeState,
    timestep_count: usize,
) -> Result<(), Error> {
    for values in [
    ...
    let humidity_owners = predecessor.cp426_supply_humidity_ratio_state_owner_count;
    let enthalpy_owners = predecessor.cp426_supply_enthalpy_state_owner_count;
    let temperature_owners = predecessor
        ...
        .ok_or_else(|| violation("predecessor_temperature_owner_overflow", 0, usize::MAX))?;
...
fn local_shape_is_exact(
    snapshot: Snapshot,
    predecessor: crate::ideal_loads::PurchasedAirCalcCoolingZeroSupplyMassFlowSupplyTemperatureMixedAirAssignmentSnapshot,
) -> bool {
    let assignment =
...
fn violation(field: &'static str, expected: usize, actual: usize) -> Error {
    Error::CalcCoolingZeroSupplyMassFlowSensibleOutputPositiveZeroAssignmentLifecycleInvariant {
        field,
        expected,
        actual,
    }
...
mod tests {
    ...
    fn hot_validator_is_bounded_and_keeps_the_numerical_dto_out() {
        let source = include_str!(
            "cooling_zero_supply_mass_flow_sensible_output_positive_zero_assignment_validation.rs"
        )
        .split_once("#[cfg(test)]")
        .map_or(
            include_str!(
        ...
        for required in ["predecessor_route_counts", "predecessor_cp427_snapshot"] {
