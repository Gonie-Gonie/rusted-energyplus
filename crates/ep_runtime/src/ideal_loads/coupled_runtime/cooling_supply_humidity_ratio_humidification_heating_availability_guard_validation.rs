//! Release validation for the CP369 Cooling humidification heating-availability guard.
...
pub(super) fn validate_lifecycle(
    lifecycle: &PurchasedAirCalcCoolingSupplyHumidityRatioHumidificationHeatingAvailabilityGuardLifecycleSummary,
    predecessor_lifecycle: &PurchasedAirCalcCoolingDefaultSupplyHumidityRatioCaseBreakLifecycleSummary,
    ...
    if binding.system.dehumidification_control_type != DehumidificationControlType::None
        ...
        || PURCHASED_AIR_CALC_COOLING_DEFAULT_SUPPLY_HUMIDITY_RATIO_CASE_BREAK_SOURCE_ORDER.len() != 1
        || state.system != binding.ideal_loads_air_system
        || predecessor.system != binding.ideal_loads_air_system
        || !cooling_supply_humidity_ratio_humidification_heating_availability_guard_latest_metadata_is_consistent(state, timestep_count)
        || !cooling_default_supply_humidity_ratio_case_break_snapshot_is_exact_direct_release(*predecessor_latest)
        || *predecessor_latest
            != latest_output.calculation_cooling_default_supply_humidity_ratio_case_break
        || !cooling_supply_humidity_ratio_humidification_heating_availability_guard_snapshot_is_exact_direct_release(*latest)
...
fn validate_counts(
    state: &PurchasedAirCalcCoolingSupplyHumidityRatioHumidificationHeatingAvailabilityGuardRuntimeState,
    predecessor: &PurchasedAirCalcCoolingDefaultSupplyHumidityRatioCaseBreakRuntimeState,
    timestep_count: usize,
) -> Result<(), Error> {
    ...
    for (field, expected, actual) in [
        ...
        ("heating_on_read_count", active, state.heating_on_read_count),
        (
...
pub(super) fn expected_snapshot(
    ...
    PurchasedAirCalcCoolingSupplyHumidityRatioHumidificationHeatingAvailabilityGuardSnapshot {
        ...
        predecessor_dehumidification_control_default_supply_humidity_ratio_case_exited_via_break: predecessor.dehumidification_control_default_supply_humidity_ratio_case_exited_via_break,
...
fn violation(field: &'static str, expected: usize, actual: usize) -> Error {
    Error::CalcCoolingSupplyHumidityRatioHumidificationHeatingAvailabilityGuardLifecycleInvariant {
        field,
        expected,
        actual,
    }
