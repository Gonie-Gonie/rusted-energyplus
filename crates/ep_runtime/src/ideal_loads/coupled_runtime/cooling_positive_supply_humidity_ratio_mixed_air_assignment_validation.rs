//! Release validation for the bounded cooling positive-supply humidity-ratio assignment.
...
pub(super) fn snapshot_matches_release(
    ...
    let predecessor = output.calculation_cooling_positive_supply_temperature_mixed_air_limit;
    let mixed_air = output.calculation_cooling_mixed_air_call;
    let snapshot = output.calculation_cooling_positive_supply_humidity_ratio_mixed_air_assignment;
...
fn source_lineage_matches(
    ...
    if !snapshot.supply_humidity_ratio_mixed_air_assignment_executed {
        return true;
    }
...
pub(super) fn validate_lifecycle(
    lifecycle: &PurchasedAirCalcCoolingPositiveSupplyHumidityRatioMixedAirAssignmentLifecycleSummary,
    predecessor_lifecycle: &PurchasedAirCalcCoolingPositiveSupplyTemperatureMixedAirLimitLifecycleSummary,
    ...
    if lifecycle.source
        ...
        || !mixed_air_limit_snapshots_match_exact_bits(
            predecessor_latest,
...
fn mixed_air_limit_snapshots_match_exact_bits(
...
fn violation(field: &'static str, expected: usize, actual: usize) -> Error {
    Error::CalcCoolingPositiveSupplyHumidityRatioMixedAirAssignmentLifecycleInvariant {
        field,
        expected,
        actual,
    }
