//! Coupled-runtime validation for CP416 psychrometric supply-humidity-ratio assignment evidence.
...
pub(in crate::ideal_loads) fn snapshot_matches_release(
    ...
    let expected = private_cooling_post_saturation_capacity_limit_dehumidification_supply_humidity_ratio_assignment_characterization(predecessor);

    snapshot.system == binding.ideal_loads_air_system
        && snapshot.parent_call_ordinal == call_ordinal
        && snapshot.controlled_zone == binding.zone
        && binding.system.dehumidification_control_type == DehumidificationControlType::None
        && cooling_post_saturation_capacity_limit_dehumidification_supply_temperature_saturation_mixed_air_limit_snapshot_is_exact_direct_release(predecessor)
        && cooling_post_saturation_capacity_limit_dehumidification_supply_humidity_ratio_assignment_snapshot_is_exact_direct_release(snapshot)
        && expected.is_some_and(|expected| {
            cooling_post_saturation_capacity_limit_dehumidification_supply_humidity_ratio_assignment_snapshots_match_bit_exact(snapshot, expected)
        })
}

pub(in crate::ideal_loads) fn validate_lifecycle(
    lifecycle: &Lifecycle,
    predecessor_cp415: &PredecessorLifecycle,
    timestep_count: usize,
    latest_output: &DirectZonePurchasedAirScheduledCouplingOutput,
    binding: &DirectZonePurchasedAirModelBinding<'_>,
) -> Result<(), Error> {
    let state = &lifecycle.state;
    let predecessor = &predecessor_cp415.state;
    if lifecycle.source
        != PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_SUPPLY_HUMIDITY_RATIO_ASSIGNMENT_SOURCE
        || lifecycle.first_excluded_source
            != PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_SUPPLY_HUMIDITY_RATIO_ASSIGNMENT_FIRST_EXCLUDED_SOURCE
        || predecessor_cp415.source
            != PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_SUPPLY_TEMPERATURE_SATURATION_MIXED_AIR_LIMIT_SOURCE
        || predecessor_cp415.first_excluded_source
            != PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_SUPPLY_TEMPERATURE_SATURATION_MIXED_AIR_LIMIT_FIRST_EXCLUDED_SOURCE
        || PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_SUPPLY_HUMIDITY_RATIO_ASSIGNMENT_SOURCE_ORDER
    ...
    let unchanged_humidity_ratio =
        humidity_ratio_owners
            ...
            .ok_or_else(|| {
                violation(
                    "humidity_ratio_preservation_partition_underflow",
                    assignments,
                    humidity_ratio_owners,
                )
            })?;
    ...
    for (field, expected, actual) in [
        ...
        (
            "cp415_assignment_equals_cp416_assignment",
    ...
    let expected = private_cooling_post_saturation_capacity_limit_dehumidification_supply_humidity_ratio_assignment_characterization(predecessor_latest)
        .ok_or_else(|| violation("latest_owner_lineage_ready", 1, 0))?;
    let output_latest = latest_output
        .calculation_cooling_post_saturation_capacity_limit_dehumidification_supply_humidity_ratio_assignment;
    if !cooling_post_saturation_capacity_limit_dehumidification_supply_humidity_ratio_assignment_snapshots_match_bit_exact(latest, expected)
        || !cooling_post_saturation_capacity_limit_dehumidification_supply_humidity_ratio_assignment_snapshots_match_bit_exact(latest, output_latest)
...
fn violation(field: &'static str, expected: usize, actual: usize) -> Error {
    Error::CalcCoolingPostSaturationCapacityLimitDehumidificationSupplyHumidityRatioAssignmentLifecycleInvariant {
        field,
        expected,
        actual,
    }
...
mod tests {
    ...
    #[test]
    fn conceptual_cp416_contract_is_54_routes_18_assignments_and_72_sites() {
