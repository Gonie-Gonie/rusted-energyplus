//! Release validation for the bounded constant-SHR supply-humidity-ratio overdrying limit.
...
pub(super) fn snapshot_matches_release(
    ...
    let snapshot = output.calculation_cooling_constant_shr_supply_humidity_ratio_overdrying_limit;
...
pub(super) fn validate_lifecycle(
    ...
    if binding.system.dehumidification_control_type != DehumidificationControlType::None
        ...
        || lifecycle.first_excluded_source
            != PURCHASED_AIR_CALC_COOLING_CONSTANT_SHR_SUPPLY_HUMIDITY_RATIO_OVERDRYING_LIMIT_FIRST_EXCLUDED_SOURCE
        || PURCHASED_AIR_CALC_COOLING_CONSTANT_SHR_SUPPLY_HUMIDITY_RATIO_OVERDRYING_LIMIT_SOURCE_ORDER.len()
            != 6
        ...
        || !snapshots_match_exact_bits(
            latest,
            &latest_output.calculation_cooling_constant_shr_supply_humidity_ratio_overdrying_limit,
        )
...
fn violation(field: &'static str, expected: usize, actual: usize) -> Error {
    Error::CalcCoolingConstantShrSupplyHumidityRatioOverdryingLimitLifecycleInvariant {
        field,
        expected,
        actual,
    }
