...
use crate::ideal_loads::{
    ...
    PurchasedAirCalcCoolingHumidistatCaseBreakLifecycleSummary as Lifecycle,
    PurchasedAirCalcCoolingHumidistatCaseBreakSnapshot as Snapshot,
    PurchasedAirCalcCoolingHumidistatSupplyHumidityRatioMixedAirLimitLifecycleSummary as PredecessorLifecycle,
...
use super::super::cooling_humidistat_case_break_validation::{
    expected_snapshot, validate_lifecycle,
};

#[test]
fn cp363_lifecycle_matches_outputs_and_cp362() {
    let fixture = validator_fixture();
    assert!(fixture.is_some(), "CP363 validator fixture");
    let Some((model, output, lifecycle, predecessor)) = fixture else {
        return;
    };
    assert!(
        validate(&model, &output, &lifecycle, &predecessor).is_ok(),
        "canonical CP363 coupled lifecycle"
    );
...
#[test]
fn cp363_route_source_latest_and_predecessor_corruption_are_rejected() {
    let fixture = validator_fixture();
    assert!(fixture.is_some(), "CP363 corruption fixture must succeed");
    let Some((model, output, lifecycle, predecessor)) = fixture else {
        return;
    };
    ...
    h_predecessor
        .state
        .dehumidification_control_none_case_completed_skip_count = 0;
    h.state.dehumidification_control_humidistat_case_break_count = 1;
    h.state.source_site_execution_count = 1;
    h_predecessor
        .state
        .dehumidification_control_humidistat_supply_humidity_ratio_mixed_air_limit_count = 1;
    assert_invariant(
        validate(&model, &output, &h, &h_predecessor),
        "direct_humidistat_case_break_count",
        0,
        1,
    );
    ...
    assert!(latest.is_some(), "CP363 latest route snapshot must exist");
    ...
    assert!(latest.is_some(), "CP363 latest ordinal snapshot must exist");
    let Some(latest) = latest else {
        return;
    };
    ...
    assert!(
        latest.is_some(),
        "CP363 predecessor-link snapshot must exist"
    );
    let Some(latest) = latest else {
        return;
    };
...
#[test]
fn cp363_expected_snapshot_maps_q_h_and_csh_predecessor_routes() {
    let fixture = validator_fixture();
    assert!(
        fixture.is_some(),
        "CP363 route-mapping fixture must succeed"
    );
    let Some((_, output, _, _)) = fixture else {
        return;
    };
    let base = output.calculation_cooling_humidistat_supply_humidity_ratio_mixed_air_limit;
...
fn validator_fixture() -> Option<(
    ...
    .ok()?;
    let lifecycle = purchased_air_calc_cooling_humidistat_case_break_lifecycle_summary(
        &runtime,
...
fn validate(
    ...
    assert!(binding.is_ok(), "CP363 binding");
...
fn assert_invariant(
    ...
    assert_eq!(
        result,
        Err(Error::CalcCoolingHumidistatCaseBreakLifecycleInvariant {
            field,
            expected,
            actual,
        })
...
fn forge_latest_route_to_unit_off(snapshot: &mut Snapshot) {
    ...
    snapshot.dehumidification_control_humidistat_case_exited_via_break = false;
    snapshot.dehumidification_control_constant_supply_humidity_ratio_case_selected_skip = false;
}
