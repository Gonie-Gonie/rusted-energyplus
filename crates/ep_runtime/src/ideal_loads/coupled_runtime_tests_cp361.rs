...
#[test]
fn cp361_coupled_direct_none_is_exact_null_skip_and_cp345_stays_owner() {
    ...
    let projected =
        super::super::cooling_humidistat_supply_humidity_ratio_for_dehumidification_minimum_limit_validation::expected_snapshot(
            predecessor,
        );
    assert!(
        super::super::cooling_humidistat_supply_humidity_ratio_for_dehumidification_minimum_limit_validation::
            snapshots_match_bit_exact(&snapshot, &projected)
    );
    let mut structural = snapshot;
    structural.source_order = &["forged-cp361-source-order"];
    assert!(
        !super::super::cooling_humidistat_supply_humidity_ratio_for_dehumidification_minimum_limit_validation::
            snapshots_match_bit_exact(&structural, &projected)
    );
    let mut numeric = snapshot;
    numeric.minimum_cooling_supply_air_humidity_ratio_for_maximum_read = true;
    numeric.minimum_cooling_supply_air_humidity_ratio = Some(-0.0);
    assert!(
        !super::super::cooling_humidistat_supply_humidity_ratio_for_dehumidification_minimum_limit_validation::
    ...
    assert_eq!(result.values[0].to_bits(), owner.to_bits());
}
