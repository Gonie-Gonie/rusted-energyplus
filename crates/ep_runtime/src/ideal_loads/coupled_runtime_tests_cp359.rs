...
#[test]
fn cp359_coupled_direct_none_is_exact_skip_and_cp345_remains_numerical_owner() {
    ...
    assert!(state.latest.is_some(), "latest CP359 snapshot");
    ...
    let predecessor = simulation
        ...
        .state
        .latest;
    assert!(predecessor.is_some(), "latest CP358 snapshot");
    ...
    let projected =
        super::super::cooling_humidistat_moisture_demand_assignment_validation::expected_snapshot(
            predecessor,
        );
    assert!(
        super::super::cooling_humidistat_moisture_demand_assignment_validation::
            snapshots_match_bit_exact(&snapshot, &projected)
    );
    let mut structural = snapshot;
    structural.source_order = &["forged-cp359-source-order"];
    assert!(
        !super::super::cooling_humidistat_moisture_demand_assignment_validation::
            snapshots_match_bit_exact(&structural, &projected)
    );
    let mut numeric = snapshot;
    numeric.zone_dehumidifying_setpoint_moisture_demand_read = true;
    numeric.zone_dehumidifying_setpoint_moisture_demand_kg_per_s = Some(-0.0);
    numeric.zone_dehumidifying_setpoint_moisture_demand_assigned = true;
    numeric.assigned_zone_dehumidifying_setpoint_moisture_demand_kg_per_s = Some(-0.0);
    numeric.resulting_zone_dehumidifying_setpoint_moisture_demand_kg_per_s = Some(-0.0);
    assert!(
        !super::super::cooling_humidistat_moisture_demand_assignment_validation::
    ...
    let owner_value = owner.assigned_supply_humidity_ratio;
    assert!(owner_value.is_some(), "CP345 numerical humidity value");
    let Some(owner_value) = owner_value else {
        return;
    };
