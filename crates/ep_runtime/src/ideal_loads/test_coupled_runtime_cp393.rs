//! Non-vacuous CP393 coupled-runtime integration tests.
...
use crate::ideal_loads::{
    ...
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationControlConstantSensibleHeatRatioSupplyHumidityRatioAssignmentLifecycleSummary as Predecessor,
...
use super::super::cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_case_break_validation::{
    snapshot_matches_release, validate_lifecycle,
};

#[test]
fn cp393_preserves_cp392_terminal_carriers_and_skips_break_on_direct_routes() {
    for (limit, maximum_capacity_w, availability) in [
        ...
        let cp392 = output.calculation_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_supply_humidity_ratio_assignment;
        assert!(
...
#[test]
fn cp393_rejects_cp392_carrier_bit_drift_and_route_drift() {
    ...
    let value = predecessor
        ...
        .as_mut()
        .expect("CP392 retained enthalpy");
...
#[test]
fn cp393_validation_remains_independent_of_numerical_output_state() {
    let (model, mut output, lifecycle, predecessor) =
        validator_fixture(IdealLoadsLimit::LimitCapacity, 0.020, 500.0, 1.0, 2);
    let binding = bind_direct_zone_purchased_air_model(&model).expect("CP393 binding");
    output
        .coupling
        .purchased_air
        .calculation
...
fn validator_fixture(
    ...
    let cache = precompute_schedule_cache(&model.typed, 1).expect("CP393 schedule cache");
    let binding = bind_direct_zone_purchased_air_model(&model).expect("CP393 direct binding");
    ...
    for step in 0..steps {
        ...
        latest = Some(
            ...
            .expect("CP393 coupling"),
        );
    }
    let output = latest.expect("at least one CP393 step");
    let system = output.initialization.system;
    let lifecycle = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_case_break_lifecycle_summary(&runtime, system).expect("CP393 lifecycle");
    let predecessor = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_supply_humidity_ratio_assignment_lifecycle_summary(&runtime, system).expect("CP392 lifecycle");
