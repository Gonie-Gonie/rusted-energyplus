//! Non-vacuous CP395 coupled-runtime integration tests.

use super::*;
use crate::ideal_loads::{
    DirectZonePurchasedAirScheduledCouplingInput, DirectZonePurchasedAirScheduledCouplingOutput,
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationControlHumidistatCaseEntryLifecycleSummary as Predecessor,
...
use super::super::cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_supply_humidity_ratio_assignment_validation::{
    snapshot_matches_release, validate_lifecycle,
};

#[test]
fn cp395_preserves_cp394_recursive_carriers_and_skips_assignment_on_direct_routes() {
    for (limit, maximum_capacity_w, availability) in [
        ...
        let cp394 = output.calculation_cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_case_entry;
        assert!(
...
#[test]
fn cp395_rejects_cp394_recursive_carrier_bit_drift_and_route_drift() {
    ...
    let value = predecessor
        ...
        .as_mut()
        .expect("CP393 recursive enthalpy");
...
fn validator_fixture(
    ...
    let cache = precompute_schedule_cache(&model.typed, 1).expect("CP395 schedule cache");
    let binding = bind_direct_zone_purchased_air_model(&model).expect("CP395 direct binding");
    ...
    for step in 0..steps {
        ...
        latest = Some(
            ...
            .expect("CP395 coupling"),
        );
    }
    let output = latest.expect("at least one CP395 step");
    let system = output.initialization.system;
    let lifecycle = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_supply_humidity_ratio_assignment_lifecycle_summary(&runtime, system).expect("CP395 lifecycle");
    let predecessor = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_case_entry_lifecycle_summary(&runtime, system).expect("CP394 lifecycle");
