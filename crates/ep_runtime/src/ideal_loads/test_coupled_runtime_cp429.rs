//! CP429 coupled-runtime accounting and no-feed contracts.

use crate::ideal_loads::PurchasedAirCalcCoolingZeroSupplyMassFlowTotalOutputPositiveZeroAssignmentRuntimeState;
use ep_model::IdealLoadsAirSystemId;

#[test]
fn cp429_conceptual_contract_has_59_outcomes_58_inactive_one_assignment_and_one_site() {
    assert_eq!(
...
#[test]
fn cp429_snapshot_schema_is_exactly_317_117_2_1_and_binding_is_129() {
    let source =
        include_str!("calc/cooling_zero_supply_mass_flow_total_output_positive_zero_assignment.rs");
    let snapshot = source
        .split_once(
            "pub struct PurchasedAirCalcCoolingZeroSupplyMassFlowTotalOutputPositiveZeroAssignmentSnapshot",
        )
...
#[test]
fn cp429_new_state_has_two_zeroed_lossless_route_partitions() {
    let state =
...
#[test]
fn cp429_binding_pipeline_and_validator_keep_numerical_dto_unchanged() {
    let binding = include_str!("binding.rs");
    let pipeline = include_str!("../../../ep_run/src/pipeline.rs");
    let validator = include_str!(
        "coupled_runtime/cooling_zero_supply_mass_flow_total_output_positive_zero_assignment_validation.rs"
    )
    .split_once("#[cfg(test)]")
    .map_or(
        include_str!(
    ...
    let marker = "cooling_zero_supply_mass_flow_total_output_positive_zero_assignment";
...
#[test]
fn cp429_integration_roots_stay_within_historical_caps() {
