//! Non-vacuous CP391 coupled-runtime integration tests.
...
use crate::ideal_loads::{
    ...
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationControlConstantSensibleHeatRatioSupplyTemperatureMixedAirLimitLifecycleSummary as Predecessor,
...
use super::super::cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_overdrying_limit_validation::{
    snapshot_matches_release, validate_lifecycle,
};

#[test]
fn cp391_preserves_cp390_enthalpy_and_temperature_on_representative_direct_routes() {
    for (limit, maximum_capacity_w, availability) in [
        ...
        let cp390 = output.calculation_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_supply_temperature_mixed_air_limit;
        assert_eq!(
            snapshot
                .preexisting_supply_enthalpy_j_per_kg
                .map(f64::to_bits),
            cp390.resulting_supply_enthalpy_j_per_kg.map(f64::to_bits)
        );
        assert_eq!(
            snapshot
        ...
        assert_eq!(
            ...
            cp390.resulting_supply_temperature_c.map(f64::to_bits)
        );
        assert!(
            !snapshot
...
#[test]
fn cp391_rejects_one_bit_drift_in_cp390_retained_result() {
    ...
    let value = predecessor
        ...
        .resulting_supply_enthalpy_j_per_kg
        .as_mut()
        .expect("CP390 retained enthalpy");
...
#[test]
fn cp391_rejects_cp390_source_order_corruption() {
    let (model, output, lifecycle, mut predecessor) =
        validator_fixture(IdealLoadsLimit::LimitCapacity, 0.020, 500.0, 1.0, 1);
    predecessor
        .state
        ...
        .as_mut()
        .expect("CP390 latest")
        .source_order = &["forged-cp390-source-order"];
    assert_eq!(
        validate(&model, &output, &lifecycle, &predecessor, 1),
        Err(latest_violation())
    );
...
#[test]
fn cp391_rejects_route_drift_and_remains_evidence_only() {
    let (model, output, mut lifecycle, predecessor) =
        validator_fixture(IdealLoadsLimit::LimitCapacity, 0.020, 500.0, 1.0, 2);
    let mut output = output;
    let binding = bind_direct_zone_purchased_air_model(&model).expect("CP391 binding");
    output
        .coupling
        .prediction
        .predicted_loads
...
fn validator_fixture(
    ...
    let cache = precompute_schedule_cache(&model.typed, 1).expect("CP391 schedule cache");
    let binding = bind_direct_zone_purchased_air_model(&model).expect("CP391 direct binding");
    ...
    for step in 0..steps {
        ...
        latest = Some(
            ...
            .expect("CP391 coupling"),
        );
    }
    let output = latest.expect("at least one CP391 step");
    let system = output.initialization.system;
    let lifecycle = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_overdrying_limit_lifecycle_summary(&runtime, system).expect("CP391 lifecycle");
    let predecessor = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_supply_temperature_mixed_air_limit_lifecycle_summary(&runtime, system).expect("CP390 lifecycle");
