...
use super::super::cooling_supply_humidity_ratio_humidification_supply_humidity_ratio_maximum_assignment_validation::{
    expected_snapshot, validate_lifecycle,
};

#[test]
fn cp375_lifecycle_matches_cp374_and_does_not_feed_numerical_result() {
    let fixture = validator_fixture();
    assert!(fixture.is_some(), "CP375 validator fixture");
...
#[test]
fn cp375_direct_counters_and_latest_corruption_are_rejected() {
    let fixture = validator_fixture();
    assert!(fixture.is_some(), "CP375 corruption fixture");
    let Some((model, output, lifecycle, predecessor)) = fixture else {
        return;
    };
    ...
    assert!(latest.state.latest.is_some(), "CP375 latest must exist");
    let Some(latest_snapshot) = latest.state.latest.as_mut() else {
        return;
    };
    latest_snapshot.source_shaped_two_argument_maximum_evaluated = true;
    assert_eq!(
    ...
    assert!(
        ...
        "CP374 predecessor latest must exist"
    );
    let Some(predecessor_latest_snapshot) = predecessor_latest.state.latest.as_mut() else {
        return;
    };
    predecessor_latest_snapshot.source_shaped_two_argument_minimum_evaluated = true;
    assert_eq!(
...
fn set_humidistat_assignment_one(lifecycle: &mut Lifecycle) {
    lifecycle
        .state
        .dehumidification_control_humidistat_supply_humidity_ratio_maximum_assignment_count = 1;
}

fn set_none_assignment_one(lifecycle: &mut Lifecycle) {
    lifecycle
        .state
        .dehumidification_control_none_supply_humidity_ratio_maximum_assignment_count = 1;
}
...
fn set_assignment_one(lifecycle: &mut Lifecycle) {
    lifecycle
        .state
        .purchased_air_supply_humidity_ratio_assignment_count = 1;
}
...
fn set_left_read_one(lifecycle: &mut Lifecycle) {
    lifecycle
        .state
        .purchased_air_supply_humidity_ratio_for_humidification_supply_maximum_read_count = 1;
}

fn set_right_read_one(lifecycle: &mut Lifecycle) {
    lifecycle
        .state
        .supply_humidity_ratio_for_humidification_for_supply_maximum_read_count = 1;
}

fn set_maximum_one(lifecycle: &mut Lifecycle) {
    lifecycle
        .state
        .source_shaped_two_argument_maximum_evaluation_count = 1;
}
...
fn validator_fixture() -> Option<(
    ...
    .ok()?;
    let lifecycle = purchased_air_calc_cooling_supply_humidity_ratio_humidification_supply_humidity_ratio_maximum_assignment_lifecycle_summary(
        &runtime,
        output.initialization.system,
    )
    .ok()?;
    let predecessor = purchased_air_calc_cooling_supply_humidity_ratio_humidification_supply_humidity_ratio_for_humidification_maximum_limit_lifecycle_summary(
        &runtime,
        output.initialization.system,
    )
    .ok()?;
