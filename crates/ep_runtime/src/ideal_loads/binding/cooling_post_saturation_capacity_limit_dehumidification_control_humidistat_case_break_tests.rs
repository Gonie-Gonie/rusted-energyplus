...
#[test]
fn binding_places_cp396_after_cp395_before_unchanged_numerical_coupling() {
    for (limit, humidity_ratio, availability, capacity) in [
        ...
        let predecessor = output.calculation_cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_supply_humidity_ratio_assignment;
        ...
        let lifecycle = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_case_break_lifecycle_summary(
            &runtime,
            system,
        )
        .expect("CP396 lifecycle");
        let predecessor_lifecycle = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_supply_humidity_ratio_assignment_lifecycle_summary(
            &runtime,
            system,
        )
        ...
        assert!(
            output
                ...
                .humidity_ratio
                .is_finite(),
            "CP396 evidence must not replace the numerical PurchasedAir output"
        );
    }
}

fn assert_carriers(
    snapshot: crate::ideal_loads::PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationControlHumidistatCaseBreakSnapshot,
