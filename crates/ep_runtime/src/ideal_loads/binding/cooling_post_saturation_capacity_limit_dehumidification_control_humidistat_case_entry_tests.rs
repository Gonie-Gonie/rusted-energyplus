...
#[test]
fn binding_places_cp394_after_cp393_before_unchanged_numerical_coupling() {
    for (limit, humidity_ratio, availability, capacity) in [
        ...
        let predecessor = output.calculation_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_case_break;
        ...
        let lifecycle = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_case_entry_lifecycle_summary(
            &runtime,
            system,
        )
        .expect("CP394 lifecycle");
        let predecessor_lifecycle = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_case_break_lifecycle_summary(
            &runtime,
            system,
        )
        ...
        assert!(
            output
                ...
                .humidity_ratio
                .is_finite(),
            "CP394 evidence must not replace the numerical PurchasedAir output"
        );
    }
}

fn assert_carriers(
    snapshot: crate::ideal_loads::PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationControlHumidistatCaseEntrySnapshot,
