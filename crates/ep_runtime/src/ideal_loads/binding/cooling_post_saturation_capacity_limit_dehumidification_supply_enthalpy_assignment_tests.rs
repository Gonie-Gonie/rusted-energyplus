use crate::{
    ideal_loads::{
        ...
        advance_direct_no_oa_calc_cooling_post_saturation_capacity_limit_dehumidification_supply_enthalpy_assignment as advance_cp417,
...
#[test]
fn cp417_binding_contract_is_source_ordered_after_cp416() {
    assert_eq!(
        PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_SUPPLY_ENTHALPY_ASSIGNMENT_SOURCE,
...
#[test]
fn public_release_assigns_psychrometric_supply_enthalpy() {
    ...
    reset_cp417(&mut runtime, predecessor.system);

    let snapshot = advance_cp417(&mut runtime, &matching_system(), predecessor)
        .expect("finite active CP417 release");
    assert!(
    ...
    let temperature = predecessor
        ...
        .expect("CP416 supply temperature");
    let humidity_ratio = predecessor
        .resulting_supply_humidity_ratio
        .expect("CP416 supply humidity ratio");
    ...
    assert_eq!(
        snapshot.assigned_supply_enthalpy_j_per_kg.map(f64::to_bits),
        Some(expected.to_bits()),
    );
    assert_eq!(
...
#[test]
fn public_release_rejects_forged_cp416_hidden_witness_transactionally() {
    let (mut runtime, output) = active_fixture();
    let predecessor = output
        .calculation_cooling_post_saturation_capacity_limit_dehumidification_supply_humidity_ratio_assignment;
    reset_cp417(&mut runtime, predecessor.system);
    let mut forged = predecessor;
    forged.source_order = &["forged-hidden-cp416-witness"];
    ...
    assert!(advance_cp417(&mut runtime, &matching_system(), predecessor).is_err());
    assert_eq!(
        runtime
            .units
            .get(&predecessor.system)
...
fn reset_cp417(
    ...
    runtime
        ...
        .expect("CP417 unit")
        .calc_cooling_post_saturation_capacity_limit_dehumidification_supply_enthalpy_assignment =
        Cp417State::new(system);
    runtime.clear_cooling_post_saturation_capacity_limit_dehumidification_supply_enthalpy_assignment_latest_witness_for_test(system);
}
