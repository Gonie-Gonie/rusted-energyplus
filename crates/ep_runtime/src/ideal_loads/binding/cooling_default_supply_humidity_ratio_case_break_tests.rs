...
#[test]
fn binding_orders_cp367_then_cp368_before_numerical_and_does_not_feed_result() {
    for (cooling_limit, maximum_capacity_w) in [
        ...
        assert!(case.is_some(), "CP368 binding fixture must succeed");
        ...
        assert!(summary.is_ok(), "CP368 lifecycle summary must be available");
...
#[test]
fn binding_cp368_is_complete_skip_for_every_direct_route() {
    for (load, availability, capacity, expected) in [
        ...
        assert!(case.is_some(), "CP368 route fixture must succeed");
...
#[test]
fn binding_rejects_corrupt_cp367_without_mutation() {
    ...
    let binding = bind_direct_zone_purchased_air_model(&model).expect("CP368 binding");
    ...
    .expect("canonical CP368 call");
    let system = output.initialization.system;
    runtime
        .units
        .get_mut(&system)
        .expect("retained CP368 unit")
    ...
    runtime.clear_cooling_default_supply_humidity_ratio_case_break_latest_witness_for_test(system);
    let before = runtime
        .units
        .get(&system)
        .expect("retained CP368 unit")
    ...
    let mut corrupt = output.calculation_cooling_default_supply_humidity_ratio_mixed_air_assignment;
    ...
    assert!(
        runtime
            .cooling_default_supply_humidity_ratio_case_break_latest_witness(system)
            .is_none()
    );
}
