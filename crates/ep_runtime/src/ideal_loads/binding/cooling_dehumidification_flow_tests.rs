use super::*;
use crate::ideal_loads::calc::cooling_dehumidification_flow_snapshot_is_exact_direct_release;
...
#[test]
fn scheduled_binding_executes_cooling_dehumidification_flow_before_numerical_coupling() {
    ...
    .expect("source-ordered CP319 cooling coupling");
    ...
    assert!(cooling_dehumidification_flow_snapshot_is_exact_direct_release(flow));
    ...
    assert_eq!(
        flow.predecessor_cooling_on_body_entered,
        predecessor.cooling_on_body_entered
    );
    assert_eq!(
    ...
    assert!(flow.supply_mass_flow_rate_for_dehumidification_reset_assigned);
    assert_eq!(
        flow.reset_supply_mass_flow_rate_for_dehumidification_kg_per_s
            .expect("dehumidification reset")
            .to_bits(),
        0.0_f64.to_bits()
    );
    ...
    assert!(flow.dehumidification_control_type_read);
    assert_eq!(
        flow.dehumidification_control_type,
        Some(DehumidificationControlType::None)
    );
    assert_eq!(flow.dehumidification_control_type_humidistat, Some(false));
    assert!(!flow.dehumidification_control_body_entered);
    assert!(!flow.zone_dehumidifying_setpoint_moisture_demand_read);
    assert!(!flow.minimum_cooling_supply_air_humidity_ratio_read);
    assert!(!flow.zone_humidity_ratio_read);
    assert!(!flow.delta_humidity_ratio_calculated);
    assert!(!flow.dehumidification_flow_body_entered);
    assert!(!flow.supply_mass_flow_rate_for_dehumidification_assigned);
    assert_eq!(
        flow.resulting_supply_mass_flow_rate_for_dehumidification_kg_per_s
            .expect("reset dehumidification candidate")
            .to_bits(),
        0.0_f64.to_bits()
    );

    let lifecycle = purchased_air_calc_cooling_dehumidification_flow_lifecycle_summary(
        &runtime,
    ...
    assert_eq!(state.dehumidification_control_type_humidistat_count, 0);
...
#[test]
fn scheduled_binding_records_unit_off_and_non_cooling_cp319_skips() {
        ...
        [(0.0, 3_000.0, true, false), (1.0, 0.0, false, true)]
    {
        ...
        .expect("source-ordered CP319 skip coupling");
        ...
        assert!(!flow.cooling_on_body_entered);
        assert!(!flow.dehumidification_control_type_read);
        assert_eq!(flow.dehumidification_control_type, None);
        assert_eq!(
            flow.resulting_supply_mass_flow_rate_for_dehumidification_kg_per_s,
            None
        );
...
#[test]
fn public_cooling_dehumidification_flow_replay_is_whole_state_transactional() {
    let (model, cache) = fixture(|_| {});
    let binding = bind_direct_zone_purchased_air_model(&model).expect("bounded model binding");
    let mut zone_state = zone_state_for_temp_independent_load(3_000.0);
    let mut runtime = PurchasedAirRuntimeState::default();
    ...
    .expect("source-ordered CP319 call");
    ...
    *flow_state = PurchasedAirCalcCoolingDehumidificationFlowRuntimeState::new(
        binding.ideal_loads_air_system,
    );
