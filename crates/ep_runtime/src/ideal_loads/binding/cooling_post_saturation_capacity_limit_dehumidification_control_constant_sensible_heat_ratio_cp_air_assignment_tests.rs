...
fn run_case(
    ...
    .expect("source-ordered CP387 coupling");
    (runtime, output)
}

#[test]
fn binding_completes_cp387_as_exact_none_selector_skip_after_cp386() {
    for (limit, humidity_ratio, capacity, availability) in [
        ...
        let lifecycle = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_constant_sensible_heat_ratio_cp_air_assignment_lifecycle_summary(
            &runtime,
            system,
        )
        .expect("CP387 lifecycle");
        let predecessor_lifecycle =
            purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_switch_lifecycle_summary(
                &runtime,
                system,
            )
...
fn assert_complete_null(
    snapshot: crate::ideal_loads::PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationControlConstantSensibleHeatRatioCpAirAssignmentSnapshot,
) {
