...
fn run_case(
    ...
    .expect("source-ordered CP336 coupling");
    (runtime, output)
}

#[test]
fn scheduled_binding_evaluates_and_assigns_canonical_supply_enthalpy() {
    ...
    let assignment = output.calculation_cooling_positive_supply_enthalpy_assignment;
    let supply_temperature_c = temperature
        .assigned_supply_temperature_c
        .expect("CP334 supply temperature");
    let supply_humidity_ratio = humidity
        .assigned_supply_humidity_ratio
        .expect("CP335 supply humidity ratio");
    let expected = energyplus_psy_h_fn_tdb_w(supply_temperature_c, supply_humidity_ratio);

    assert!(assignment.supply_enthalpy_assignment_executed);
    assert!(cooling_positive_supply_enthalpy_assignment_snapshot_is_exact_direct_release(
        assignment,
    ));
    assert_eq!(
    ...
    let state = purchased_air_calc_cooling_positive_supply_enthalpy_assignment_lifecycle_summary(
        &runtime,
        output.initialization.system,
    )
    .expect("CP336 lifecycle")
    .state;
    assert_counter_shape(&state, true, false, false, false);
}

#[test]
fn scheduled_binding_skips_cp336_after_the_positive_guard_falls_through() {
    let (runtime, output) = run_case(
    ...
    .expect("CP336 guard-false lifecycle")
    .state;
    assert_counter_shape(&state, false, false, false, true);
}

#[test]
fn scheduled_binding_preserves_unit_off_and_non_cooling_cp336_skip_routes() {
        ...
        [(0.0, 3_000.0, true, false), (1.0, 0.0, false, true)]
    {
        ...
        let state =
            ...
            .expect("CP336 skipped lifecycle")
            .state;
...
fn assert_snapshot_has_no_source_values(
    snapshot: crate::ideal_loads::PurchasedAirCalcCoolingPositiveSupplyEnthalpyAssignmentSnapshot,
) {
...
fn assert_counter_shape(
    ...
    assert_eq!(state.supply_enthalpy_assignment_count, executions);
    ...
    assert_eq!(state.supply_enthalpy_assignment_write_count, executions);
}
