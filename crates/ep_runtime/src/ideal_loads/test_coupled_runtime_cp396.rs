//! Non-vacuous CP396 coupled-runtime integration tests.
...
use crate::ideal_loads::{
    ...
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationControlHumidistatSupplyHumidityRatioAssignmentLifecycleSummary as Predecessor,
...
use super::super::cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_case_break_validation::{
    snapshot_matches_release, validate_lifecycle,
};

#[test]
fn cp396_preserves_cp395_terminal_carriers_and_skips_break_on_direct_routes() {
    for (limit, maximum_capacity_w, availability) in [
        ...
        let snapshot = output.calculation_cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_case_break;
        let cp395 = output.calculation_cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_supply_humidity_ratio_assignment;
...
#[test]
fn cp396_rejects_cp395_carrier_bit_drift_and_route_drift() {
    ...
    let value = predecessor
        ...
        .as_mut()
        .expect("CP395 retained enthalpy");
...
#[test]
fn cp396_validation_remains_independent_of_numerical_output_state() {
    let (model, mut output, lifecycle, predecessor) =
        validator_fixture(IdealLoadsLimit::LimitCapacity, 0.020, 500.0, 1.0, 2);
    let binding = bind_direct_zone_purchased_air_model(&model).expect("CP396 binding");
    output
        .coupling
        .purchased_air
        .calculation
...
fn validator_fixture(
    ...
    let cache = precompute_schedule_cache(&model.typed, 1).expect("CP396 schedule cache");
    let binding = bind_direct_zone_purchased_air_model(&model).expect("CP396 direct binding");
    ...
    for step in 0..steps {
        ...
        latest = Some(
            ...
            .expect("CP396 coupling"),
        );
    }
    let output = latest.expect("at least one CP396 step");
    let system = output.initialization.system;
    let lifecycle = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_case_break_lifecycle_summary(&runtime, system).expect("CP396 lifecycle");
    let predecessor = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_supply_humidity_ratio_assignment_lifecycle_summary(&runtime, system).expect("CP395 lifecycle");
...
fn latest_violation() -> Error {
    Error::CalcCoolingPostSaturationCapacityLimitDehumidificationControlHumidistatCaseBreakLifecycleInvariant {
