//! Non-vacuous CP380 coupled-runtime integration tests.
...
use crate::ideal_loads::{
    ...
    PurchasedAirCalcCoolingSupplyEnthalpyPostSaturationAssignmentLifecycleSummary as Predecessor,
...
use super::super::cooling_post_saturation_capacity_limit_guard_validation::{
    snapshot_matches_release, validate_lifecycle,
};

#[test]
fn cp380_follows_cp379_and_locks_all_four_selector_shapes() {
    for (limit, expected_sites, expected_body) in [
        (IdealLoadsLimit::LimitCapacity, 3, true),
        (IdealLoadsLimit::LimitFlowRateAndCapacity, 5, true),
        (IdealLoadsLimit::NoLimit, 4, false),
        (IdealLoadsLimit::LimitFlowRate, 4, false),
    ] {
        let (model, output, lifecycle, predecessor) = validator_fixture(limit);
        ...
        assert_eq!(snapshot.capacity_limit_body_entered, expected_body);
        assert_eq!(snapshot.active_guard_false_fallthrough, !expected_body);
        assert_eq!(snapshot.first_cooling_limit, Some(limit));
        assert_eq!(
            snapshot.second_cooling_limit,
            (limit != IdealLoadsLimit::LimitCapacity).then_some(limit),
        );
        assert_eq!(
            lifecycle.state.capacity_limit_guard_evaluation_count,
            predecessor
                .state
...
#[test]
fn cp380_rejects_control_corruption_but_ignores_numerical_results() {
    let (model, output, lifecycle, predecessor) = validator_fixture(IdealLoadsLimit::NoLimit);
    ...
    bad.state
        ...
        .as_mut()
        .expect("CP380 latest")
        .configured_cooling_limit_owned_read = false;
    assert_eq!(
        validate(&model, &output, &bad, &predecessor),
        Err(
...
fn validator_fixture(
    ...
    let cache = precompute_schedule_cache(&model.typed, 1).expect("CP380 schedule cache");
    let binding = bind_direct_zone_purchased_air_model(&model).expect("CP380 direct binding");
