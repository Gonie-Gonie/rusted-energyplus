...
#[test]
fn cp355_coupled_direct_none_route_is_complete_skip_and_numerical_humidity_remains_unfed() {
    ...
    let schedule_cache =
        precompute_schedule_cache(&model.typed, 1).expect("one CP355 schedule sample");
    ...
    .expect("valid CP355 direct simulation");
    let summary = &simulation.summary;
    let lifecycle =
        &summary.calc_cooling_constant_shr_supply_humidity_ratio_minimum_limit_lifecycle;
    let state = &lifecycle.state;
    let snapshot = state.latest.expect("latest CP355 snapshot");
    let predecessor = summary
        .calc_cooling_constant_shr_supply_humidity_ratio_overdrying_limit_lifecycle
        .state
        .latest
        .expect("latest CP354 predecessor");

    assert_eq!(
        lifecycle.source,
        crate::ideal_loads::PURCHASED_AIR_CALC_COOLING_CONSTANT_SHR_SUPPLY_HUMIDITY_RATIO_MINIMUM_LIMIT_SOURCE
    );
    assert_eq!(
        lifecycle.first_excluded_source,
        crate::ideal_loads::PURCHASED_AIR_CALC_COOLING_CONSTANT_SHR_SUPPLY_HUMIDITY_RATIO_MINIMUM_LIMIT_FIRST_EXCLUDED_SOURCE
    );
    assert_eq!(
        crate::ideal_loads::PURCHASED_AIR_CALC_COOLING_CONSTANT_SHR_SUPPLY_HUMIDITY_RATIO_MINIMUM_LIMIT_SOURCE_ORDER
            .len(),
        4
    );
    ...
    let expected =
        super::super::cooling_constant_shr_supply_humidity_ratio_minimum_limit_validation::expected_snapshot(
            predecessor,
        );
    assert!(
        super::super::cooling_constant_shr_supply_humidity_ratio_minimum_limit_validation::snapshots_match_exact_bits(
            &snapshot,
            &expected,
        )
    );
    let mut corrupted = snapshot;
    corrupted.source_order = &["forged-cp355-source-order"];
    assert!(
        !super::super::cooling_constant_shr_supply_humidity_ratio_minimum_limit_validation::snapshots_match_exact_bits(
            &corrupted,
            &expected,
    ...
    assert!(
        ...
        "CP355 complete-null evidence must not replace numerical supply humidity"
    );
}
