...
use crate::ideal_loads::{
    ...
    PurchasedAirCalcCoolingConstantSupplyHumidityRatioCaseBreakLifecycleSummary as PredecessorLifecycle,
    PurchasedAirCalcCoolingDefaultSupplyHumidityRatioMixedAirAssignmentLifecycleSummary as Lifecycle,
...
use super::super::cooling_default_supply_humidity_ratio_mixed_air_assignment_validation::{
    expected_snapshot, validate_lifecycle,
};

#[test]
fn cp367_lifecycle_matches_outputs_and_cp366_without_feeding_numerical_result() {
    let fixture = validator_fixture();
    assert!(fixture.is_some(), "CP367 validator fixture");
    let Some((model, output, lifecycle, predecessor)) = fixture else {
        return;
    };
    assert!(
        validate(&model, &output, &lifecycle, &predecessor).is_ok(),
        "canonical CP367 coupled lifecycle"
    );
    ...
    assert_eq!(
        snapshot,
        expected_snapshot(output.calculation_cooling_constant_supply_humidity_ratio_case_break)
    );
    ...
    assert_eq!(
        ...
        "CP367 control-only evidence must not feed the coupling result"
    );
}

#[test]
fn cp367_route_source_latest_and_predecessor_corruption_are_rejected() {
    let fixture = validator_fixture();
    assert!(fixture.is_some(), "CP367 corruption fixture");
    let Some((model, output, lifecycle, predecessor)) = fixture else {
        return;
    };
    ...
    for (field, mutate) in [
        (
            ...
            set_read_count as fn(&mut Lifecycle),
        ),
        (
            "supply_humidity_ratio_assignment_count",
            set_assignment_count as fn(&mut Lifecycle),
        ),
        (
    ...
    assert!(latest_snapshot.is_some(), "CP367 latest snapshot");
    ...
    assert!(predecessor_latest.is_some(), "CP366 predecessor snapshot");
...
fn set_read_count(lifecycle: &mut Lifecycle) {
    lifecycle.state.mixed_air_humidity_ratio_read_count = 1;
}

fn set_assignment_count(lifecycle: &mut Lifecycle) {
...
#[test]
fn cp367_expected_snapshot_maps_all_typed_selected_routes_to_default_skip() {
    let fixture = validator_fixture();
    assert!(fixture.is_some(), "CP367 route-mapping fixture");
    let Some((_, output, _, _)) = fixture else {
        return;
    };
    let base = output.calculation_cooling_constant_supply_humidity_ratio_case_break;
    ...
    for control in [
        ...
        predecessor.dehumidification_control_humidistat_case_completed_skip =
            control == DehumidificationControlType::Humidistat;
        predecessor.dehumidification_control_constant_supply_humidity_ratio_case_exited_via_break =
            control == DehumidificationControlType::ConstantSupplyHumidityRatio;
...
fn validate(
    ...
    assert!(binding.is_ok(), "CP367 binding");
