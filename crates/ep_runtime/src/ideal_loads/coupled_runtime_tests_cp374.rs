...
use super::super::cooling_supply_humidity_ratio_humidification_supply_humidity_ratio_for_humidification_maximum_limit_validation::{
    expected_snapshot, validate_lifecycle,
};

#[test]
fn cp374_lifecycle_matches_cp373_and_does_not_feed_numerical_result() {
    let fixture = validator_fixture();
    assert!(fixture.is_some(), "CP374 validator fixture");
...
#[test]
fn cp374_direct_counters_and_latest_corruption_are_rejected() {
    let fixture = validator_fixture();
    assert!(fixture.is_some(), "CP374 corruption fixture");
    let Some((model, output, lifecycle, predecessor)) = fixture else {
        return;
    };

    for mutate in [
        set_humidistat_limit_one as fn(&mut Lifecycle),
        set_none_limit_one,
    ...
    assert!(latest.state.latest.is_some(), "CP374 latest must exist");
    let Some(latest_snapshot) = latest.state.latest.as_mut() else {
        return;
    };
    latest_snapshot.source_shaped_two_argument_minimum_evaluated = true;
    assert_eq!(
    ...
    assert!(
        ...
        "CP373 predecessor latest must exist"
    );
    let Some(predecessor_latest_snapshot) = predecessor_latest.state.latest.as_mut() else {
        return;
    };
    predecessor_latest_snapshot.supply_humidity_ratio_for_humidification_assigned = true;
    assert_eq!(
...
fn set_humidistat_limit_one(lifecycle: &mut Lifecycle) {
    lifecycle.state.dehumidification_control_humidistat_supply_humidity_ratio_for_humidification_maximum_limit_count = 1;
}

fn set_none_limit_one(lifecycle: &mut Lifecycle) {
    lifecycle.state.dehumidification_control_none_supply_humidity_ratio_for_humidification_maximum_limit_count = 1;
}
...
fn set_left_read_one(lifecycle: &mut Lifecycle) {
    lifecycle
        .state
        .supply_humidity_ratio_for_humidification_for_maximum_limit_minimum_read_count = 1;
}

fn set_right_read_one(lifecycle: &mut Lifecycle) {
    lifecycle
        .state
        .maximum_heating_supply_air_humidity_ratio_for_minimum_read_count = 1;
}

fn set_minimum_one(lifecycle: &mut Lifecycle) {
    lifecycle
        .state
...
fn validator_fixture() -> Option<(
    ...
    .ok()?;
    let lifecycle = purchased_air_calc_cooling_supply_humidity_ratio_humidification_supply_humidity_ratio_for_humidification_maximum_limit_lifecycle_summary(
        &runtime,
        output.initialization.system,
    )
    .ok()?;
    let predecessor = purchased_air_calc_cooling_supply_humidity_ratio_humidification_supply_humidity_ratio_for_humidification_assignment_lifecycle_summary(
        &runtime,
        output.initialization.system,
    )
    .ok()?;
