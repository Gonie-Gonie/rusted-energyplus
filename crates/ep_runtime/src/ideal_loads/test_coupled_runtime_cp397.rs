//! Non-vacuous CP397 coupled-runtime integration tests.

use super::*;
use crate::ideal_loads::{
    DirectZonePurchasedAirScheduledCouplingInput, DirectZonePurchasedAirScheduledCouplingOutput,
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationControlHumidistatCaseBreakLifecycleSummary as Predecessor,
...
use super::super::cooling_post_saturation_capacity_limit_dehumidification_control_none_case_entry_validation::{
    snapshot_matches_release, validate_lifecycle,
};

#[test]
fn cp397_preserves_cp396_terminal_carriers_and_tracks_direct_none_entry() {
...
#[test]
fn cp397_rejects_cp396_carrier_bit_drift_and_route_drift() {
    ...
    let value = predecessor
        ...
        .as_mut()
        .expect("CP396 retained enthalpy");
...
#[test]
fn cp397_validation_remains_independent_of_numerical_output_state() {
    let (model, mut output, lifecycle, predecessor) =
        validator_fixture(IdealLoadsLimit::LimitCapacity, 0.020, 500.0, 1.0, 2);
    let binding = bind_direct_zone_purchased_air_model(&model).expect("CP397 binding");
    output
        .coupling
        .purchased_air
        .calculation
...
fn validator_fixture(
    ...
    let cache = precompute_schedule_cache(&model.typed, 1).expect("CP397 schedule cache");
    let binding = bind_direct_zone_purchased_air_model(&model).expect("CP397 direct binding");
    ...
    for step in 0..steps {
        ...
        latest = Some(
            ...
            .expect("CP397 coupling"),
        );
    }
    let output = latest.expect("at least one CP397 step");
    let system = output.initialization.system;
    let lifecycle = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_none_case_entry_lifecycle_summary(&runtime, system).expect("CP397 lifecycle");
    let predecessor = purchased_air_calc_cooling_post_saturation_capacity_limit_dehumidification_control_humidistat_case_break_lifecycle_summary(&runtime, system).expect("CP396 lifecycle");
