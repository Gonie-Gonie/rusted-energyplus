//! CP441 heating maximum-flow continue-warning timestamp call-site assertions.
...
pub(super) fn assert_direct(runtime: &Value, results: &Value) {
    let lifecycle = &runtime[CP441_KEY];
    let predecessor = &runtime[CP440_KEY];
    assert_eq!(
        lifecycle["source"],
    ...
    for field in route_fields {
        ...
        assert_eq!(values.len(), 36, "CP441 {field} width");
        for (index, value) in values.iter().enumerate() {
            if !PUBLIC.contains(&index) {
                assert_eq!(count_value(value), 0, "CP441 {field} route {index}");
            }
        }
    }
    assert_eq!(
        lifecycle["heating_outdoor_air_maximum_flow_continue_warning_timestamp_call_route_counts"],
        lifecycle["predecessor_continue_warning_call_route_counts"]
    );
    let calls = sum(array(
        lifecycle,
        "heating_outdoor_air_maximum_flow_continue_warning_timestamp_call_route_counts",
    ...
    for field in [
        "heating_outdoor_air_maximum_flow_continue_warning_timestamp_call_site_count",
        "source_site_execution_count",
        "cp440_outdoor_air_flow_maximum_heating_output_error_count_state_owner_count",
        "unchanged_outdoor_air_flow_maximum_heating_output_error_count_preservation_count",
    ] {
        assert_eq!(count(lifecycle, field), 0, "CP441 {field}");
    }
    for (owner, predecessor_preservation, preservation) in [
        (
            "cp440_supply_humidity_ratio_state_owner_count",
            "unchanged_supply_humidity_ratio_preservation_count",
            "unchanged_supply_humidity_ratio_preservation_count",
        ),
        (
            "cp440_supply_enthalpy_state_owner_count",
            "unchanged_supply_enthalpy_preservation_count",
            "unchanged_supply_enthalpy_preservation_count",
        ),
        (
            "cp440_supply_temperature_state_owner_count",
...
pub(super) fn assert_non_direct(runtime: &Map<String, Value>) {
    assert!(runtime.contains_key(CP441_KEY));
    assert!(
...
fn assert_schema_and_binding_cardinalities() {
    let core = include_str!(
        "../../../ep_runtime/src/ideal_loads/calc/heating_outdoor_air_maximum_flow_continue_warning_timestamp_call.rs"
    );
    let snapshot = core
        .split_once(
...
fn array<'a>(lifecycle: &'a Value, field: &str) -> &'a Vec<Value> {
    lifecycle[field].as_array().expect("CP441 route array")
}
...
fn count_value(value: &Value) -> u64 {
    value.as_u64().expect("CP441 unsigned count")
}
