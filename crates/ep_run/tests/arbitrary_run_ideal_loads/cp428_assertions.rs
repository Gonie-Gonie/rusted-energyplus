...
#[path = "cp429_assertions.rs"]
mod cp429_assertions;
...
pub(super) fn assert_direct(runtime: &Value, results: &Value) {
    ...
    cp429_assertions::assert_direct(runtime, results);
}
...
pub(super) fn assert_non_direct(runtime: &Map<String, Value>) {
    ...
    cp429_assertions::assert_non_direct(runtime);
}
...
fn assert_schema_and_binding_cardinalities() {
    ...
    assert_eq!(binding.matches("    pub calculation_").count(), 124);
}
