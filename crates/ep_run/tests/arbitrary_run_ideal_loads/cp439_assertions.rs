//! CP439 heating maximum-flow first-warning call-site assertions.

#[path = "cp440_assertions.rs"]
mod cp440_assertions;
...
const MARKER: &str = "heating_outdoor_air_maximum_flow_first_warning_call_site_reached";
const ORDER: [&str; 1] = ["reach-heating-outdoor-air-maximum-flow-first-warning-call-site"];
const PUBLIC: [usize; 13] = [0, 1, 2, 3, 4, 5, 6, 7, 8, 20, 21, 26, 27];

pub(super) fn assert_direct(runtime: &Value, results: &Value) {
    let lifecycle = &runtime[CP439_KEY];
    let predecessor = &runtime[CP438_KEY];
    assert_eq!(
        lifecycle["source"],
        json!("EnergyPlus 26.1 PurchasedAirManager.cc:2366-2370")
    );
    assert_eq!(
        lifecycle["first_excluded_source"],
        json!("EnergyPlus 26.1 PurchasedAirManager.cc:2371")
    );
    ...
    for field in route_fields {
        ...
        assert_eq!(values.len(), 36, "CP439 {field} width");
        for (index, value) in values.iter().enumerate() {
            if !PUBLIC.contains(&index) {
                assert_eq!(count_value(value), 0, "CP439 {field} route {index}");
            }
        }
    }
    assert_eq!(
        lifecycle["heating_outdoor_air_maximum_flow_first_warning_call_route_counts"],
        lifecycle["predecessor_first_warning_counter_increment_route_counts"]
    );
    let calls = sum(array(
        lifecycle,
        "heating_outdoor_air_maximum_flow_first_warning_call_route_counts",
    ...
    for field in [
        ...
        assert_eq!(count(lifecycle, field), 0, "CP439 {field}");
    }
    for (owner, predecessor_preservation, preservation) in [
        (
            "cp438_supply_humidity_ratio_state_owner_count",
            "unchanged_supply_humidity_ratio_preservation_count",
            "unchanged_supply_humidity_ratio_preservation_count",
        ),
        (
            "cp438_supply_enthalpy_state_owner_count",
            "unchanged_supply_enthalpy_preservation_count",
            "unchanged_supply_enthalpy_preservation_count",
        ),
        (
            "cp438_supply_temperature_state_owner_count",
    ...
    for (key, value) in predecessor_latest {
        if !["source", "first_excluded_source", "source_order"].contains(&key.as_str()) {
            assert_eq!(
                latest.get(key),
                Some(value),
                "CP439 changed CP438 field {key}"
            );
        }
    }
    assert_eq!(latest[MARKER], json!(false));
    assert_schema_and_binding_cardinalities();
    assert!(!results.to_string().contains(CP439_KEY));
    cp440_assertions::assert_direct(runtime, results);
}

pub(super) fn assert_non_direct(runtime: &Map<String, Value>) {
    assert!(runtime.contains_key(CP439_KEY));
    assert!(
        runtime[CP439_KEY].is_null(),
        "non-direct runtime must not publish CP439 evidence"
    );
    cp440_assertions::assert_non_direct(runtime);
}
...
fn assert_schema_and_binding_cardinalities() {
    let core = include_str!(
        "../../../ep_runtime/src/ideal_loads/calc/heating_outdoor_air_maximum_flow_first_warning_call.rs"
    );
    let snapshot = core
        .split_once(
...
fn array<'a>(lifecycle: &'a Value, field: &str) -> &'a Vec<Value> {
    lifecycle[field].as_array().expect("CP439 route array")
}
...
fn count_value(value: &Value) -> u64 {
    value.as_u64().expect("CP439 unsigned count")
}
