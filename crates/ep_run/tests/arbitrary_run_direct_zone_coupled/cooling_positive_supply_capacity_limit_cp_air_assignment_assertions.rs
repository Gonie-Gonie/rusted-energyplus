//! CP338 run-summary assertions shared by direct-Zone integration routes.

use serde_json::Value;

use super::{assert_exact_object_keys, string_array};

const SOURCE: &str = "EnergyPlus 26.1 PurchasedAirManager.cc:2196";
const FIRST_EXCLUDED_SOURCE: &str = "EnergyPlus 26.1 PurchasedAirManager.cc:2197";
...
pub(super) fn assert_cooling_positive_supply_capacity_limit_cp_air_assignment(
    runtime: &Value,
    expected_calls: u64,
    expected_unit_off_skips: u64,
    expected_non_cooling_skips: u64,
) {
    let lifecycle = &runtime["purchased_air_calc_cooling_positive_supply_capacity_limit_cp_air_assignment_lifecycle"];
    assert!(lifecycle.is_object(), "direct runtime must publish CP338");
    ...
    assert_eq!(
        lifecycle["capacity_limit_guard_false_fallthrough_skip_count"],
        predecessor["active_guard_false_fallthrough_count"]
    );
    assert_eq!(
        lifecycle["capacity_limit_cp_air_assignment_count"],
        predecessor["capacity_limit_body_entry_count"]
    );

    let assignments = lifecycle["capacity_limit_cp_air_assignment_count"]
        .as_u64()
        .expect("CP338 assignment count");
    let positive_skips = lifecycle["positive_guard_false_fallthrough_skip_count"]
        .as_u64()
        .expect("CP338 positive-guard skips");
    let capacity_skips = lifecycle["capacity_limit_guard_false_fallthrough_skip_count"]
        .as_u64()
        .expect("CP338 capacity-guard skips");
    assert_eq!(
    ...
    assert_eq!(
        ...
        predecessor_latest["capacity_limit_guard_evaluated"]
    );
    assert_eq!(
        latest["predecessor_capacity_limit_body_entered"],
        predecessor_latest["capacity_limit_body_entered"]
    );
    assert_eq!(
    ...
    assert_eq!(
        latest["capacity_limit_cp_air_assignment_executed"],
        predecessor_latest["capacity_limit_body_entered"]
    );

    if latest["capacity_limit_cp_air_assignment_executed"] != true {
        assert_eq!(latest["mixed_air_humidity_ratio_read"], false);
        assert!(latest["mixed_air_humidity_ratio"].is_null());
        assert!(latest["mixed_air_humidity_ratio_ieee_bits"].is_null());
    ...
    assert_eq!(latest["mixed_air_humidity_ratio_read"], true);
