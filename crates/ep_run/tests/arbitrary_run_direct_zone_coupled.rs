...
#[path = "arbitrary_run_direct_zone_coupled/cooling_positive_supply_capacity_limit_sensible_output_assignment_assertions.rs"]
mod cooling_positive_supply_capacity_limit_sensible_output_assignment_assertions;
#[path = "arbitrary_run_direct_zone_coupled/cooling_positive_supply_capacity_limit_sensible_output_guard_assertions.rs"]
mod cooling_positive_supply_capacity_limit_sensible_output_guard_assertions;
#[path = "arbitrary_run_direct_zone_coupled/cooling_positive_supply_cp_air_assignment_assertions.rs"]
mod cooling_positive_supply_cp_air_assignment_assertions;
#[path = "arbitrary_run_direct_zone_coupled/cooling_positive_supply_enthalpy_assignment_assertions.rs"]
mod cooling_positive_supply_enthalpy_assignment_assertions;
#[path = "arbitrary_run_direct_zone_coupled/cooling_positive_supply_humidity_ratio_mixed_air_assignment_assertions.rs"]
mod cooling_positive_supply_humidity_ratio_mixed_air_assignment_assertions;
#[path = "arbitrary_run_direct_zone_coupled/cooling_positive_supply_temperature_assignment_assertions.rs"]
mod cooling_positive_supply_temperature_assignment_assertions;
#[path = "arbitrary_run_direct_zone_coupled/cooling_positive_supply_temperature_minimum_limit_assertions.rs"]
mod cooling_positive_supply_temperature_minimum_limit_assertions;
#[path = "arbitrary_run_direct_zone_coupled/cooling_positive_supply_temperature_mixed_air_limit_assertions.rs"]
mod cooling_positive_supply_temperature_mixed_air_limit_assertions;
...
use cooling_positive_supply_capacity_limit_sensible_output_assignment_assertions::assert_cooling_positive_supply_capacity_limit_sensible_output_assignment;
use cooling_positive_supply_capacity_limit_sensible_output_guard_assertions::assert_cooling_positive_supply_capacity_limit_sensible_output_guard;
...
use cooling_positive_supply_humidity_ratio_mixed_air_assignment_assertions::assert_cooling_positive_supply_humidity_ratio_mixed_air_assignment;
use cooling_positive_supply_temperature_assignment_assertions::assert_cooling_positive_supply_temperature_assignment;
use cooling_positive_supply_temperature_minimum_limit_assertions::assert_cooling_positive_supply_temperature_minimum_limit;
use cooling_positive_supply_temperature_mixed_air_limit_assertions::assert_cooling_positive_supply_temperature_mixed_air_limit;
...
const CALC_COOLING_HUMIDIFICATION_FLOW_SOURCE: &str =
    "EnergyPlus 26.1 PurchasedAirManager.cc:2133-2144";
const CALC_COOLING_HUMIDIFICATION_FLOW_FIRST_EXCLUDED_SOURCE: &str =
    "EnergyPlus 26.1 PurchasedAirManager.cc:2147";
const CALC_COOLING_HUMIDIFICATION_FLOW_SOURCE_ORDER: [&str; 26] = [
...
fn assert_persistent_init_lifecycle(summary: &Value, expected_calls: u64) {
    ...
    assert_cooling_supply_mass_flow_very_small_guard(runtime, expected_calls, 0, expected_calls, 0);
    assert_cooling_supply_mass_flow_very_small_guard_body(
        runtime,
        expected_calls,
        0,
        expected_calls,
    ...
    assert_cooling_positive_supply_cp_air_assignment(runtime, expected_calls, 0, expected_calls);
    assert_cooling_positive_supply_temperature_assignment(
        runtime,
        expected_calls,
        0,
        expected_calls,
    );
    assert_cooling_positive_supply_temperature_minimum_limit(
        runtime,
        expected_calls,
        0,
        expected_calls,
    );
    assert_cooling_positive_supply_temperature_mixed_air_limit(
        runtime,
        expected_calls,
        0,
        expected_calls,
    );
    assert_cooling_positive_supply_humidity_ratio_mixed_air_assignment(
        runtime,
        expected_calls,
        0,
        expected_calls,
    );
    assert_cooling_positive_supply_enthalpy_assignment(runtime, expected_calls, 0, expected_calls);
    assert_cooling_positive_supply_capacity_limit_guard(
        runtime,
        expected_calls,
        0,
        expected_calls,
    ...
    assert_cooling_positive_supply_capacity_limit_sensible_output_assignment(
        runtime,
        expected_calls,
        0,
        expected_calls,
    );
    assert_cooling_positive_supply_capacity_limit_sensible_output_guard(
        runtime,
        expected_calls,
        0,
        expected_calls,
...
fn assert_cooling_humidification_flow(
    runtime: &Value,
    expected_calls: u64,
    expected_non_cooling_skips: u64,
    expected_cooling_entries: u64,
) {
    let flow = &runtime["purchased_air_calc_cooling_humidification_flow_lifecycle"];
    assert!(
        flow.is_object(),
        "direct runtime must publish the CP320 key"
    );
    assert_exact_object_keys(
        flow,
        &[
    ...
    assert_eq!(flow["source"], CALC_COOLING_HUMIDIFICATION_FLOW_SOURCE);
    assert_eq!(
        flow["first_excluded_source"],
        CALC_COOLING_HUMIDIFICATION_FLOW_FIRST_EXCLUDED_SOURCE
    );
    assert_eq!(flow["transition_count"], expected_calls);
    assert_eq!(flow["cooling_body_entry_count"], expected_cooling_entries);
    assert_eq!(flow["non_cooling_skip_count"], expected_non_cooling_skips);
    ...
    assert_eq!(
        string_array(&latest["source_order"]),
        CALC_COOLING_HUMIDIFICATION_FLOW_SOURCE_ORDER
    );
    let active = expected_cooling_entries > 0;
    ...
    if active {
        ...
        assert!(latest["heating_on"].is_null());
        assert!(latest["humidification_control_type"].is_null());
        assert!(latest["resulting_supply_mass_flow_rate_for_humidification_kg_per_s"].is_null());
    }
    assert_eq!(latest["dehumidification_control_type_first_read"], false);
    assert!(latest["first_dehumidification_control_type"].is_null());
    assert_eq!(
...
-> Result<(), Box<dyn std::error::Error>> {
    for (name, limit, first_matches, second_comparisons, second_matches, flow_reads) in [
        ...
        assert_cooling_positive_supply_cp_air_assignment(&summary["rust_runtime"], 2, 0, 0);
        assert_cooling_positive_supply_temperature_assignment(&summary["rust_runtime"], 2, 0, 0);
        assert_cooling_positive_supply_temperature_minimum_limit(&summary["rust_runtime"], 2, 0, 0);
        assert_cooling_positive_supply_temperature_mixed_air_limit(
            &summary["rust_runtime"],
            2,
            0,
        ...
        assert_cooling_positive_supply_enthalpy_assignment(&summary["rust_runtime"], 2, 0, 0);
        assert_cooling_positive_supply_capacity_limit_guard(
            &summary["rust_runtime"],
            2,
            0,
        ...
        let (comparison_false_fallthroughs, adjustment_body_entries) = match limit {
            "LimitCapacity" => (1, 1),
            "LimitFlowRateAndCapacity" => (2, 0),
            _ => (0, 0),
        };
        assert_cooling_positive_supply_capacity_limit_sensible_output_guard(
            &summary["rust_runtime"],
            ...
            0,
            0,
            comparison_false_fallthroughs,
            adjustment_body_entries,
        );
...
#[test]
fn no_limit_cooling_publishes_active_cp331_and_cp332_json_lineage()
-> Result<(), Box<dyn std::error::Error>> {
    let case_dir = unique_case_dir("no-limit-cooling-cp331-cp332")?;
    let input_path = case_dir.join("direct-zone-no-limit-cooling.epJSON");
    let weather_path = case_dir.join("weather.epw");
    let output_dir = case_dir.join("out");
    write_text(&input_path, &no_limit_cooling_fixture())?;
    write_text(&weather_path, ONE_DAY_EPW)?;
    ...
    let summary = read_json(&output_dir.join("run-summary.json"))?;
    let runtime = &summary["rust_runtime"];
    assert_cooling_supply_mass_flow_positive_guard(runtime, 2, 0, 0, 2);
    assert_eq!(
        runtime["purchased_air_calc_cooling_supply_mass_flow_positive_guard_lifecycle"]["positive_supply_mass_flow_body_entry_count"],
        2
    );
    assert_cooling_positive_supply_cp_air_assignment(runtime, 2, 0, 0);
    assert_cooling_positive_supply_temperature_assignment(runtime, 2, 0, 0);
    assert_cooling_positive_supply_temperature_minimum_limit(runtime, 2, 0, 0);
    assert_cooling_positive_supply_temperature_mixed_air_limit(runtime, 2, 0, 0);
    ...
    assert_cooling_positive_supply_capacity_limit_sensible_output_assignment(runtime, 2, 0, 0);
    assert_cooling_positive_supply_capacity_limit_sensible_output_guard(runtime, 2, 0, 0, 0, 0);
    assert_eq!(
...
#[test]
fn high_capacity_cooling_publishes_cp340_false_fallthrough()
-> Result<(), Box<dyn std::error::Error>> {
    let case_dir = unique_case_dir("high-capacity-cooling-cp340-false")?;
    let input_path = case_dir.join("direct-zone-high-capacity-cooling.epJSON");
    let weather_path = case_dir.join("weather.epw");
    let output_dir = case_dir.join("out");
    let fixture = finite_limit_cooling_fixture("LimitCapacity").replace(
        "\"maximum_total_cooling_capacity\": 300.0",
        "\"maximum_total_cooling_capacity\": 1000000000.0",
    );
    write_text(&input_path, &fixture)?;
    write_text(&weather_path, ONE_DAY_EPW)?;
    ...
    let runtime = &summary["rust_runtime"];
    assert_cooling_positive_supply_capacity_limit_sensible_output_assignment(runtime, 2, 0, 0);
    assert_cooling_positive_supply_capacity_limit_sensible_output_guard(runtime, 2, 0, 0, 2, 0);
    Ok(())
}
...
fn no_limit_cooling_fixture() -> String {
    DIRECT_ZONE_COUPLED_EPJSON
        .replace(
            "\"Heating Setpoint\": {\"hourly_value\": 30.0}",
            "\"Heating Setpoint\": {\"hourly_value\": 10.0}",
        )
