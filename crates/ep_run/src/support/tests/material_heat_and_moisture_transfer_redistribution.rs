...
use ep_model::MaterialHeatAndMoistureTransferRedistributionId;
use ep_raw_model::parse_epjson_str;

#[test]
fn redistribution_blocks_used_and_unused_targets_before_runtime()
-> Result<(), Box<dyn std::error::Error>> {
    let raw = parse_epjson_str(
        r#"{
            ...
            "MaterialProperty:HeatAndMoistureTransfer:Redistribution": {
                "Used Redistribution": {"material_name":"Used","number_of_redistribution_points":1,"moisture_content_1":10,"liquid_transport_coefficient_1":1e-9},
                "Unused Redistribution": {"material_name":"Unused","number_of_redistribution_points":1,"moisture_content_1":10,"liquid_transport_coefficient_1":1e-9}
            },
    ...
    assert!(assessment.typed_objects.iter().any(|entry| {
        entry.object_type == "MaterialProperty:HeatAndMoistureTransfer:Redistribution"
            && entry.count == 2
            && entry.status == "typed"
    }));
    ...
    assert!(assessment.diagnostics.diagnostics.iter().any(|diagnostic| {
        diagnostic.code == "UnsupportedSurfaceBoundary"
            && diagnostic.object_type.as_deref()
                == Some("MaterialProperty:HeatAndMoistureTransfer:Redistribution")
    }));
    Ok(())
}
