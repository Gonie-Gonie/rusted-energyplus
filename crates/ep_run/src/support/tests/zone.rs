...
#[test]
fn resolved_surface_assignment_to_the_sole_generated_default_adds_no_partition_boundary()
-> Result<(), Box<dyn std::error::Error>> {
    const EPJSON: &str = r#"{
    ...
    for value in ["\"Zone One\"", "\"\""] {
        ...
        assert!(
            !assessment
                .unsupported_objects
                .iter()
                .any(|entry| entry.object_type == "Space"
                    || entry.object_type == "SpaceList"
                    || entry.object_type == "BuildingSurface:Detailed space_name")
        );
        assert!(
            !assessment
                .diagnostics
...
#[test]
fn invalid_explicit_surface_space_assignments_are_compile_errors()
-> Result<(), Box<dyn std::error::Error>> {
    const EPJSON: &str = r#"{
        "Zone":{__ZONES__},
        "Material:NoMass":{
    ...
    for (zones, space_name) in [
        (r#""Zone One":{"volume":100}"#, r#""Missing""#),
        (r#""Zone One":{"volume":100}"#, "3"),
        (
            r#""Zone One":{"volume":100},"Zone Two":{"volume":100}"#,
            r#""Zone Two""#,
        ),
    ] {
        let epjson = EPJSON
        ...
        let result = compile_raw_model(&raw);
        assert!(result.has_errors(), "{space_name}: expected compile error");
...
#[test]
fn generated_remainder_spaces_are_counted_as_unsupported_partition_topology()
-> Result<(), Box<dyn std::error::Error>> {
    let raw = parse_epjson_str(
        r#"{
            "Zone":{"Zone One":{"volume":100}},
    ...
    let model = result
        .model
        .as_ref()
        .ok_or_else(|| std::io::Error::other("expected remainder-space model"))?;
    assert_eq!(
        model
            .spaces
            .iter()
    ...
    assert_eq!(
        model
            .spaces
            .iter()
            .filter(|space| space.origin == ep_model::SpaceOrigin::AutoZoneRemainder)
            .count(),
        1
    );
