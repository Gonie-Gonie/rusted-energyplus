...
use ep_model::MaterialHeatAndMoistureTransferDiffusionId;
use ep_raw_model::parse_epjson_str;

#[test]
fn diffusion_blocks_used_and_unused_targets_before_runtime()
-> Result<(), Box<dyn std::error::Error>> {
    let raw = parse_epjson_str(
        r#"{
            ...
            "MaterialProperty:HeatAndMoistureTransfer:SorptionIsotherm": {
                "Used Curve": {"material_name":"Used","number_of_isotherm_coordinates":1,"relative_humidity_fraction_1":0.5,"moisture_content_1":0.5},
                "Unused Curve": {"material_name":"Unused","number_of_isotherm_coordinates":1,"relative_humidity_fraction_1":0.5,"moisture_content_1":0.5}
            },
            "MaterialProperty:HeatAndMoistureTransfer:Diffusion": {
                "Used Diffusion": {"material_name":"Used","number_of_data_pairs":1,"relative_humidity_fraction_1":0.5,"water_vapor_diffusion_resistance_factor_1":1e-9},
                "Unused Diffusion": {"material_name":"Unused","number_of_data_pairs":1,"relative_humidity_fraction_1":0.5,"water_vapor_diffusion_resistance_factor_1":1e-9}
            },
    ...
    assert!(assessment.typed_objects.iter().any(|entry| {
        entry.object_type == "MaterialProperty:HeatAndMoistureTransfer:Diffusion"
            && entry.count == 2
            && entry.status == "typed"
    }));
    ...
    assert!(assessment.diagnostics.diagnostics.iter().any(|diagnostic| {
        diagnostic.code == "UnsupportedSurfaceBoundary"
            && diagnostic.object_type.as_deref()
                == Some("MaterialProperty:HeatAndMoistureTransfer:Diffusion")
    }));
    Ok(())
}
