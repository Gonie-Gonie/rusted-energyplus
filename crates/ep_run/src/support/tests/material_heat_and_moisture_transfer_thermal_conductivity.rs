...
use ep_model::MaterialHeatAndMoistureTransferThermalConductivityId;
use ep_raw_model::parse_epjson_str;

#[test]
fn thermal_conductivity_blocks_used_and_unused_targets_before_runtime()
-> Result<(), Box<dyn std::error::Error>> {
    let raw = parse_epjson_str(
        r#"{
            ...
            "MaterialProperty:HeatAndMoistureTransfer:ThermalConductivity": {
                "Used Conductivity": {"material_name":"Used","number_of_thermal_coordinates":1,"moisture_content_1":2,"thermal_conductivity_1":0.8},
                "Unused Conductivity": {"material_name":"Unused","number_of_thermal_coordinates":1,"moisture_content_1":2,"thermal_conductivity_1":0.4}
            },
    ...
    assert!(assessment.typed_objects.iter().any(|entry| {
        entry.object_type == "MaterialProperty:HeatAndMoistureTransfer:ThermalConductivity"
            && entry.count == 2
            && entry.status == "typed"
    }));
    ...
    assert!(assessment.diagnostics.diagnostics.iter().any(|diagnostic| {
        diagnostic.code == "UnsupportedSurfaceBoundary"
            && diagnostic.object_type.as_deref()
                == Some("MaterialProperty:HeatAndMoistureTransfer:ThermalConductivity")
    }));
    Ok(())
}
