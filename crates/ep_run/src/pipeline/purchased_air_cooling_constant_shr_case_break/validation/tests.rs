...
#[test]
fn snapshot_source_identity_route_and_break_corruptions_fail_closed() {
    let init = valid_init_lifecycle();
    ...
    for field in [
        ...
        assert!(corrupted.state.latest.is_some(), "CP357 snapshot");
        let Some(snapshot) = corrupted.state.latest.as_mut() else {
            return;
        };
        match field {
            "source" => snapshot.source = "forged-cp357-source",
            "source_order" => snapshot.source_order = &["forged-cp357-source-order"],
...
fn validate(
    ...
    validate_direct_lifecycle(
        ...
        DirectLifecyclePredecessors {
            mixed_air_limit_cp356: Some(predecessor),
        },
        Some(init),
        Some(1),
    )
...
fn lifecycles(
    ...
    let predecessor_latest =
        PurchasedAirCalcCoolingConstantShrSupplyHumidityRatioMixedAirLimitSnapshot {
            ...
            dehumidification_control_constant_supply_humidity_ratio_case_selected_skip:
                constant_supply,
            supply_humidity_ratio_for_mixed_air_limit_minimum_read: constant_shr,
            supply_humidity_ratio_before_mixed_air_limit: constant_shr.then_some(0.0077),
            mixed_air_humidity_ratio_for_minimum_read: constant_shr,
            mixed_air_humidity_ratio: constant_shr.then_some(0.007),
            source_shaped_two_argument_minimum_evaluated: constant_shr,
            minimum_supply_humidity_ratio: constant_shr.then_some(0.007),
            supply_humidity_ratio_assignment_performed: constant_shr,
            assigned_supply_humidity_ratio: constant_shr.then_some(0.007),
            resulting_supply_humidity_ratio: constant_shr.then_some(0.007),
        };
    ...
    predecessor_state.dehumidification_control_constant_sensible_heat_ratio_supply_humidity_ratio_mixed_air_limit_count =
    ...
    predecessor_state.supply_humidity_ratio_for_mixed_air_limit_minimum_read_count =
        usize::from(constant_shr);
    predecessor_state.mixed_air_humidity_ratio_for_minimum_read_count = usize::from(constant_shr);
    predecessor_state.source_shaped_two_argument_minimum_evaluation_count =
        usize::from(constant_shr);
    ...
    state.dehumidification_control_constant_supply_humidity_ratio_case_selected_skip_count =
        usize::from(constant_supply);
    state.source_site_execution_count = usize::from(constant_shr);
