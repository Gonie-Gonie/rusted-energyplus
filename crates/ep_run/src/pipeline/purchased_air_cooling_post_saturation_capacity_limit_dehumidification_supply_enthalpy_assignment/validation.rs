//! Fail-closed validation for CP417 direct-release supply-enthalpy evidence.
...
use ep_runtime::{
    ...
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationSupplyHumidityRatioAssignmentRuntimeState as PredecessorState,
...
pub(in crate::pipeline) fn validate_direct_lifecycle(
    ...
    predecessor_cp416: Option<&PredecessorLifecycle>,
    init_lifecycle: Option<&PurchasedAirInitLifecycleSummary>,
    coupling_call_count: Option<usize>,
) -> Result<(), String> {
    let lifecycle =
        lifecycle.ok_or_else(|| "direct-zone IdealLoads CP417 evidence is missing".to_string())?;
    let predecessor = predecessor_cp416
        .ok_or_else(|| "direct-zone IdealLoads CP417 CP416 evidence is missing".to_string())?;
    let init = init_lifecycle.ok_or_else(|| {
        "direct-zone IdealLoads CP417 initialization evidence is missing".to_string()
    })?;
    let calls = coupling_call_count
        .ok_or_else(|| "direct-zone IdealLoads CP417 coupling call count is missing".to_string())?;
    if calls == 0 {
        return Err("direct-zone IdealLoads CP417 coupling call count is invalid".to_string());
    }
    ...
    let system = init
        ...
        .first()
        .copied()
        .ok_or_else(|| "direct-zone IdealLoads CP417 declared system is missing".to_string())?;
    let zone = init
        .controlled_zone
        .ok_or_else(|| "direct-zone IdealLoads CP417 controlled Zone is missing".to_string())?;
    let latest = lifecycle
        .state
        .latest
        .ok_or_else(|| "direct-zone IdealLoads CP417 latest evidence is missing".to_string())?;
    let predecessor_latest = predecessor
        .state
        .latest
        .ok_or_else(|| "direct-zone IdealLoads CP417 CP416 latest is missing".to_string())?;
    ...
    {
        return Err("direct-zone IdealLoads CP417 latest lineage is invalid".to_string());
    }
    Ok(())
}
...
fn validate_provenance(
    ...
    if lifecycle.source
        ...
        || PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_SUPPLY_HUMIDITY_RATIO_ASSIGNMENT_SOURCE_ORDER.is_empty()
    {
        return Err("direct-zone IdealLoads CP417 provenance is invalid".to_string());
    }
    Ok(())
}
...
fn validate_public_route_contract(
    ...
    if state.predecessor_route_counts != predecessor.predecessor_route_counts
        ...
        || state.supply_enthalpy_assignment_route_counts
            != predecessor.supply_humidity_ratio_assignment_route_counts
    {
        return Err("direct-zone IdealLoads CP417 route lineage is invalid".to_string());
    }
    for values in [
    ...
    let predecessor_humidity_ratio_assignments = checked_sum(
        &state.predecessor_supply_humidity_ratio_assignment_route_counts,
        "predecessor humidity-ratio assignment partition",
    )?;
    let assignments = checked_sum(
        &state.supply_enthalpy_assignment_route_counts,
        "supply-enthalpy assignment partition",
    )?;
    ...
    let guard_outcomes = guard_false.checked_add(guard_body).ok_or_else(|| {
        "direct-zone IdealLoads CP417 guard outcome partition overflowed".to_string()
    })?;
    let inactive = transitions
        .checked_sub(assignments)
        .ok_or_else(|| "direct-zone IdealLoads CP417 inactive partition underflowed".to_string())?;
    ...
    let unchanged_enthalpy = enthalpy_owners.checked_sub(assignments).ok_or_else(|| {
        "direct-zone IdealLoads CP417 enthalpy preservation underflowed".to_string()
    })?;
    let sites = assignments
        .checked_mul(EXPECTED_SOURCE_ORDER.len())
        .ok_or_else(|| "direct-zone IdealLoads CP417 site count overflowed".to_string())?;
    ...
    for (field, expected, actual) in [
        ...
        (
            "cp416_predecessor_saturation_temperature_assignment_count",
...
fn validate_route_evidence(
    ...
    predecessor_humidity_ratio_assignment_routes: &[usize; 36],
    ...
    for index in 0..routes.len() {
        let outcomes = guard_false_routes[index]
            ...
            .ok_or_else(|| {
                format!("direct-zone IdealLoads CP417 route {index} outcome overflowed")
            })?;
        ...
        ensure_count(
            predecessor_humidity_ratio_assignment_routes[index],
            predecessor_limit_routes[index],
            "predecessor_humidity_ratio_assignment_limit_route_partition",
        )?;
        ensure_count(
            assignment_routes[index],
            predecessor_humidity_ratio_assignment_routes[index],
            "enthalpy_assignment_predecessor_humidity_ratio_assignment_route_partition",
        )?;
    }
    Ok(())
}
...
fn sum_predecessor_indices(
    ...
    for predecessor_index in 0..30 {
        ...
        if include(predecessor_index) {
            total = values[logical..logical + width]
                ...
                .ok_or_else(|| format!("direct-zone IdealLoads CP417 {label} overflowed"))?;
        }
        logical += width;
    }
    if logical == values.len() {
        ...
        Err("direct-zone IdealLoads CP417 logical route width is invalid".to_string())
    }
}
...
fn ensure_public_routes_only(values: &[usize; 36]) -> Result<(), String> {
    for (index, count) in values.iter().enumerate() {
        if !PUBLIC_LOGICAL_INDICES.contains(&index) && *count != 0 {
            return Err(format!(
                "direct-zone IdealLoads CP417 non-direct route {index} is active"
            ));
        }
    }
    Ok(())
...
fn ensure_count(actual: usize, expected: usize, field: &str) -> Result<(), String> {
    if actual == expected {
        ...
        Err(format!(
            "direct-zone IdealLoads CP417 invariant {field} expected {expected}, got {actual}"
        ))
    }
}
