//! Fail-closed validation for CP361 direct-release evidence.
...
pub(in crate::pipeline) struct DirectLifecyclePredecessors<'a> {
    pub(in crate::pipeline) supply_humidity_ratio_assignment_cp360: Option<
        &'a PurchasedAirCalcCoolingHumidistatSupplyHumidityRatioForDehumidificationAssignmentLifecycleSummary,
    >,
}
...
pub(in crate::pipeline) fn validate_direct_lifecycle(
    ...
    let lifecycle = lifecycle.ok_or_else(|| {
        "direct-zone IdealLoads runtime did not expose Humidistat supply-humidity-ratio-for-dehumidification minimum-limit evidence"
            .to_string()
    })?;
    let predecessor = predecessors
        .supply_humidity_ratio_assignment_cp360
        .ok_or_else(|| {
            "direct-zone IdealLoads Humidistat supply-humidity-ratio-for-dehumidification minimum limit has no CP360 evidence"
                .to_string()
        })?;
    let init = init_lifecycle.ok_or_else(|| {
        "direct-zone IdealLoads Humidistat supply-humidity-ratio-for-dehumidification minimum limit has no initialization evidence"
            .to_string()
    })?;
    let calls = coupling_call_count.ok_or_else(|| {
        "direct-zone IdealLoads Humidistat supply-humidity-ratio-for-dehumidification minimum limit has no coupling call count"
            .to_string()
    })?;
    if calls == 0
        || lifecycle.source
    ...
    {
        return Err(
            "direct-zone IdealLoads Humidistat supply-humidity-ratio-for-dehumidification minimum-limit provenance is invalid"
                .into(),
        );
    }
    ...
    let latest = state.latest.as_ref().ok_or_else(|| {
        "direct-zone IdealLoads Humidistat supply-humidity-ratio-for-dehumidification minimum limit has no latest snapshot"
            .to_string()
    })?;
    let predecessor_latest = predecessor_state.latest.as_ref().ok_or_else(|| {
        "direct-zone IdealLoads Humidistat supply-humidity-ratio-for-dehumidification minimum limit has no latest CP360 snapshot"
            .to_string()
    })?;
    let expected_system = init.declared_system_order.first().copied().ok_or_else(|| {
        "direct-zone IdealLoads Humidistat supply-humidity-ratio-for-dehumidification minimum limit has no declared system"
            .to_string()
    })?;
    let expected_zone = init.controlled_zone.ok_or_else(|| {
        "direct-zone IdealLoads Humidistat supply-humidity-ratio-for-dehumidification minimum limit has no controlled Zone"
            .to_string()
    })?;
    ...
    {
        return Err(
            "direct-zone IdealLoads Humidistat supply-humidity-ratio-for-dehumidification minimum-limit latest state is not release-ready"
                .into(),
        );
    }
    Ok(())
...
fn validate_source_counters(
    ...
    let source_sites = executed
        ...
        .ok_or_else(|| {
            "direct-zone IdealLoads Humidistat supply-humidity-ratio-for-dehumidification minimum-limit source counter overflow"
                .to_string()
        })?;
    for (field, expected, actual) in [
        (
...
fn expected_snapshot(
    predecessor: PurchasedAirCalcCoolingHumidistatSupplyHumidityRatioForDehumidificationAssignmentSnapshot,
) -> PurchasedAirCalcCoolingHumidistatSupplyHumidityRatioForDehumidificationMinimumLimitSnapshot {
    PurchasedAirCalcCoolingHumidistatSupplyHumidityRatioForDehumidificationMinimumLimitSnapshot {
        source:
            PURCHASED_AIR_CALC_COOLING_HUMIDISTAT_SUPPLY_HUMIDITY_RATIO_FOR_DEHUMIDIFICATION_MINIMUM_LIMIT_SOURCE,
        ...
        dehumidification_control_constant_supply_humidity_ratio_case_selected_skip:
            predecessor
                .dehumidification_control_constant_supply_humidity_ratio_case_selected_skip,
        supply_humidity_ratio_for_dehumidification_for_minimum_limit_maximum_read: false,
        supply_humidity_ratio_for_dehumidification_before_minimum_limit: None,
        ...
        maximum_supply_humidity_ratio_for_dehumidification: None,
        supply_humidity_ratio_for_dehumidification_assignment_performed: false,
...
fn snapshots_match_exact_bits(
    left: &PurchasedAirCalcCoolingHumidistatSupplyHumidityRatioForDehumidificationMinimumLimitSnapshot,
    right: &PurchasedAirCalcCoolingHumidistatSupplyHumidityRatioForDehumidificationMinimumLimitSnapshot,
) -> bool {
    let values_match = [
        (
...
fn ensure_count(actual: usize, expected: usize, field: &str) -> Result<(), String> {
    if actual == expected {
        ...
        Err(format!(
            "direct-zone IdealLoads Humidistat supply-humidity-ratio-for-dehumidification minimum limit {field} expected {expected}, got {actual}"
        ))
    }
}
