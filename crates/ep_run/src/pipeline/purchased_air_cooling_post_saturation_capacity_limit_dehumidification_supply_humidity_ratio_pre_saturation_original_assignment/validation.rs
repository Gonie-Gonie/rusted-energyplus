//! Fail-closed validation for CP411 direct-release evidence.
...
use ep_runtime::{
    ...
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationControlDefaultCaseBreakRuntimeState as PredecessorState,
...
const EXPECTED_SOURCE_ORDER: [&str; 2] = [
...
pub(in crate::pipeline) fn validate_direct_lifecycle(
    ...
    predecessor_cp410: Option<&PredecessorLifecycle>,
    init_lifecycle: Option<&PurchasedAirInitLifecycleSummary>,
    coupling_call_count: Option<usize>,
) -> Result<(), String> {
    let lifecycle =
        lifecycle.ok_or_else(|| "direct-zone IdealLoads CP411 evidence is missing".to_string())?;
    let predecessor = predecessor_cp410
        .ok_or_else(|| "direct-zone IdealLoads CP411 CP410 evidence is missing".to_string())?;
    let init = init_lifecycle.ok_or_else(|| {
        "direct-zone IdealLoads CP411 initialization evidence is missing".to_string()
    })?;
    let calls = coupling_call_count
        .ok_or_else(|| "direct-zone IdealLoads CP411 coupling call count is missing".to_string())?;
    if calls == 0 {
        return Err("direct-zone IdealLoads CP411 coupling call count is invalid".to_string());
    }
    ...
    let system = init
        ...
        .first()
        .copied()
        .ok_or_else(|| "direct-zone IdealLoads CP411 declared system is missing".to_string())?;
    let zone = init
        .controlled_zone
        .ok_or_else(|| "direct-zone IdealLoads CP411 controlled Zone is missing".to_string())?;
    let latest = lifecycle
        .state
        .latest
        .ok_or_else(|| "direct-zone IdealLoads CP411 latest evidence is missing".to_string())?;
    let predecessor_latest = predecessor
        .state
        .latest
        .ok_or_else(|| "direct-zone IdealLoads CP411 CP410 latest is missing".to_string())?;
    ...
    {
        return Err("direct-zone IdealLoads CP411 latest lineage is invalid".to_string());
    }
    Ok(())
}
...
fn validate_provenance(
    ...
    if lifecycle.source
        ...
        || PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_CONTROL_DEFAULT_CASE_BREAK_SOURCE_ORDER.is_empty()
    {
        return Err("direct-zone IdealLoads CP411 provenance is invalid".to_string());
    }
    Ok(())
}
...
fn validate_public_route_contract(
    ...
    {
        return Err("direct-zone IdealLoads CP411 route lineage is invalid".to_string());
    }
    for values in [
    ...
    let inactive = transitions
        ...
        .ok_or_else(|| "direct-zone IdealLoads CP411 inactive partition underflowed".to_string())?;
    ...
    let sites = assignments
        ...
        .ok_or_else(|| "direct-zone IdealLoads CP411 site count overflowed".to_string())?;
...
fn validate_route_evidence(
    ...
    for index in 0..routes.len() {
        let branch_count = guard_false_routes[index]
            ...
            .ok_or_else(|| {
                format!("direct-zone IdealLoads CP411 route {index} branch count overflowed")
            })?;
...
fn ensure_public_routes_only(values: &[usize; 30]) -> Result<(), String> {
    for (index, count) in values.iter().enumerate() {
        if !matches!(index, 0..=8 | 20 | 24) && *count != 0 {
            return Err(format!(
                "direct-zone IdealLoads CP411 non-direct route {index} is active"
            ));
        }
    }
    Ok(())
...
fn checked_sum(values: &[usize], label: &str) -> Result<usize, String> {
    values.iter().try_fold(0usize, |sum, value| {
        sum.checked_add(*value)
            .ok_or_else(|| format!("direct-zone IdealLoads CP411 {label} overflowed"))
    })
}
...
fn ensure_count(actual: usize, expected: usize, field: &str) -> Result<(), String> {
    if actual == expected {
        ...
        Err(format!(
            "direct-zone IdealLoads CP411 invariant {field} expected {expected}, got {actual}"
        ))
    }
}
