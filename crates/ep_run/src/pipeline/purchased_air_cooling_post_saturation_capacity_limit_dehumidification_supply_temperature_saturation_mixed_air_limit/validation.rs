//! Fail-closed validation for CP415 direct-release evidence.
...
use ep_runtime::{
    ...
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationSupplyTemperatureSaturationAssignmentRuntimeState as PredecessorState,
...
pub(in crate::pipeline) fn validate_direct_lifecycle(
    ...
    predecessor_cp414: Option<&PredecessorLifecycle>,
    mixed_air_owner_cp329: Option<&MixedAirLifecycle>,
    init_lifecycle: Option<&PurchasedAirInitLifecycleSummary>,
    coupling_call_count: Option<usize>,
) -> Result<(), String> {
    let lifecycle =
        lifecycle.ok_or_else(|| "direct-zone IdealLoads CP415 evidence is missing".to_string())?;
    let predecessor = predecessor_cp414
        .ok_or_else(|| "direct-zone IdealLoads CP415 CP414 evidence is missing".to_string())?;
    let mixed_air = mixed_air_owner_cp329
        .ok_or_else(|| "direct-zone IdealLoads CP415 CP329 owner is missing".to_string())?;
    let init = init_lifecycle.ok_or_else(|| {
        "direct-zone IdealLoads CP415 initialization evidence is missing".to_string()
    })?;
    let calls = coupling_call_count
        .ok_or_else(|| "direct-zone IdealLoads CP415 coupling call count is missing".to_string())?;
    if calls == 0 {
        return Err("direct-zone IdealLoads CP415 coupling call count is invalid".to_string());
    }
    ...
    let system = init
        ...
        .first()
        .copied()
        .ok_or_else(|| "direct-zone IdealLoads CP415 declared system is missing".to_string())?;
    let zone = init
        .controlled_zone
        .ok_or_else(|| "direct-zone IdealLoads CP415 controlled Zone is missing".to_string())?;
    let latest = lifecycle
        .state
        .latest
        .ok_or_else(|| "direct-zone IdealLoads CP415 latest evidence is missing".to_string())?;
    let predecessor_latest = predecessor
        .state
        .latest
        .ok_or_else(|| "direct-zone IdealLoads CP415 CP414 latest is missing".to_string())?;
    ...
    {
        return Err("direct-zone IdealLoads CP415 latest lineage is invalid".to_string());
    }
    Ok(())
}
...
fn validate_provenance(
    ...
    if lifecycle.source
        ...
        || PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_SUPPLY_TEMPERATURE_SATURATION_ASSIGNMENT_SOURCE_ORDER.is_empty()
        || PURCHASED_AIR_CALC_COOLING_MIXED_AIR_CALL_SOURCE_ORDER.is_empty()
    {
        return Err("direct-zone IdealLoads CP415 provenance is invalid".to_string());
    }
    Ok(())
}
...
fn validate_public_route_contract(
    ...
    if state.predecessor_route_counts != predecessor.predecessor_route_counts
        ...
        || state.supply_temperature_mixed_air_limit_route_counts
            != predecessor.supply_temperature_saturation_assignment_route_counts
    {
        return Err("direct-zone IdealLoads CP415 route lineage is invalid".to_string());
    }
    for values in [
    ...
    let guard_outcomes = guard_false.checked_add(body_entries).ok_or_else(|| {
        "direct-zone IdealLoads CP415 guard outcome partition overflowed".to_string()
    })?;
    ...
    let inactive = transitions
        ...
        .ok_or_else(|| "direct-zone IdealLoads CP415 inactive partition underflowed".to_string())?;
    ...
    let unchanged_temperature = temperature_owners.checked_sub(assignments).ok_or_else(|| {
        "direct-zone IdealLoads CP415 temperature preservation underflowed".to_string()
    })?;
    let sites = assignments
        .checked_mul(
            PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_SUPPLY_TEMPERATURE_SATURATION_MIXED_AIR_LIMIT_SOURCE_ORDER.len(),
        )
        .ok_or_else(|| "direct-zone IdealLoads CP415 site count overflowed".to_string())?;
    ...
    for (field, expected, actual) in [
        ...
        (
            "mixed_air_limit_route_partition",
            predecessor_assignments,
            assignments,
        ),
        (
        ...
        (
            "cp414_temperature_owner_lineage",
    ...
    if mixed_air.state.cooling_call_count < assignments {
        return Err(format!(
            "direct-zone IdealLoads CP415 invariant cp329_owner_coverage expected at least {assignments}, got {}",
...
fn validate_route_evidence(
    ...
    mixed_air_limit_routes: &[usize; 36],
) -> Result<(), String> {
    for index in 0..routes.len() {
        let outcomes = guard_false_routes[index]
            .checked_add(guard_body_routes[index])
            .ok_or_else(|| {
                format!("direct-zone IdealLoads CP415 route {index} outcome overflowed")
            })?;
        ...
        ensure_count(
            mixed_air_limit_routes[index],
            predecessor_assignment_routes[index],
            "mixed_air_limit_predecessor_assignment_route_partition",
        )?;
    }
    Ok(())
}
    ...
    for predecessor_index in 0..30 {
        ...
        if include(predecessor_index) {
            total = values[logical..logical + width]
                ...
                .ok_or_else(|| format!("direct-zone IdealLoads CP415 {label} overflowed"))?;
        }
        logical += width;
    }
    Ok(total)
...
fn ensure_public_routes_only(values: &[usize; 36]) -> Result<(), String> {
    for (index, count) in values.iter().enumerate() {
        if !PUBLIC_LOGICAL_INDICES.contains(&index) && *count != 0 {
            return Err(format!(
                "direct-zone IdealLoads CP415 non-direct route {index} is active"
            ));
        }
    }
    Ok(())
...
fn ensure_count(actual: usize, expected: usize, field: &str) -> Result<(), String> {
    if actual == expected {
        ...
        Err(format!(
            "direct-zone IdealLoads CP415 invariant {field} expected {expected}, got {actual}"
        ))
    }
}
