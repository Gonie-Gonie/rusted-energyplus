//! Fail-closed validation for CP434 direct-release evidence.
...
const ORDER: &[&str] = &["assign-local-operating-mode-deadband"];

pub(in crate::pipeline) fn validate_direct_lifecycle(
    lifecycle: Option<&Lifecycle>,
    predecessor_cp433: Option<&PredecessorLifecycle>,
    init_lifecycle: Option<&PurchasedAirInitLifecycleSummary>,
    coupling_call_count: Option<usize>,
) -> Result<(), String> {
    let lifecycle =
        lifecycle.ok_or_else(|| "direct-zone IdealLoads CP434 evidence is missing".to_string())?;
    let predecessor = predecessor_cp433
        .ok_or_else(|| "direct-zone IdealLoads CP434 CP433 evidence is missing".to_string())?;
    let init = init_lifecycle.ok_or_else(|| {
        "direct-zone IdealLoads CP434 initialization evidence is missing".to_string()
    })?;
    let calls = coupling_call_count
        .ok_or_else(|| "direct-zone IdealLoads CP434 coupling call count is missing".to_string())?;
    if calls == 0
        || lifecycle.source != PURCHASED_AIR_CALC_HEATING_OPERATING_MODE_DEADBAND_ASSIGNMENT_SOURCE
        || lifecycle.first_excluded_source
            != PURCHASED_AIR_CALC_HEATING_OPERATING_MODE_DEADBAND_ASSIGNMENT_FIRST_EXCLUDED_SOURCE
        || predecessor.source != PURCHASED_AIR_CALC_HEATING_MODE_GUARD_ELSE_BRANCH_ENTRY_SOURCE
        || predecessor.first_excluded_source
            != PURCHASED_AIR_CALC_HEATING_MODE_GUARD_ELSE_BRANCH_ENTRY_FIRST_EXCLUDED_SOURCE
        || PURCHASED_AIR_CALC_HEATING_OPERATING_MODE_DEADBAND_ASSIGNMENT_SOURCE_ORDER != ORDER
    {
        return Err("direct-zone IdealLoads CP434 provenance is invalid".to_string());
    }
    ...
    let system = init
        ...
        .first()
        .copied()
        .ok_or_else(|| "direct-zone IdealLoads CP434 declared system is missing".to_string())?;
    let zone = init
        .controlled_zone
        .ok_or_else(|| "direct-zone IdealLoads CP434 controlled Zone is missing".to_string())?;
    let latest = lifecycle
        .state
        .latest
        .ok_or_else(|| "direct-zone IdealLoads CP434 latest evidence is missing".to_string())?;
    let predecessor_latest = predecessor.state.latest.ok_or_else(|| {
        "direct-zone IdealLoads CP434 CP433 latest evidence is missing".to_string()
    })?;
    if lifecycle.state.system != system
        ...
        || latest.source != PURCHASED_AIR_CALC_HEATING_OPERATING_MODE_DEADBAND_ASSIGNMENT_SOURCE
    ...
    {
        return Err("direct-zone IdealLoads CP434 latest lineage is invalid".to_string());
    }
    Ok(())
}
...
fn validate_public_route_contract(
    state: &State,
    predecessor: &ep_runtime::PurchasedAirCalcHeatingModeGuardElseBranchEntryRuntimeState,
) -> Result<(), String> {
    ...
    {
        return Err("direct-zone IdealLoads CP434 route lineage is invalid".to_string());
    }
    for index in 0..36 {
        for values in [
            ...
            if !PUBLIC.contains(&index) && values[index] != 0 {
                return Err(format!(
                    "direct-zone IdealLoads CP434 non-direct route {index} is active"
                ));
            }
        }
    }
    ...
    let inactive = transitions
        ...
        .ok_or_else(|| "direct-zone IdealLoads CP434 inactive partition underflowed".to_string())?;
...
fn checked_sum(values: &[usize]) -> Result<usize, String> {
    values.iter().try_fold(0usize, |sum, value| {
        sum.checked_add(*value)
            .ok_or_else(|| "direct-zone IdealLoads CP434 count overflowed".to_string())
    })
}
...
fn ensure_count(actual: usize, expected: usize, field: &str) -> Result<(), String> {
    if actual == expected {
        ...
        Err(format!(
            "direct-zone IdealLoads CP434 invariant {field} expected {expected}, got {actual}"
        ))
    }
}
...
mod tests {
    #[test]
    fn validator_is_structural_and_has_no_numerical_or_dto_feed() {
