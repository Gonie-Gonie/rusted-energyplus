//! Fail-closed validation for CP416 direct-release evidence.
...
use ep_runtime::{
    ...
    PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_SUPPLY_HUMIDITY_RATIO_ASSIGNMENT_SOURCE_ORDER,
    ...
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationSupplyHumidityRatioAssignmentLifecycleSummary as Lifecycle,
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationSupplyHumidityRatioAssignmentRuntimeState as State,
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationSupplyTemperatureSaturationMixedAirLimitLifecycleSummary as PredecessorLifecycle,
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationSupplyTemperatureSaturationMixedAirLimitRuntimeState as PredecessorState,
...
pub(in crate::pipeline) fn validate_direct_lifecycle(
    ...
    predecessor_cp415: Option<&PredecessorLifecycle>,
    init_lifecycle: Option<&PurchasedAirInitLifecycleSummary>,
    coupling_call_count: Option<usize>,
) -> Result<(), String> {
    let lifecycle =
        lifecycle.ok_or_else(|| "direct-zone IdealLoads CP416 evidence is missing".to_string())?;
    let predecessor = predecessor_cp415
        .ok_or_else(|| "direct-zone IdealLoads CP416 CP415 evidence is missing".to_string())?;
    let init = init_lifecycle.ok_or_else(|| {
        "direct-zone IdealLoads CP416 initialization evidence is missing".to_string()
    })?;
    let calls = coupling_call_count
        .ok_or_else(|| "direct-zone IdealLoads CP416 coupling call count is missing".to_string())?;
    if calls == 0 {
        return Err("direct-zone IdealLoads CP416 coupling call count is invalid".to_string());
    }
    ...
    let system = init
        ...
        .first()
        .copied()
        .ok_or_else(|| "direct-zone IdealLoads CP416 declared system is missing".to_string())?;
    let zone = init
        .controlled_zone
        .ok_or_else(|| "direct-zone IdealLoads CP416 controlled Zone is missing".to_string())?;
    let latest = lifecycle
        .state
        .latest
        .ok_or_else(|| "direct-zone IdealLoads CP416 latest evidence is missing".to_string())?;
    let predecessor_latest = predecessor
        .state
        .latest
        .ok_or_else(|| "direct-zone IdealLoads CP416 CP415 latest is missing".to_string())?;
    ...
    {
        return Err("direct-zone IdealLoads CP416 latest lineage is invalid".to_string());
    }
    Ok(())
}
...
fn validate_provenance(
    ...
    if lifecycle.source
        ...
        || PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_SUPPLY_TEMPERATURE_SATURATION_MIXED_AIR_LIMIT_SOURCE_ORDER.is_empty()
    {
        return Err("direct-zone IdealLoads CP416 provenance is invalid".to_string());
    }
    Ok(())
}
...
fn validate_public_route_contract(
    ...
    if state.predecessor_route_counts != predecessor.predecessor_route_counts
        ...
        || state.supply_humidity_ratio_assignment_route_counts
            != predecessor.supply_temperature_mixed_air_limit_route_counts
    {
        return Err("direct-zone IdealLoads CP416 route lineage is invalid".to_string());
    }
    for values in [
    ...
    let guard_body = checked_sum(
        &state.predecessor_guard_body_entry_route_counts,
        "guard-body partition",
    )?;
    let predecessor_assignments = checked_sum(
        &state.predecessor_supply_temperature_saturation_assignment_route_counts,
        "predecessor saturation-temperature assignment partition",
    )?;
    let predecessor_limits = checked_sum(
        &state.predecessor_supply_temperature_mixed_air_limit_route_counts,
        "predecessor mixed-air-limit partition",
    )?;
    let assignments = checked_sum(
        &state.supply_humidity_ratio_assignment_route_counts,
        "humidity-ratio assignment partition",
    )?;
    let active_outcomes = checked_sum(
        &state.predecessor_route_counts[FIRST_ACTIVE_LOGICAL_INDEX..],
        "active outcome partition",
    )?;
    let guard_outcomes = guard_false.checked_add(guard_body).ok_or_else(|| {
        "direct-zone IdealLoads CP416 guard outcome partition overflowed".to_string()
    })?;
    let inactive = transitions
        .checked_sub(assignments)
        .ok_or_else(|| "direct-zone IdealLoads CP416 inactive partition underflowed".to_string())?;
    ...
    let unchanged_humidity_ratio =
        humidity_ratio_owners
            .checked_sub(assignments)
            .ok_or_else(|| {
                "direct-zone IdealLoads CP416 humidity-ratio preservation underflowed".to_string()
            })?;
    let sites = assignments
        .checked_mul(EXPECTED_SOURCE_ORDER.len())
        .ok_or_else(|| "direct-zone IdealLoads CP416 site count overflowed".to_string())?;
    ...
    for (field, expected, actual) in [
        ...
        (
            "predecessor_saturation_temperature_assignment_count",
            predecessor_assignments,
            state.predecessor_supply_temperature_saturation_assignment_count,
        ),
        (
            "cp415_predecessor_saturation_temperature_assignment_count",
            predecessor.predecessor_supply_temperature_saturation_assignment_count,
            state.predecessor_supply_temperature_saturation_assignment_count,
        ),
        (
            "predecessor_mixed_air_limit_count",
            predecessor_limits,
            state.predecessor_supply_temperature_saturation_mixed_air_limit_count,
        ),
        (
            "cp415_mixed_air_limit_count",
            predecessor.supply_temperature_saturation_mixed_air_limit_count,
            state.predecessor_supply_temperature_saturation_mixed_air_limit_count,
        ),
        (
            "humidity_ratio_assignment_count",
            assignments,
            state.supply_humidity_ratio_assignment_count,
        ),
        (
            "cp415_limit_equals_cp416_assignment",
            predecessor.supply_temperature_saturation_mixed_air_limit_count,
            assignments,
        ),
        (
        ...
        (
            "cp415_supply_humidity_ratio_state_owner_count",
            humidity_ratio_owners,
            state.cp415_supply_humidity_ratio_state_owner_count,
        ),
        (
            "cp415_humidity_ratio_owner_lineage",
            predecessor.cp414_supply_humidity_ratio_state_owner_count,
            state.cp415_supply_humidity_ratio_state_owner_count,
        ),
        (
        ...
        (
            "cp415_supply_enthalpy_state_owner_count",
            enthalpy_owners,
            state.cp415_supply_enthalpy_state_owner_count,
        ),
        (
            "cp415_enthalpy_owner_lineage",
            predecessor.cp414_supply_enthalpy_state_owner_count,
            state.cp415_supply_enthalpy_state_owner_count,
        ),
        (
        ...
        (
            "cp415_supply_temperature_state_owner_count",
            temperature_owners,
            state.cp415_supply_temperature_state_owner_count,
        ),
        (
            "cp415_temperature_owner_lineage",
            predecessor.cp414_supply_temperature_state_owner_count,
            state.cp415_supply_temperature_state_owner_count,
        ),
        (
...
fn validate_route_evidence(
    ...
    predecessor_assignment_routes: &[usize; 36],
    predecessor_limit_routes: &[usize; 36],
    ...
    for index in 0..routes.len() {
        let outcomes = guard_false_routes[index]
            ...
            .ok_or_else(|| {
                format!("direct-zone IdealLoads CP416 route {index} outcome overflowed")
            })?;
        ...
        ensure_count(
            ...
            "predecessor_assignment_body_route_partition",
        )?;
        ensure_count(
            predecessor_limit_routes[index],
            predecessor_assignment_routes[index],
            "predecessor_limit_assignment_route_partition",
        )?;
        ensure_count(
            assignment_routes[index],
            predecessor_limit_routes[index],
            "assignment_predecessor_limit_route_partition",
        )?;
    }
    Ok(())
}
    ...
    for predecessor_index in 0..30 {
        ...
        if include(predecessor_index) {
            total = values[logical..logical + width]
                ...
                .ok_or_else(|| format!("direct-zone IdealLoads CP416 {label} overflowed"))?;
        }
        logical += width;
    }
    if logical == values.len() {
        Ok(total)
    } else {
        Err("direct-zone IdealLoads CP416 logical route width is invalid".to_string())
    }
}
...
fn ensure_public_routes_only(values: &[usize; 36]) -> Result<(), String> {
    for (index, count) in values.iter().enumerate() {
        if !PUBLIC_LOGICAL_INDICES.contains(&index) && *count != 0 {
            return Err(format!(
                "direct-zone IdealLoads CP416 non-direct route {index} is active"
            ));
        }
    }
    Ok(())
...
fn ensure_count(actual: usize, expected: usize, field: &str) -> Result<(), String> {
    if actual == expected {
        ...
        Err(format!(
            "direct-zone IdealLoads CP416 invariant {field} expected {expected}, got {actual}"
        ))
    }
}
