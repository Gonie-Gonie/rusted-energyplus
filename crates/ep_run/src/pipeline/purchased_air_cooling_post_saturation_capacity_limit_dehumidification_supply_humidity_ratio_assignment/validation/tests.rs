...
#[test]
fn public_cp416_validator_requires_cp415_and_runtime_identity() {
    ...
    let error = validate_direct_lifecycle(Some(&lifecycle), None, None, Some(1))
        .expect_err("CP416 must require CP415 before runtime validation");
    assert!(error.contains("CP415 evidence is missing"));
}

#[test]
fn route_evidence_requires_cp416_assignment_to_equal_cp415_limit() {
    let mut routes = [0; 36];
    let mut guard_false = [0; 36];
    let mut guard_body = [0; 36];
    let mut predecessor_assignment = [0; 36];
    let mut predecessor_limit = [0; 36];
    let mut assignment = [0; 36];
    routes[20] = 2;
    guard_false[20] = 1;
    guard_body[20] = 1;
    predecessor_assignment[20] = 1;
    predecessor_limit[20] = 1;
    assignment[20] = 1;
    assert!(
