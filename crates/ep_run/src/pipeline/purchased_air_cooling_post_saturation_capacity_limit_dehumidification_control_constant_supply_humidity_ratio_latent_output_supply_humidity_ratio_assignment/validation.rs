//! Fail-closed validation for CP404 direct-release evidence.
...
use ep_runtime::{
    ...
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationControlConstantSupplyHumidityRatioLatentOutputSupplyTemperatureMixedAirAssignmentRuntimeState as PredecessorState,
...
pub(in crate::pipeline) fn validate_direct_lifecycle(
    ...
    predecessor_cp403: Option<&PredecessorLifecycle>,
    init_lifecycle: Option<&PurchasedAirInitLifecycleSummary>,
    coupling_call_count: Option<usize>,
) -> Result<(), String> {
    let lifecycle =
        lifecycle.ok_or_else(|| "direct-zone IdealLoads CP404 evidence is missing".to_string())?;
    let predecessor = predecessor_cp403
        .ok_or_else(|| "direct-zone IdealLoads CP404 CP403 evidence is missing".to_string())?;
    let init = init_lifecycle.ok_or_else(|| {
        "direct-zone IdealLoads CP404 initialization evidence is missing".to_string()
    })?;
    let calls = coupling_call_count
        .ok_or_else(|| "direct-zone IdealLoads CP404 coupling call count is missing".to_string())?;
    ...
    if calls == 0
        ...
        || predecessor.first_excluded_source
            != PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_CONTROL_CONSTANT_SUPPLY_HUMIDITY_RATIO_LATENT_OUTPUT_SUPPLY_TEMPERATURE_MIXED_AIR_ASSIGNMENT_FIRST_EXCLUDED_SOURCE
        || PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_CONTROL_CONSTANT_SUPPLY_HUMIDITY_RATIO_LATENT_OUTPUT_SUPPLY_HUMIDITY_RATIO_ASSIGNMENT_SOURCE_ORDER.len()
            != 4
    {
        return Err("direct-zone IdealLoads CP404 provenance is invalid".to_string());
    }
    ...
    let system = init
        ...
        .first()
        .copied()
        .ok_or_else(|| "direct-zone IdealLoads CP404 declared system is missing".to_string())?;
    let zone = init
        .controlled_zone
        .ok_or_else(|| "direct-zone IdealLoads CP404 controlled Zone is missing".to_string())?;
    let latest = lifecycle
        .state
        .latest
        .ok_or_else(|| "direct-zone IdealLoads CP404 latest evidence is missing".to_string())?;
    let predecessor_latest = predecessor.state.latest.ok_or_else(|| {
        "direct-zone IdealLoads CP404 CP403 latest evidence is missing".to_string()
    })?;
    ...
    {
        return Err("direct-zone IdealLoads CP404 latest lineage is invalid".to_string());
    }
    Ok(())
}
...
fn validate_public_route_contract(
    ...
    {
        return Err("direct-zone IdealLoads CP404 route lineage is invalid".to_string());
    }
    for (index, count) in state.predecessor_route_counts.iter().enumerate() {
        if !matches!(index, 0..=8 | 20 | 24) && *count != 0 {
            return Err(format!(
                "direct-zone IdealLoads CP404 non-direct route {index} is active"
            ));
        }
        let successors = state.predecessor_guard_false_fallthrough_route_counts[index]
            ...
            .ok_or_else(|| {
                "direct-zone IdealLoads CP404 successor partition overflowed".to_string()
            })?;
    ...
    let sites = assignments
        .checked_mul(4)
        .ok_or_else(|| "direct-zone IdealLoads CP404 site count overflowed".to_string())?;
    let humidity_owners = checked_selected_sum(
        &state.predecessor_route_counts,
        &[18, 19, 22, 23, 26, 28],
...
fn checked_selected_sum(
    ...
    indices.iter().try_fold(0usize, |sum, index| {
        sum.checked_add(values[*index])
            .ok_or_else(|| format!("direct-zone IdealLoads CP404 {label} overflowed"))
    })
}
...
fn ensure_count(actual: usize, expected: usize, field: &str) -> Result<(), String> {
    if actual == expected {
        ...
        Err(format!(
            "direct-zone IdealLoads CP404 invariant {field} expected {expected}, got {actual}"
        ))
    }
}
