//! Exact CP403-to-CP404 lineage and source-operation validation.
...
pub(super) fn operation_shape_is_exact(snapshot: Snapshot, predecessor: Predecessor) -> bool {
    ...
    // CP385 remains the recursive same-call SupplyEnthalpy owner; CP403 carries
    // its assignment marker and exact resulting enthalpy into this checkpoint.
    if !predecessor.predecessor_supply_enthalpy_assignment_executed {
        return false;
    }
    let (
...
fn local_flags(snapshot: Snapshot) -> [bool; 7] {
    [
...
fn inherited_values(snapshot: Snapshot) -> [Option<f64>; 40] {
    [
...
fn predecessor_values(snapshot: Predecessor) -> [Option<f64>; 40] {
    [
        snapshot.predecessor_cp397_resulting_supply_humidity_ratio,
        snapshot.predecessor_cp397_resulting_supply_enthalpy_j_per_kg,
        snapshot.predecessor_cp397_resulting_supply_temperature_c,
