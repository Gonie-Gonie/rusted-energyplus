...
fn checked_partition_and_all_zero_source_counters_fail_closed() {
    let mut state = PurchasedAirCalcCoolingDefaultSupplyHumidityRatioCaseBreakRuntimeState::new(
    ...
    executed.unit_off_skip_count = 0;
    executed.non_cooling_skip_count = 0;
    executed.dehumidification_control_default_supply_humidity_ratio_case_break_count = 1;
    assert!(validate_source_counters(&executed).is_err());

    let mut sourced = PurchasedAirCalcCoolingDefaultSupplyHumidityRatioCaseBreakRuntimeState::new(
        IdealLoadsAirSystemId(0),
    );
...
#[test]
fn expected_snapshot_preserves_all_typed_routes_and_never_executes_default_case_break() {
    ...
    assert!(
        !none_case.dehumidification_control_default_supply_humidity_ratio_case_exited_via_break
    );
    ...
    assert!(
        !constant_shr.dehumidification_control_default_supply_humidity_ratio_case_exited_via_break
    );
    ...
    assert!(
        !humidistat.dehumidification_control_default_supply_humidity_ratio_case_exited_via_break
    );
...
#[test]
fn direct_release_and_immediate_cp367_predecessor_are_strict() {
...
fn direct_lifecycles() -> (
    ...
    let predecessor = PurchasedAirCalcCoolingDefaultSupplyHumidityRatioMixedAirAssignmentLifecycleSummary {
