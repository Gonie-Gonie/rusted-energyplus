//! Fail-closed validation for CP368 direct-release evidence.
...
pub(in crate::pipeline) struct DirectLifecyclePredecessors<'a> {
    pub(in crate::pipeline) assignment_cp367: Option<
        &'a PurchasedAirCalcCoolingDefaultSupplyHumidityRatioMixedAirAssignmentLifecycleSummary,
    >,
}

pub(in crate::pipeline) fn validate_direct_lifecycle(
    lifecycle: Option<&PurchasedAirCalcCoolingDefaultSupplyHumidityRatioCaseBreakLifecycleSummary>,
    predecessors: DirectLifecyclePredecessors<'_>,
    init_lifecycle: Option<&PurchasedAirInitLifecycleSummary>,
    coupling_call_count: Option<usize>,
) -> Result<(), String> {
    let lifecycle = lifecycle.ok_or_else(|| {
        "direct-zone IdealLoads runtime did not expose default supply-humidity-ratio case break evidence"
            .to_string()
    })?;
    let predecessor = predecessors.assignment_cp367.ok_or_else(|| {
        "direct-zone IdealLoads default supply-humidity-ratio case break has no CP367 evidence"
            .to_string()
    })?;
    let init = init_lifecycle.ok_or_else(|| {
        "direct-zone IdealLoads default supply-humidity-ratio case break has no initialization evidence"
            .to_string()
    })?;
    let calls = coupling_call_count.ok_or_else(|| {
        "direct-zone IdealLoads default supply-humidity-ratio case break has no coupling call count"
            .to_string()
    })?;
    let expected_system = init.declared_system_order.first().copied().ok_or_else(|| {
        "direct-zone IdealLoads default supply-humidity-ratio case break has no declared system"
            .to_string()
    })?;
    let expected_zone = init.controlled_zone.ok_or_else(|| {
        "direct-zone IdealLoads default supply-humidity-ratio case break has no controlled Zone"
            .to_string()
    })?;
...
fn validate_release_state(
    lifecycle: &PurchasedAirCalcCoolingDefaultSupplyHumidityRatioCaseBreakLifecycleSummary,
    predecessor: &PurchasedAirCalcCoolingDefaultSupplyHumidityRatioMixedAirAssignmentLifecycleSummary,
    ...
    if calls == 0
        ...
        || PURCHASED_AIR_CALC_COOLING_DEFAULT_SUPPLY_HUMIDITY_RATIO_CASE_BREAK_SOURCE_ORDER
            .len()
            != 1
        || PURCHASED_AIR_CALC_COOLING_DEFAULT_SUPPLY_HUMIDITY_RATIO_MIXED_AIR_ASSIGNMENT_SOURCE_ORDER.len()
            != 2
    {
        return Err(
            "direct-zone IdealLoads default supply-humidity-ratio case break provenance is invalid"
                .into(),
        );
    }
    ...
    let latest = state.latest.as_ref().ok_or_else(|| {
        "direct-zone IdealLoads default supply-humidity-ratio case break has no latest snapshot"
            .to_string()
    })?;
    let predecessor_latest = predecessor_state.latest.as_ref().ok_or_else(|| {
        "direct-zone IdealLoads default supply-humidity-ratio case break has no latest CP367 snapshot"
            .to_string()
    })?;
    ...
    {
        return Err(
            "direct-zone IdealLoads default supply-humidity-ratio case break latest state is not release-ready"
                .into(),
        );
    }
    Ok(())
...
fn validate_route_partition(
    state: &PurchasedAirCalcCoolingDefaultSupplyHumidityRatioCaseBreakRuntimeState,
) -> Result<(), String> {
...
fn predecessor_latest_is_exact_direct_shape(
    ...
    !snapshot.dehumidification_control_default_supply_humidity_ratio_mixed_air_assignment_executed
        && (unit_off || non_cooling || positive_guard_false || none_case)
}

fn latest_route_has_cumulative_evidence(
    state: &PurchasedAirCalcCoolingDefaultSupplyHumidityRatioCaseBreakRuntimeState,
    predecessor_state: &PurchasedAirCalcCoolingDefaultSupplyHumidityRatioMixedAirAssignmentRuntimeState,
    latest: PurchasedAirCalcCoolingDefaultSupplyHumidityRatioMixedAirAssignmentSnapshot,
) -> bool {
...
fn ensure_count(actual: usize, expected: usize, field: &str) -> Result<(), String> {
    if actual == expected {
        ...
        Err(format!(
            "direct-zone IdealLoads default supply-humidity-ratio case break {field} expected {expected}, got {actual}"
        ))
    }
}
