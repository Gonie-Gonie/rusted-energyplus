//! Fail-closed validation for CP440 direct-release evidence.
...
use ep_runtime::{
    ...
    PurchasedAirCalcHeatingOutdoorAirMaximumFlowContinueWarningCallLifecycleSummary as Lifecycle,
    PurchasedAirCalcHeatingOutdoorAirMaximumFlowContinueWarningCallRuntimeState as State,
    PurchasedAirCalcHeatingOutdoorAirMaximumFlowFirstWarningCallLifecycleSummary as PredecessorLifecycle,
    PurchasedAirCalcHeatingOutdoorAirMaximumFlowFirstWarningCallRuntimeState as PredecessorState,
...
const ORDER: &[&str] = &["reach-heating-outdoor-air-maximum-flow-continue-warning-call-site"];

pub(in crate::pipeline) fn validate_direct_lifecycle(
    lifecycle: Option<&Lifecycle>,
    predecessor_cp439: Option<&PredecessorLifecycle>,
    init_lifecycle: Option<&PurchasedAirInitLifecycleSummary>,
    coupling_call_count: Option<usize>,
) -> Result<(), String> {
    let lifecycle =
        lifecycle.ok_or_else(|| "direct-zone IdealLoads CP440 evidence is missing".to_string())?;
    let predecessor = predecessor_cp439
        .ok_or_else(|| "direct-zone IdealLoads CP440 CP439 evidence is missing".to_string())?;
    let init = init_lifecycle.ok_or_else(|| {
        "direct-zone IdealLoads CP440 initialization evidence is missing".to_string()
    })?;
    let calls = coupling_call_count
        .ok_or_else(|| "direct-zone IdealLoads CP440 coupling call count is missing".to_string())?;
    if calls == 0
        || lifecycle.source
            != PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_CONTINUE_WARNING_CALL_SOURCE
        ...
        || PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_CONTINUE_WARNING_CALL_SOURCE_ORDER
            != ORDER
    {
        return Err("direct-zone IdealLoads CP440 provenance is invalid".to_string());
    }
    ...
    let system = init
        ...
        .first()
        .copied()
        .ok_or_else(|| "direct-zone IdealLoads CP440 declared system is missing".to_string())?;
    let zone = init
        .controlled_zone
        .ok_or_else(|| "direct-zone IdealLoads CP440 controlled Zone is missing".to_string())?;
    let latest = lifecycle
        .state
        .latest
        .ok_or_else(|| "direct-zone IdealLoads CP440 latest evidence is missing".to_string())?;
    let predecessor_latest = predecessor.state.latest.ok_or_else(|| {
        "direct-zone IdealLoads CP440 CP439 latest evidence is missing".to_string()
    })?;
    ...
    {
        return Err("direct-zone IdealLoads CP440 latest lineage is invalid".to_string());
    }
    Ok(())
}
...
fn validate_public_route_contract(
    ...
    if state.transition_count != predecessor.transition_count
        ...
        || state.heating_outdoor_air_maximum_flow_continue_warning_call_route_counts
            != state.predecessor_first_warning_call_route_counts
    {
        return Err("direct-zone IdealLoads CP440 route lineage is invalid".to_string());
    }
    for index in 0..36 {
        let arrays = [
        ...
        {
            return Err(format!(
                "direct-zone IdealLoads CP440 route {index} is not public-release-ready"
            ));
        }
    }
    let transitions = checked_sum(&state.predecessor_route_counts)?;
    let call_sites =
        checked_sum(&state.heating_outdoor_air_maximum_flow_continue_warning_call_route_counts)?;
    let inactive = transitions
        .checked_sub(call_sites)
        .ok_or_else(|| "direct-zone IdealLoads CP440 transition partition underflow".to_string())?;
    for (field, expected, actual) in [
        ...
        ("public_continue_warning_call_count", 0, call_sites),
    ] {
...
fn checked_sum(values: &[usize]) -> Result<usize, String> {
    values.iter().try_fold(0usize, |sum, value| {
        sum.checked_add(*value)
            .ok_or_else(|| "direct-zone IdealLoads CP440 route count overflow".to_string())
    })
}
...
fn ensure_count(actual: usize, expected: usize, field: &str) -> Result<(), String> {
    if actual == expected {
        ...
        Err(format!(
            "direct-zone IdealLoads CP440 {field} expected {expected}, got {actual}"
        ))
    }
}
...
mod tests {
    #[test]
    fn validator_uses_nine_route_arrays_one_source_site_and_no_numerical_input() {
        ...
        assert_eq!(route_arrays.len(), 9);
        for field in route_arrays {
            assert!(source.contains(field), "{field}");
        }
        assert!(source.contains("public_continue_warning_call_count"));
