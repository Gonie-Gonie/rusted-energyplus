//! Exact CP394-to-CP395 compact pipeline lineage validation.
...
pub(super) fn links_to_predecessor(snapshot: Snapshot, predecessor: Predecessor) -> bool {
    predecessor.source == PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_CONTROL_HUMIDISTAT_CASE_ENTRY_SOURCE
        ...
        && predecessor.source_order == PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_CONTROL_HUMIDISTAT_CASE_ENTRY_SOURCE_ORDER
        ...
        && snapshot.predecessor_dehumidification_control_humidistat_case_entered == predecessor.dehumidification_control_humidistat_case_entered
        && recursive_values(snapshot).into_iter().zip(predecessor_recursive_values(predecessor)).all(|(left, right)| option_bits_equal(left, right))
}
