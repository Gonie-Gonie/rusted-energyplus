...
fn six_compact_carriers_serialize_with_adjacent_ieee_sidecars() {
    ...
    let object = value.as_object().expect("CP398 object");
    assert_eq!(
        object
            .keys()
    ...
    assert_eq!(
        value["dehumidification_control_none_or_constant_supply_humidity_ratio_shared_case_entered"],
        true
    );
}
...
{
    let predecessor = crate::pipeline::purchased_air_cooling_post_saturation_capacity_limit_dehumidification_control_none_case_entry::test_snapshot(value, active);
    PurchasedAirCalcCoolingPostSaturationCapacityLimitDehumidificationControlConstantSupplyHumidityRatioCaseEntrySnapshot {
