//! Exact CP411-to-CP412 direct-pipeline lineage validation.
...
pub(super) fn lineage_is_exact(snapshot: Snapshot, predecessor: Predecessor) -> bool {
    snapshot.system == predecessor.system
        ...
        && predecessor_values(snapshot)
            .into_iter()
            .zip(predecessor_numeric_values(predecessor))
            .all(|(left, right)| option_bits_equal(left, right))
        && assignment_shape_is_exact(snapshot, predecessor)
}

fn assignment_shape_is_exact(snapshot: Snapshot, predecessor: Predecessor) -> bool {
    ...
    let carriers_preserved = option_bits_equal(
    ...
    let owners_match = snapshot.cp411_retained_supply_humidity_ratio_state_owned
    ...
    if !temperature.is_finite() || !pressure.is_finite() || pressure <= 0.0 {
        return false;
    }
...
fn inherited_flags(snapshot: Snapshot) -> [bool; 40] {
    [
...
fn predecessor_numeric_values(snapshot: Predecessor) -> [Option<f64>; 12] {
    [
        snapshot.predecessor_cp409_resulting_supply_humidity_ratio,
        snapshot.predecessor_cp409_resulting_supply_enthalpy_j_per_kg,
        snapshot.predecessor_cp409_resulting_supply_temperature_c,
