//! Fail-closed validation for CP353 direct-release evidence.
...
pub(in crate::pipeline) struct DirectLifecyclePredecessors<'a> {
    pub(in crate::pipeline) supply_enthalpy_assignment_cp352: Option<
        &'a PurchasedAirCalcCoolingPositiveSupplyPostCapacityLimitDehumidificationControlConstantSensibleHeatRatioSupplyEnthalpyAssignmentLifecycleSummary,
    >,
}
...
pub(in crate::pipeline) fn validate_direct_lifecycle(
    ...
    let lifecycle = lifecycle.ok_or_else(|| {
        "direct-zone IdealLoads runtime did not expose constant-SHR overdrying-limit evidence"
            .to_string()
    })?;
    let predecessor = predecessors
        .supply_enthalpy_assignment_cp352
        .ok_or_else(|| {
            "direct-zone IdealLoads constant-SHR overdrying limit has no CP352 evidence".to_string()
        })?;
    let init = init_lifecycle.ok_or_else(|| {
        "direct-zone IdealLoads constant-SHR overdrying limit has no initialization evidence"
            .to_string()
    })?;
    let calls = coupling_call_count.ok_or_else(|| {
        "direct-zone IdealLoads constant-SHR overdrying limit has no coupling call count"
            .to_string()
    })?;
    if calls == 0
        || lifecycle.source
    ...
    {
        return Err(
            "direct-zone IdealLoads constant-SHR overdrying-limit provenance is invalid"
                .to_string(),
        );
    }
    ...
    let latest = state.latest.as_ref().ok_or_else(|| {
        "direct-zone IdealLoads constant-SHR overdrying limit has no latest snapshot".to_string()
    })?;
    let predecessor_latest = predecessor_state.latest.as_ref().ok_or_else(|| {
        "direct-zone IdealLoads constant-SHR overdrying limit has no latest CP352 snapshot"
            .to_string()
    })?;
    let expected_system = init.declared_system_order.first().copied().ok_or_else(|| {
        "direct-zone IdealLoads constant-SHR overdrying limit has no declared system".to_string()
    })?;
    let expected_zone = init.controlled_zone.ok_or_else(|| {
        "direct-zone IdealLoads constant-SHR overdrying limit has no controlled Zone".to_string()
    })?;
    ...
    {
        return Err(
            "direct-zone IdealLoads constant-SHR overdrying-limit latest state is not release-ready"
                .to_string(),
        );
    }
    Ok(())
...
fn validate_source_counters(
    ...
    let source_sites = executed
        ...
        .ok_or_else(|| {
            "direct-zone IdealLoads constant-SHR overdrying-limit source counter overflow"
                .to_string()
        })?;
    for (field, expected, actual) in [
        (
...
fn ensure_count(actual: usize, expected: usize, field: &str) -> Result<(), String> {
    if actual == expected {
        ...
        Err(format!(
            "direct-zone IdealLoads constant-SHR overdrying limit {field} expected {expected}, got {actual}"
        ))
    }
}
