...
use ep_runtime::{
    IdealLoadsInitFlags, PURCHASED_AIR_CALC_COOLING_CONSTANT_SHR_CASE_BREAK_SOURCE_ORDER,
    PURCHASED_AIR_INIT_LIFECYCLE_SOURCE, PurchasedAirCalcCoolingConstantShrCaseBreakRuntimeState,
...
fn source_counter_mismatch_fails_closed() {
    let mut state =
        PurchasedAirCalcCoolingHumidistatCaseEntryRuntimeState::new(IdealLoadsAirSystemId(0));
...
#[test]
fn snapshot_source_system_ordinal_zone_route_and_entry_corruptions_fail_closed() {
    let init = valid_init_lifecycle();
    ...
    for field in [
        ...
        match field {
            "source" => snapshot.source = "forged-cp358-source",
            "first_excluded_source" => {
                snapshot.first_excluded_source = "forged-cp358-first-excluded"
            }
            ...
            "entry" => snapshot.dehumidification_control_humidistat_case_entered = true,
            _ => unreachable!(),
        }
        assert!(
...
fn validate(
    ...
    predecessor: &PurchasedAirCalcCoolingConstantShrCaseBreakLifecycleSummary,
    init: &PurchasedAirInitLifecycleSummary,
) -> Result<(), String> {
    validate_direct_lifecycle(
        Some(lifecycle),
        DirectLifecyclePredecessors {
            case_break_cp357: Some(predecessor),
        },
        Some(init),
        Some(1),
    )
...
fn lifecycles(
    ...
    let predecessor_latest = PurchasedAirCalcCoolingConstantShrCaseBreakSnapshot {
        ...
        dehumidification_control_constant_sensible_heat_ratio_case_exited_via_break: constant_shr,
    ...
    predecessor_state.dehumidification_control_constant_sensible_heat_ratio_case_break_count =
    ...
    predecessor_state.source_site_execution_count = usize::from(constant_shr);
    predecessor_state.latest = Some(predecessor_latest);
    let predecessor = PurchasedAirCalcCoolingConstantShrCaseBreakLifecycleSummary {
        source: PURCHASED_AIR_CALC_COOLING_CONSTANT_SHR_CASE_BREAK_SOURCE,
        first_excluded_source:
            PURCHASED_AIR_CALC_COOLING_CONSTANT_SHR_CASE_BREAK_FIRST_EXCLUDED_SOURCE,
    ...
    state.dehumidification_control_constant_sensible_heat_ratio_case_completed_skip_count =
        usize::from(constant_shr);
    state.dehumidification_control_humidistat_case_entry_count = usize::from(humidistat);
    state.dehumidification_control_constant_supply_humidity_ratio_case_selected_skip_count =
        usize::from(constant_supply);
    state.source_site_execution_count = usize::from(humidistat);
