//! CP385 cumulative counter and preserved 23-route validation.
...
pub(super) fn validate(
    ...
    if routes != predecessor_route_counts(predecessor) {
        return Err("direct-zone IdealLoads CP385 predecessor route counters are invalid".into());
    }
    if routes[5..].iter().any(|count| *count != 0) {
        return Err(
            "direct-zone IdealLoads CP385 private direct-route counters are nonzero".into(),
        );
    }
    ...
    let source_sites = checked_mul(assignments, 6, "source sites")?;
    ...
    for (field, actual, expected) in [
        ...
        (
            "predecessor_maximum_assignment_count",
            state.dehumidification_total_output_maximum_capacity_assignment_count,
...
fn refined_route_index(snapshot: Snapshot) -> Option<usize> {
    ...
    let successor = if snapshot.predecessor_active_capacity_limit_guard_false_fallthrough {
        ...
        2
    } else if snapshot.supply_enthalpy_assignment_executed {
        3
    } else {
        return None;
    };
...
fn checked_sum(values: &[usize], field: &str) -> Result<usize, String> {
    values.iter().try_fold(0usize, |sum, value| {
        sum.checked_add(*value)
            .ok_or_else(|| format!("direct-zone IdealLoads CP385 {field} overflow"))
    })
}
...
fn ensure(actual: usize, expected: usize, field: &str) -> Result<(), String> {
    if actual == expected {
        ...
        Err(format!(
            "direct-zone IdealLoads CP385 {field} is invalid: expected {expected}, got {actual}"
        ))
    }
}
...
mod tests {
    ...
    #[test]
    fn six_site_counter_mismatch_fails_closed() {
        ...
        assert!(validate(&state, &PredecessorState::new(IdealLoadsAirSystemId(0)), 0).is_err());
    }
}
