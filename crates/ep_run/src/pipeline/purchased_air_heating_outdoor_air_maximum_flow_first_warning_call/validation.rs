//! Fail-closed validation for CP439 direct-release evidence.
...
use ep_runtime::{
    ...
    PurchasedAirCalcHeatingOutdoorAirMaximumFlowFirstWarningCallLifecycleSummary as Lifecycle,
    PurchasedAirCalcHeatingOutdoorAirMaximumFlowFirstWarningCallRuntimeState as State,
    PurchasedAirCalcHeatingOutdoorAirMaximumFlowFirstWarningCounterIncrementLifecycleSummary as PredecessorLifecycle,
...
const ORDER: &[&str] = &["reach-heating-outdoor-air-maximum-flow-first-warning-call-site"];

pub(in crate::pipeline) fn validate_direct_lifecycle(
    lifecycle: Option<&Lifecycle>,
    predecessor_cp438: Option<&PredecessorLifecycle>,
    init_lifecycle: Option<&PurchasedAirInitLifecycleSummary>,
    coupling_call_count: Option<usize>,
) -> Result<(), String> {
    let lifecycle =
        lifecycle.ok_or_else(|| "direct-zone IdealLoads CP439 evidence is missing".to_string())?;
    let predecessor = predecessor_cp438
        .ok_or_else(|| "direct-zone IdealLoads CP439 CP438 evidence is missing".to_string())?;
    let init = init_lifecycle.ok_or_else(|| {
        "direct-zone IdealLoads CP439 initialization evidence is missing".to_string()
    })?;
    let calls = coupling_call_count
        .ok_or_else(|| "direct-zone IdealLoads CP439 coupling call count is missing".to_string())?;
    if calls == 0
        || lifecycle.source
            != PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_FIRST_WARNING_CALL_SOURCE
        ...
        || PURCHASED_AIR_CALC_HEATING_OUTDOOR_AIR_MAXIMUM_FLOW_FIRST_WARNING_CALL_SOURCE_ORDER
            != ORDER
    {
        return Err("direct-zone IdealLoads CP439 provenance is invalid".to_string());
    }
    ...
    let system = init
        ...
        .first()
        .copied()
        .ok_or_else(|| "direct-zone IdealLoads CP439 declared system is missing".to_string())?;
    let zone = init
        .controlled_zone
        .ok_or_else(|| "direct-zone IdealLoads CP439 controlled Zone is missing".to_string())?;
    let latest = lifecycle
        .state
        .latest
        .ok_or_else(|| "direct-zone IdealLoads CP439 latest evidence is missing".to_string())?;
    let predecessor_latest = predecessor.state.latest.ok_or_else(|| {
        "direct-zone IdealLoads CP439 CP438 latest evidence is missing".to_string()
    })?;
    ...
    {
        return Err("direct-zone IdealLoads CP439 latest lineage is invalid".to_string());
    }
    Ok(())
}
...
fn validate_public_route_contract(
    ...
    if state.transition_count != predecessor.transition_count
        ...
        || state.heating_outdoor_air_maximum_flow_first_warning_call_route_counts
            != state.predecessor_first_warning_counter_increment_route_counts
    {
        return Err("direct-zone IdealLoads CP439 route lineage is invalid".to_string());
    }
    for index in 0..36 {
        let arrays = [
        ...
        {
            return Err(format!(
                "direct-zone IdealLoads CP439 route {index} is not public-release-ready"
            ));
        }
    }
    ...
    let inactive = transitions
        ...
        .ok_or_else(|| "direct-zone IdealLoads CP439 transition partition underflow".to_string())?;
    for (field, expected, actual) in [
        ...
        ("public_first_warning_call_count", 0, call_sites),
    ] {
...
fn checked_sum(values: &[usize]) -> Result<usize, String> {
    values.iter().try_fold(0usize, |sum, value| {
        sum.checked_add(*value)
            .ok_or_else(|| "direct-zone IdealLoads CP439 route count overflow".to_string())
    })
}
...
fn ensure_count(actual: usize, expected: usize, field: &str) -> Result<(), String> {
    if actual == expected {
        ...
        Err(format!(
            "direct-zone IdealLoads CP439 {field} expected {expected}, got {actual}"
        ))
    }
}
...
mod tests {
    #[test]
    fn validator_uses_eight_route_arrays_one_source_site_and_no_numerical_input() {
        ...
        assert_eq!(route_arrays.len(), 8);
        for field in route_arrays {
            assert!(source.contains(field), "{field}");
        }
        assert!(source.contains("public_first_warning_call_count"));
