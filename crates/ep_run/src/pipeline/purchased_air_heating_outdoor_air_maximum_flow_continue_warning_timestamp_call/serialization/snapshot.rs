//! Lossless JSON serialization for one CP441 continue-warning timestamp call-site snapshot.
...
pub(in crate::pipeline) fn snapshot_json(
    snapshot: PurchasedAirCalcHeatingOutdoorAirMaximumFlowContinueWarningTimestampCallSnapshot,
) -> Value {
    let predecessor =
        heating_outdoor_air_maximum_flow_continue_warning_timestamp_call_predecessor_cp440_snapshot(
            snapshot,
        );
    let mut value = cp440_snapshot_json(predecessor);
    ...
    target.insert(
        "heating_outdoor_air_maximum_flow_continue_warning_timestamp_call_site_reached".to_string(),
        json!(
...
mod tests {
    #[test]
    fn serializer_source_preserves_cp440_json_and_appends_only_the_call_marker() {
        let source = include_str!("snapshot.rs")
            .split_once("#[cfg(test)]")
            .map_or(include_str!("snapshot.rs"), |(production, _)| production);
        assert!(source.contains("cp440_snapshot_json(predecessor)"));
