...
#[test]
fn forty_lossless_values_serialize_with_exact_ieee_sidecars() {
    let value = snapshot_json(snapshot(Some(-0.0), true));
    let object = value.as_object().expect("CP403 object");
    assert_eq!(
        object
            .keys()
...
#[test]
fn forty_nonfinite_values_project_null_and_keep_ieee_payload() {
    let nan = f64::from_bits(0x7ff8_0000_0000_0403);
    let value = snapshot_json(snapshot(Some(nan), true));
    for field in numeric_fields() {
        assert!(value[field].is_null(), "{field}");
        assert_eq!(value[format!("{field}_ieee_bits")], "0x7ff8000000000403");
    }
}
...
pub(in crate::pipeline) fn snapshot(value: Option<f64>, active: bool) -> Snapshot {
    let predecessor = crate::pipeline::purchased_air_cooling_post_saturation_capacity_limit_dehumidification_control_constant_supply_humidity_ratio_latent_output_guard::test_snapshot(value, active);
    let assigned = if active { value } else { None };
    Snapshot {
        ...
        source_order: PURCHASED_AIR_CALC_COOLING_POST_SATURATION_CAPACITY_LIMIT_DEHUMIDIFICATION_CONTROL_CONSTANT_SUPPLY_HUMIDITY_RATIO_LATENT_OUTPUT_SUPPLY_TEMPERATURE_MIXED_AIR_ASSIGNMENT_SOURCE_ORDER,
        ...
        predecessor_dehumidification_control_none_or_constant_supply_humidity_ratio_latent_output_capacity_guard_evaluated: predecessor.dehumidification_control_none_or_constant_supply_humidity_ratio_latent_output_capacity_guard_evaluated,
        predecessor_cp401_retained_cooling_latent_output_owned_read: predecessor.cp401_retained_cooling_latent_output_owned_read,
        predecessor_cooling_latent_output_read: predecessor.cooling_latent_output_read,
        predecessor_cp402_cooling_latent_output_w: value,
        predecessor_cp321_maximum_total_cooling_capacity_owned_read: predecessor.cp321_maximum_total_cooling_capacity_owned_read,
        predecessor_cp340_same_call_maximum_total_cooling_capacity_bit_corroborated: predecessor.cp340_same_call_maximum_total_cooling_capacity_bit_corroborated,
        predecessor_maximum_total_cooling_capacity_read: predecessor.maximum_total_cooling_capacity_read,
        predecessor_maximum_total_cooling_capacity_w: value,
        predecessor_cooling_latent_output_maximum_total_cooling_capacity_comparison_evaluated: predecessor.cooling_latent_output_maximum_total_cooling_capacity_comparison_evaluated,
        predecessor_cooling_latent_output_greater_than_or_equal_to_maximum_total_cooling_capacity: predecessor.cooling_latent_output_greater_than_or_equal_to_maximum_total_cooling_capacity,
        predecessor_dehumidification_control_none_or_constant_supply_humidity_ratio_latent_output_capacity_adjustment_body_entered: active,
        predecessor_dehumidification_control_none_or_constant_supply_humidity_ratio_latent_output_capacity_guard_false_fallthrough: predecessor.dehumidification_control_none_or_constant_supply_humidity_ratio_latent_output_capacity_guard_false_fallthrough,
        predecessor_cp401_retained_supply_humidity_ratio_state_owned: predecessor.cp401_retained_supply_humidity_ratio_state_owned,
        predecessor_cp401_retained_supply_enthalpy_state_owned: predecessor.cp401_retained_supply_enthalpy_state_owned,
        predecessor_cp401_retained_supply_temperature_state_owned: predecessor.cp401_retained_supply_temperature_state_owned,
        ...
        dehumidification_control_none_or_constant_supply_humidity_ratio_latent_output_capacity_adjustment_supply_temperature_mixed_air_assignment_executed: active,
        cp329_retained_mixed_air_temperature_owned_read: active,
        cp402_same_call_mixed_air_temperature_bit_corroborated: active,
        mixed_air_temperature_read: active,
        mixed_air_temperature_c: assigned,
        supply_temperature_assigned: active,
        assigned_supply_temperature_c: assigned,
        cp402_retained_supply_humidity_ratio_state_owned: value.is_some(),
        cp402_retained_supply_enthalpy_state_owned: value.is_some(),
        cp402_retained_supply_temperature_state_owned: value.is_some(),
        resulting_supply_humidity_ratio: value,
        resulting_supply_enthalpy_j_per_kg: value,
        resulting_supply_temperature_c: if active { assigned } else { value },
    }
}
