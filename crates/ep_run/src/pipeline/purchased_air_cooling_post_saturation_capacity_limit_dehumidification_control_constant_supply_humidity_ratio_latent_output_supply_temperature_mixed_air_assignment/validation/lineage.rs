//! Exact CP402-to-CP403 lineage and CP329 mixed-air-temperature ownership.
...
pub(super) fn operation_shape_is_exact(
    ...
    if owner.source != PURCHASED_AIR_CALC_COOLING_MIXED_AIR_CALL_SOURCE
        ...
        || !owner.calc_purch_air_mixed_air_called
        || !owner.no_outdoor_air_fallback_entered
        || !owner.mixed_air_temperature_assigned
    {
        return false;
    }
    let (Some(carried), Some(owned), Some(read), Some(assigned), Some(resulting)) = (
    ...
    [owned, read, assigned, resulting]
        .into_iter()
        .all(|value| value.to_bits() == carried.to_bits())
}

fn local_flags(snapshot: Snapshot) -> [bool; 5] {
    [
...
fn inherited_values(snapshot: Snapshot) -> [Option<f64>; 35] {
    [
...
fn predecessor_values(snapshot: Predecessor) -> [Option<f64>; 35] {
    [
        snapshot.predecessor_cp397_resulting_supply_humidity_ratio,
        snapshot.predecessor_cp397_resulting_supply_enthalpy_j_per_kg,
        snapshot.predecessor_cp397_resulting_supply_temperature_c,
