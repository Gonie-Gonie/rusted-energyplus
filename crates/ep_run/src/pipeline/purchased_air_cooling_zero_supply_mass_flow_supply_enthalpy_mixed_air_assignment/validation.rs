//! Fail-closed validation for CP425 direct-release evidence.
...
pub(in crate::pipeline) fn validate_direct_lifecycle(
    ...
    predecessor_cp424: Option<&PredecessorLifecycle>,
    init_lifecycle: Option<&PurchasedAirInitLifecycleSummary>,
    coupling_call_count: Option<usize>,
) -> Result<(), String> {
    let lifecycle =
        lifecycle.ok_or_else(|| "direct-zone IdealLoads CP425 evidence is missing".to_string())?;
    let predecessor = predecessor_cp424
        .ok_or_else(|| "direct-zone IdealLoads CP425 CP424 evidence is missing".to_string())?;
    let init = init_lifecycle.ok_or_else(|| {
        "direct-zone IdealLoads CP425 initialization evidence is missing".to_string()
    })?;
    let calls = coupling_call_count
        .ok_or_else(|| "direct-zone IdealLoads CP425 coupling call count is missing".to_string())?;
    if calls == 0
        || lifecycle.source
            != PURCHASED_AIR_CALC_COOLING_ZERO_SUPPLY_MASS_FLOW_SUPPLY_ENTHALPY_MIXED_AIR_ASSIGNMENT_SOURCE
        ...
        || PURCHASED_AIR_CALC_COOLING_ZERO_SUPPLY_MASS_FLOW_SUPPLY_ENTHALPY_MIXED_AIR_ASSIGNMENT_SOURCE_ORDER
            != ORDER
    {
        return Err("direct-zone IdealLoads CP425 provenance is invalid".to_string());
    }
    ...
    let system = init
        ...
        .first()
        .copied()
        .ok_or_else(|| "direct-zone IdealLoads CP425 declared system is missing".to_string())?;
    let zone = init
        .controlled_zone
        .ok_or_else(|| "direct-zone IdealLoads CP425 controlled Zone is missing".to_string())?;
    let latest = lifecycle
        .state
        .latest
        .ok_or_else(|| "direct-zone IdealLoads CP425 latest evidence is missing".to_string())?;
    let predecessor_latest = predecessor.state.latest.ok_or_else(|| {
        "direct-zone IdealLoads CP425 CP424 latest evidence is missing".to_string()
    })?;
    ...
    {
        return Err("direct-zone IdealLoads CP425 latest lineage is invalid".to_string());
    }
    Ok(())
}
...
fn validate_public_route_contract(
    state: &State,
    predecessor: &ep_runtime::PurchasedAirCalcCoolingSupplyMassFlowPositiveGuardElseBranchEntryRuntimeState,
) -> Result<(), String> {
    if state.transition_count != predecessor.transition_count
        || state.predecessor_route_counts != predecessor.predecessor_route_counts
    {
        return Err("direct-zone IdealLoads CP425 route lineage is invalid".to_string());
    }
    for index in 0..36 {
        for values in [
            ...
            if !PUBLIC.contains(&index) && values[index] != 0 {
                return Err(format!(
                    "direct-zone IdealLoads CP425 non-direct route {index} is active"
                ));
            }
        }
    ...
    let inactive = transitions
        ...
        .ok_or_else(|| "direct-zone IdealLoads CP425 inactive partition underflowed".to_string())?;
    let sites = assignments
        .checked_mul(2)
        .ok_or_else(|| "direct-zone IdealLoads CP425 site count overflowed".to_string())?;
...
fn checked_sum(values: &[usize]) -> Result<usize, String> {
    values.iter().try_fold(0usize, |sum, value| {
        sum.checked_add(*value)
            .ok_or_else(|| "direct-zone IdealLoads CP425 count overflowed".to_string())
    })
}
...
fn ensure_count(actual: usize, expected: usize, field: &str) -> Result<(), String> {
    if actual == expected {
        ...
        Err(format!(
            "direct-zone IdealLoads CP425 invariant {field} expected {expected}, got {actual}"
        ))
    }
}
...
#[cfg(test)]
mod tests {
    use super::lineage::active_enthalpy_chain_is_exact;
    ...
    #[test]
    fn validator_stays_on_the_bounded_cp424_prefix_and_local_assignment() {
