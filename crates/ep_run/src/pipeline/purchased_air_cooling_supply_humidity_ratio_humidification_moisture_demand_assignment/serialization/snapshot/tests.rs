...
#[test]
fn direct_release_serializes_null_values_and_ieee_sidecars() {
    let value = snapshot_json(snapshot(None, false));
    assert_eq!(
        value["humidification_moisture_demand_assignment_executed"],
        false
    );
    assert_eq!(
        value["zone_humidifying_setpoint_moisture_demand_read"],
        false
    );
    assert_eq!(
        value["zone_humidifying_setpoint_moisture_demand_assigned"],
        false
    );
...
fn snapshot(
    ...
    PurchasedAirCalcCoolingSupplyHumidityRatioHumidificationMoistureDemandAssignmentSnapshot {
        ...
        assigned_zone_humidifying_setpoint_moisture_demand_kg_per_s: scalar,
        resulting_zone_humidifying_setpoint_moisture_demand_kg_per_s: scalar,
    }
}
