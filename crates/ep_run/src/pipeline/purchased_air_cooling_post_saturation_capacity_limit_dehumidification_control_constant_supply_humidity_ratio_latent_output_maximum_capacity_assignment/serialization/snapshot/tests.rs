...
fn snapshot(value: Option<f64>) -> Snapshot {
    Snapshot {
        ...
        predecessor_dehumidification_control_none_or_constant_supply_humidity_ratio_latent_output_capacity_adjustment_supply_humidity_ratio_assignment_executed: false,
        predecessor_cp403_retained_supply_humidity_ratio_state_owned: false,
        predecessor_cp403_retained_supply_temperature_state_owned: false,
        predecessor_cp403_retained_supply_enthalpy_state_owned: false,
        predecessor_cp404_cp403_retained_supply_temperature_owned_read: false,
        predecessor_supply_temperature_for_humidity_ratio_inversion_read: false,
        predecessor_cp404_supply_temperature_c: value,
        predecessor_cp404_cp403_retained_supply_enthalpy_owned_read: false,
        predecessor_supply_enthalpy_for_humidity_ratio_inversion_read: false,
        predecessor_cp404_supply_enthalpy_j_per_kg: value,
        predecessor_psychrometric_supply_humidity_ratio_evaluated: false,
        predecessor_cp404_psychrometric_supply_humidity_ratio: value,
        predecessor_supply_humidity_ratio_assignment_performed: false,
        predecessor_cp404_assigned_supply_humidity_ratio: value,
        predecessor_cp404_resulting_supply_humidity_ratio: value,
        predecessor_cp404_resulting_supply_enthalpy_j_per_kg: value,
        predecessor_cp404_resulting_supply_temperature_c: value,
        dehumidification_control_none_or_constant_supply_humidity_ratio_latent_output_maximum_capacity_assignment_executed: false,
        preexisting_cooling_latent_output_w: value,
        cp404_retained_maximum_total_cooling_capacity_owned_read: false,
        maximum_total_cooling_capacity_read: false,
        maximum_total_cooling_capacity_w: value,
        cooling_latent_output_assigned: false,
        assigned_cooling_latent_output_w: value,
        resulting_cooling_latent_output_w: value,
        cp404_retained_supply_humidity_ratio_state_owned: false,
        cp404_retained_supply_enthalpy_state_owned: false,
        cp404_retained_supply_temperature_state_owned: false,
...
#[test]
fn snapshot_serializes_all_fifty_four_numeric_values_with_adjacent_ieee_sidecars() {
    let encoded = snapshot_json(snapshot(Some(-0.0)));
    let Some(object) = encoded.as_object() else {
        assert!(encoded.is_object(), "CP405 snapshot must be an object");
        return;
    };
    assert_eq!(object.len(), 215);
    assert_eq!(
        object
            .keys()
    ...
    assert_eq!(encoded["preexisting_cooling_latent_output_w"], 0.0);
    assert_eq!(
        encoded["preexisting_cooling_latent_output_w_ieee_bits"],
        "0x8000000000000000"
    );
    assert_eq!(
        encoded["predecessor_cp404_psychrometric_supply_humidity_ratio_ieee_bits"],
        "0x8000000000000000"
    );
}
...
fn non_finite_projection_is_null_while_ieee_payload_remains_authoritative() {
    ...
    assert!(encoded["assigned_cooling_latent_output_w"].is_null());
    assert_eq!(
        encoded["assigned_cooling_latent_output_w_ieee_bits"],
        "0x7ff8000000000042"
    );
}
