//! Fail-closed validation for CP429 direct-release evidence.
...
pub(in crate::pipeline) fn validate_direct_lifecycle(
    ...
    predecessor_cp428: Option<&PredecessorLifecycle>,
    init_lifecycle: Option<&PurchasedAirInitLifecycleSummary>,
    coupling_call_count: Option<usize>,
) -> Result<(), String> {
    let lifecycle =
        lifecycle.ok_or_else(|| "direct-zone IdealLoads CP429 evidence is missing".to_string())?;
    let predecessor = predecessor_cp428
        .ok_or_else(|| "direct-zone IdealLoads CP429 CP428 evidence is missing".to_string())?;
    let init = init_lifecycle.ok_or_else(|| {
        "direct-zone IdealLoads CP429 initialization evidence is missing".to_string()
    })?;
    let calls = coupling_call_count
        .ok_or_else(|| "direct-zone IdealLoads CP429 coupling call count is missing".to_string())?;
    if calls == 0
        || lifecycle.source
            != PURCHASED_AIR_CALC_COOLING_ZERO_SUPPLY_MASS_FLOW_TOTAL_OUTPUT_POSITIVE_ZERO_ASSIGNMENT_SOURCE
        ...
        || PURCHASED_AIR_CALC_COOLING_ZERO_SUPPLY_MASS_FLOW_TOTAL_OUTPUT_POSITIVE_ZERO_ASSIGNMENT_SOURCE_ORDER
            != ORDER
    {
        return Err("direct-zone IdealLoads CP429 provenance is invalid".to_string());
    }
    ...
    let system = init
        ...
        .first()
        .copied()
        .ok_or_else(|| "direct-zone IdealLoads CP429 declared system is missing".to_string())?;
    let zone = init
        .controlled_zone
        .ok_or_else(|| "direct-zone IdealLoads CP429 controlled Zone is missing".to_string())?;
    let latest = lifecycle
        .state
        .latest
        .ok_or_else(|| "direct-zone IdealLoads CP429 latest evidence is missing".to_string())?;
    let predecessor_latest = predecessor.state.latest.ok_or_else(|| {
        "direct-zone IdealLoads CP429 CP428 latest evidence is missing".to_string()
    })?;
    ...
    {
        return Err("direct-zone IdealLoads CP429 latest lineage is invalid".to_string());
    }
    Ok(())
}
...
fn validate_public_route_contract(
    state: &State,
    predecessor: &ep_runtime::PurchasedAirCalcCoolingZeroSupplyMassFlowSensibleOutputPositiveZeroAssignmentRuntimeState,
) -> Result<(), String> {
    if state.transition_count != predecessor.transition_count
        || state.predecessor_route_counts != predecessor.predecessor_route_counts
    {
        return Err("direct-zone IdealLoads CP429 route lineage is invalid".to_string());
    }
    for index in 0..36 {
        for values in [
            ...
            if !PUBLIC.contains(&index) && values[index] != 0 {
                return Err(format!(
                    "direct-zone IdealLoads CP429 non-direct route {index} is active"
                ));
            }
        }
    ...
    let inactive = transitions
        ...
        .ok_or_else(|| "direct-zone IdealLoads CP429 inactive partition underflowed".to_string())?;
...
fn checked_sum(values: &[usize]) -> Result<usize, String> {
    values.iter().try_fold(0usize, |sum, value| {
        sum.checked_add(*value)
            .ok_or_else(|| "direct-zone IdealLoads CP429 count overflowed".to_string())
    })
}
...
fn ensure_count(actual: usize, expected: usize, field: &str) -> Result<(), String> {
    if actual == expected {
        ...
        Err(format!(
            "direct-zone IdealLoads CP429 invariant {field} expected {expected}, got {actual}"
        ))
    }
}
