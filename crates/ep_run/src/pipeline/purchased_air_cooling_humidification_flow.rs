//! Run-summary evidence for the bounded PurchasedAir cooling humidification flow.
...
use ep_runtime::{
    ...
    PurchasedAirCalcCoolingHumidificationFlowSnapshot, PurchasedAirInitLifecycleSummary,
};
...
pub(super) fn validate_direct_lifecycle(
    lifecycle: Option<&PurchasedAirCalcCoolingHumidificationFlowLifecycleSummary>,
    predecessor_lifecycle: Option<&PurchasedAirCalcCoolingDehumidificationFlowLifecycleSummary>,
    init_lifecycle: Option<&PurchasedAirInitLifecycleSummary>,
    coupling_call_count: Option<usize>,
) -> Result<(), String> {
    let lifecycle = lifecycle.ok_or_else(|| {
        "direct-zone IdealLoads runtime did not expose cooling humidification-flow evidence"
            .to_string()
    })?;
    let predecessor_lifecycle = predecessor_lifecycle.ok_or_else(|| {
        "direct-zone IdealLoads cooling humidification flow has no dehumidification-flow evidence"
            .to_string()
    })?;
    let init_lifecycle = init_lifecycle.ok_or_else(|| {
        "direct-zone IdealLoads cooling humidification flow has no initialization evidence"
            .to_string()
    })?;
    let coupling_call_count = coupling_call_count.ok_or_else(|| {
        "direct-zone IdealLoads cooling humidification flow has no coupling call count".to_string()
    })?;
    let state = &lifecycle.state;
    let predecessor = &predecessor_lifecycle.state;
    if coupling_call_count == 0
        || lifecycle.source != PURCHASED_AIR_CALC_COOLING_HUMIDIFICATION_FLOW_SOURCE
        || lifecycle.first_excluded_source
            != PURCHASED_AIR_CALC_COOLING_HUMIDIFICATION_FLOW_FIRST_EXCLUDED_SOURCE
        || predecessor_lifecycle.source != PURCHASED_AIR_CALC_COOLING_DEHUMIDIFICATION_FLOW_SOURCE
        || predecessor_lifecycle.first_excluded_source
            != PURCHASED_AIR_CALC_COOLING_DEHUMIDIFICATION_FLOW_FIRST_EXCLUDED_SOURCE
    {
        return Err(
            "direct-zone IdealLoads cooling humidification-flow provenance is invalid".to_string(),
        );
    }
    ...
    let predecessor_cooling_count = predecessor
        ...
        .ok_or_else(|| {
            "direct-zone IdealLoads cooling humidification-flow predecessor skip partition is invalid"
                .to_string()
        })?;
    ...
    let latest = state.latest.as_ref().ok_or_else(|| {
        "direct-zone IdealLoads cooling humidification flow has no latest snapshot".to_string()
    })?;
    let latest_predecessor = predecessor.latest.as_ref().ok_or_else(|| {
        "direct-zone IdealLoads cooling humidification flow has no latest dehumidification-flow snapshot"
            .to_string()
    })?;
    let expected_system = init_lifecycle
        ...
        .copied()
        .ok_or_else(|| {
            "direct-zone IdealLoads cooling humidification flow has no declared system".to_string()
        })?;
    let expected_zone = init_lifecycle.controlled_zone.ok_or_else(|| {
        "direct-zone IdealLoads cooling humidification flow has no controlled Zone".to_string()
    })?;
    ...
    {
        return Err(
            "direct-zone IdealLoads cooling humidification-flow latest state is not release-ready"
                .to_string(),
        );
    }
    Ok(())
...
fn latest_matches_release(
    flow: &PurchasedAirCalcCoolingHumidificationFlowSnapshot,
    ...
    flow.source == PURCHASED_AIR_CALC_COOLING_HUMIDIFICATION_FLOW_SOURCE
        ...
        && predecessor.source == PURCHASED_AIR_CALC_COOLING_DEHUMIDIFICATION_FLOW_SOURCE
        && predecessor.first_excluded_source
            == PURCHASED_AIR_CALC_COOLING_DEHUMIDIFICATION_FLOW_FIRST_EXCLUDED_SOURCE
        && predecessor.source_order == PURCHASED_AIR_CALC_COOLING_DEHUMIDIFICATION_FLOW_SOURCE_ORDER
