...
use ep_model::{
    ...
    MaterialHeatAndMoistureTransferThermalConductivity,
    MaterialHeatAndMoistureTransferThermalConductivityId,
    MaterialHeatAndMoistureTransferThermalConductivityPoint, MaterialId,
...
impl<'a> Compiler<'a> {
    ...
    fn parse_material_heat_and_moisture_transfer_suctions(&mut self, model: &mut TypedModel) {
        ...
        for (name, object) in self.objects(OBJECT_TYPE) {
            ...
            let Some(sorption_last_moisture_content) = sorption_isotherm
                .effective_points
                .last()
            ...
            effective_points.push(MaterialHeatAndMoistureTransferSuctionPoint {
                moisture_content_kg_per_m3: sorption_last_moisture_content,
    ...
    fn parse_material_heat_and_moisture_transfer_redistributions(
        &mut self,
        model: &mut TypedModel,
    ) {
        ...
        for (name, object) in self.objects(OBJECT_TYPE) {
            let diagnostics_before_fields = self.diagnostics.len();
            let material_name = self.required_string(OBJECT_TYPE, &name, &object, "material_name");
            let number_of_redistribution_points = self
                .required_number(
                    OBJECT_TYPE,
                    &name,
                    &object,
                ...
                .and_then(|value| {
                    if value.fract() != 0.0 {
                        self.error(
                            ...
                            format!(
                                "{OBJECT_TYPE}/{name} field number_of_redistribution_points must be an integer, got {value}"
                            ),
                        );
                        None
                    } else if !(1.0..=MAX_POINTS as f64).contains(&value) {
                        self.error(
                            ...
                            Some("number_of_redistribution_points"),
                            format!(
                                "{OBJECT_TYPE}/{name} field number_of_redistribution_points must be between 1 and {MAX_POINTS}, got {value}"
                            ),
                        );
                        None
                    } else {
            ...
            for point in 1..=MAX_POINTS {
                ...
                parsed_points.push(MaterialHeatAndMoistureTransferRedistributionPoint {
                    moisture_content_kg_per_m3: moisture_content_kg_per_m3.unwrap_or(0.0),
                    liquid_transport_coefficient_m2_per_s: liquid_transport_coefficient_m2_per_s
                        .unwrap_or(0.0),
                });
            ...
            let (Some(material_name), Some(number_of_redistribution_points)) =
                (material_name, number_of_redistribution_points)
            else {
                continue;
            };
            ...
            let Some(sorption_last_moisture_content) = sorption_isotherm
                ...
                self.error(
                    "InvalidHeatAndMoistureTransferRedistributionDependency",
                    OBJECT_TYPE,
                    Some(&name),
                    Some("material_name"),
                    format!(
            ...
            if model
                ...
                .iter()
                .any(|redistribution| redistribution.reference_material == reference_material)
            {
                self.error(
                    ...
                    Some("material_name"),
                    format!(
                        "{OBJECT_TYPE}/{name} repeats a material that already has redistribution data"
                    ),
                );
                continue;
            }
            ...
            let input_points =
                parsed_points[..usize::from(number_of_redistribution_points)].to_vec();
            let mut effective_points = input_points.clone();
            let Some(last_input_point) = input_points.last() else {
                self.error(
                    "InvalidHeatAndMoistureTransferRedistributionDerivedState",
                    OBJECT_TYPE,
                    Some(&name),
                    None,
                    format!("{OBJECT_TYPE}/{name} has no active redistribution point"),
                );
                continue;
            };
            effective_points.push(MaterialHeatAndMoistureTransferRedistributionPoint {
                moisture_content_kg_per_m3: sorption_last_moisture_content,
                liquid_transport_coefficient_m2_per_s: last_input_point
                    .liquid_transport_coefficient_m2_per_s,
            });
            ...
            model
                .material_heat_and_moisture_transfer_redistributions
                .push(MaterialHeatAndMoistureTransferRedistribution {
                    id: MaterialHeatAndMoistureTransferRedistributionId(id_value),
                    name: NormalizedName::new(&name),
                    reference_material,
                    reference_sorption_isotherm,
                    number_of_redistribution_points,
    ...
    fn parse_material_heat_and_moisture_transfer_thermal_conductivities(
        &mut self,
        model: &mut TypedModel,
    ) {
        ...
        for (name, object) in self.objects(OBJECT_TYPE) {
            let diagnostics_before_fields = self.diagnostics.len();
            let material_name = self.required_string(OBJECT_TYPE, &name, &object, "material_name");
            let number_of_thermal_coordinates = self
                .required_number(
                    OBJECT_TYPE,
                    &name,
                    &object,
                ...
                .and_then(|value| {
                    if value.fract() != 0.0 {
                        self.error(
                            ...
                            format!(
                                "{OBJECT_TYPE}/{name} field number_of_thermal_coordinates must be an integer, got {value}"
                            ),
                        );
                        None
                    } else if !(1.0..=MAX_POINTS as f64).contains(&value) {
                        self.error(
                            ...
                            Some("number_of_thermal_coordinates"),
                            format!(
                                "{OBJECT_TYPE}/{name} field number_of_thermal_coordinates must be between 1 and {MAX_POINTS}, got {value}"
                            ),
                        );
                        None
                    } else {
            ...
            for point in 1..=MAX_POINTS {
                let moisture_content_field = format!("moisture_content_{point}");
                let thermal_conductivity_field = format!("thermal_conductivity_{point}");
                let moisture_content_kg_per_m3 = if point == 1 {
                    self.required_number_bounded(
                        OBJECT_TYPE,
                        &name,
                ...
                let thermal_conductivity_w_per_m_k = if point == 1 {
                    self.required_number_bounded(
                        OBJECT_TYPE,
                        &name,
                        &object,
                        &thermal_conductivity_field,
                        (0.0, false),
                        (f64::INFINITY, false),
                    )
                } else {
                ...
                parsed_points.push(MaterialHeatAndMoistureTransferThermalConductivityPoint {
                    moisture_content_kg_per_m3: moisture_content_kg_per_m3.unwrap_or(0.0),
                    thermal_conductivity_w_per_m_k: thermal_conductivity_w_per_m_k.unwrap_or(0.0),
                });
            }
            if self.diagnostics.len() != diagnostics_before_fields {
                continue;
            }
            let (Some(material_name), Some(number_of_thermal_coordinates)) =
                (material_name, number_of_thermal_coordinates)
            else {
                continue;
            };
            ...
            let Some(sorption_last_moisture_content) = sorption_isotherm
                ...
                self.error(
                    "InvalidHeatAndMoistureTransferThermalConductivityDependency",
                    OBJECT_TYPE,
                    Some(&name),
                    Some("material_name"),
                    format!(
            ...
            if model
                ...
                .iter()
                .any(|thermal_conductivity| {
                    thermal_conductivity.reference_material == reference_material
                })
            {
                self.error(
                    ...
                    Some("material_name"),
                    format!(
                        "{OBJECT_TYPE}/{name} repeats a material that already has thermal-conductivity data"
                    ),
                );
                continue;
            }

            let input_points = parsed_points[..usize::from(number_of_thermal_coordinates)].to_vec();
            let mut effective_points = input_points.clone();
            let Some(last_input_point) = input_points.last() else {
                self.error(
                    "InvalidHeatAndMoistureTransferThermalConductivityDerivedState",
                    OBJECT_TYPE,
                    Some(&name),
                    None,
                    format!("{OBJECT_TYPE}/{name} has no active thermal conductivity point"),
                );
                continue;
            };
            effective_points.push(MaterialHeatAndMoistureTransferThermalConductivityPoint {
                moisture_content_kg_per_m3: sorption_last_moisture_content,
                thermal_conductivity_w_per_m_k: last_input_point.thermal_conductivity_w_per_m_k,
            });
            ...
            model
                .material_heat_and_moisture_transfer_thermal_conductivities
                .push(MaterialHeatAndMoistureTransferThermalConductivity {
                    id: MaterialHeatAndMoistureTransferThermalConductivityId(id_value),
                    name: NormalizedName::new(&name),
                    reference_material,
                    reference_sorption_isotherm,
                    number_of_thermal_coordinates,
    ...
    fn parse_window_shade_equivalent_layer_materials(&mut self, model: &mut TypedModel) {
        const OBJECT_TYPE: &str = "WindowMaterial:Shade:EquivalentLayer";

        for (name, object) in self.objects(OBJECT_TYPE) {
            let shade_beam_beam_solar_transmittance = self.number_bounded_default(
                OBJECT_TYPE,
                &name,
                &object,
            ...
            let front_side_shade_beam_diffuse_solar_transmittance = self.required_number_bounded(
                OBJECT_TYPE,
                &name,
                &object,
            ...
            let back_side_shade_beam_diffuse_solar_transmittance = self.required_number_bounded(
                OBJECT_TYPE,
                &name,
                &object,
            ...
            let front_side_shade_beam_diffuse_solar_reflectance = self.required_number_bounded(
                OBJECT_TYPE,
                &name,
                &object,
            ...
            let back_side_shade_beam_diffuse_solar_reflectance = self.required_number_bounded(
                OBJECT_TYPE,
                &name,
                &object,
            ...
            let shade_beam_beam_visible_transmittance_at_normal_incidence = self
                .number_bounded_default(
                    OBJECT_TYPE,
                    &name,
                    &object,
            ...
            let shade_beam_diffuse_visible_transmittance_at_normal_incidence = self
                .number_bounded_default(
                    OBJECT_TYPE,
                    &name,
                    &object,
            ...
            let shade_beam_diffuse_visible_reflectance_at_normal_incidence = self
                .number_bounded_default(
                    OBJECT_TYPE,
                    &name,
                    &object,
            ...
            let shade_material_infrared_transmittance = self.number_bounded_default(
                OBJECT_TYPE,
                &name,
                &object,
            ...
            let front_side_shade_material_infrared_emissivity = self.number_bounded_default(
                OBJECT_TYPE,
                &name,
                &object,
            ...
            let back_side_shade_material_infrared_emissivity = self.number_bounded_default(
                OBJECT_TYPE,
                &name,
                &object,
            ...
            let mut combinations_valid = true;
            let mut invalid_sum = |field: &'static str, label: &str, sum: f64| {
                if sum >= 1.0 {
                    self.error(
                        ...
                        Some(field),
                        format!(
                            "{OBJECT_TYPE}/{name} {label} optical-property sum must be less than 1.0, got {sum}"
                        ),
                    );
                    combinations_valid = false;
                }
            ...
            if let (Some(front_transmittance), Some(front_reflectance)) = (
                ...
                invalid_sum(
                    "shade_beam_beam_solar_transmittance",
                    "front solar",
                    shade_beam_beam_solar_transmittance + front_transmittance + front_reflectance,
                );
            }
            if let (Some(back_transmittance), Some(back_reflectance)) = (
                back_side_shade_beam_diffuse_solar_transmittance,
                back_side_shade_beam_diffuse_solar_reflectance,
            ) {
                invalid_sum(
                    "shade_beam_beam_solar_transmittance",
                    "back solar",
                    shade_beam_beam_solar_transmittance + back_transmittance + back_reflectance,
                );
            }
            invalid_sum(
                "shade_beam_beam_visible_transmittance_at_normal_incidence",
            ...
            let (
                Some(front_side_shade_beam_diffuse_solar_transmittance),
                Some(back_side_shade_beam_diffuse_solar_transmittance),
                Some(front_side_shade_beam_diffuse_solar_reflectance),
            ...
            model.materials.push(Material {
                id,
                name: normalized_name,
                definition: MaterialDefinition::WindowShadeEquivalentLayer(
                    WindowShadeEquivalentLayerMaterial {
                        roughness: MaterialSurfaceRoughness::MediumRough,
                        front_solar: WindowShadeEquivalentLayerSideOpticalProperties {
                            beam_beam_transmittance: shade_beam_beam_solar_transmittance,
                        ...
                        infrared_transmittance: shade_material_infrared_transmittance,
                        front_infrared_emissivity: front_side_shade_material_infrared_emissivity,
                        back_infrared_emissivity: back_side_shade_material_infrared_emissivity,
...
mod tests {
    ...
    mod material_property_heat_and_moisture_transfer_thermal_conductivity;
