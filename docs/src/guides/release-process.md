...
for local inspection. The GitHub Release page stays intentionally small: the
workflow uploads the binary zip plus the curated public evidence PDFs selected
by `scripts/release/select-release-assets.ps1`. HTML, JSON, and Markdown
outputs remain local evidence artifacts unless a release explicitly opts into
publishing them.
