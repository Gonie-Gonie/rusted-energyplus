...
| Zone | yes | partial | yes | yes | partial | partial | partial | partial | complete public declaration fields plus bounded nominal-control, ZoneGroup, and last-nonblank local-environment node-link side effects; local convection overrides, zone grouping, and local environments fail closed before runtime; existing first-zone numerical boundaries are unchanged |
