...
| Construction | yes | partial | yes | yes | partial | partial | partial | partial | ordered opaque layer stack used for first-zone UA and compared against EIO `Construction CTF`; official `1ZoneUncontrolled` static EIO fields are v0.23 conformance, while `material_opaque_variants_001` adds nonblocking exact ordered-layer diagnostic evidence only. CP103 adds private immutable thermochromic series/child projections in master-ID then state order without granting child ConstructionIds, mutating the name/count/graph arenas, or weakening the thermochromic all-definition runtime block. CP105 adds only sorted/deduplicated positive known-used and source-mark-shaped known-CTF-used ConstructionId metadata; the latter does not assert AirBoundary CTF coefficient state. Absence proves nothing and changes no object count, graph, runtime support, warning, or capability. Global child integration, switching, unused inference, CTF/CondFD selection, window behavior, reporting, and conformance remain deferred |
