...
### v0.1.0 — Reproducible Setup / Oracle Release

목표:

```text
개발 환경과 기준 oracle을 release한다.
```
...
### v0.2.0 — RawModel / epJSON Inspection

목표:

```text
실제 IDF/epJSON을 읽고 raw model summary를 만든다.
```
...
### v0.3.0 — TypedModel / Reference Resolution

목표:

```text
RawModel을 typed model로 변환하고 name reference를 typed ID로 해석한다.
```
...
### v0.4.0 — Model Compiler / Compile Artifacts
...
### v0.5.0 — Weather/Schedule Compatibility
...
### v0.6.0 — Graph Validation / Execution Plan
...
### v0.7.0 — SimulationState / ResultStore / First Runnable Subset
...
### v0.9.0 — IdealLoads Compatibility
...
### v0.11.0 — Air/Plant Graph Preview
...
```text
eplus-rs 0.7.0 for EnergyPlus 26.1.0 compatibility
```
...
```text
eplus-rs-v0.7.0-windows-x64.zip
  bin/
    eplus-rs.exe
  scripts/
  docs/
...
```text
eplus-rs-v0.7.0-windows-x64-offline-oracle.zip
