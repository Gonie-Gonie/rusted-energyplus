...
| Opaque conduction | CTF setup and surface balance reporting | surface and zone conduction result series | no-mass adiabatic conduction series in `surface_temperature_nomass_001` | see `conduction-map.md`; no official ExampleFile CTF parity |
...
| Warmup and reporting semantics | warmup convergence and ESO reporting filters | future warmup-aware runtime/report context | current reports compare run-period ESO only | see `warmup-semantics-map.md`; official dynamic cases are baseline candidates |
