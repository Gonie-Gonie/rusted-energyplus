...
- `src/EnergyPlus/SurfaceGeometry.cc` for aggregate unused-HAMT object counting
  and its warning boundary
...
| typed material overlays | 11 / 12 | `MaterialProperty:VariableAbsorptance` is typed in a separate overlay arena after its eligible base-material and schedule dependencies are available. `MaterialProperty:PhaseChangeHysteresis`, `MaterialProperty:PhaseChange`, `MaterialProperty:VariableThermalConductivity`, `MaterialProperty:MoisturePenetrationDepth:Settings`, and all six `MaterialProperty:HeatAndMoistureTransfer:*` objects are typed in separate attachment arenas keyed by their existing material targets. None is a `MaterialDefinition` variant. The twelfth in-boundary `MaterialProperty:*` object is the standalone glazing spectral dataset, not an overlay. |
| complete bounded public-object slices | 33 / 34 | The 21 complete base-definition slices, standalone glazing spectral dataset, variable-absorptance overlay, phase-change-hysteresis attachment, phase-change temperature-enthalpy attachment, variable-thermal-conductivity attachment, EMPD settings attachment, and all six HAMT attachments are complete within their declared bounded compiler contracts. |
...
| wholly deferred public objects | 0 / 34 | No in-boundary EnergyPlus 26.1 public material object remains wholly deferred. The three unsupported `WindowMaterial:Glazing` optical branches keep that one object partial. |
...
`MaterialProperty:PhaseChange` and
`MaterialProperty:VariableThermalConductivity` are not part of common startup.
EnergyPlus reads them lazily when CondFD initialization invokes
`GetCondFDInput`; selecting
`HeatBalanceAlgorithm = ConductionFiniteDifference` is sufficient even with no
surfaces, while a non-CondFD algorithm skips these semantic checks. The routine
first reads `HeatBalanceSettings:ConductionFiniteDifference`, then PhaseChange,
and then VariableThermalConductivity. Rust publishes both immutable attachments after
...
- internally generated F/C-factor material targets remain outside the executable boundary; typed PhaseChange, VariableThermalConductivity, EMPD settings, and all six HAMT attachments may coexist on the same public target while every combination remains run-blocking
...
all three alongside PhaseChange in source-relative order. Every PhaseChange
attachment, including one on an unused or NoMass material, blocks arbitrary
runtime execution. Rust does not allocate `MaterialFD`, replace an empty table
with the source three-point `-100` sentinel, interpolate enthalpy, derive
specific heat or conductivity, allocate CondFD construction/surface/node state
or history, reproduce Hysteresis/VTC precedence, or execute EMS and reporting
paths. Source-accepted one- and two-point tables are typed but never executed
because downstream CondFD assumes at least three points. The object emits no
dedicated EIO row, so this checkpoint adds compiler and support-boundary tests
but no case manifest, proof variable, runtime numerical claim, or conformance
claim.

## Variable Thermal Conductivity Attachment Typed Contract

`MaterialProperty:VariableThermalConductivity` is a complete bounded typed-input
attachment, not a material variant. Its object key is the referenced material
name itself; there is no independent attachment name, namespace, coefficient,
or fixed numeric field. Each valid record owns a typed ID, retains a normalized
target snapshot, resolves a `MaterialId`, and is stored after PhaseChange in a
separate deterministic `MaterialVariableThermalConductivity` arena. The
underlying `MaterialDefinition` stays unchanged.

The executable EnergyPlus 26.1 epJSON schema makes the source-ordered `values`
array optional and gives it no item-count cap. Every present item must contain
both `temperature` and `thermal_conductivity`. Rust therefore preserves absent
or empty arrays and zero, one, two, three, one hundred, 101, and larger complete
pair counts. It does not invent the stale textual IDD requirement for three
pairs or its stated one-hundred-pair ceiling; the same IDD actually declares a
101st pair. Native epJSON `{}` succeeds in EnergyPlus, unlike the PhaseChange
empty-object presence-count defect.

Rust requires both members of every pair to be finite. Temperatures must
strictly increase, while conductivity has no sign, monotonicity, or
cross-field rule: negative, zero, equal, or decreasing finite conductivity is
valid. EnergyPlus IDF execution can retain NaN, infinity, or an overflowing
exponent as a numeric value; the VTC reader itself does not reject nonfinite
conductivity and only its ordinary temperature comparison can reject some
nonfinite temperature placements. Rust's finite-only rule is therefore an
explicit bounded fail-closed divergence. ConvertInputFormat serializing such
IDF scalars as JSON `null` does not change that source-execution fact.

The source `Group::Regular` target gate admits public `Material` and
`Material:NoMass`; AirGap, InfraredTransparent, RoofVegetation, and every window
family fail closed. Internal `~FC_Concrete` and `~FC_Insulation_n` targets remain
outside the public bounded set. Blank or missing targets, wrong families,
malformed/incomplete/nonfinite pairs, nonincreasing temperatures, and a second
case-insensitive VTC target fail before attachment identity or ownership is
reserved. EnergyPlus instead validates only when CondFD initializes and assigns
later repeated records into the same material-indexed `TempCond` state. Rust
validates eagerly and rejects repetition because it does not recover reliable
source declaration order. The same target may independently retain typed
VariableAbsorptance, PhaseChangeHysteresis, and PhaseChange attachments.

Every VTC attachment, including one on an unused or NoMass material, blocks
arbitrary runtime execution. Rust does not allocate `MaterialFD`, materialize
the source three-point `-100` sentinel for an empty table, interpolate or clamp
conductivity, or execute the source first-three-value activation rule. It also
does not execute fallback to base conductivity or PhaseChange `tk1`, the later
Hysteresis conductivity override, the final EMS override, CondFD construction,
surface, and node state, or reporting. EMPD and HAMT keep algorithm-local state
and do not directly consume the VTC table; combined execution and the upstream
Hysteresis-plus-EMPD/HAMT subclass-lifetime hazards remain nonclaims.
Source-accepted one- and two-point tables remain typed but never execute because
consumers directly assume at least three points. The object emits no dedicated
EIO row, so this checkpoint adds no case manifest, proof variable, runtime
numerical claim, or conformance claim.
...
- a separate deterministic `MaterialVariableThermalConductivity` attachment arena whose records own a typed ID, normalized target snapshot, resolved `MaterialId`, and source-ordered vector of complete temperature/conductivity points; the object key is not published as a separate name map
...
- compile failure before attachment ID or target reservation for a blank or missing target, a non-Regular-group public target, malformed/incomplete/nonfinite values, nonincreasing temperatures, or a second case-insensitive VariableThermalConductivity attachment for one material

history_state_ownership:
- no mutable `MaterialFD`, `TempEnth`, `TempCond`, sentinel, CondFD-node property, or timestep history in this checkpoint; the compiled model owns immutable PhaseChange and VariableThermalConductivity attachment descriptors while every downstream consumer remains unsupported

unsupported_state:
- the preceding `HeatBalanceSettings:ConductionFiniteDifference` pass, `MaterialFD` allocation and `tk1`/`TempEnth`/`TempCond` plus both three-point `-100` sentinel states, construction/surface/node allocation, interpolation, activation, specific-heat/conductivity and PhaseChange/VTC/Hysteresis precedence execution, EMS, generic CondFD reporting, and conformance

inactive_branches:
- `Material:NoMass` passes the source `Group::Regular` gate even though downstream R-only CondFD execution bypasses both temperature-enthalpy and temperature-conductivity tables
- zero-point PhaseChange and VariableThermalConductivity tables are valid typed empty state, and Rust does not materialize either source three-point `-100` sentinel; Rust applies the executable-schema coefficient default to an entirely empty PhaseChange object instead of reproducing its native-epJSON negative-presence-count defect, while a native empty VTC object succeeds
- EnergyPlus skips target and table-order validation for both attachment families when CondFD initialization does not invoke `GetCondFDInput`; selecting `HeatBalanceAlgorithm = ConductionFiniteDifference` invokes the reader even with no surfaces, while Rust deliberately validates every definition eagerly and fails closed

unsupported_active_branches:
- every valid PhaseChange or VariableThermalConductivity attachment is typed but run-blocking, including one attached only to an unused material
- source-accepted one- and two-point tables in either family remain typed but are never executed because downstream CondFD directly assumes the first three points; first-three enthalpy/conductivity activation quirks and fallback behavior are also not executed
...
The dependent SorptionIsotherm, Suction, Redistribution, Diffusion, and
ThermalConductivity passes are now typed in their own immutable arenas.
SorptionIsotherm requires this Settings attachment. Suction, Redistribution,
Diffusion, and ThermalConductivity are independent siblings that each require
the resulting typed SorptionIsotherm; none depends on another sibling. The
ThermalConductivity reader appends a Sorption moisture endpoint. Later
active-construction validation may synthesize a constant-conductivity table
from the positive base conductivity only when that object is absent; Rust does
not materialize that runtime fallback. Used HAMT constructions require every
layer to be a HAMT material and allocate a nonlinear cell grid. This checkpoint
executes none of those consumers. Every Settings, SorptionIsotherm, Suction,
Redistribution, Diffusion, or ThermalConductivity attachment, including one on
an unused material, blocks arbitrary runtime. The six fixed HAMT surface outputs, three per-cell
output families, HAMT cells and origins EIO, generic Material Nominal Resistance
EIO, and SQLite porosity path remain unsupported; none of the six typed HAMT
objects has a dedicated EIO row. No manifest, comparator, proof variable,
runtime numerical claim, or conformance claim is added.

## Heat And Moisture Transfer Sorption Isotherm Attachment Typed Contract

`MaterialProperty:HeatAndMoistureTransfer:SorptionIsotherm` is the complete
bounded second `GetHeatBalHAMTInput` pass. It is an immutable attachment, not a
material variant. Its outer epJSON instance key is nonsemantic and may be blank
or collide case-insensitively; Rust retains only a normalized diagnostic
snapshot and creates no name map. The required inner `material_name` is the
semantic target. It must resolve to a public `Material` that already owns the
typed Settings attachment, and each record stores both its `MaterialId` and
`MaterialHeatAndMoistureTransferSettingsId`. The base definition and Settings
record remain unchanged.

The executable schema and textual IDD expose a fixed tail of 25 coordinate
pairs, not an extension array. `number_of_isotherm_coordinates` is required and
must be an exact integer from 1 through 25. Pair 1 is required; pairs 2 through
25 are optional. Rust validates every supplied scalar, including fields beyond
the declared count, then retains only the declared active prefix. Missing
optional fields inside that prefix become source-equivalent zero, including a
half-present pair, while valid supplied fields beyond the count are ignored.
Each relative-humidity fraction is finite and in inclusive `[0,1]`; each
moisture content is finite and nonnegative with no upper bound. Input order,
duplicate relative humidity, descending moisture, and moisture above the
Settings-derived saturation endpoint remain valid. Rust's exact-integer rule is
schema-faithful and deliberately rejects EnergyPlus IDF's observed rounding of
count `1.5` to 2.

Each attachment owns both the active zero-filled input prefix and the
source-effective vector. Rust appends `(1.01, porosity * 1000)` and `(0, 0)`,
reproduces the source nested-swap ascending-relative-humidity sort, then runs up
to 100 left-to-right passes that replace each descending adjacent moisture pair
with its mean. Any correction is retained as one warning state. Because moisture
has no schema upper bound, adding two finite values can overflow during the
source-equivalent mean; Rust detects any derived nonfinite point and fails
closed before identity reservation. Its finite-only scalar validation also
deliberately rejects source-retained RH NaN and moisture NaN/positive infinity;
active-HAMT oracle probes reached denominator severe diagnostics and a Windows
access violation for the nonfinite moisture cases, so those crash paths are not
reproduced.

Missing or blank targets, missing Settings, invalid fields, derived nonfinite
state, and a second case-insensitive SorptionIsotherm for one `MaterialId` fail
before attachment identity or ownership is reserved. EnergyPlus performs this
semantic read only after the first actual HAMT surface and effectively lets a
later duplicate target replace the earlier active curve; Rust validates every
definition eagerly and rejects repetition. The typed Suction and Redistribution
passes independently consume the indexed last source-effective Sorption moisture
point to materialize their respective appended liquid-transport endpoints, while
the typed Diffusion pass independently consumes the indexed last
source-effective Sorption relative humidity for its appended vapor-transport
endpoint. The typed ThermalConductivity pass independently consumes the indexed
last source-effective Sorption moisture for its appended conductivity endpoint.
Runtime interpolation and every other sorption consumer, construction completeness, cell allocation and history,
outputs and reports, exact warning text/order/count, runtime numerics, and
conformance remain unsupported. Every definition,
including one on an unused material, blocks arbitrary runtime. No manifest,
comparator, or proof variable is added.

## Heat And Moisture Transfer Suction Attachment Typed Contract

`MaterialProperty:HeatAndMoistureTransfer:Suction` is the complete bounded
third `GetHeatBalHAMTInput` pass. It is an immutable attachment rather than a
material variant. Its outer epJSON instance key is nonsemantic and may be blank
or collide case-insensitively; Rust retains only a normalized diagnostic
snapshot and creates no name map. The required inner `material_name` is the
semantic target. It must resolve to a public `Material` that already owns a
typed SorptionIsotherm attachment and therefore a typed Settings attachment.
Each record stores its `MaterialId` and
`MaterialHeatAndMoistureTransferSorptionIsothermId`; the base definition and
both prerequisite attachments remain unchanged.

The executable schema and textual IDD expose a fixed tail of 25 suction pairs,
not an extension array. `number_of_suction_points` is required and must be an
exact integer from 1 through 25. Pair 1 is required; pairs 2 through 25 are
optional. Rust validates every supplied scalar, including fields beyond the
declared count, then retains only the declared active prefix. Missing optional
fields inside that prefix become source-equivalent zero, including a
half-present pair, while valid supplied fields beyond the count are ignored.
Moisture content and liquid transport coefficient are each finite and
nonnegative with no upper bound. Input order, repeated or descending moisture,
repeated coefficients, and any relationship between the two columns remain
valid: the source performs no suction sort, correction, monotonicity check,
uniqueness check, or warning.

Each attachment owns both the active zero-filled input prefix and the
source-effective vector. Rust preserves that prefix in authored order, then
appends exactly one point whose moisture content is the last point of the
source-effective SorptionIsotherm and whose liquid transport coefficient is
copied from the last active Suction point. An authored count of 25 therefore
produces 26 source-effective points. The dependency intentionally uses the
corrected Sorption vector rather than recomputing `porosity * 1000`. EnergyPlus
checks only the shared `hasHAMT` state before this pass and then immediately
indexes the Sorption endpoint; without Sorption the default negative index is
unsafe. Rust deliberately requires the typed Sorption dependency and fails
closed instead of preserving that invalid source path.

Missing or blank targets, missing SorptionIsotherm, missing required fields,
malformed/nonfinite/negative supplied fields, invalid counts, and a second
case-insensitive Suction attachment for one `MaterialId` fail before attachment
identity or ownership is reserved. Rust exact-integer and finite-only rules are
deliberate bounded fail-closes; exact EnergyPlus IDF fractional-count and
nonfinite handling remain unclaimed. Eager validation, dependency safety, and
duplicate rejection also fail closed relative to source lazy first-surface
reading and later-effective replacement. The typed Redistribution, Diffusion,
and ThermalConductivity source-order siblings may coexist on the same target and
each independently requires SorptionIsotherm, not Suction or another sibling. Every definition,
including one on an unused material, blocks arbitrary runtime. Diffusion
and ThermalConductivity interpolation, rain-selected Suction and
ordinary Redistribution interpolation, construction and cell state, outputs and
reports, exact diagnostics/order/multiplicity, runtime numerics, and conformance
remain unsupported. No manifest, comparator, or proof variable is added.

## Heat And Moisture Transfer Redistribution Attachment Typed Contract

`MaterialProperty:HeatAndMoistureTransfer:Redistribution` is the complete
bounded fourth `GetHeatBalHAMTInput` pass. It is an immutable attachment rather
than a material variant. Its outer epJSON instance key is nonsemantic and may be
blank or collide case-insensitively; Rust retains only a normalized diagnostic
snapshot and creates no name map. The required inner `material_name` is the
semantic target. It must resolve to a public `Material` that already owns a
typed SorptionIsotherm attachment and therefore a typed Settings attachment.
Suction is a source-order predecessor but not a dependency: a target with
Material, Settings, SorptionIsotherm, and Redistribution but no Suction remains
valid typed input. Each record stores its `MaterialId` and
`MaterialHeatAndMoistureTransferSorptionIsothermId`; the base definition and
prerequisite attachments remain unchanged.

The executable schema and textual IDD expose a fixed tail of 25 redistribution
pairs, not an extension array. `number_of_redistribution_points` is required and
must be an exact integer from 1 through 25. Pair 1 is required; pairs 2 through
25 are optional. Rust validates every supplied scalar, including fields beyond
the declared count, then retains only the declared active prefix. Missing
optional fields inside that prefix become source-equivalent zero independently,
including either half of a pair, while valid supplied fields beyond the count
are ignored. Moisture content and liquid transport coefficient are each finite
and nonnegative with no upper bound. Authored order, repeated or descending
moisture, repeated coefficients, and every relationship between the two columns
remain valid: the source performs no redistribution-table sorting, correction,
monotonicity or uniqueness check, clamp, or table-validation warning.

Each attachment owns both the active zero-filled input prefix and the
source-effective vector. Rust preserves that prefix in authored order, then
appends exactly one point whose moisture content is the indexed last point of
the source-effective SorptionIsotherm and whose liquid transport coefficient is
copied from the last active Redistribution point. An authored count of 25
therefore produces 26 source-effective points. The dependency intentionally
uses the corrected Sorption vector rather than its maximum or a fresh
`porosity * 1000` calculation. EnergyPlus checks only shared HAMT state before
this pass and then indexes the Sorption endpoint without a distinct guard; Rust
requires the typed Sorption dependency and fails closed on that unsafe path.

Missing or blank targets, missing SorptionIsotherm, missing required fields,
malformed/nonfinite/negative supplied fields, invalid counts, and a second
case-insensitive Redistribution attachment for one `MaterialId` fail before
attachment identity or ownership is reserved. Rust exact-integer and finite-only
rules are deliberate bounded fail-closes; exact EnergyPlus IDF fractional-count,
nonfinite, and missing-Sorption failure behavior remain unclaimed. Eager
validation, dependency safety, and duplicate rejection also fail closed relative
to source lazy first-surface reading and later-effective replacement. The typed
Diffusion and ThermalConductivity source-order siblings may coexist on the same
target and independently require SorptionIsotherm, not Suction, Redistribution,
or each other. Every definition,
including one on an unused material, blocks arbitrary runtime. EnergyPlus
normally interpolates Redistribution when its rain condition is false and
Suction when it is true; both paths, Diffusion and ThermalConductivity
interpolation, construction and cell state, outputs and reports,
exact diagnostics/order/multiplicity, runtime numerics, and conformance remain
unsupported. No manifest, comparator, or proof variable is added.

## Heat And Moisture Transfer Diffusion Attachment Typed Contract

`MaterialProperty:HeatAndMoistureTransfer:Diffusion` is the complete bounded
fifth `GetHeatBalHAMTInput` pass. It is an immutable attachment rather than a
material variant. Its outer epJSON instance key is nonsemantic and may be blank
or collide case-insensitively; Rust retains only a normalized diagnostic
snapshot and creates no name map. The required inner `material_name` is the
semantic target. It must resolve to a public `Material` that already owns a
typed SorptionIsotherm attachment and therefore a typed Settings attachment.
Suction and Redistribution are source-order predecessors but neither is a
dependency: a target with Material, Settings, SorptionIsotherm, and Diffusion but
neither liquid-transport attachment remains valid typed input. Each record
stores its `MaterialId` and
`MaterialHeatAndMoistureTransferSorptionIsothermId`; the base definition and
prerequisite or optional sibling attachments remain unchanged.

The executable schema and textual IDD expose a fixed tail of 25 diffusion pairs,
not an extension array. `number_of_data_pairs` is required and must be an exact
integer from 1 through 25. Pair 1 is required; pairs 2 through 25 are optional.
Rust validates every supplied scalar, including fields beyond the declared
count, then retains only the declared active prefix. Missing optional fields
inside that prefix independently become source-equivalent zero, including
either half of a pair, while valid supplied fields beyond the count are ignored.
Relative-humidity fraction is finite and in inclusive `[0,1]`; water-vapor
diffusion resistance factor is finite and nonnegative with no upper bound.
Authored order, repeated or descending relative humidity, repeated or arbitrarily
ordered resistance factors, and every relationship between the columns remain
valid: the source performs no diffusion-table sorting, correction, monotonicity
or uniqueness check, clamp, cross-field rule, or table-validation warning.

Each attachment owns both the active zero-filled input prefix and the
source-effective vector. Rust preserves that prefix in authored order, then
appends exactly one point whose relative humidity is the indexed last point of
the source-effective SorptionIsotherm and whose water-vapor diffusion resistance
factor is copied from the last active Diffusion point. An authored count of 25
therefore produces 26 source-effective points. Within the bounded Sorption
contract, the appended relative humidity is 1.01. EnergyPlus checks only shared
HAMT state before this pass and then indexes the Sorption endpoint without a
distinct guard; Rust requires the typed Sorption dependency and fails closed on
that unsafe path without inventing a dependency on Suction or Redistribution.

Missing or blank targets, missing SorptionIsotherm, missing required fields,
malformed/nonfinite/out-of-range supplied fields, invalid counts, and a second
case-insensitive Diffusion attachment for one `MaterialId` fail before
attachment identity or ownership is reserved. Rust exact-integer and finite-only
rules are deliberate bounded fail-closes; exact EnergyPlus IDF fractional-count,
nonfinite, and missing-Sorption failure behavior remain unclaimed. Eager
validation, dependency safety, and duplicate rejection also fail closed relative
to source lazy first-surface reading and later-effective replacement. Every
definition, including one on an unused material, blocks arbitrary runtime.
The typed ThermalConductivity source-order successor may coexist on the same
target and independently requires SorptionIsotherm, not Suction, Redistribution,
or Diffusion. Diffusion and ThermalConductivity interpolation, construction and
cell state, outputs and reports, exact diagnostics/order/multiplicity including
the aggregate unused-HAMT warning, runtime numerics, and conformance remain
unsupported. No manifest, comparator, or proof variable is added.

## Heat And Moisture Transfer Thermal Conductivity Attachment Typed Contract

`MaterialProperty:HeatAndMoistureTransfer:ThermalConductivity` is the complete
bounded sixth material pass of `GetHeatBalHAMTInput`. It is an immutable
attachment rather than a material variant. Its outer epJSON instance key is
nonsemantic and may be blank or collide case-insensitively; Rust retains only a
normalized diagnostic snapshot and creates no name map. The required inner
`material_name` is the semantic target. It must resolve to a public `Material`
that already owns a typed SorptionIsotherm attachment and therefore a typed
Settings attachment. Suction, Redistribution, and Diffusion are source-order
predecessors but are not dependencies: Material, Settings, SorptionIsotherm,
and ThermalConductivity alone remain valid typed input. Each record stores its
`MaterialId` and `MaterialHeatAndMoistureTransferSorptionIsothermId`; the base
definition and prerequisite or optional sibling attachments remain unchanged.

The executable schema and textual IDD expose a fixed tail of 25 thermal
coordinate pairs, not an extension array. `number_of_thermal_coordinates` is
required and must be an exact integer from 1 through 25. Pair 1 is required;
pairs 2 through 25 are optional. Rust validates every supplied scalar,
including fields beyond the declared count, then retains only the declared
active prefix. Missing optional fields inside that prefix independently become
source-equivalent zero, including either half of a pair, while valid supplied
fields beyond the count are ignored. Every supplied moisture content is finite
and nonnegative; every supplied thermal conductivity is finite and strictly
positive. Neither column has an upper bound. Consequently an omitted optional
active conductivity becomes source numeric-blank zero even though an explicitly
supplied zero conductivity is rejected. Authored order, repeated or descending
moisture, repeated or arbitrarily ordered conductivity, and every relationship
between the columns remain valid: the source performs no thermal-conductivity
table sorting, correction, monotonicity or uniqueness check, clamp, cross-field
rule, or table-validation warning.

Each attachment owns both the active zero-filled input prefix and the
source-effective vector. Rust preserves that prefix in authored order, then
appends exactly one point whose moisture content is the indexed last point of
the source-effective SorptionIsotherm and whose thermal conductivity is copied
from the last active ThermalConductivity point. An authored count of 25
therefore produces 26 source-effective points. The dependency intentionally
uses the corrected Sorption vector rather than its maximum or a fresh
`porosity * 1000` calculation. EnergyPlus checks only shared HAMT state before
this pass and then indexes the Sorption endpoint without a distinct guard; Rust
requires the typed Sorption dependency and fails closed on that unsafe path
without inventing a dependency on Suction, Redistribution, or Diffusion.

Missing or blank targets, missing SorptionIsotherm, missing required fields,
malformed/nonfinite/out-of-range supplied fields, invalid counts, and a second
case-insensitive ThermalConductivity attachment for one `MaterialId` fail
before attachment identity or ownership is reserved. Rust exact-integer and
finite-only rules are deliberate bounded fail-closes; exact EnergyPlus IDF
fractional-count, nonfinite, and missing-Sorption failure behavior remain
unclaimed. Eager validation, dependency safety, and duplicate rejection also
fail closed relative to source lazy first-surface reading and later-effective
replacement. Rust does not synthesize an attachment when the object is absent.
EnergyPlus later creates `(0, base conductivity)` and `(indexed last Sorption
moisture, base conductivity)` only for an active HAMT construction that lacks
this table and has positive base conductivity, while emitting a warning; that
construction-local fallback and its error branches remain unsupported. Every
typed definition, including one on an unused material, blocks arbitrary
runtime. Thermal-conductivity interpolation, construction and cell state,
outputs and reports, exact diagnostics/order/multiplicity including the
aggregate unused-HAMT warning, runtime numerics, and conformance remain
...
- deterministic compiler-ordered `MaterialProperty:HeatAndMoistureTransfer:SorptionIsotherm` definitions after Settings; each has a nonsemantic outer-key snapshot, a semantic inner material target, and a required exact-integer coordinate count from 1 through 25
- the fixed 25-pair sorption tail: pair 1 is required, pairs 2 through 25 are optional, every supplied scalar is validated, missing optional active-prefix scalars become zero, and positions beyond the declared count are ignored after validation; relative humidity is finite in inclusive [0,1] and moisture content is finite and nonnegative without an upper bound
- deterministic compiler-ordered `MaterialProperty:HeatAndMoistureTransfer:Suction` definitions after SorptionIsotherm; each has a nonsemantic outer-key snapshot, a semantic inner material target, and a required exact-integer suction-point count from 1 through 25
- the fixed 25-pair suction tail: pair 1 is required, pairs 2 through 25 are optional, every supplied scalar is validated, missing optional active-prefix scalars become zero, and positions beyond the declared count are ignored after validation; moisture content and liquid transport coefficient are finite and nonnegative without upper or cross-field bounds, and authored order is retained without sorting or correction
- deterministic compiler-ordered `MaterialProperty:HeatAndMoistureTransfer:Redistribution` definitions after Suction; each has a nonsemantic outer-key snapshot, a semantic inner material target, and a required exact-integer redistribution-point count from 1 through 25
- the fixed 25-pair redistribution tail: pair 1 is required, pairs 2 through 25 are optional, every supplied scalar is validated, missing optional active-prefix scalars become zero, and positions beyond the declared count are ignored after validation; moisture content and liquid transport coefficient are finite and nonnegative without upper or cross-field bounds, and authored order is retained without sorting or correction
- deterministic compiler-ordered `MaterialProperty:HeatAndMoistureTransfer:Diffusion` definitions after Redistribution; each has a nonsemantic outer-key snapshot, a semantic inner material target, and a required exact-integer data-pair count from 1 through 25
...
- a separate deterministic `MaterialHeatAndMoistureTransferSorptionIsotherm` attachment arena whose records own a typed ID, normalized outer-key snapshot, resolved `MaterialId`, Settings ID, declared count, count-selected zero-filled input points, source-effective points, and a moisture-adjustment flag; the outer key is not published as a name map
- source-effective Sorption points formed by appending (1.01, porosity*1000) and (0,0), the source nested-swap ascending-relative-humidity sort, and up to 100 left-to-right adjacent-moisture averaging passes; any correction records one warning state and any derived nonfinite point fails closed
- a separate deterministic `MaterialHeatAndMoistureTransferSuction` attachment arena whose records own a typed ID, normalized outer-key snapshot, resolved `MaterialId`, SorptionIsotherm ID, declared count, count-selected zero-filled input points, and source-effective points; the outer key is not published as a name map
- source-effective Suction points formed by preserving the active input prefix exactly, then appending one point whose moisture content is the indexed last source-effective Sorption moisture and whose liquid transport coefficient is the last active Suction coefficient; no Suction sort, correction, warning, monotonicity, uniqueness, or cross-column rule is invented
- a separate deterministic `MaterialHeatAndMoistureTransferRedistribution` attachment arena whose records own a typed ID, normalized outer-key snapshot, resolved `MaterialId`, SorptionIsotherm ID, declared count, count-selected zero-filled input points, and source-effective points; the outer key is not published as a name map
- source-effective Redistribution points formed by preserving the active input prefix exactly, then appending one point whose moisture content is the indexed last source-effective Sorption moisture and whose liquid transport coefficient is the last active Redistribution coefficient; no Redistribution-table sorting, correction, monotonicity or uniqueness check, clamp, cross-column rule, or table-validation warning is invented
- a separate deterministic `MaterialHeatAndMoistureTransferDiffusion` attachment arena whose records own a typed ID, normalized outer-key snapshot, resolved `MaterialId`, SorptionIsotherm ID, declared count, count-selected zero-filled input points, and source-effective points; the outer key is not published as a name map
...
| HAMT 2 | 29 | `MaterialProperty:HeatAndMoistureTransfer:SorptionIsotherm` | second HAMT pass; settings-dependent fixed-tail sorption dataset | complete bounded typed attachment for public Material targets with existing typed Settings; all definitions runtime-blocked |
| HAMT 3 | 30 | `MaterialProperty:HeatAndMoistureTransfer:Suction` | third HAMT pass; sorption-dependent fixed-tail suction dataset | complete bounded typed attachment for public Material targets with existing typed SorptionIsotherm; all definitions runtime-blocked |
...
the base payload unchanged while its runtime upgrade remains blocked. The
...
only as a nonsemantic diagnostic snapshot. The SorptionIsotherm attachment is a
second separate arena whose semantic inner target must resolve the same
`MaterialId` as an existing Settings attachment. It retains the declared active
input prefix and source-effective endpoint/sort/correction vector. The Suction
attachment is a third separate arena whose semantic inner target must resolve
the same `MaterialId` as an existing SorptionIsotherm attachment. It retains its
declared authored-order input prefix and appends the indexed last Sorption
moisture with the last active Suction coefficient. The Redistribution attachment
is a fourth separate arena with the same SorptionIsotherm dependency but no
Suction dependency. It retains its own authored-order prefix and appends the
indexed last Sorption moisture with the last active Redistribution coefficient.
The Diffusion attachment is a fifth separate arena with the same
SorptionIsotherm dependency but no Suction or Redistribution dependency. It
retains its authored-order relative-humidity/resistance-factor prefix and appends
the indexed last Sorption relative humidity with the last active Diffusion
factor. The ThermalConductivity attachment is a sixth separate arena with the
same SorptionIsotherm dependency but no Suction, Redistribution, or Diffusion
...
The payload uses the dedicated `MaterialFamily::SimpleGlazing` rather than
pretending that the whole-system input is ordinary detailed glass. The bounded
ordinary-`Construction` consumer accepts it only as the sole layer, retains the
same material ID as the outside layer and sole graph target, and classifies the
construction as fenestration. Multi-layer and shaded uses continue to fail
closed with `UnsupportedSimpleGlazingSystemConstruction`; this intentionally
avoids EnergyPlus 26.1's downstream validation holes. Arbitrary-run assessment
independently counts and blocks every typed definition, including consumed and
unused definitions, as
...
unsupported_state:
- multi-layer or shaded Construction packing and all construction-derived thermal state, specialized WindowMaterial:Glazing and WindowConstruction report state, angular or hemispherical optical tables, surface/window thermal state, daylighting, ratings, output variables, and runtime state; the separate GetConstructData mapping owns only sole-layer typed identity and graph linkage
...
placement, overlong, and mixed opaque/window stacks; it separately accepts a
sole whole-system SimpleGlazing layer while rejecting every multi-layer use. A
...
the unavailable thermochromic-child relationship, typed coverage, the
single-layer ordinary-`Construction` identity/kind/graph mapping, multi-layer
fail-closed boundary, and the consumed-plus-unused all-definition arbitrary-run
block.
...
`MaterialProperty:VariableThermalConductivity` model and compiler tests lock
both actual public `Group::Regular` target variants (`Material` and
`Material:NoMass`); absent/empty and zero, one, two, three, 100, 101, and 102
complete-pair arrays; finite negative temperatures and negative, zero, equal,
or decreasing conductivity; strict temperature-only ordering; explicit null,
malformed, incomplete, and nonfinite fail-closed rejection; exact wrong-family
targets including RoofVegetation; normalized target snapshots;
case-insensitive duplicate-target rejection; validation before ID or target
reservation; coexistence with VariableAbsorptance, PhaseChangeHysteresis, and
PhaseChange; typed coverage; and object-count inclusion. The support-boundary
test attaches VTC to used and unused Regular/NoMass materials and requires one
explicit all-definition `UnsupportedSurfaceBoundary` run block. No test claims
source lazy no-CondFD validation, source acceptance of nonfinite IDF scalars,
repeated-target overwrite, internal F/C-factor targets, `MaterialFD`, sentinel
...
`MaterialProperty:HeatAndMoistureTransfer:SorptionIsotherm` model and compiler
tests lock nonsemantic blank and case-colliding outer keys; semantic inner target
resolution through an existing typed Settings attachment; the exact-integer
count range 1 through 25; the fixed 25-pair tail with required pair 1, optional
active-prefix zero fill, and validation before declared-count truncation; finite
inclusive `[0,1]` relative humidity and finite nonnegative unbounded moisture;
unordered and duplicate RH acceptance; endpoint insertion; exact source-order
sorting and the 100-pass adjacent-moisture correction cap with warning state and
possible residual inversion; and deliberate derived-overflow fail-close. Oracle
evidence separately records schema-faithful
Rust rejection of IDF fractional-count rounding and source-retained nonfinite
states that can enter an active-HAMT severe/access-violation path. Tests also
lock the public Material-plus-Settings target boundary, case-insensitive
duplicate-target rejection, validation before ID reservation, coexistence with
all earlier typed material attachments, typed coverage, and object-count
inclusion. The support-boundary test attaches SorptionIsotherm records to used
and unused materials and requires one explicit all-definition
`UnsupportedSurfaceBoundary` run block. No test claims source lazy semantic
validation, repeated-target replacement, nonfinite or crash-path parity, the
typed Suction, Redistribution, Diffusion, and ThermalConductivity runtime
consumers, runtime interpolation, construction/cell state,
output variables,
EIO or SQLite reporting, runtime numerics, or conformance.

`MaterialProperty:HeatAndMoistureTransfer:Suction` model and compiler tests
lock nonsemantic blank and case-colliding outer keys; semantic inner target
resolution through an existing typed SorptionIsotherm attachment; the
exact-integer count range 1 through 25; the fixed 25-pair tail with required
pair 1, component-wise optional active-prefix zero fill, and validation before
declared-count truncation; and finite nonnegative unbounded moisture content and
liquid transport coefficient. They retain unordered, duplicate, and descending
authored points without a Suction sort, correction, warning, monotonicity,
uniqueness, or cross-column rule. The source-effective vector appends exactly
one point: the indexed last source-effective Sorption moisture and the last
active Suction coefficient. A residual-inversion Sorption fixture separately
locks that this dependency uses the last point rather than the maximum or a
fresh `porosity * 1000` calculation. Tests also lock count 1 and 25 boundaries,
the resulting count-plus-one vector, valid inactive-field ignore and invalid
inactive-field rejection, public Material-plus-Sorption dependency, deliberate
missing-Sorption fail-close, case-insensitive duplicate-target rejection,
validation before ID reservation, coexistence with all earlier typed material
attachments, typed coverage, and object-count inclusion. The support-boundary
test attaches Suction records to used and unused materials and requires one
explicit all-definition `UnsupportedSurfaceBoundary` run block. No test claims
source lazy semantic validation, repeated-target replacement, fractional-count
rounding, exact source nonfinite or missing-Sorption failure behavior, the typed
Redistribution, Diffusion, and ThermalConductivity siblings' runtime consumers,
runtime interpolation, construction/cell state,
output variables, EIO or SQLite reporting, runtime numerics, or conformance.

`MaterialProperty:HeatAndMoistureTransfer:Redistribution` model and compiler
tests lock nonsemantic blank and case-colliding outer keys; semantic inner target
resolution through an existing typed SorptionIsotherm attachment; explicit
success without any Suction attachment; the exact-integer count range 1 through
25; the fixed 25-pair tail with required pair 1, component-wise optional
active-prefix zero fill, and validation before declared-count truncation; and
finite nonnegative unbounded moisture content and liquid transport coefficient.
They retain unordered, duplicate, and descending authored points without
Redistribution-table sorting, correction, monotonicity or uniqueness checks,
clamping, a cross-column rule, or a table-validation warning. The
source-effective vector appends exactly one point: the
indexed last source-effective Sorption moisture and the last active
Redistribution coefficient. A residual-inversion Sorption fixture separately
locks that this dependency uses the last point rather than the maximum or a
fresh `porosity * 1000` calculation. Tests also lock count 1 and 25 boundaries,
the resulting count-plus-one vector, valid inactive-field ignore and invalid
inactive-field rejection, public Material-plus-Sorption dependency, deliberate
...
validation before ID reservation, coexistence with all earlier typed material
attachments, typed coverage, and object-count inclusion. The support-boundary
test attaches Redistribution records to used and unused materials and requires
one explicit all-definition `UnsupportedSurfaceBoundary` run block. No test
claims source lazy semantic validation, repeated-target replacement,
fractional-count rounding, exact source nonfinite or missing-Sorption failure
behavior, rain-selected Suction or ordinary Redistribution interpolation, the
typed Diffusion and ThermalConductivity runtime consumers,
construction/cell state, output variables, EIO or SQLite reporting, runtime
numerics, or conformance.

`MaterialProperty:HeatAndMoistureTransfer:Diffusion` model and compiler tests
lock nonsemantic blank and case-colliding outer keys; semantic inner target
resolution through an existing typed SorptionIsotherm attachment; explicit
success without either Suction or Redistribution; the exact-integer count range
1 through 25; the fixed 25-pair tail with required pair 1, component-wise
optional active-prefix zero fill, and validation before declared-count
truncation; and finite inclusive `[0,1]` relative humidity plus finite
nonnegative unbounded water-vapor diffusion resistance factor. They retain
unordered, duplicate, and descending authored relative humidity and arbitrary
resistance-factor order without diffusion-table sorting, correction,
monotonicity or uniqueness checks, clamping, a cross-field rule, or a
table-validation warning. The source-effective vector appends exactly one
point: the indexed last source-effective Sorption relative humidity and the last
active Diffusion factor. Tests separately lock that the endpoint copies the
Sorption relative-humidity column rather than moisture. They also lock count 1
and 25 boundaries, the resulting count-plus-one vector, valid inactive-field
ignore and invalid inactive-field rejection, public Material-plus-Sorption
dependency, deliberate missing-Sorption fail-close, case-insensitive
duplicate-target rejection, validation before ID reservation, coexistence with
all earlier typed material attachments, typed coverage, and object-count
inclusion. The support-boundary test attaches Diffusion records to used and
unused materials and requires one explicit all-definition
`UnsupportedSurfaceBoundary` run block. No test claims source lazy semantic
validation, repeated-target replacement, fractional-count rounding, exact
source nonfinite or missing-Sorption failure behavior, Diffusion and
ThermalConductivity interpolation, construction/cell state, output
variables, EIO or SQLite reporting, runtime numerics, or conformance.

`MaterialProperty:HeatAndMoistureTransfer:ThermalConductivity` model and compiler
tests lock nonsemantic blank and case-colliding outer keys; semantic inner target
resolution through an existing typed SorptionIsotherm attachment; explicit
success without Suction, Redistribution, or Diffusion; the exact-integer count
range 1 through 25; the fixed 25-pair tail with required pair 1, component-wise
optional active-prefix zero fill, and validation before declared-count
truncation; finite nonnegative unbounded supplied moisture content; and finite
strictly-positive unbounded supplied thermal conductivity. Missing optional
active conductivity retains source numeric-blank zero even though supplied zero
is rejected. Tests retain unordered, duplicate, and descending authored
moisture with arbitrary conductivity order without table sorting, correction,
monotonicity or uniqueness checks, clamping, a cross-field rule, or a warning.
The source-effective vector appends the indexed last source-effective Sorption
moisture and the last active conductivity. A residual-inversion Sorption fixture
locks the indexed last point rather than the maximum or a fresh
`porosity * 1000` calculation. Tests also lock count 1 and 25 boundaries, the
resulting count-plus-one vector, valid inactive-field ignore and invalid
inactive-field rejection, public Material-plus-Sorption dependency, deliberate
missing-Sorption fail-close, case-insensitive duplicate-target rejection,
validation before ID reservation, coexistence with all earlier typed material
attachments, typed coverage, and object-count inclusion. The support-boundary
test attaches ThermalConductivity records to used and unused materials and
requires one explicit all-definition `UnsupportedSurfaceBoundary` run block. No
test claims source lazy semantic validation, repeated-target replacement,
fractional-count rounding, exact source nonfinite or missing-Sorption failure
behavior, construction-local absent-table fallback and warnings,
