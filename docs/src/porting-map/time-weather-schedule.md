...
`calendar_dst_fixed_date_hourly_exact_001` is the fixed-date external DST
claim. Its EPW declares a fixed February 29 through March 1 period,
...
forms, RunPeriod disabling, malformed headers, and policy precedence remain
...
All three DST checkpoints exclude the IDF daylight-saving object,
...
1. **Remaining DST gate.** The fixed-date EPW period, one literal EPW
   fourth-Monday-through-last-Wednesday boundary pair, and the exact
   last-Sunday-October-through-last-Sunday-March end-side window now have
   external exact gates. Promote other date-rule pairs, the October start-side
   boundary, generic southern/wrap behavior, and actual cross-year execution
   beyond unit evidence; add RunPeriod disabling and the IDF DST object; and
...
by 48 active. The weekday-rule fixture adds only the literal EPW start/end pair
...
The IDF DST object,
RunPeriod disabling, policy precedence, schedule
DST shift and hour-24 rollover, special days, Rust raw ESO serialization,
...
boundary, the fixed-date DST case keeps only its 72-sample boundary, the literal
...
hours, and the fixed IDF special-day case
...
behavior beyond the three exact EPW DST cases, special-day behavior beyond the fixed IDF,
