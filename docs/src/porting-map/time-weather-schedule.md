...
years, months, date rules, disabling and precedence, DST clock effects,
...
1. **Remaining DST gate.** The fixed-date EPW period, one literal EPW
   ...
   execution beyond unit evidence; add RunPeriod disabling, precedence, and the
   IDF DST object; and
...
end-side case. Broader years/months/rules, disabling, precedence, DST clock
effects, actual weather, and broad `WeatherManager` remain unclaimed. The IDF DST object,
...
behavior beyond the four exact EPW DST cases, special-day behavior beyond the fixed IDF,
