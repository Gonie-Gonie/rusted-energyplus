...
| compact schedule input/default ownership | `Sched::ProcessScheduleInput` | EnergyPlus 26.1 lines 1248-1477 create the internal compact day/week schedules and dispatch their intervals. Lines 1253 and 1404-1418 make the `Interpolate` directive optional and retain the day schedule's default `No` state when the next field is `Until`. CP43 exercises only that omitted-directive default; accepting any explicit `Interpolate` directive remains unclaimed. |
| compact interval minute expansion | `Sched::ProcessIntervalFields` | EnergyPlus 26.1 lines 2733-2961 decode each `Until` endpoint, fill the 1,440-minute day, and choose flat versus Linear minute values. The CP43 fixture exercises only aligned endpoints and the flat-fill path under default Interpolate:No; Average, Linear, non-aligned warning and resulting sampling behavior, incomplete/overlap errors, and other interval shapes remain unclaimed. |
| day-schedule timestep population | `Sched::DaySchedule::populateFromMinuteVals` | EnergyPlus 26.1 lines 211-239 either averages each zone-timestep window or selects the minute at each timestep end. The CP43 default-No case locks only default-No end-minute sampling at 15, 30, 45, and 60 minutes for `Timestep,4`; Average and Linear remain unclaimed. |
...
CP43 aligned default-No case, other schedule families,
...
separate exact CP43 aligned default-No case, other schedule families,
