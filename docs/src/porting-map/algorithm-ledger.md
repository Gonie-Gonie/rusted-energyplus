...
| Thermostat and IdealLoads no-OA sensible branch | `PurchasedAirManager.cc`, `ZoneEquipmentManager.cc`, `ZoneTempPredictorCorrector.cc`, and node/source maps | `ep_runtime::ideal_loads` helper, zone equipment demand state, and supply-node update | `ideal_loads_no_oa_sensible_conformance_001`, `ideal_loads_capacity_limit_conformance_001`, and `ideal_loads_flow_limit_conformance_001` promoted conformance gates | declared no-OA/no-limit, numeric capacity-limit, and numeric flow-limit thermostat, IdealLoads rate, supply-node temperature, and supply-node flow variables only |
