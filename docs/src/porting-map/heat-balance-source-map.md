...
39. CP168 next, after the already mapped CP125 `UpdateFinalSurfaceHeatBalance` and CP126 `UpdateThermalHistories` definitions, maps `CalculateZoneMRT`, declared at `HeatBalanceSurfaceManager.hh` lines 140-141 and implemented at `HeatBalanceSurfaceManager.cc` lines 5583-5699
...
ledger. Its required
project-contract entry is after `calc_heat_balance_inside_surf_2_ctf_only` and
before `manage_air_heat_balance`, preserving the inside-balance
parent/optimized-child/tail grouping. It adds no EnergyPlus source inventory,
...
CP168 next, after the already mapped CP125 `UpdateFinalSurfaceHeatBalance` and
CP126 `UpdateThermalHistories` definitions, maps `CalculateZoneMRT`, declared
at `HeatBalanceSurfaceManager.hh` lines 140-141 and implemented at
`HeatBalanceSurfaceManager.cc` lines 5583-5699.
