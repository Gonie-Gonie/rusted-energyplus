...
44. CP173 next maps required `sumSurfQdotRadHVAC`, declared at `HeatBalanceSurfaceManager.hh` line 171, implemented at `HeatBalanceSurfaceManager.cc` lines 9277-9285, and called in production only by `CalcHeatBalanceInsideSurf` line 7788
...
aggregation are global. The CP172 general child samples
...
CP172 now separately adds the required general-child row without promoting
...
`routine.calc_heat_balance_inside_surf_2` immediately after
`calc_heat_balance_inside_surf` and before
`calc_heat_balance_inside_surf_2_ctf_only` in both the Surface-manager routine
ledger and heat-balance project-contract list. It adds no EnergyPlus source
...
CP173 next maps required `sumSurfQdotRadHVAC`, declared at
`HeatBalanceSurfaceManager.hh` line 171, implemented at
`HeatBalanceSurfaceManager.cc` lines 9277-9285, and called in production only
by `CalcHeatBalanceInsideSurf` line 7788.
