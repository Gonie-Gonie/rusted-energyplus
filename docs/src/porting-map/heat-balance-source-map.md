...
42. CP171 next maps required `GetQdotConvOutPerArea`, declared at `HeatBalanceSurfaceManager.hh` line 173 and implemented at `HeatBalanceSurfaceManager.cc` lines 7723-7736
...
CP171 next maps required `GetQdotConvOutPerArea`, declared at
`HeatBalanceSurfaceManager.hh` line 173, implemented at
`HeatBalanceSurfaceManager.cc` lines 7723-7736, and called in production only
by `CalcHeatBalanceOutsideSurf` line 7717. CP120 previously retained that helper
only as source dependency context.
