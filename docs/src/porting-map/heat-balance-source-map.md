...
38. CP167 next in source-definition order maps `UpdateNonRepresentativeSurfaceResults`, declared at `HeatBalanceSurfaceManager.hh` line 134 and implemented at `HeatBalanceSurfaceManager.cc` lines 5022-5174; its sole call is CP166 line 4985 under `UseRepresentativeSurfaceCalculations`, with no direct unit-test call
...
Representative mode can still call CP167, whose absent-argument Zone loops are
also empty, and CP166 still reaches its final global Kiva loop.
...
`calc_heat_balance_outside_surf` in the mapped routine ledger. Its required
...
CP167 next maps `UpdateNonRepresentativeSurfaceResults`, declared at
