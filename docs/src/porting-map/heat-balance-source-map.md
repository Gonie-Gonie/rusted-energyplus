...
34. CP163 next in source-definition order maps `ComputeDifSolExcZonesWIZWindows`, declared at `HeatBalanceSurfaceManager.hh` line 117, implemented at `HeatBalanceSurfaceManager.cc` lines 4473-4644, and called under `InterZoneWindow` by `InitSurfaceHeatBalance` lines 435-439
...
`state_mapped` and 113 `source_mapped`, with 64 required. CP163 next maps
...
direct unit-test calls at lines 3173, 3186, and 3193 remain for CP163.
