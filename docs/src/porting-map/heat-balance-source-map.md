...
shared child-call reachability boundary, and CP172 carries it into the
separate general-child map without turning unreachable thresholds into
claimed runtime parity.
...
chain to couple Dome/diffuser outside fields. CP172 invokes the shared
surface-temperature checker on ordinary entries outside the normal limits,
then `IZSurfs` copies each raw paired inside-history value into both current
...
CP174 next maps `TestSurfTempCalcHeatBalanceInsideSurf`, declared at
`HeatBalanceSurfaceManager.hh` lines 192-193 and defined immediately after
CP173 at `HeatBalanceSurfaceManager.cc` line 9287.
