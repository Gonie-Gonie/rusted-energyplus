...
41. CP170 next maps `ReportIntMovInsInsideSurfTemp`, declared at `HeatBalanceSurfaceManager.hh` line 153 and implemented at `HeatBalanceSurfaceManager.cc` lines 6932-6940
...
rows. CP169 now separately adds the representative-report child row; the
movable-insulation and shading-report children still add no rows, and
`SolarShading.cc` or its header remains absent from the Surface algorithm's
...
CP170 next maps `ReportIntMovInsInsideSurfTemp`, declared at
