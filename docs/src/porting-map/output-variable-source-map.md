...
| `Zone Ideal Loads Zone Latent Heating Rate` | detailed | `PurchasedAirManager.cc::CalcPurchAirLoads` and `ReportPurchasedAir` | no-OA `ConstantSensibleHeatRatio` latent split in the diagnostic compare lane | diagnostic-only for `ideal_loads_constant_shr_diagnostic_001`; no broad humidity-control conformance |
| `Zone Ideal Loads Zone Latent Cooling Rate` | detailed | `PurchasedAirManager.cc::CalcPurchAirLoads`, `ReportPurchasedAir`, and EnergyPlus psychrometric helpers | no-OA `ConstantSensibleHeatRatio` cooling latent split with EPW barometric-pressure saturation clamp | diagnostic-only for `ideal_loads_constant_shr_diagnostic_001`; no broad humidity-control conformance |
| `Zone Ideal Loads Supply Air Sensible Heating Rate` | detailed | `PurchasedAirManager.cc::ReportPurchasedAir` | no-OA `ConstantSensibleHeatRatio` supply-air sensible heating report row in the diagnostic compare lane | diagnostic-only for `ideal_loads_constant_shr_diagnostic_001`; no broad humidity-control conformance |
| `Zone Ideal Loads Supply Air Sensible Cooling Rate` | detailed | `PurchasedAirManager.cc::ReportPurchasedAir` | no-OA `ConstantSensibleHeatRatio` supply-air sensible cooling report row in the diagnostic compare lane | diagnostic-only for `ideal_loads_constant_shr_diagnostic_001`; no broad humidity-control conformance |
| `Zone Ideal Loads Supply Air Latent Heating Rate` | detailed | `PurchasedAirManager.cc::ReportPurchasedAir` | no-OA `ConstantSensibleHeatRatio` supply-air latent heating report row in the diagnostic compare lane | diagnostic-only for `ideal_loads_constant_shr_diagnostic_001`; no broad humidity-control conformance |
| `Zone Ideal Loads Supply Air Latent Cooling Rate` | detailed | `PurchasedAirManager.cc::ReportPurchasedAir` and EnergyPlus psychrometric helpers | no-OA `ConstantSensibleHeatRatio` supply-air latent cooling report row with EnergyPlus enthalpy constants | diagnostic-only for `ideal_loads_constant_shr_diagnostic_001`; no broad humidity-control conformance |
...
| `System Node Humidity Ratio` | detailed | `node-state-source-map.md`; `NodeInputManager.cc`; `ZoneTempPredictorCorrector.cc`; `PurchasedAirManager.cc`; `ZoneEquipmentManager.cc` | `supply_node_update_from_result` in the IdealLoads compare lane; finite-limit and Constant SHR return-node proof rows from oracle ESO | diagnostic proof rows for IdealLoads no-OA, finite-limit, and Constant SHR cases; not humidity conformance |
