...
| `Surface Outside Face Convection Heat Transfer Coefficient` | hourly | `src/EnergyPlus/HeatBalanceSurfaceManager.cc`; `src/EnergyPlus/ConvectionCoefficients.cc` | diagnostic exterior coefficient helper; explicit `SurfaceConvectionAlgorithm:Outside,DOE-2` now selects the DOE-2 coefficient path | diagnostic-only for official dynamic heat-balance case |
