...
| `Zone Ideal Loads Heat Recovery Total Heating Rate` | detailed | `PurchasedAirManager.cc::CalcPurchAirMixedAir` and `ReportPurchasedAir` | inactive heat recovery reports zero total heating | diagnostic-only for `ideal_loads_outdoor_air_design_flow_diagnostic_001`, `ideal_loads_outdoor_air_flow_area_diagnostic_001`, and `ideal_loads_outdoor_air_air_changes_diagnostic_001` |
...
`ideal_loads_outdoor_air_air_changes_diagnostic_001` reports are
diagnostic-only Flow/Zone, Flow/Area, and AirChanges/Hour outdoor-air
design-flow results for IdealLoads
