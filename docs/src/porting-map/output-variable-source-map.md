...
| `Zone Thermostat Heating Setpoint Temperature` | detailed | thermostat setup/output registration in EnergyPlus HVAC/zone predictor code | thermostat schedule values in the IdealLoads diagnostic compare lane | diagnostic zero-failure evidence for `ideal_loads_no_oa_sensible_conformance_001`; not conformance |
| `Zone Thermostat Cooling Setpoint Temperature` | detailed | thermostat setup/output registration in EnergyPlus HVAC/zone predictor code | thermostat schedule values in the IdealLoads diagnostic compare lane | diagnostic zero-failure evidence for `ideal_loads_no_oa_sensible_conformance_001`; not conformance |
| `Zone Ideal Loads Zone Total Heating Rate` | detailed | IdealLoads HVAC component implementation and zone equipment managers | `calc_no_oa_no_limit_sensible_compat` sampled by the IdealLoads diagnostic compare lane | diagnostic zero-failure evidence for `ideal_loads_no_oa_sensible_conformance_001`; not conformance |
| `Zone Ideal Loads Zone Total Cooling Rate` | detailed | IdealLoads HVAC component implementation and zone equipment managers | `calc_no_oa_no_limit_sensible_compat` sampled by the IdealLoads diagnostic compare lane | diagnostic zero-failure evidence for `ideal_loads_no_oa_sensible_conformance_001`; not conformance |
...
| `System Node Mass Flow Rate` | detailed | `node-state-source-map.md`; `DataZoneEquipment.cc`; `PurchasedAirManager.cc` | `supply_node_update_from_result` using source-order pre-update zone state in the IdealLoads diagnostic compare lane | diagnostic zero-failure evidence for `ideal_loads_no_oa_sensible_conformance_001`; not conformance |
