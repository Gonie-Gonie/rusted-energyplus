...
| `System Node Temperature` | detailed | `node-state-source-map.md`; `NodeInputManager.cc`; `ZoneTempPredictorCorrector.cc`; `PurchasedAirManager.cc`; `ZoneEquipmentManager.cc` | `supply_node_update_from_result` in the IdealLoads compare lane; finite-limit return-node proof rows from oracle ESO | conformance for the IdealLoads supply node in `ideal_loads_no_oa_sensible_conformance_001` only; finite-limit rows diagnostic-only |
| `System Node Humidity Ratio` | detailed | `node-state-source-map.md`; `NodeInputManager.cc`; `ZoneTempPredictorCorrector.cc`; `PurchasedAirManager.cc`; `ZoneEquipmentManager.cc` | `supply_node_update_from_result` in the IdealLoads compare lane; finite-limit return-node proof rows from oracle ESO | diagnostic proof rows for IdealLoads no-OA and finite-limit cases; not humidity conformance |
| `System Node Mass Flow Rate` | detailed | `node-state-source-map.md`; `DataZoneEquipment.cc`; `PurchasedAirManager.cc` | `supply_node_update_from_result` using source-order pre-update zone state in the IdealLoads compare lane | conformance for the IdealLoads supply node in `ideal_loads_no_oa_sensible_conformance_001` only; finite-limit rows diagnostic-only |
