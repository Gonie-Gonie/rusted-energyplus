...
| `Zone Ideal Loads Zone Latent Heating Rate` | detailed | `PurchasedAirManager.cc::CalcPurchAirLoads` and `ReportPurchasedAir` | no-OA `ConstantSensibleHeatRatio` and `ConstantSupplyHumidityRatio` latent split in the diagnostic compare lane | diagnostic-only for `ideal_loads_constant_shr_diagnostic_001` and `ideal_loads_constant_supply_humidity_diagnostic_001`; no broad humidity-control conformance |
...
| `Zone Ideal Loads Supply Air Latent Heating Rate` | detailed | `PurchasedAirManager.cc::ReportPurchasedAir` | no-OA `ConstantSensibleHeatRatio` and `ConstantSupplyHumidityRatio` supply-air latent heating report row in the diagnostic compare lane | diagnostic-only for `ideal_loads_constant_shr_diagnostic_001` and `ideal_loads_constant_supply_humidity_diagnostic_001`; no broad humidity-control conformance |
...
| `Zone System Predicted Moisture Load to Humidifying Setpoint Moisture Transfer Rate` | detailed | `DataZoneEnergyDemands.cc`; `ZoneTempPredictorCorrector.cc` | oracle proof input copied into `ZoneSysEnergyDemand::remaining_output_req_to_humid_sp_kg_per_s` for the Humidistat diagnostic lane | diagnostic-only for `ideal_loads_humidistat_dehumidification_diagnostic_001`; not user-facing conformance |
