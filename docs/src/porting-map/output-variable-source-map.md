...
| `Zone Ideal Loads Supply Air Total Heating Fuel Energy Rate` | detailed | `PurchasedAirManager.cc::ReportPurchasedAir` fuel efficiency branch | no-OA supply-air total heating rate divided by blank value 1.0 or a constant fuel-efficiency schedule value | diagnostic-only for `ideal_loads_no_oa_sensible_conformance_001` and `ideal_loads_fuel_efficiency_diagnostic_001`; facility meters are oracle-MTR tracked separately; no Rust meter time-series conformance |
| `Zone Ideal Loads Supply Air Total Cooling Fuel Energy Rate` | detailed | `PurchasedAirManager.cc::ReportPurchasedAir` fuel efficiency branch | no-OA supply-air total cooling rate divided by blank value 1.0 or a constant fuel-efficiency schedule value | diagnostic-only for `ideal_loads_no_oa_sensible_conformance_001` and `ideal_loads_fuel_efficiency_diagnostic_001`; facility meters are oracle-MTR tracked separately; no Rust meter time-series conformance |
...
| `Zone Ideal Loads Zone Cooling Fuel Energy Rate` | detailed | `PurchasedAirManager.cc::ReportPurchasedAir` fuel efficiency branch | no-OA zone total cooling rate divided by blank value 1.0 or a constant fuel-efficiency schedule value | diagnostic-only for `ideal_loads_no_oa_sensible_conformance_001` and `ideal_loads_fuel_efficiency_diagnostic_001`; facility meters are oracle-MTR tracked separately; no Rust meter time-series conformance |
...
| `System Node Mass Flow Rate` | detailed | `node-state-source-map.md`; `DataZoneEquipment.cc`; `PurchasedAirManager.cc` | `supply_node_update_from_result` using source-order pre-update zone state in the IdealLoads compare lane | conformance for the IdealLoads supply node in `ideal_loads_no_oa_sensible_conformance_001` only |
...
sizing, Rust meter time-series comparison, non-constant efficiency schedules,
or broad HVAC compatibility. The energy and blank/constant Schedule:Constant
...
`DistrictCooling:Facility` meters are tracked only as oracle-MTR diagnostic
requests.
