...
- `official_1zone_uncontrolled_dynamic_conformance_candidate_001` declared
  ...
  conduction, zone opaque aggregate conduction, floor storage, and exact-zero
  named wall/roof sky/ground diffuse plus named wall/roof incident total/beam
  solar rows, named wall/roof absorbed solar heat gain rate rows, and the roof
  absorbed solar heat gain per-area row only
...
| Heat balance | no-mass zone MAT, no-mass surface inside/outside temperature, no-mass adiabatic conduction series, selected official `1ZoneUncontrolled` dynamic weather/wetbulb/rain-status/zone-air/surface-temperature/surface-conduction, conduction-per-area, named wall/roof incident total/beam solar per-area rows, exact-zero named wall/roof sky/ground diffuse incident solar per-area rows, named wall/roof absorbed solar heat gain rate rows, the roof absorbed solar heat gain per-area row, zone opaque aggregate conduction variables, and the declared floor `Surface Heat Storage Rate` / `Surface Heat Storage Rate per Area` rows under dedicated aggregate/flux/storage/solar-rate/solar-flux tolerances | official `1ZoneUncontrolled` broad diagnostic decomposition, broad weather processor, absorbed/non-incident solar outside the declared absorbed rows, radiation/convection diagnostics, and non-promoted probe lanes | broad CTF storage parity, EnergyPlus warmup convergence parity outside the official candidate, broad weather processor compatibility, absorbed/non-incident solar outside the declared rows, radiation exchange, fenestration, infiltration, zone air predictor/corrector parity, or general heat-balance compatibility |
...
- official dynamic heat-balance conformance reports that run a Rust warmup
  ...
  zone opaque aggregate conduction, and exact-zero named wall/roof sky/ground
  diffuse incident solar rows plus named wall/roof incident total/beam solar
  rows, named wall/roof absorbed solar heat gain rate rows, and the roof
  absorbed solar heat gain per-area row in the compatibility-candidate lane
  while keeping broad storage/radiation/absorbed-solar outside those declared
  rows/convection decomposition evidence diagnostic-only
...
- official dynamic heat-balance diagnostic reports that retain the broader
  surface/radiation/absorbed-solar outside the declared absorbed rows/convection
  decomposition and probe lanes without promoting those diagnostic variables
