...
- `official_1zone_uncontrolled_dynamic_conformance_candidate_001` declared
  weather drybulb/wetbulb/sky/horizontal-infrared/rain-status, roof
  surface-local outdoor-air drybulb/wetbulb/wind speed/wind direction,
  zone-air, surface temperature, surface conduction, inside/outside
  convection coefficients, inside convection/radiation source rows, declared
  wall/roof exterior convection/net-radiation source rows, roof exterior
  convection/net-radiation per-area rows, zone opaque aggregate conduction
  including outside
  gain/loss aggregates, floor storage, exact-zero named wall/roof sky/ground
  diffuse plus named wall/roof incident total/beam solar rows, named wall/roof
  absorbed solar heat gain rate rows, and the roof absorbed solar heat gain
  per-area row only
...
| Heat balance | no-mass zone MAT, no-mass surface inside/outside temperature, no-mass adiabatic conduction series, selected official `1ZoneUncontrolled` dynamic weather drybulb/wetbulb/sky/horizontal-infrared/rain-status, roof surface-local outdoor-air drybulb/wetbulb/wind, zone-air, surface-temperature/surface-conduction, conduction-per-area, hourly surface inside-face heat-balance iteration-count row, inside convection/radiation source rows, inside/outside convection coefficient rows, declared wall/roof exterior convection/net-radiation source rows, roof exterior convection/net-radiation per-area rows, named wall/roof incident total/beam solar per-area rows, exact-zero named wall/roof sky/ground diffuse incident solar per-area rows, named wall/roof absorbed solar heat gain rate rows, the roof absorbed solar heat gain per-area row, zone opaque aggregate conduction variables including outside-face gain/loss aggregates, and the declared floor `Surface Heat Storage Rate` / `Surface Heat Storage Rate per Area` rows under dedicated aggregate/flux/storage/solar-rate/solar-flux/coefficient tolerances | official `1ZoneUncontrolled` broad diagnostic decomposition, broad weather processor, absorbed/non-incident solar outside the declared absorbed rows, exterior source diagnostics outside the declared wall/roof rate and roof per-area rows, and non-promoted probe lanes | broad CTF storage parity, EnergyPlus warmup convergence parity outside the official candidate, broad weather processor compatibility, absorbed/non-incident solar outside the declared rows, exterior source rows outside the declared wall/roof rate and roof per-area rows, fenestration, infiltration, zone air predictor/corrector parity, or general heat-balance compatibility |
...
- official dynamic heat-balance conformance reports that run a Rust warmup
  ...
  ESO values to run-period samples, gate declared weather drybulb/wetbulb/sky/horizontal-infrared/rain-status, roof surface-local outdoor-air drybulb/wetbulb/wind, zone-air,
  roof/wall/floor face-temperature, conduction, conduction per-area, and floor
  Surface Heat Storage Rate plus Surface Heat Storage Rate per Area series,
  inside convection/radiation source rows, inside/outside convection
  coefficient rows, zone opaque aggregate conduction including outside gain/loss
  aggregates, and exact-zero named wall/roof sky/ground diffuse incident solar
  rows plus named wall/roof incident total/beam solar rows, named wall/roof
  absorbed solar heat gain rate rows, and the roof absorbed solar heat gain
  per-area row in the compatibility-candidate lane while keeping broad
  storage/radiation/absorbed-solar outside those declared rows/exterior
  convection decomposition evidence diagnostic-only
...
- official dynamic heat-balance diagnostic reports that retain the broader
  surface/radiation/absorbed-solar outside the declared absorbed rows/exterior
  convection heat-gain decomposition and probe lanes without promoting those
