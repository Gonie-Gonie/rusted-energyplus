...
| HVAC, node, plant | no-OA/no-limit IdealLoads sensible conformance for declared thermostat, IdealLoads rate, and supply-node temperature/flow variables only | node proof rows, IdealLoads diagnostic proof rows, IdealLoads Flow/Zone outdoor-air mass/standard-density volume-flow, no-humidity report-rate, and mixed-air-state diagnostic evidence, and plant-loop baseline/diagnostic reports | broad HVAC, broad node, full IdealLoads, supply-air-state/economizer/heat-recovery/DCV/humidity-control, meter, and plant numerical conformance |
...
- diagnostic-only IdealLoads Flow/Zone outdoor-air design-flow evidence for
  ...
  sensible/latent/total report rates, and mixed-air state, with supply-air
  state, DCV, economizer, heat recovery, active humidity controls, meters, and
