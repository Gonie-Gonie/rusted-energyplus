...
| HVAC, node, plant | no-OA/no-limit IdealLoads sensible conformance for declared thermostat, IdealLoads rate, and supply-node temperature/flow variables only | node proof rows, blank-efficiency IdealLoads fuel energy-rate proof rows, IdealLoads Flow/Zone outdoor-air mass/standard-density volume-flow, no-humidity report-rate, supply-air-state, mixed-air-state, and inactive economizer/heat-recovery diagnostic evidence, and plant-loop baseline/diagnostic reports | broad HVAC, broad node, full IdealLoads, active economizer/heat-recovery/DCV/humidity-control/saturation-limit branches, fuel meters, non-unity efficiency schedules, and plant numerical conformance |
...
- limited IdealLoads no-OA/no-limit sensible conformance for declared
  ...
  supply-node temperature/mass-flow Detailed series, with blank-efficiency fuel
  energy-rate rows kept diagnostic-only and humidity, predictor/corrector
  proof rows, outdoor-air, sizing, fuel meters, non-unity efficiency
  schedules, and broad HVAC compatibility kept outside the claim
