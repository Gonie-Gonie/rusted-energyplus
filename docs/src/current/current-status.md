...
- limited IdealLoads no-OA ReportPurchasedAir energy conformance for declared
  ...
  blank fuel-efficiency rows promoted only in the separate
  `ideal_loads_blank_fuel_efficiency_conformance_candidate_001` case; raw
  rate, thermostat, demand, humidity, node, and facility meter rows are kept
...
- limited IdealLoads no-OA blank fuel-efficiency conformance for declared
  ...
  only; constant and non-constant efficiency schedules remain outside this
  claim
- limited IdealLoads no-OA constant Schedule:Constant fuel-efficiency
  ...
  kept diagnostic proof only; blank and non-constant efficiency schedules
