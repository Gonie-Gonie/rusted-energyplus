...
- limited IdealLoads outdoor-air `DifferentialEnthalpy` economizer conformance
  ...
  minimum
- diagnostic-only IdealLoads remaining Sensible heat-recovery
  and Enthalpy heat-recovery outdoor-air evidence for outdoor-air mass flow,
  standard-density volume flow, no-humidity sensible/latent/total report
  rates, supply-air state, mixed-air state, inactive heat-recovery reports,
  and heat-recovery active-time/rate rows, with active
  DCV, active humidity controls, heat-recovery saturation-limit branches,
  meters, and broad OA compatibility outside the claim
