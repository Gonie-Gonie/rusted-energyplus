...
- `ideal_loads_constant_supply_humidity_cooling_conformance_candidate_001`
  ...
  supply-node variables, ReportPurchasedAir energy/fuel rows, and hourly
...
- `ideal_loads_humidistat_dehumidification_conformance_candidate_001`
  ...
  variables, ReportPurchasedAir energy/fuel rows, and hourly
  `DistrictHeatingWater:Facility`/`DistrictCooling:Facility` meters only
- `ideal_loads_humidistat_humidification_conformance_candidate_001` declared
  ...
  ReportPurchasedAir energy/fuel rows, and hourly
  `DistrictHeatingWater:Facility`/`DistrictCooling:Facility` meters only
...
- limited IdealLoads no-OA `ConstantSupplyHumidityRatio` cooling/heating
  ...
  rows, supply-air report rows, ReportPurchasedAir energy/fuel rows,
  supply-node temperature/mass-flow/humidity Detailed series, and hourly
  `DistrictHeatingWater:Facility`/`DistrictCooling:Facility` meters, with
  opposite-side rate rows, return-node/zone-air humidity rows, and broader
  meter frequencies kept diagnostic-only
- limited IdealLoads no-OA Humidistat dehumidification/humidification
  ...
  supply-node temperature/mass-flow/humidity Detailed series, plus hourly
  `DistrictHeatingWater:Facility`/`DistrictCooling:Facility` meters, with
  EnergyPlus moisture-demand rows, opposite-side rows, return-node/zone-air
  humidity rows, and broader meter frequencies kept diagnostic-only
