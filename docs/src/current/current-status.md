...
| Heat balance | no-mass zone MAT, no-mass surface inside/outside temperature, no-mass adiabatic conduction series, selected official `1ZoneUncontrolled` dynamic weather/wetbulb/rain-status/zone-air/surface-temperature/surface-conduction, conduction-per-area, named wall/roof incident total/beam solar per-area rows, exact-zero named wall/roof sky/ground diffuse incident solar per-area rows, zone opaque aggregate conduction variables, and the declared floor `Surface Heat Storage Rate` / `Surface Heat Storage Rate per Area` rows under dedicated aggregate/flux/storage tolerances | official `1ZoneUncontrolled` broad diagnostic decomposition, broad weather processor, absorbed/non-incident solar/radiation/convection diagnostics, and non-promoted probe lanes | broad CTF storage parity, EnergyPlus warmup convergence parity outside the official candidate, broad weather processor compatibility, absorbed/non-incident solar, radiation exchange, fenestration, infiltration, zone air predictor/corrector parity, or general heat-balance compatibility |
...
- official dynamic heat-balance conformance reports that run a Rust warmup
  ...
  rows in the compatibility-candidate lane while keeping broad
  storage/radiation/absorbed-solar/convection decomposition evidence
  diagnostic-only
...
- official dynamic heat-balance diagnostic reports that retain the broader
  surface/radiation/absorbed-solar/convection decomposition and probe lanes
  without promoting those diagnostic variables
