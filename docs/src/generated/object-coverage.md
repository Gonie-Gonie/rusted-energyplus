...
| ZoneHVAC:IdealLoadsAirSystem | hvac | typed | ideal_loads_no_oa_sensible_conformance_001 | Typed input support plus limited no-OA/no-limit IdealLoads sensible conformance evidence and diagnostic outdoor-air design-flow/mixed-air evidence for declared output variables only; outdoor-air conformance, humidity, economizer, heat recovery, sizing, meters, and broad HVAC compatibility remain outside the claim. |
| DesignSpecification:OutdoorAir | hvac | diagnostic | ideal_loads_outdoor_air_design_flow_diagnostic_001 | Typed for IdealLoads outdoor-air reference/schedule preservation plus diagnostic-only Flow/Zone design-flow mass, standard-density volume flow, sensible report-rate, and no-heat-recovery mixed-air state parity; no outdoor-air latent load, DCV, economizer, heat recovery, or numerical OA conformance claim. |
